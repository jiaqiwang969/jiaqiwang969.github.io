...
该程序使用几何多网格(GMG)预处理程序解决一个平流-扩散问题。该预处理程序的基本原理在 step-16 中讨论；这里我们讨论非对称PDE所需的必要变化。此外，我们还介绍了块平滑的概念（与 step-16 中的点平滑相比），并检查了加法和乘法平滑器的DoF重编号的效果。
...
direction</i>， $f$ 是一个源。有几个注意事项。

1. 如果 $\boldsymbol{\beta}=\boldsymbol{0}$ ，这是 step-16 中解决的拉普拉斯方程（以及其他许多地方）。

2. 如果 $\varepsilon=0$ ，那么这就是 step-9 中解决的静止平流方程。

3. 人们可以为这个问题定义一个无量纲数，称为<i>Peclet number</i>。  $\mathcal{P} \dealcoloneq \frac{\|\boldsymbol{\beta}\|
L}{\varepsilon}$  ，其中 $L$  是域的长度尺度。它描述了我们所考虑的那种方程的特点。如果 $\mathcal{P}>1$ ，我们说问题是<i>advection-dominated</i>，否则如果 $\mathcal{P}<1$ ，我们会说问题是<i>diffusion-dominated</i>。

在本教程的讨论中，我们将关注以平流为主的流动。这是很复杂的情况。我们知道，对于扩散主导的问题，标准的Galerkin方法可以很好地工作，我们也知道简单的多网格方法，如 step-16 中定义的方法非常有效。另一方面，对于平流主导的问题，标准Galerkin方法会导致振荡和不稳定的离散，简单的求解器往往不是很有效。因此，本教程程序旨在解决这两个问题。
...
其中总和是在所有单元 $K$ 上进行的，每个单元都有内积， $\delta_K$ 是定义在 @cite john2006discontinuity 中的一个单元常数稳定参数。

从本质上讲，加入离散强形式残差会增强双线性形式 $a(\cdot,\cdot)$ 的矫顽力，从而增加离散解的稳定性。这种方法通常被称为<i>streamline
diffusion</i>或<i>SUPG</i>（流线型上风/Petrov-Galerkin）。
...
与 step-16 不同的是，我们的测试问题包含一个平流项。特别是在扩散常数较小的情况下 $\varepsilon$ ，信息是沿着给定的平流方向的流线传输的。这意味着，如果平滑器允许信息在单一平滑器应用中沿下游方向传播，那么平滑器可能会更有效。如果我们想按照这些未知数（或未知数块）的列举顺序一次解决一个未知数（或未知数块），那么这个信息传播特性需要相应地重新排列自由度或单元（对于基于单元的平滑器），以便更上游的自由度被提前处理（指数较低），更下游的自由度被推迟处理（指数较大）。排序的影响将在结果部分可见。

现在让我们简单定义一下本教程中使用的平滑器。关于更详细的介绍，我们参考 @cite KanschatNotesIterative 以及书籍 @cite smith2004domain 和 @cite toselli2006domain  。施瓦兹预处理器需要一个分解 

@f{align*}{
V = \sum_{j=1}^J V_j
@f} 

我们的有限元空间  $V$  的分解。每个子问题  $V_j$  也有一个基于双线性形式  $a(\cdot,\cdot)$  的里兹投影  $P_j: V \rightarrow V_j$  。这个投影为每个子问题 $V_j$ 诱导出一个局部算子  $A_j$  。如果 $\Pi_j:V\rightarrow V_j$ 是对 $V_j$ 的正交投影，就可以表明 $A_jP_j=\Pi_j^TA$ 。

由此，我们可以为算子 $A$ 定义一个<i>additive Schwarz preconditioner</i>作为 
...
请注意，通过为每个自由度定义一个子问题 $V_j$ ，我们可以将逐点（一次一个未知数）的雅可比方法解释为加法施瓦茨方法。然后， $A_j^{-1}$ 成为与 $A$ 的对角线项的逆的乘法。

对于本教程中使用的 "Block Jacobi "方法，我们为当前层次上的网格的每个单元定义一个子问题 $V_j$ 。注意，我们使用的是连续有限元，所以这些块是重叠的，因为两个单元之间的界面上的自由度都属于两个子问题。对子问题（在deal.II中它们被称为 "块"）进行操作的施瓦茨算子的逻辑在RelaxationBlock类中实现。块状雅可比 "方法是在RelaxationBlockJacobi类中实现的。该类的许多方面（例如如何定义块以及如何反转局部子问题 $A_j$ ）可以在平滑器数据中配置，详见 RelaxationBlock::AdditionalData  和 DoFTools::make_cell_patches() 。

到目前为止，我们讨论了加法平滑器，其中更新可以独立应用，并且在单一平滑器应用中没有信息流动。一个<i>multiplicative Schwarz preconditioner</i>解决了这个问题，其定义为 

@f{align*}{
 B^{-1} = \left( I- \prod_{j=1}^J \left(I-P_j\right) \right) A^{-1}.
@f} 

与上述相反，子问题 $V_j$ 上的更新是按顺序应用的。这意味着在颠倒子问题 $A_j$ 时得到的更新立即被用于 $A_{j+1}$ 。这在写出项目的时候就可以看到。
...
当把子空间 $V_j$ 定义为整个自由度块时，这个方法在RelaxationBlockSOR类中实现，并在本教程中选择 "块SOR "时使用。RelaxationBlockSOR类也是从RelaxationBlock派生的。因此，加法和乘法的施瓦兹方法都在一个统一的框架内实现。
...
</table>  <a name="CommProg"></a> <h1> The commented program</h1> 
...
由于我们将使用MeshWorker框架，第一步是定义以下由 MeshWorker::mesh_loop(): 使用的assemble_cell()函数所需要的结构 `ScratchData`包含一个FEValues对象，这是组装一个单元的局部贡献所需要的，而`CopyData`包含一个单元的局部贡献的输出以及复制到全局系统的必要信息。它们的目的在WorkStream类的文档中也有解释）。
...
Incompressible Fluid Dynamics by Elman, Silvester, and Wathen</a>第118页上的例3.1.3的改编。主要区别在于我们在域的中心添加了一个洞，其边界条件为零的迪里希特。   


为了进行完整的描述，我们需要首先实现零右手边的类（当然我们可以直接使用 Functions::ZeroFunction):   
...
如果 $x=1$ ，或者如果 $x>0.5$ 和 $y=-1$ ，将边界设置为1。
...
流线扩散法有一个稳定常数，我们需要能够计算出来。这个参数如何计算的选择来自于<a
...
这是程序的主类，看起来应该与  step-16  非常相似。主要的区别是，由于我们是在运行时定义我们的多网格平滑器，我们选择定义一个函数`create_smoother()`和一个类对象`mg_smoother`，这是一个  `std::unique_ptr`  派生自MGSmoother的平滑器。请注意，对于从RelaxationBlock派生的平滑器，我们必须为每个级别包括一个`smoother_data`对象。这将包含关于单元格排序和单元格矩阵倒置方法的信息。
...
如果我们使用流线扩散，我们必须将其贡献添加到单元矩阵和单元右手边。如果我们不使用流线扩散，设置 $\delta=0$ 就会否定下面的贡献，我们就只能使用标准的Galerkin有限元组合。
...
在这里，我们采用 MeshWorker::mesh_loop() 来翻阅单元格，为我们组装system_matrix、system_rhs和所有mg_matrices。
...
如果 $(i,j)$ 是一个`interface_out` dof对，那么 $(j,i)$ 是一个`interface_in` dof对。注意：对于 "interface_in"，我们加载接口条目的转置，即，dof对 $(j,i)$ 的条目被存储在 "interface_in(i,j)"。这是对对称情况的优化，允许在solve()中设置边缘矩阵时只使用一个矩阵。然而，在这里，由于我们的问题是非对称的，我们必须同时存储`interface_in`和`interface_out`矩阵。
...
在求解系统之前，我们必须首先设置多网格预处理程序。这需要设置各级之间的转换、粗略矩阵求解器和平滑器。这个设置几乎与 Step-16 相同，主要区别在于上面定义的各种平滑器，以及由于我们的问题是非对称性的，所以我们需要不同的界面边缘矩阵。实际上，在本教程中，这些接口矩阵是空的，因为我们只使用全局细化，因此没有细化边。然而，我们在这里仍然包括了这两个矩阵，因为如果人们简单地切换到自适应细化方法，程序仍然可以正确运行）。) 
...
在第0个循环中，我们在正方形 <code>[-1,1]^dim</code> 上生成网格，半径为3/10单位的洞以原点为中心。对于`manifold_id`等于1的对象（即与洞相邻的面），我们指定一个球面流形。
...
以下各表给出了GMRES的迭代次数，以减少初始残差的系数 $10^8$ 。我们选择了足够数量的平滑步骤（基于方法），以获得独立于网格大小的迭代数。从下面的表格可以看出，该方法确实是 $\mathcal{O}(n)$  。
...
然而，乘法的这一特点取决于 $\varepsilon$ 的值。当我们增加 $\varepsilon$ ，问题变得更加以扩散为主时，我们在网格上的信息传播更加均匀，在平流方向上重新编号的优势就会减弱。相反，在 $\varepsilon=0$ 的极端情况下（仅有平流），我们有一个一阶PDE，具有正确重编号的乘法成为有效的求解器。正确的下游编号可能导致方法只需要一次迭代，因为信息可以从流入边界向下游传播，而没有相反方向的信息传输。然而，请注意，在 $\varepsilon=0$ 的情况下，必须对这种情况下的边界条件给予特别关注）。


<a name="Pointvsblocksmoothers"></a><h4> %Point vs. block smoothers </h4>


我们将把结果限制在使用下游重新编号的运行上。下面是对 $Q_1$ 和 $Q_3$ 两个元素的所有四个平滑器的交叉比较。
...
我们看到，对于 $Q_1$ ，两个乘法平滑器需要的平滑步骤和迭代次数的组合都比加法平滑器小。然而，当我们将度数增加到 $Q_3$ 元素时，在平滑步骤和迭代次数方面，块平滑器有明显的优势。具体来说，块状SOR平滑器在度数上给出了恒定的迭代次数，而块状Jacobi平滑器的迭代次数只增加了约38%，而Jacobi和SOR的迭代次数分别为75%和183%。

<a name="Cost"></a><h3> Cost </h3>


迭代次数并不能完全说明一种平滑器比另一种平滑器的最优性。很明显，我们必须检查迭代的成本。块状平滑器在这里处于不利地位，因为它们必须为每个单元构建和反转一个单元矩阵。下面是一个具有74,496个DoF的 $Q_3$ 元素的求解时间的比较。
...
还有几个重要的点要提到。
  ...
  <ol>   <li>  对于平行分布的网格，乘法不能在整个领域内执行。这是因为它们一次操作一个单元，下游的单元只有在上游的单元已经完成后才能被处理。这在单个处理器上是没有问题的。处理器只是一个接一个地浏览单元的列表。然而，在并行的情况下，这将意味着一些处理器是空闲的，因为上游处理器还没有完成对当前处理器所拥有的上游单元的工作。一旦上游处理器完成，下游处理器就可以开始工作，但那时上游处理器已经没有工作了。换句话说，在这些平稳的步骤中，大部分时间，大多数处理器实际上是空闲的。这不是获得良好的并行可扩展性的方法! 

我们可以使用一种混合方法，即在每个子域上应用乘法平滑器，但是当你增加子域的数量时，该方法接近于加法的行为。这是这些方法的一个主要缺点。  </li>   

  <li>  目前对块平滑器的研究表明，很快我们将能够计算单元矩阵的逆，比目前在deal.II里面做的要便宜得多。这项研究是基于快速对角线化方法（可追溯到20世纪60年代），并在光谱界使用了约20年（见，例如，<a
...
and Fischer</a>）。目前，人们正在努力将这些方法推广到DG，并使其更加稳健。此外，人们似乎应该能够利用无矩阵实现的优势，以及在域的内部，单元矩阵往往看起来非常相似的事实，允许更少的矩阵逆计算。  </li>  </ol>  。 
...
改变平滑步骤的数量和平滑器松弛参数（在 <code>Smoother::AdditionalData()</code> 中设置在 <code>create_smoother()</code> 里面，只对点平滑器有必要），以便我们对一个 $Q_5$ 元素保持恒定的迭代次数。
...
该代码已被设置为能在自适应细化的网格中正常工作（界面矩阵已创建并设置）。设计一个合适的细化标准或尝试KellyErrorEstimator类。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-63.cc"  。 
