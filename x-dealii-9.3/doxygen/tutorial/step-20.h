...
这个程序致力于两个方面：使用混合有限元--特别是Raviart-Thomas元--以及使用块状矩阵来定义求解器、预处理器和使用系统矩阵的子结构的嵌套版本。我们要解决的方程仍然是泊松方程，尽管有一个矩阵值的系数。@f{eqnarray*}
...
我们将扩展 step-21 中的这个教程程序，不仅要解决混合拉普拉斯方程，还要增加另一个描述两种流体的混合运输的方程。

这里涉及的方程属于矢量值问题的范畴。这个主题的顶层概述可以在 @ref vector_valued 模块中找到。


<a name="Theequations"></a><h3>The equations</h3> 


在上述形式中，泊松方程（即右手边为非零的拉普拉斯方程）通常被认为是多孔介质中流体流动的良好模型方程。当然，人们通常通过<a href="https://en.wikipedia.org/wiki/Navier%E2%80%93Stokes_equations">Navier-Stokes
equations</a>来模拟流体流动，或者，如果流体速度很慢或粘度很大，则通过<a href="https://en.wikipedia.org/wiki/Stokes_flow">Stokes
equations</a>（我们在 step-22 中涉及）。在这两个模型中，第一个模型的作用力是惯性和粘性摩擦，而在第二个模型中，只有粘性摩擦--即一个流体粒子对附近的粒子所施加的力。如果你在一个大的领域里有自由流动，例如管道、河流或空气中，这是很合适的。另一方面，如果流体被限制在孔隙中，那么孔壁对流体施加的摩擦力变得越来越重要，而内部粘性摩擦力变得越来越不重要。如果这两种效应都很重要，那么建立模型首先会导致<a href="https://en.wikipedia.org/wiki/Darcy%27s_law#Brinkman_form_of_Darcy's_law">Brinkman
...
达西方程对驱动流动的压力进行建模。由于求解变量是压力，我们在此使用 $p$ 的名称，而不是通常用于求解偏微分方程的 $u$ 的名称）。拉普拉斯方程的这种观点的典型应用是为地下水流或油藏中的碳氢化合物流动建模。在这些应用中， $K$ 是渗透性张量，即衡量土壤或岩石基质对流体流动的阻力大小。

在上述应用中，数值方案的一个理想特征是它应该是局部保守的，也就是说，无论什么东西流入一个单元，也会从该单元流出（或者如果源不为零，差值等于每个单元的源项的积分）。然而，事实证明，拉普拉斯方程的通常离散化（如 step-3 、 step-4 或 step-6 中使用的那些）并不满足这一特性。但是，人们可以通过选择不同的问题表述和特定的有限元空间组合来实现这一点。


<a name="Formulationweakformanddiscreteproblem"></a><h3>Formulation, weak form, and discrete problem</h3> 


为此，我们首先引入了第二个变量，称为速度，  ${\mathbf u}=-K\nabla p$  。根据其定义，速度是压力梯度的负方向的一个矢量，乘以渗透性张量。如果渗透率张量与单位矩阵成正比，这个方程就很容易理解和直观：渗透率越高，速度就越高；速度与压力梯度成正比，从高压区到低压区（因此是负号）。
...
u}$ 乘以 $K^{-1}$ ，因为这使得方程组是对称的：其中一个方程有梯度，第二个方程有负发散，这两个方程当然是彼此相邻的，结果是一个对称的双线性形式，因此，在 $K$ 是对称张量的共同假设下，是对称的系统矩阵。

这个问题的弱式是通过将两个方程与测试函数相乘，并将一些项进行部分积分而得到的。@f{eqnarray*}
...
为了得到良好的解决，我们必须在空间 $H({\textrm{div}})=\{{\mathbf w}\in L^2(\Omega)^d:\ {\textrm{div}}\ {\mathbf w}\in L^2\}$ 中寻找 $\mathbf u$ 、 $\mathbf v$ 和 $L^2$ 的解和检验函数。几乎每一本关于有限元理论的书都说过一个众所周知的事实，如果选择离散的有限元空间来逼近 ${\mathbf u},p$ 是不恰当的，那么产生的离散问题是不稳定的，离散的解不会收敛到精确的解。这里考虑的问题的一些细节--属于 "鞍点问题 "的范畴 
...
为了克服这个问题，已经为 ${\mathbf u},p$ 开发了一些不同的有限元对，导致了稳定的离散问题。其中一个对子是对速度 ${\mathbf u}$ 使用Raviart-Thomas空间，对压力 $DQ(k)$ 使用不连续元素类 $p$ 。关于这些空间的细节，我们特别参考Brezzi和Fortin的关于混合有限元方法的书，但许多其他关于有限元理论的书，例如Brenner和Scott的经典书，也说明了相关结果。在任何情况下，在适当选择函数空间的情况下，离散的表述如下。找到 ${\mathbf
...
在继续之前，让我们简短地停顿一下，说明上述函数空间的选择为我们提供了所需的局部守恒特性。特别是，由于压力空间由不连续的片断多项式组成，我们可以选择测试函数 $q$ 作为在任何给定单元 $K$ 上等于1，其他地方为0的函数。如果我们也到处选择 ${\mathbf v}=0$ （记住，上面的弱式必须对<i>all</i>离散测试函数 $q,v$ 成立），那么把这些测试函数的选择放到上面的弱式表述中，特别意味着@f{eqnarray*}
...
如果你现在记得 ${\mathbf u}$ 是速度，那么左边的积分正是穿过单元 $K$ 边界的（离散）通量。那么声明是，通量必须等于 $K$ 内的源上的积分。特别是，如果没有源（即 $f=0$ 在 $K$ 中），那么声明是<i>total</i>通量为零，也就是说，任何流入一个单元的东西都必须通过单元边界的其他部分流出。这就是我们所说的<i>local conservation</i>，因为它对每个单元都成立。

另一方面，通常的连续 $Q_k$ 元素在用于压力时不会产生这种属性（例如，我们在 step-43 中这样做），因为我们不能选择一个离散的测试函数 $q_h$ ，在一个单元 $K$ 上为一，在其他地方为零：它将是不连续的，因此不在有限元空间内。严格来说，我们只能说上面的证明对连续元素不起作用。这些元素是否仍然可能导致局部守恒是一个不同的问题，因为人们可以认为不同的证明可能仍然有效；然而，在现实中，这个属性确实不成立）。) 
...
deal.II库（当然）实现了任意阶的Raviart-Thomas元素  $RT(k)$   $k$  ，以及不连续的元素  $DG(k)$  。如果我们暂时忘记它们的特殊属性，那么我们就必须解决一个离散问题@f{eqnarray*}
  A(x_h,w_h) = F(w_h),
@f}

的双线性形式和右手边，以及 $x_h=\{{\mathbf u}_h,p_h\}$  ,  $w_h=\{{\mathbf v}_h,q_h\}$  。 $x_h$ 和 $w_h$ 都来自空间 $X_h=RT(k)\times DQ(k)$ ，其中 $RT(k)$ 本身就是一个 $dim$ 维函数的空间，以适应流速是矢量值的事实。那么必要的问题是：我们如何在程序中做到这一点？

矢量值元素已经在以前的教程程序中讨论过了，第一次是在  step-8  中详细讨论。那里的主要区别是，矢量值空间 $V_h$ 的所有分量都是统一的：位移矢量的 $dim$ 分量都是相等的，来自同一个函数空间。因此，我们可以做的是建立 $V_h$ 作为 $dim$ 乘以通常的 $Q(1)$ 有限元空间的外积，并以此确保我们所有的形状函数只有一个非零矢量分量。因此，我们在 step-8 中所做的不是处理矢量值的形状函数，而是查看（标量）唯一的非零分量，并使用 <code>fe.system_to_component_index(i).first</code> 调用来计算出这实际上是哪个分量。

这对Raviart-Thomas元素不起作用：由于它们的构造满足空间 $H({\textrm{div}})$ 的某些规律性属性， $RT(k)$ 的形状函数通常在其所有矢量分量中都是非零的。由于这个原因，如果应用 <code>fe.system_to_component_index(i).first</code> 来确定形状函数 $i$ 的唯一非零分量，就会产生一个例外。我们真正需要做的是在 <em> 中获得一个形状函数的所有 </em> 矢量分量。在deal.II的字典中，我们把这样的有限元称为  <em>  非原始  </em>  ，而把标量的有限元或者每个矢量值的形状函数只在一个矢量分量中不为零的有限元称为  <em>  原始  </em>  。

那么对于非原始元素，我们要怎么做呢？为了弄清楚这个问题，让我们回到教程的程序中，几乎是最开始的时候。在那里，我们了解到，我们使用 <code>FEValues</code> 类来确定正交点的形状函数的值和梯度。例如，我们会调用  <code>fe_values.shape_value(i,q_point)</code>  来获得  <code>i</code>  的形状函数在正交点上的数值  <code>q_point</code>  。后来，在  step-8  和其他教程程序中，我们了解到这个函数调用也适用于矢量值的形状函数（原始有限元），它返回形状函数  <code>i</code>  在正交点  <code>q_point</code>  的唯一非零分量的值。
...
我们现在可以尝试用向量分量来重写上面的双线性形式。例如，在2d中，第一项可以这样改写（注意， $u_0=x_0, u_1=x_1, p=x_2$ ）。@f{eqnarray*}
...
这充其量是繁琐的，容易出错的，而且不是独立的维度。有一些明显的方法可以使事情与维度无关，但最终，这些代码根本不漂亮。如果我们能够简单地提取形状函数 ${\mathbf u}$ 和 $p$ 的分量，那就更好了。在程序中，我们以如下方式进行。
...
这段代码的作用是，给定一个 <code>fe_values</code> 对象，在正交点 <code>q</code> 提取形状函数 <code>i</code> 的第一个 $dim$ 分量的值，也就是该形状函数的速度分量。换句话说，如果我们把形状函数 $x_h^i$ 写成元组 $\{{\mathbf u}_h^i,p_h^i\}$ ，那么该函数返回这个元组的速度部分。请注意，速度当然是一个 <code>dim</code> 维度的张量，并且该函数返回一个相应的对象。同样地，在我们用压力提取器下标的地方，我们提取标量压力分量。整个机制在 @ref vector_valued 模块中有更详细的描述。

在实践中，如果我们在每个最外层的循环中只评估一次形状函数、它们的梯度和发散，并存储结果，我们可以做得更好一些，因为这样可以节省一些原本重复的计算（通过提前计算所有相关的量，然后只在实际的循环中插入结果，甚至可以节省更多的重复操作，关于这种方法的实现见 step-22 ）。最后的结果是这样的，在每个空间维度上工作。
...
有一个最后的项我们必须注意：右手边包含项 $(g,{\mathbf v}\cdot {\mathbf n})_{\partial\Omega}$ ，构成压力边界条件的弱执行。我们已经在 step-7 中看到了如何处理面积分：本质上与域积分完全相同，只是我们必须使用FEFaceValues类而不是 <code>FEValues</code> 。为了计算边界项，我们只需在所有的边界面上进行循环并在那里进行积分。该机制的工作方式与上述相同，即提取器类也对FEFaceValues对象工作。
...
- 它是<a href="https://en.wikipedia.org/wiki/Definiteness_of_a_matrix">indefinite</a>，也就是说，它既有正特征值也有负特征值。  我们不想在这里证明这个属性，但请注意，对于所有形式为 $\left(\begin{array}{cc} M & B \\ B^T & 0 \end{array}\right)$ 的矩阵都是如此，比如这里的 $M$ 就是正定的。

- 该矩阵在右下方有一个零块（在双线性形式中没有将压力 $p$ 与压力测试函数 $q$ 耦合的项）。
...
对于我们期望用这个程序运行的矩阵大小，迄今为止最简单的方法是直接使用一个直接求解器（特别是与deal.II捆绑的SparseDirectUMFPACK类）。  step-29 走的就是这条路线，并表明解决<i>any</i>线性系统只需3、4行代码就可以完成。

但是，这是一个教程。我们教如何做事情。因此，在下文中，我们将介绍一些可用于类似这种情况的技术。也就是说，我们将考虑线性系统不是由一个大矩阵和向量组成的，而是要将矩阵分解为<i>blocks</i>，这些矩阵对应于系统中出现的各个运算符。我们注意到所得到的求解器并不是最优的--有更好的方法来有效地计算这个系统，例如在 step-22 的结果部分所解释的方法，或者我们在 step-43 中用于类似于当前问题的方法。在这里，我们的目标仅仅是介绍新的求解技术以及如何在交易中实现它们。


<a name="SolvingusingtheSchurcomplement"></a><h4>Solving using the Schur complement</h4>


鉴于使用上述标准求解器和预处理器的困难，让我们再看一下矩阵。如果我们对自由度进行排序，使所有的速度变量排在所有的压力变量之前，那么我们可以将线性系统 $Ax=b$ 细分为以下几块。@f{eqnarray*}
  \left(\begin{array}{cc}
    M & B \\ B^T & 0
  \end{array}\right)
  \left(\begin{array}{cc}
...
通过区块消除法，我们可以按以下方式对这个系统重新排序（用系统的第一行乘以 $B^TM^{-1}$ ，然后用第二行减去）。@f{eqnarray*}
...
因此，如果我们能够计算出 $S$ ，我们就可以对其应用共轭梯度法。然而，计算 $S$ 是昂贵的，因为它需要我们计算（可能很大的）矩阵 $M$ 的逆；而 $S$ 实际上也是一个全矩阵，因为即使 $M$ 是稀疏的，其逆 $M^{-1}$ 通常也是一个密集矩阵。另一方面，CG算法并不要求我们真正拥有 $S$ 的表示：只需与它形成矩阵-向量乘积即可。我们可以利用矩阵乘积是关联的这一事实，分步进行（也就是说，我们可以通过设置括号的方式使乘积更方便计算）。为了计算 $Sv=(B^TM^{-1}B)v=B^T(M^{-1}(Bv))$ ，我们 <ol>  <li> 计算 $w = B v$ ； <li> 用CG方法解决 $My = w$ 的 $y=M^{-1}w$ ，适用于正定和对称的质量矩阵 $M$ ； <li> 计算 $z=B^Ty$ ，得到 $z=Sv$  。  </ol>  注意我们如何从右到左评估表达式 $B^TM^{-1}Bv$ 以避免矩阵-矩阵乘积；这样，我们所要做的就是评估矩阵-向量乘积。

在下文中，我们必须想出表示矩阵 $S$ 的方法，以便在共轭梯度求解器中使用它，以及定义我们可以预设涉及 $S$ 的线性系统解决方案的方法，并处理与矩阵 $M$ 的线性系统求解（上述第二步骤）。

  @note  这个考虑的关键点是要认识到，为了实现一个迭代求解器，如CG或GMRES，我们实际上从来不需要矩阵的实际<i>elements</i>! 所需要的只是我们能够形成矩阵-向量乘积。对于预处理程序也是如此。在deal.II中，我们对这一要求进行了编码，只要求给予求解器类的矩阵和预处理器有一个 <code>vmult()</code> 成员函数来做矩阵-向量乘积。一个类如何选择实现这个函数对求解器来说并不重要。因此，类可以通过，例如，做一连串的乘积和线性求解来实现它，正如上面所讨论的。
...
作为第一个例子，让我们构建一个代表 $M^{-1}$ 的LinearOperator对象。这意味着每当这个运算符的 <code>vmult()</code> 函数被调用时，它必须解决一个线性系统。这就要求我们指定一个解算器（和相应的）前置条件。假设 <code>M</code> 是对系统矩阵左上块的引用，我们可以写成。
...
我们没有使用SolverControl，而是使用了ReductionControl类，该类在达到绝对容限（我们选择 $10^{-18}$ ）或残差减少某一系数（此处为 $10^{-10}$ ）时停止迭代。相反，SolverControl类只检查绝对公差。在我们的案例中，我们必须使用ReductionControl来解决一个小问题。我们将送入 <code>op_M_inv</code> 的右手边基本上是由残差形成的，随着外部迭代的进行，残差的规范自然会大大降低。这使得用绝对公差来控制非常容易出错。

我们现在有一个LinearOperator  <code>op_M_inv</code>  ，我们可以用它来构造更复杂的算子，如Schur补码  $S$  。假设 <code>B</code> 是对右上角区块的引用，构造一个LinearOperator  <code>op_S</code> 只需两行。
...
在这里，三个LinearOperator对象的乘法得到一个复合对象 <code>op_S</code> whose <code>vmult()</code> ，该函数首先应用 $B$ ，然后是 $M^{-1}$ （即用 $M$ 解方程），最后是 $B^T$ 到任何给定的输入向量。在这个意义上， <code>op_S.vmult()</code> 类似于以下代码。
...
(  <code>tmp1</code> and <code>tmp2</code>  是两个临时向量)。这种方法背后的关键点是，我们实际上从未创建过矩阵的内积。相反，每当我们必须与 <code>op_S</code> 进行矩阵向量乘法时，我们只需按上述顺序运行所有单独的 <code>vmult</code> 操作。
...
尽管这两种方法完全等同，但LinearOperator类比这种手工方法有很大的优势。它提供了所谓的<i><a href="https://en.wikipedia.org/wiki/Syntactic_sugar">syntactic sugar</a><a href="https://en.wikipedia.org/wiki/Syntactic_sugar">syntactic sugar</a></i>。在数学上，我们认为 $S$ 是复合矩阵 $S=B^TM^{-1}B$ ，LinearOperator类允许你或多或少地逐字写出来。
...
现在我们要做的就是形成定义 $P$ 和 $U$ 的两个方程的右边，然后分别用舒尔补码矩阵和质量矩阵来解决它们。例如，第一个方程的右手边为  $B^TM^{-1}F-G$  。这可以按以下方式实现。
...
该类允许<a href="https://en.wikipedia.org/wiki/Lazy_evaluation">lazy evaluation</a>涉及向量和线性运算符的表达式。这是通过存储计算表达式来实现的，并且只在对象被转换为向量对象，或者 PackagedOperation::apply() （或 PackagedOperation::apply_add()) 被手动调用时才执行计算。假设 <code>F</code> and <code>G</code> 是右手边的两个向量，我们可以简单地写出。
...
有了这些先决条件，解决 $P$ 和 $U$ 是创建另一个求解器和逆的问题。
...
@endcode 
  ...
  @note 我们在这个例子中手工开发的功能在库中已经很容易获得。看看schur_complement(), condense_schur_rhs(), and postprocess_schur_solution()。


<a name="ApreconditionerfortheSchurcomplement"></a><h4>A preconditioner for the Schur complement</h4>


有人可能会问，如果我们有一个Schur补码的预处理程序是否会有帮助  $S=B^TM^{-1}B$  。一般来说，答案是：当然。问题是，我们对这个舒尔补码矩阵一无所知。我们不知道它的条目，我们所知道的只是它的作用。另一方面，我们必须认识到，我们的求解器是昂贵的，因为在每次迭代中，我们必须与舒尔补矩阵做一次矩阵-向量乘积，这意味着我们必须在每次迭代中对质量矩阵做一次反转。

有不同的方法对这样的矩阵进行预处理。一个极端是使用一些便宜的方法，因此对每次迭代的工作没有实际影响。另一个极端是使用本身非常昂贵的预处理程序，但作为回报，它确实降低了用 $S$ 求解所需的迭代次数。

我们将沿着第二种方法进行尝试，既是为了提高程序的性能，也是为了展示一些技术。为此，让我们回顾一下，理想的预处理程序当然是 $S^{-1}$ ，但这是无法实现的。然而，将@f{eqnarray*}
  \tilde S^{-1} = [B^T ({\textrm{diag}\ }M)^{-1}B]^{-1}
@f}作为预处理程序如何？

作为先决条件如何？这意味着每次我们必须做一个预处理步骤时，我们实际上必须用 $\tilde S$ 来解决。起初，这看起来几乎和立即用 $S$ 求解一样昂贵。然而，请注意，在内迭代中，我们不必计算 $M^{-1}$ ，而只需计算其对角线的逆值，这很便宜。

值得庆幸的是，LinearOperator框架使得这一点非常容易写出来。我们之前已经对 $M$ 矩阵使用了雅可比预处理程序（ <code>preconditioner_M</code> ）。所以剩下的就是写出近似的舒尔补码应该是什么样子。
...
注意这个算子的不同之处在于，它只是做了一次雅可比扫频（即与对角线的逆数相乘），而不是与整个 $M^{-1}$ 相乘。（这就是与 $M$ 的单一雅可比预处理步骤的定义：它是与 $M$ 的对角线的逆数相乘；换句话说，对向量 $x$ 的 $({\textrm{diag}\ }M)^{-1}x$ 操作正是 PreconditionJacobi 所做的）。

有了这些，我们几乎完成了预处理程序：它应该是近似舒尔补码的逆运算。我们再次通过使用inverse_operator()函数创建一个线性算子来实现这一点。然而这一次我们想为CG求解器选择一个相对较小的容忍度（即反转 <code>op_aS</code> ）。理由是 <code>op_aS</code> is only coarse approximation to <code>op_S</code> ，所以我们实际上不需要完全反转它。然而，这产生了一个微妙的问题： <code>preconditioner_S</code> 将被用于最后的外层CG迭代以创建一个正交基础。但为了使其发挥作用，每次调用都必须是精确的线性操作。我们通过使用IterationNumberControl来确保这一点，它允许我们将执行的CG迭代次数固定为一个固定的小数字（在我们的例子中为30）。
...
在这个教程程序中，我们将以上述的混合公式求解拉普拉斯方程。由于我们想在程序中监控解的收敛性，我们选择右手边、边界条件和系数，以便恢复我们已知的解函数。特别是，我们选择压力解@f{eqnarray*}
  p = -\left(\frac \alpha 2 xy^2 + \beta x - \frac \alpha 6 x^3\right),
@f}

对于系数我们选择单位矩阵 $K_{ij}=\delta_{ij}$ 来简化。因此，确切的速度满足@f{eqnarray*}
  {\mathbf u} =
  \left(\begin{array}{cc}
...
选择这个解是因为它完全没有发散，使得它成为不可压缩流体流动的一个现实的测试案例。因此，右手边等于 $f=0$  ，作为边界值，我们必须选择 $g=p|_{\partial\Omega}$  。

在本程序的计算中，我们选择 $\alpha=0.3,\beta=1$  。你可以在<a name="#Results">results section
below</a>中找到所得到的解决方案，在注释程序之后。<a name="CommProg"></a> <h1> The commented program</h1>
...
由于这个程序只是对 step-4 的改编，在头文件方面没有太多的新东西。在deal.II中，我们通常按照base-lac-grid-dofs-fe-numerics的顺序列出包含文件，然后是C++标准包含文件。
...
最后一步和以前的所有程序一样。我们把所有与这个程序相关的代码放到一个命名空间中。(这个想法在  step-7  中首次提出) 。
...
同样，由于这是对  step-6  的改编，主类与该教程程序中的主类几乎相同。就成员函数而言，主要区别在于构造函数将Raviart-Thomas元素的度数作为参数（并且有一个相应的成员变量来存储这个值），并且增加了 <code>compute_error</code> 函数，在该函数中，不出意外，我们将计算精确解和数值解之间的差异，以确定我们计算的收敛性。
...
我们的下一个任务是定义我们问题的右手边（即原始拉普拉斯方程中压力的标量右手边），压力的边界值，以及一个描述压力和精确解的速度的函数，以便以后计算误差。请注意，这些函数分别有一个、一个和 <code>dim+1</code> 个分量，我们将分量的数量传递给 <code>Function@<dim@></code> 基类。对于精确解，我们只声明实际上一次性返回整个解向量（即其中的所有成分）的函数。下面是各自的声明。
...
除了其他方程数据外，我们还想使用渗透性张量，或者更好的是--因为这是在弱形式中出现的全部--渗透性张量的逆，  <code>KInverse</code>  。对于验证解的精确性和确定收敛顺序的目的来说，这个张量的作用大于帮助。因此，我们将简单地把它设置为同一矩阵。     
...
可能毫不奇怪，deal.II也有一个基类，不仅适用于标量和一般矢量值的函数（ <code>Function</code> 基类），也适用于返回固定维度和等级的张量的函数，即 <code>TensorFunction</code> 模板。在这里，所考虑的函数返回一个dim-by-dim矩阵，即一个等级为2、维度为 <code>dim</code> 的张量。然后我们适当地选择基类的模板参数。     
 ...
 <code>TensorFunction</code> 类所提供的接口本质上等同于 <code>Function</code> 类。特别是，存在一个 <code>value_list</code> 函数，它接收一个评估函数的点的列表，并在第二个参数中返回函数的值，一个张量的列表。
...
<a name="MixedLaplaceProblemMixedLaplaceProblem"></a> <h4>MixedLaplaceProblem::MixedLaplaceProblem</h4> <h4>MixedLaplaceProblem::MixedLaplaceProblem</h4>




在这个类的构造函数中，我们首先存储传入的关于我们将使用的有限元的度数的值（例如，度数为0，意味着使用RT(0)和DG(0)），然后构造属于介绍中描述的空间 $X_h$ 的矢量值元素。构造函数的其余部分与早期的教程程序一样。   
...
同样值得比较的是，我们从基本元素中构建这个元素的方式，与我们在 step-8 中的方式相比较：在那里，我们将其构建为 <code>fe (FE_Q@<dim@>(1), dim)</code> ，即我们简单地使用 <code>dim</code> copies of the <code>FE_Q(1)</code> 元素，每个坐标方向上的位移都有一份。
...
然而，接下来事情就变得不同了。正如在介绍中提到的，我们要将矩阵细分为对应于速度和压力这两个不同类型变量的块。为此，我们首先要确保与速度和压力相对应的指数不混在一起。首先是所有速度自由度，然后是所有压力自由度。这样一来，全局矩阵就很好地分离成一个 $2 \times 2$ 系统。为了实现这一点，我们必须根据自由度的矢量分量对其重新编号，这个操作已经很方便地实现了。
...
接下来，我们要弄清楚这些块的大小，以便我们可以分配适当的空间量。为此，我们调用了 DoFTools::count_dofs_per_fe_component() 函数，该函数计算了某个向量分量的多少个形状函数是非零的。我们有 <code>dim+1</code> 个向量分量， DoFTools::count_dofs_per_fe_component() 将计算有多少个形状函数属于这些分量中的每个。     
...
利用这一知识，我们可以从 <code>dim</code> 中的任何第一个 <code>dofs_per_component</code> 元素中得到速度形状函数的数量，然后用下面这个来初始化向量和矩阵块大小，以及创建输出。     
  ...
  @note  如果你觉得这个概念难以理解，你可能要考虑用函数  DoFTools::count_dofs_per_fe_block()  来代替，就像我们在  step-22  的相应代码中做的那样。你可能还想阅读一下术语表中 @ref GlossBlock "块 "和 @ref GlossComponent "组件 "之间的区别。
...
同样地，组装线性系统的函数在本例的介绍中已经大部分讨论过了。在它的顶部，发生的是所有常规的步骤，此外，我们不仅为单元项分配正交和 <code>FEValues</code> 对象，而且还为面项分配。之后，我们为变量定义通常的缩写，并为本地矩阵和右手贡献分配空间，以及保存当前单元本地自由度的全局数字的阵列。
...
最后，我们需要几个提取器，用来获取矢量值形状函数的速度和压力成分。它们的功能和使用在 @ref
vector_valued 报告中有详细描述。基本上，我们将把它们作为下面FEValues对象的下标：FEValues对象描述了形状函数的所有矢量分量，而在订阅后，它将只指速度（一组从零分量开始的 <code>dim</code> 分量）或压力（位于 <code>dim</code> 位置的标量分量）。
...
得到压力后，我们可以计算速度。方程为 $MU=-BP+F$  ，我们通过首先计算右手边，然后与代表质量矩阵逆的对象相乘来解决它。
...
在deal.II中， <code>ComponentSelectFunction</code> 正是这样做的：它想知道它要表示的函数应该有多少个矢量分量（在我们的例子中，这将是 <code>dim+1</code> ，用于联合速度-压力空间），哪个个体或范围的分量应该等于1。因此，我们在函数的开头定义了两个这样的掩码，接下来是一个代表精确解的对象和一个向量，我们将在其中存储由 <code>integrate_difference</code> 计算的单元误差。
...
正如在 step-7 中已经讨论过的，我们必须认识到，不可能精确地整合误差。我们所能做的就是用正交法对这个积分进行近似。这实际上在这里出现了一个小小的转折：如果我们像人们可能倾向于做的那样天真地选择一个 <code>QGauss@<dim@>(degree+1)</code> 类型的对象（这就是我们用于积分线性系统的对象），就会发现误差非常小，根本不遵循预期的收敛曲线。现在的情况是，对于这里使用的混合有限元，高斯点恰好是超收敛点，其中的点误差比其他地方小得多（而且收敛的阶数更高）。因此，这些点不是特别好的积分点。为了避免这个问题，我们简单地使用梯形法则，并在每个坐标方向上迭代 <code>degree+2</code> 次（同样如 step-7 中的解释）。
...
最后一个有趣的函数是我们生成图形输出的函数。请注意，所有的速度分量都得到相同的解名 "u"。再加上使用 DataComponentInterpretation::component_is_part_of_vector ，这将导致 DataOut<dim>::write_vtu() 生成单个速度分量的矢量表示，更多信息请参见 step-22 或 @ref VVOutput 模块中的 "生成图形输出 "部分。最后，对于高阶元素来说，在图形输出中每个单元只显示一个双线性四边形似乎不合适。因此，我们生成大小为(度数+1)x(度数+1)的斑块来捕捉解决方案的全部信息内容。关于这方面的更多信息，请参见 step-7 的教程程序。
...
我们从 step-6 而不是 step-4 偷来的主函数。它几乎等同于  step-6  中的函数（当然，除了改变了类名），唯一的例外是我们将有限元空间的度数传递给混合拉普拉斯问题的构造函数（这里，我们使用零阶元素）。
...
如果我们按原样运行程序，对于我们使用的 $32\times 32$ 网格，我们得到这样的输出（因为我们使用片状常数，所以总共有1024个单元，1024个压力自由度，2112个速度，因为Raviart-Thomas元素定义了每个面的一个自由度，有 $1024 + 32 = 1056$ 个面平行于 $x$ -轴，同样数量平行于 $y$  -轴）。
...
作为补充说明，请注意左图中的x-速度在x方向上是连续的，而y-速度在y方向上是连续的。其他方向上的流场是不连续的。这非常明显地反映了Raviart-Thomas元素的连续性特性，事实上，它只在空间H(div)而不是在空间 $H^1$ 。最后，压力场是完全不连续的，但鉴于我们选择了 <code>FE_DGQ(0)</code> 作为该求解分量的有限元，这不应该令人惊讶。
...
该程序提供了两个明显的玩耍和观察收敛的地方：使用的有限元的程度（传递给 <code>MixedLaplaceProblem</code> class from <code>main()</code> 的构造器）和细化水平（在 <code>MixedLaplaceProblem::make_grid_and_dofs</code> 中确定）。人们可以做的是改变这些值，并在以后的程序运行过程中观察计算出的误差。




如果这样做，就会发现压力变量的 $L_2$ 误差有如下模式。  <table align="center" class="doxtable">
  <tr>
    <th></th>
...
我们可以用速度变量中的 $L_2$ 误差做同样的实验。  <table align="center" class="doxtable">
  <tr>
    <th></th>
    <th colspan="3" align="center">Finite element order</th>
  </tr>
...
正如介绍中提到的，这里使用的Schur补码求解器并不是可以想象的最好的（也不打算成为一个特别好的）。更好的解算器可以在文献中找到，并且可以使用这里介绍的相同的块矩阵技术来构建。我们在 step-22 中再次提到这个主题，在那里我们首先为斯托克斯方程建立了一个Schur补数求解器，就像我们在这里所做的那样，然后在<a
href="step_22.html#improved-solver">Improved Solvers</a>部分讨论了基于求解系统整体但基于单个块的预处理的更好方法。我们还将在 step-43 中再来讨论这个问题。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-20.cc"  。 
