...
正弦戈登方程的名称是对所谓的克莱因-戈登方程的双关语，克莱因-戈登方程是质量不为零的粒子的薛定谔方程的相对论版本。这种相似性不仅仅是表面的，正弦-戈登方程已经被证明可以模拟一些统一场现象，如亚原子粒子的相互作用（见，例如，Perring &amp; Skyrme in Nuclear %Physics <b>31</b>）和超导体结中的约瑟夫森（量子）效应（见，例如<a
...
Nonlinear Waves</i>（第17章，第10-13节）。

  @note  我们将在 step-58 中介绍量子力学的一个单独的非线性方程，即非线性Schr&ouml;dinger方程。

<a name="Statementoftheproblem"></a><h3>Statement of the problem</h3>

我们希望解决的正弦-戈登初始边界值问题（IBVP）由以下方程组成。\f{eqnarray*}
  u_{tt}-\Delta u &=& -\sin(u) \quad\mbox{for}\quad (x,t) \in \Omega \times (t_0,t_f],\\
  {\mathbf n} \cdot \nabla u &=& 0 \quad\mbox{for}\quad (x,t) \in \partial\Omega
           \times (t_0,t_f],\\
  u(x,t_0) &=& u_0(x).
\f} 这是一个类似于我们在  step-23  和  step-24  中讨论的波浪方程的非线性方程。我们选择强制执行零诺伊曼边界条件，以使波从我们领域的边界上反射出去。然而，应该注意的是，迪里希特边界条件并不适合这个问题。尽管正弦-戈登方程的解是局部的，但只有在 $x=\pm\infty$ 处指定（迪里希特）边界条件才有意义，否则要么不存在解，要么只存在微不足道的解 $u=0$ 。

然而，上述方程的形式对于数值离散化来说并不理想。如果我们要直接准确地离散二阶时间导数，那么我们将需要一个大的模板（即需要在内存中保留几个时间步长），这可能变得很昂贵。因此，与我们在 step-23 和 step-24 中的做法完全类似，我们将二阶（时间上）正弦-戈登方程分割成两个一阶（时间上）方程系统，我们称之为分割或速度公式。为此，通过设置 $v = u_t$ ，很容易看到正弦-戈登方程等同于\f{eqnarray*}
  u_t - v &=& 0,\\
  v_t - \Delta u &=& -\sin(u).
\f} 
...
现在，我们可以用 $\theta$ 方法对分裂公式进行时间离散，该方法的模板只有两个时间步长。通过选择 $\theta\in [0,1]$ ，后者的离散化允许我们从一系列的方案中进行选择。特别是，如果我们选择 $\theta=0$ 或 $\theta=1$ ，我们可以分别得到一阶精确的显式或隐式欧拉方法。另一个重要的选择是 $\theta=\frac{1}{2}$ ，它给出了二阶精确的Crank-Nicolson方案。因此，上标 $n$ 表示在 $n^{\mathrm{th}}$ 时间步长的变量值，即在 $t=t_n \dealcoloneq n k$ ，其中 $k$ 是（固定）时间步长。因此，时间分解的正弦戈登方程的拆分表述成为\f{eqnarray*}
...
在这一点上，我们似乎可以直接对方程进行空间离散。虽然这对第二个方程（在 $v^n$ 中是线性的）来说是正确的，但这对所有的 $\theta$ 来说是不可行的，因为上面的第一个方程是非线性的。因此，必须实现一个非线性求解器，然后可以将方程在空间中离散化并求解。

为此，我们可以使用牛顿方法。给定非线性方程 $F(u^n) = 0$ ，我们对 $u^n$ 产生连续的近似值，如下。\f{eqnarray*}
  \mbox{ Find } \delta u^n_l \mbox{ s.t. } F'(u^n_l)\delta u^n_l = -F(u^n_l)
  \mbox{, set }  u^n_{l+1} = u^n_l + \delta u^n_l.
\f} 迭代可以用旧的时间步长进行初始化，即 $u^n_0 = u^{n-1}$  ，最终会产生分裂公式的第一个方程的解（见上文）。对于这里所考虑的正弦戈登方程的时间离散化，我们有 \f{eqnarray*}
  ...
  F'(u^n_l) &=& 1-k^2\theta^2\Delta + k^2\theta^2\cos\left[\theta u^n_l
                        + (1-\theta) u^{n-1}\right].
\f} 注意，虽然  $F(u^n_l)$  是一个函数，但  $F'(u^n_l)$  是一个算子。
...
\f}注意，我们在涉及拉普拉斯算子的所有项上使用了分项积分和零诺依曼边界条件。此外， $F(\cdot)$ 和 $F'(\cdot)$ 如上定义， $(\cdot,\cdot)_{\Omega}$ 表示域 $\Omega$ 上通常的 $L^2$ 内积，即 $(f,g)_{\Omega} = \int_\Omega fg
\,\mathrm{d}x$  。最后，注意到第一个方程实际上是一个迭代程序的定义，所以它在每个时间步骤中被多次求解，直到满足一个停止标准。

<a name="Discretizationoftheweakformulationinspace"></a><h3>Discretization of the weak formulation in space</h3>

使用有限元法，我们将变量公式在空间中离散化。为此，让 $V_h$ 是一个具有节点基 $\{\varphi_1,\ldots,\varphi_N\}$ 的有限维 $H^1(\Omega)$ -符合的有限元空间（ $\mathrm{dim}\, V_h = N
...
H^1(\Omega)$  。因此，变量公式的有限维版本要求我们在每个时间步长上解决以下矩阵方程。@f{eqnarray*}
...
对于第一个方程，我们可以用什么解法？让我们来看看我们必须反转的矩阵。@f[
  (M+k^2\theta^2(A + N))_{ij} =
  \int_\Omega (1+k^2\theta^2 \cos \alpha)
  \varphi_i\varphi_j \; dx
  + k^2 \theta^2 \int_\Omega \nabla\varphi_i\nabla\varphi_j \; dx,
@f]为一些 $\alpha$ ，这取决于现在和以前的解法。首先，请注意，该矩阵是对称的。此外，如果时间步长 $k$ 足够小，即如果 $k\theta<1$ ，那么该矩阵也将是正定的。在下面的程序中，情况总是这样的，所以我们将使用共轭梯度法和SSOR法作为预处理。然而，我们应该记住，如果我们碰巧使用更大的时间步长，这将失败。幸运的是，在这种情况下，求解器将只是抛出一个异常，表明收敛失败，而不是默默地产生一个错误的结果。如果发生这种情况，那么我们可以简单地用能够处理不确定对称系统的方法取代CG方法。GMRES求解器通常是处理所有 "坏 "线性系统的标准方法，但它也是一个缓慢的方法。可能更好的是利用对称性的求解器，例如，SymmLQ，它也在deal.II中实现。

这个程序对 step-23 和 @ref
step_24 " step-24 "进行了巧妙的优化：如果你仔细阅读上述公式，就会发现，速度 $V$ 只出现在与质量矩阵的乘积中。因此，在 step-23 和 step-24 中，我们有点浪费：在每个时间步骤中，我们将用质量矩阵求解一个线性系统，只是在下一个时间步骤中再次将该系统的解乘以 $M$ 。当然，这是可以避免的，我们在这个程序中就是这样做的。


<a name="Thetestcase"></a><h3>The test case</h3> 


正弦戈登方程有几个分析解，包括一维和二维的。特别是，本程序是计算一个具有单一激波状孤波初始条件的问题的解。 这个解是由Leibbrandt在Phys.Rev.Lett.中给出的。\b 41(7)中给出，并在 <code>ExactSolution</code> 类中实现。
...
因为这样更容易发挥，用于设置&mdash;惊喜！&mdash;我们模拟的初始值的 <code>InitialValues</code> 类只是查询描述初始时值的精确解的类，而不是重复实现一个解函数。<a name="CommProg"></a><h1> The commented program</h1> 
...
关于include文件的解释，读者应该参考示例程序  step-1  到  step-4  。它们的标准顺序是 <code>base</code> -- <code>lac</code> -- <code>grid</code>  --  <code>dofs</code> -- <code>fe</code> -- <code>numerics</code> （因为每一类大致都是建立在前面的基础上），然后是一些用于文件输入/输出和字符串流的C++头文件。
...
整个解决问题的算法被封装在这个类中。和以前的例子程序一样，这个类在声明时有一个模板参数，就是空间维度，这样我们就可以在一个、两个或三个空间维度上求解正弦-戈登方程。关于问题的独立于维度的类封装的更多信息，读者应该参考  step-3  和  step-4  。   
...
最后， <code>output_timestep_skip</code> 变量存储了在生成图形输出之前每次所需的时间步数。这一点在使用细网格（因此也是小的时间步数）时非常重要，在这种情况下，我们会运行大量的时间步数，并创建大量的输出文件，这些文件中的解看起来几乎是一样的。这只会堵塞我们的可视化程序，我们应该避免创建比我们真正感兴趣的更多的输出。因此，如果这个变量被设置为大于1的值 $n$ ，那么只有在每一个 $n$ 的时间步长才会产生输出。
...
在下面两类中，我们首先实现本程序介绍中提到的一维、二维和三维的精确解。如果想通过比较数值解和分析解来测试程序的准确性，这个时空解可能会有独立的意义（但是请注意，程序使用的是有限域，而这些是无界域的分析解）。例如，这可以用 VectorTools::integrate_difference 函数来完成。再次注意（正如在 step-23 中已经讨论过的），我们如何将时空函数描述为依赖于时间变量的空间函数，该变量可以使用FunctionTime基类中的 FunctionTime::set_time() 和 FunctionTime::get_time() 成员函数来设置和查询。
...
在本节的第二部分，我们提供初始条件。我们很懒惰（也很谨慎），不想第二次实现与上面相同的函数。相反，如果我们被询问初始条件，我们创建一个对象 <code>ExactSolution</code> ，将其设置为正确的时间，并让它计算当时精确解的任何值。
...
请注意，如果我们选择显式欧拉时间步进方案（ $\theta = 0$ ），那么我们必须选择一个时间步长 $k \le h$ ，否则该方案不稳定，解中可能出现振荡。Crank-Nicolson方案（ $\theta = \frac{1}{2}$ ）和隐式Euler方案（ $\theta=1$ ）不存在这个缺陷，因为它们是无条件稳定的。然而，即使如此，时间步长也应选择在 $h$ 的数量级上，以获得一个好的解决方案。由于我们知道我们的网格是由矩形的均匀细分而来，我们可以很容易地计算出这个时间步长；如果我们有一个不同的域， step-24 中的技术使用 GridTools::minimal_cell_diameter 也是可以的。
...
请注意，在每个时间步骤中，我们必须把对矩阵和右手边的各种贡献加起来。与 step-23 和 step-24 相比，这需要集合更多的条款，因为它们取决于前一个时间步骤或前一个非线性步骤的解决方案。我们使用函数 <code>compute_nl_matrix</code> 和 <code>compute_nl_term</code> 来做到这一点，而本函数提供了顶层逻辑。
...
首先，我们组装雅各布矩阵 $F'_h(U^{n,l})$  ，其中 $U^{n,l}$ 为方便起见被存储在向量 <code>solution</code> 中。
...
接下来我们计算右手边的向量。这只是介绍中对 $-F_h(U^{n,l})$ 的描述所暗示的矩阵-向量乘积的组合。
...
作为题外话，也许值得研究一下什么阶数的正交公式最适合于这种类型的积分。由于 $\sin(\cdot)$ 不是一个多项式，可能没有正交公式可以准确地积分这些项。通常只需确保右手边的积分达到离散化方案的相同精度即可，但也可能通过选择更精确的正交公式来改善收敛性渐近声明中的常数。
...
这是处理非线性方案的第二个函数。它计算矩阵  $N(\cdot,\cdot)$  ，它出现在  $F(\cdot)$  的雅各布项的非线性项中。正如 <code>compute_nl_term</code> 一样，我们必须让这个函数接收一个 "旧 "和一个 "新 "的解决方案作为输入，下面我们再次分别称之为 $w_{\mathrm{old}}$ 和 $w_{\mathrm{new}}$ 。
...
正如在介绍中所讨论的，该函数使用CG迭代求解器对线性方程组进行求解，该方程组是由牛顿方法的每个迭代的有限元空间离散化产生的，用于求解分裂式的（非线性）第一个方程。该系统的解实际上是 $\delta U^{n,l}$ ，所以它被存储在 <code>solution_update</code> and used to update <code>solution</code> 的 <code>run</code> 函数中。   
...
为了确认初始条件，我们必须使用函数  $u_0(x)$  来计算  $U^0$  。为此，下面我们将创建一个 <code>InitialValues</code> 类型的对象；注意，当我们创建这个对象（它来自 <code>Function</code> 类）时，我们将其内部时间变量设置为 $t_0$ ，以表明初始条件是在 $t=t_0$ 处评估的空间和时间函数。     


然后我们通过使用 <code>VectorTools::project</code> 将 $u_0(x)$ 投影到网格上，产生 $U^0$ 。我们必须使用与 step-21 相同的悬挂节点约束结构： VectorTools::project 函数需要一个悬挂节点约束对象，但要使用它，我们首先需要关闭它。
...
在每个时间步长的开始，我们必须通过牛顿方法求解拆分公式中的非线性方程---即求解 $\delta U^{n,l}$ ，然后计算 $U^{n,l+1}$ ，依此类推。这种非线性迭代的停止标准是： $\|F_h(U^{n,l})\|_2 \le 10^{-6} \|F_h(U^{n,0})\|_2$  。因此，我们需要在第一次迭代中记录残差的规范。         
...
很多时候，特别是对于细网格，我们必须选择相当小的时间步长，以使方案稳定。因此，有很多时间步数在求解过程中 "没有任何有趣的事情发生"。为了提高整体效率--特别是加快程序速度和节省磁盘空间--我们每隔 <code>output_timestep_skip</code> 个时间步数才输出解。
...
这是该程序的主要功能。它创建了一个顶层类的对象，并调用其主函数。如果在执行 <code>SineGordonProblem</code> 类的运行方法时抛出了异常，我们会在这里捕获并报告它们。关于异常的更多信息，读者应该参考  step-6  。
...
显式欧拉时间步进方案（ $\theta=0$ ）对于我们希望解决的问题来说表现得很充分。不幸的是，由于稳定性问题，必须选择一个相当小的时间步长------ $k\sim h/10$ 似乎对我们进行的大多数模拟都有效。另一方面，Crank-Nicolson方案（ $\theta=\frac{1}{2}$ ）是无条件稳定的，并且（至少对于一维呼吸器的情况）我们可以选择大到 $25h$ 的时间步长而不对解决方案产生任何不良影响。隐式欧拉方案（ $\theta=1$ ）是 "指数阻尼 "的，所以它不是解决正弦戈登方程的好选择，因为它是保守的。然而， $\theta$ -方法所提供的连续体中的一些阻尼方案对于消除边界效应引起的虚假振荡很有帮助。
...
我们讨论的第一个例子是正弦-戈登方程的所谓一维（静止）呼吸器解。正如在介绍中提到的，呼吸器有如下闭合形式的表达。\f[
u_{\mathrm{breather}}(x,t) = -4\arctan \left(\frac{m}{\sqrt{1-m^2}} \frac{\sin\left(\sqrt{1-m^2}t +c_2\right)}{\cosh(mx+c_1)} \right),
\f] 其中 $c_1$ 、 $c_2$ 和 $m<1$ 为常数。在下面的模拟中，我们选择了 $c_1=0$  ,  $c_2=0$  ,  $m=0.5$  。此外，我们知道呼吸器的振荡周期是 $2\pi\sqrt{1-m^2}$ ，因此我们选择了 $t_0=-5.4414$ 和 $t_f=2.7207$ ，这样我们可以观察到溶液的三次振荡。然后，取 $u_0(x) = u_{\mathrm{breather}}(x,t_0)$ ， $\theta=0$ 和 $k=h/10$ ，程序计算出以下解。
...
此外，由于呼吸器是正弦戈登方程的分析解，我们可以用它来验证我们的代码，尽管我们必须假设我们选择诺伊曼边界条件引入的误差与数值误差相比很小。在这种假设下，可以使用 VectorTools::integrate_difference 函数来计算数值解和本程序的 <code>ExactSolution</code> 类描述的函数之间的差异。对于上面两幅图所示的模拟，每个时间步长的有限元解的误差的 $L^2$ 准则保持在 $10^{-2}$ 的数量级。因此，我们可以得出结论，数值方法在程序中得到了正确的实现。


<a name="Afew21DSolutions"></a><h3>A few (2+1)D Solutions</h3>


在文献中可以找到的正弦戈登方程在(2+1)D中的唯一分析解是所谓的克氏孤波。它有以下闭合式表达。  @f[
    u(x,y,t) = 4 \arctan \left[a_0 e^{s\xi}\right]
  @f]与@f[
    \xi = x \cos\vartheta + \sin(\vartheta) (y\cosh\lambda + t\sinh \lambda)
  @f]，其中 $a_0$ 、 $\vartheta$ 和 $\lambda$ 为常数。在下面的模拟中，我们选择了 $a_0=\lambda=1$  。请注意，如果 $\vartheta=\pi$ 是静止的，那么它将是一个很好的解决方案，我们可以用它来验证二维的程序，因为没有发生域边界的反射。
...
为此，我们围绕 $z$ 轴旋转上面讨论的扭结解：我们让 $\vartheta=\frac{\pi}{4}$  。后者的结果是一个不与网格对齐的孤波，所以反射会立即在域的边界发生。对于下面所示的模拟，我们采取了 $u_0(x)=u_{\mathrm{kink}}(x,t_0)$ 、 $\theta=\frac{2}{3}$ 、 $k=20h$ 、 $t_0=0$ 和 $t_f=20$  。此外，我们不得不选择 $\theta=\frac{2}{3}$ ，因为对于任何 $\theta\le\frac{1}{2}$ 的边界都会产生振荡，这可能是由于方案而不是方程造成的，因此选择 $\theta$ 的值是在时间步进方案的 "指数阻尼 "频谱中的一个好位置，确保这些振荡不会被产生。
...
正弦-戈登方程的另一个有趣的解决方案（不能通过分析获得）可以通过使用两个一维呼吸器来构建以下可分离的二维初始条件来产生。\f[
  u_0(x) =
...
\f] 其中 $x=(x_1,x_2)\in{R}^2$  ,  $m=0.5<1$  与我们上面讨论的一维情况一样。对于下面的模拟，我们选择了 $\theta=\frac{1}{2}$ ,  $k=10h$ ,  $t_0=-5.4414$ 和 $t_f=2.7207$ 。这个解决方案相当有趣 
...
改变初始条件是有启发的。大多数选择不会导致保持局部的解决方案（在孤子界，这样的解决方案被称为 "静止的"，尽管解决方案确实随时间变化），而是导致解决方案，其中方程的波状特征占主导地位，波从局部初始条件的位置走了。例如，值得玩一玩 <code>InitialValues</code> 类，把对 <code>ExactSolution</code> 类的调用替换成类似这样的函数。@f[
  u_0(x,y) = \cos\left(\frac x2\right)\cos\left(\frac y2\right)
@f] 如果 $|x|,|y|\le \frac\pi 2$  ，而 $u_0(x,y)=0$ 在这个区域之外。

第二个方面是研究该方案是否是能量保全的。对于 @ref
...
@f]（我们在公式中使用 $1-\cos u$ 而不是 $-\cos u$ ，以确保对能量的所有贡献都是正的，从而使衰减解在无界域上具有有限的能量）。

除此以外，还有两个明显的领域。

- 显然，自适应性（即时间自适应网格）会对这样的问题感兴趣。它们的复杂性导致我们再次将其排除在本方案之外，尽管 @ref step_23 "  step-23 "介绍中的一般评论仍然正确。

- 更快的方案来解决这个问题。虽然今天的计算机已经足够快，可以在不长的时间内解决二维甚至三维静止的问题，但时间相关的问题提出了一个完全不同的问题类别。我们在 step-48 中讨论了这个问题，在那里我们展示了如何并行地解决这个问题，而且根本不需要组装或反转任何矩阵。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-25.cc" 。 
