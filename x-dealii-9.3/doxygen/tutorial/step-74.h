...
在本教程中，我们将展示FEInterfaceValues类的用法，该类是为组装非连续加尔金（DG）方法产生的面状项而设计的。FEInterfaceValues类提供了一种简单的方法来获得形状函数的跳变和平均值以及跨单元面的解。本教程包括以下内容。  <ol>   <li>  泊松方程的SIPG方法，在  step-39  和  step-59  中已经使用过。    <li>  使用FEInterfaceValues组装面项，使用 MeshWorker::mesh_loop(), 组装系统矩阵，这与 step-12 类似。    <li>  使用误差估计器进行自适应网格细化。    <li>  两个测试案例：平滑函数的收敛测试和奇异解的适应性网格细化测试。  </ol>   
...
@f]的影响。 为了简单起见，我们在这里假设扩散系数 $\nu$ 是常数。注意，如果 $\nu$ 是不连续的，我们在计算单元面上的跳跃项时需要考虑到这一点。

我们用 ${\mathbb T}_h$ 表示网格， $K\in{\mathbb T}_h$ 是一个网格单元。内部和边界面的集合分别用 ${\mathbb F}^i_h$ 和 ${\mathbb F}^b_h$ 表示。让 $K^0$ 和 $K^1$ 是共享一个面 $f\in F_h^i$ 的两个单元， $\mathbf n$ 是 $K^0$ 的外法向量。然后跳跃算子由 "这里减去那里 "公式给出，@f[
\jump{v} = v^0 - v^1
@f]和平均算子分别为@f[
\average{v} = \frac{v^0 + v^1}{2}
@f]。注意，当 $f\subset \partial \Omega$ 时，我们定义 $\jump{v} = v$ 和 $\average{v}=v$  。使用SIPG的离散化由以下弱式给出（更多细节可以在 @cite di2011mathematical 和其中的参考文献中找到）。
...
在上面的公式中， $\gamma$ 是惩罚常数。为了保证离散矫治性，惩罚常数必须足够大  @cite ainsworth2007posteriori  。对于文献中提出的哪些公式应该被使用，人们并没有真正达成共识。这与 step-47 的 "结果 "部分讨论的情况类似）。人们可以直接挑选一个大的常数，而其他的选择可以是  $(p+1)^2$  或  $p(p+1)$  的倍数。在本代码中，我们遵循  step-39  并使用  $\gamma = p(p+1)$  。


<a name="Aposteriorierrorestimator"></a><h3>A posteriori error estimator</h3>

在这个例子中，稍加修改，我们使用Karakashian和Pascal的误差估计器  @cite karakashian2003posteriori  @f[
...
这里我们用 $\sigma = \gamma/h_f$ 代替 $\gamma^2/h_f$ 来表示 $u_h$ 的跳跃项（ $\eta^2_{f_i}$ 和 $\eta_{f_b}^2$ 中的第一个项）。

为了计算这个估计器，在每个单元格 $K$ 中，我们计算 
...
@f]。 $0.5$ 的系数是由于整体的误差估计值只包括每个内部面一次，所以每个单元格的估计值在相邻的两个单元格中各算一个半的系数。注意，我们计算 $\eta_\text{local}^2$ 而不是 $\eta_\text{local}$ 以简化实现。然后，每个单元的误差估计方被存储在一个全局向量中，其 $l_1$ 准则等于 $\eta^2$  。
...
在第二个测试中，我们在二维的L形域 (GridGenerator::hyper_L) 上选择 Functions::LSingularityFunction 。该解在极坐标中由 $u(r,\phi) = r^{\frac{2}{3}}\sin \left(\frac{2}{3}\phi \right)$ 给出，它在原点有一个奇点。构建了一个误差估计器来检测有大误差的区域，根据该估计器自适应地细化网格。<a name="CommProg"></a> <h1> The commented program</h1>. 
...
<a name="Equationdata"></a> <h3>Equation data</h3> 这里我们定义了两个测试案例：平滑函数的收敛率和l_singularity的 Functions::LSingularityFunction.  。 
...
<a name="TheSIPGLaplaceclass"></a> <h3>The SIPGLaplace class</h3> 在这些准备工作之后，我们继续本程序的主类，称为`SIPGLaplace`。该类的整体结构与其他许多教程程序一样。主要的区别只出现在集合函数的实现上，因为我们使用FEInterfaceValues来集合面项。
...
这里的构造函数将测试案例作为输入，然后确定正确的解决方案和右手边的类。其余的成员变量以明显的方式初始化。
...
<a name="Theassemble_systemfunction"></a> <h3>The assemble_system function</h3> 这里的组装函数与  step-12  和  step-47  中的相似。与手工装配不同的是，我们只需要专注于在每个单元格、每个边界面和每个内部面的装配。单元和面的循环是由 MeshWorker::mesh_loop(). 自动处理的。    
...
最后，我们还需要一个在内部面集合面积分的函数。为了重新初始化FEInterfaceValues，我们需要向FEInterfaceValues的reinit()函数传递单元格、面和子面指数（用于自适应细化）。
...
然后，下面的lambda函数将把数据复制到全局矩阵和右手边。 虽然DG离散化中没有悬空节点约束，但我们定义了一个空的AffineConstraints对象，允许我们使用 AffineConstraints::distribute_local_to_global() 功能。
...
在定义了装配函数后，我们现在可以创建ScratchData和CopyData对象，并将它们与上面的lambda函数一起传递给 MeshWorker::mesh_loop(). 此外，我们需要指定我们要在内部面组装一次。
...
在计算了每个单元的局部贡献后，我们仍然需要一种方法将这些复制到全局向量中，以保存所有单元的误差估计值。
...
<a name="Thecompute_energy_norm_errorfunction"></a> <h3>The compute_energy_norm_error() function</h3> 接下来，我们用能量准则来评估准确性。这个函数类似于上面的误差估计器的集合。这里我们计算由@f[
\|u \|_{1,h}^2 = \sum_{K \in \Gamma_h} \nu\|\nabla u \|_K^2 +
\sum_{f \in F_i} \sigma \| [ u ] \|_f^2 +
\sum_{f \in F_b} \sigma  \|u\|_f^2.
@f]定义的能量准则的平方 因此相应的误差是@f[
...
<a name="Thecompute_errorsfunction"></a> <h3>The compute_errors() function</h3> 我们分别计算 $L_2$ 准则、 $H_1$ 半准则和能量准则中的三个误差。这些误差将被打印到屏幕上，同时也被存储在一个表格中，该表格记录了这些误差是如何随着网格细化而衰减的，并且可以在程序的最后一步输出。
...
<a name="Themainfunction"></a> <h3>The main() function</h3> 下面的 <code>main</code> 函数与之前的例子也类似，不需要注释。
...
理论上，对于多项式度数 $p$ ， $L_2$ 规范和 $H^1$ 半规范的收敛顺序应该是 $p+1$ 和 $p$ ，分别是 $p$ 。我们的数值结果与理论很一致。
...
下图提供了L型域上这个测试案例的误差与自由度数的对数图。为了解释它，让 $n$ 为自由度数，那么在均匀细化的网格上， $h$ 在二维上为 $1/\sqrt{n}$ 阶。结合前面的理论结果，我们可以看到，如果解足够光滑，我们可以预期 $L_2$ 准则的误差为 $O(n^{-\frac{p+1}{2}})$ 阶， $H^1$ 半准则的误差为 $O(n^{-\frac{p}{2}})$  。先验地，我们并不清楚在像我们用于第二个测试案例的自适应细化网格上是否会得到与 $n$ 的函数相同的行为，但我们当然可以希望。事实上，从图中我们看到，带有自适应网格细化的SIPG产生了渐进式的希望结果。

  <img width="600px" src="https://www.dealii.org/images/steps/developer/step-74.log-log-plot.png" alt="">   

此外，我们观察到误差估计器的下降速度几乎与能量准则和 $H^1$ 半准则的误差相同，而且比 $L_2$ 的误差低一阶。这表明它有能力预测具有较大误差的区域。

虽然本教程侧重于实现，但 step-59 教程程序以无矩阵求解技术在计算时间上实现了一个高效的大规模求解器。需要注意的是， step-59 教程目前不能用于包含悬空节点的网格，因为多网格界面矩阵不那么容易确定，但这仅仅是deal.II中一些界面的缺乏，没有什么根本性的问题。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-74.cc"   
