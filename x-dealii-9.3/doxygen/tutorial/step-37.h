...
这个例子展示了如何实现无矩阵方法，即不明确存储矩阵元素的方法，用于超立方体上的可变系数二阶泊松方程。该线性系统将用多网格方法求解，并使用MPI的大规模并行性。

无矩阵方法的主要动机是，在今天的处理器上，对主内存的访问（即对不适合在缓存中的对象）已经成为许多偏微分方程求解器的瓶颈。为了执行基于矩阵的矩阵-向量乘积，现代CPU花在等待数据从内存到达的时间远远多于实际进行浮点乘法和加法的时间。因此，如果我们可以通过重新计算矩阵元素来代替在内存中查找矩阵元素，或者更确切地说，这些条目所代表的运算符&mdash;，我们可能会在整体运行时间方面获胜，即使这需要大量的额外浮点运算。也就是说，用一个微不足道的实现来实现这一点是不够的，我们需要真正关注细节来获得性能。这个教程程序和上面提到的论文展示了如何实现这样一个方案，并演示了可以获得的速度提升。
...
\partial \Omega @f}，其中 $a(\mathbf x)$ 是一个可变系数。下面，我们解释如何在不明确形成矩阵的情况下实现这个问题的矩阵-向量乘积。当然，对于其他方程也可以用类似的方法进行构造。
...
为了找出我们如何编写一个执行矩阵-向量乘积的代码，但不需要存储矩阵元素，让我们先看看有限元矩阵<i>A</i>是如何组装的。@f{eqnarray*}
...
如果我们要进行矩阵-向量乘积，我们可以使用@f{eqnarray*}
...
其中<i>u</i><sub>cell</sub>是<i>u</i>在各单元自由度上的值，而<i>v</i><sub>cell</sub>=<i>A</i><sub>cell</sub><i>u</i><sub>cell</sub>相应地表示出结果。因此，实现拉普拉斯的局部作用的一个天真的尝试是使用以下代码。
...
这里我们忽略了边界条件以及我们可能有的任何挂起的节点，尽管使用AffineConstraints类来包括这两者都不是很困难。请注意，我们首先以通常的方式生成局部矩阵，作为每个局部矩阵项的所有正交点的总和。为了形成上述公式中表达的实际乘积，我们提取细胞相关自由度的 <code>src</code> 的值（<i>P</i><sub>cell,loc-glob</sub>的作用），乘以局部矩阵（<i>A</i> ]<sub>cell</sub>），最后把结果加到目标向量 <code>dst</code> （<i>P</i><sub>cell,loc-glob</sub><sup>T</sup>的动作，加在所有元素上）。原则上说，这并不难。

虽然这段代码是完全正确的，但它非常慢。对于每个单元，我们生成一个局部矩阵，这需要三个嵌套的循环，循环长度等于局部自由度的数量来计算。然后，乘法本身是由两个嵌套循环完成的，这意味着它要便宜得多。

改进的方法之一是认识到，从概念上讲，局部矩阵可以被认为是三个矩阵的乘积，@f{eqnarray*}
A_\mathrm{cell} = B_\mathrm{cell}^T D_\mathrm{cell} B_\mathrm{cell},
@f}

其中对于拉普拉斯算子的例子，<i>q</i>*dim+<i>d,i</i>）的第1个元素<i>B</i><sub>cell</sub>由 <code>fe_values.shape_grad(i,q)[d]</code> 给出。这个矩阵由 <code>dim*n_q_points</code> 行和 @p dofs_per_cell 列组成。矩阵<i>D</i><sub>cell</sub>是对角线，包含 <code>fe_values.JxW(q) * coefficient_values[q]</code> 的值（或者说，每个值的@p dim副本）。这种有限元矩阵的表示方法经常可以在工程文献中找到。

当单元格矩阵应用于一个矢量时，@f{eqnarray*}
A_\mathrm{cell}\cdot u_\mathrm{cell} = B_\mathrm{cell}^T
D_\mathrm{cell} B_\mathrm{cell} \cdot u_\mathrm{cell},
@f}

就不会形成矩阵-矩阵的乘积，而是每次将一个矩阵与一个向量从右到左相乘，这样就只形成三个连续的矩阵-向量乘积。这种方法去掉了局部矩阵计算中的三个嵌套循环，从而将一个单元格的工作复杂度从类似 $\mathcal
{O}(\mathrm{dofs\_per\_cell}^3)$ 降低到 $\mathcal
{O}(\mathrm{dofs\_per\_cell}^2)$  。对这种算法的解释是，我们首先将本地DoF上的值向量转化为正交点上的梯度向量。在第二个循环中，我们把这些梯度乘以积分权重和系数。第三个循环应用第二个梯度（转置形式），这样我们就得到了一个单元斗室上的（拉普拉斯）值矢量。

上述代码的瓶颈是对每一个 <code>cell</code> 调用 FEValues::reinit 所做的操作，它所花费的时间和其他步骤加起来一样多（至少如果网格是非结构化的；deal.II可以识别梯度在结构化网格上通常是不变的）。这当然不理想，我们希望能做得更好。reinit函数所做的是计算实空间的梯度，使用从实空间到参考单元的转换的Jacobian来转换参考单元上的梯度。这是对单元上的每个基函数和每个正交点进行的。雅各布系数并不取决于基函数，但它在不同的正交点上通常是不同的。如果你只建立一次矩阵，就像我们在以前所有的教程程序中所做的那样，没有什么需要优化的，因为 FEValues::reinit 需要在每个单元上调用。在这个过程中，转换是在计算本地矩阵元素时应用的。

然而，在无矩阵的实现中，我们将非常频繁地计算这些积分，因为迭代求解器在求解过程中会多次应用矩阵。因此，我们需要考虑是否可以缓存一些在运算器应用中被重用的数据，也就是积分计算。另一方面，我们意识到我们不能缓存太多的数据，否则我们又回到了内存访问成为主导因素的情况。因此，我们不会在矩阵<i>B</i>中存储转换后的梯度，因为一般来说，对于曲线网格的每个基函数和每个元素上的正交点，它们都是不同的。

诀窍是去掉雅各布变换的因素，首先只在参考单元上应用梯度。这个操作将局部道夫上的值向量内插到正交点上的（单位坐标）梯度向量。在这里，我们首先应用我们从梯度中分解出来的Jacobian，然后应用正交的权重，最后应用转置的Jacobian来准备第三个循环，通过单元格上的梯度测试并对正交点进行求和。

让我们再次用矩阵的方式来写。让矩阵<i>B</i><sub>cell</sub>表示单元格相关的梯度矩阵，每一行都包含正交点上的值。它由矩阵-矩阵乘积构建为@f{eqnarray*} B_\mathrm{cell} =
J_\mathrm{cell}^{-\mathrm T} B_\mathrm{ref\_cell}, @f}，其中<i>B</i><sub>ref_cell</sub>表示参考单元的梯度，<i>J</i> ]<sup>-T</sup><sub>cell</sub>表示从单位到实数单元转换的反转置Jacobian（在转换语言中，由<i>J</i><sup>-T</sup><sub>cell</sub>代表的操作是协变转换）。<i>J</i><sup>-T</sup><sub>cell</sub>是块对角线的，块的大小等于问题的维度。每个对角线块都是雅各布变换，从参考单元到实际单元。
...
注意我们是如何为参考单元梯度创建一个额外的FEValues对象，以及如何将其初始化为参考单元的。然后，实际的导数数据由反转的雅各布矩阵应用（deal.II称从实数到单元格的雅各布矩阵为inverse_jacobian，因为正向转换是从单元格到实数的）。因子 $J_\mathrm{cell}^{-1} D_\mathrm{cell} J_\mathrm{cell}^{-\mathrm T}$ 是块对角线超过正交的。在这种形式下，人们意识到可变系数（可能通过张量表示）和一般网格拓扑的雅各布变换对变换单位格导数的系数有类似的影响。

在这一点上，人们可能会想，为什么我们要分别存储矩阵 $J_\mathrm{cell}^{-\mathrm T}$ 和系数，而不是只存储完整的系数 $J_\mathrm{cell}^{-1} D_\mathrm{cell}
J_\mathrm{cell}^{-\mathrm T}$  。后者会使用更少的内存，因为张量是对称的，在三维中具有六个独立的值，而对于前者，我们需要九个条目用于反转的雅各布系数，一个用于正交权重和雅各布行列式，一个用于系数，总共是11个双项。原因是前者允许通过一个共同的缓存数据框架来实现通用的微分算子，而后者则专门存储拉普拉斯的系数。如果应用需要，这种专门化可能会得到回报，值得考虑。请注意，deal.II中的实现足够聪明，可以检测笛卡尔或仿生几何，其中雅各布系数在整个单元中是恒定的，不需要为每个单元存储（实际上在不同的单元中也常常是相同的）。

从操作数的角度来看，最后的优化是利用基函数中的张量积结构，这是最为关键的。这是可能的，因为我们已经从<i>B</i><sub>ref_cell</sub>描述的参考单元操作中剔除了梯度，即对参考单元的完全规则的数据域进行插值操作。我们举例说明在两个空间维度上降低复杂度的过程，但同样的技术也可以用在更高的维度上。在参考单元上，基函数是张量积形式的  $\phi(x,y,z) = \varphi_i(x) \varphi_j(y)$  。矩阵<i>B</i><sub>ref_cell</sub>计算第一分量的部分具有 $B_\mathrm{sub\_cell}^x = B_\mathrm{grad,x} \otimes B_\mathrm{val,y}$ 形式，其中<i>B</i><sub>grad,x</sub>和<i>B</i><sub>val,y</sub>包含所有一维正交点上所有一维基函数的评估。用含有属于基函数 $\varphi_i(x) \varphi_j(y)$ 的系数的<i>U(j,i)</i>组成矩阵<i>U</i>，我们得到 $(B_\mathrm{grad,x} \otimes
B_\mathrm{val,y})u_\mathrm{cell} = B_\mathrm{val,y} U B_\mathrm{grad,x}$  。这就把计算这个乘积的复杂度从 $p^4$ 降低到 $2 p^3$ ，其中<i>p</i>-1是有限元的度数（即，相当于，<i>p</i>是每个坐标方向上的形状函数的数量），或者一般来说 $p^{2d}$ 到 $d p^{d+1}$ 。我们之所以用多项式度数来看复杂度，是因为我们希望能够到高度数，可能增加多项式度数<i>p</i>，而不是网格分辨率。像这里使用的中等度数的好算法是独立于维度的线性多项式度数，而不是基于矩阵的方案或通过FEValues的天真评价。自20世纪80年代以来，deal.II的实现中所使用的技术已经在谱元界建立起来。

实现无矩阵和基于单元的有限元算子，与以前的教程程序中显示的通常的矩阵装配代码相比，需要有一些不同的程序设计。做到这一点的数据结构是MatrixFree类和FEEvaluation类，前者收集所有数据并对所有单元发出（并行）循环，后者利用张量积结构评估有限元基函数。

本教程中展示的无矩阵的矩阵-向量乘积的实现比使用稀疏矩阵的线性元素的矩阵-向量乘积要慢，但由于张量乘积结构降低了复杂度，并且在计算过程中减少了内存传输，所以对所有高阶元素来说都更快。当在一个多核处理器上工作时，减少内存传输的影响特别有利，因为在这个处理器上有几个处理单元共享内存的访问。在这种情况下，一个受计算约束的算法将显示出几乎完美的并行加速（除了可能通过涡轮模式改变处理器的时钟频率，这取决于有多少个核心在工作），而一个受内存传输约束的算法可能无法实现类似的加速（即使工作是完全并行的，我们可以期待像稀疏矩阵-向量产品那样的完美缩放）。这种实现方式的另一个好处是，我们不必建立稀疏矩阵本身，这也可能是相当昂贵的，这取决于基础微分方程。此外，上述框架可以简单地推广到非线性运算，正如我们在 step-48 中所证明的那样。


<a name="Combinationwithmultigrid"></a><h3>Combination with multigrid</h3>


以上，我们花了很大的力气来实现一个不实际存储矩阵元素的矩阵-向量乘积。然而，在许多用户代码中，人们想要的不仅仅是做一些矩阵-向量乘积&mdash；在解决线性系统时，人们想要尽可能少地进行这些操作。理论上，我们可以使用CG方法，而不需要预处理；然而，这对拉普拉斯的效率并不高。相反，预调节器是用来提高收敛速度的。不幸的是，大多数比较常用的预处理方法，如SSOR、ILU或代数多网格（AMG）不能在这里使用，因为它们的实现需要了解系统矩阵的元素。

一个解决方案是使用几何多栅方法，如  step-16  所示。它们被认为是非常快的，而且适合于我们的目的，因为所有的成分，包括不同网格层之间的转移，都可以用与单元格集合相关的矩阵-向量产品来表达。我们需要做的就是找到一个基于矩阵-向量乘积而不是所有矩阵条目的平滑器。一个这样的候选方法是阻尼雅可比迭代，它需要访问矩阵对角线，但它在阻尼所有高频误差方面往往不够好。雅可比方法的特性可以通过所谓的切比雪夫迭代进行几次改进。切比雪夫迭代由矩阵-向量乘积的多项式表达式描述，其中的系数可以被选择来实现某些特性，在这种情况下，可以平滑误差的高频成分，这些误差与雅可比预处理矩阵的特征值相关。在零度时，具有最佳阻尼参数的雅可比方法被检索出来，而高阶修正被用来改善平滑特性。切比雪夫平滑法在多网格中的有效性已经被证明，例如，在文章<a href="http://www.sciencedirect.com/science/article/pii/S0021999103001943">
...
因此，在多网格框架中的实现是很简单的。本程序中的多网格实现与 step-16 类似，包括自适应性。


<a name="UsingCPUdependentinstructionsvectorization"></a><h3>Using CPU-dependent instructions (vectorization)</h3>


FEEvaluation中用于评估的计算核是以优化使用计算资源的方式来编写的。为了达到这个目的，它们不对双数据类型进行操作，而是对我们称之为VectorizedArray的东西进行操作（例如，查看 FEEvaluationBase::get_value, 的返回类型，对于标量元素来说是VectorizedArray，对于矢量有限元素来说是Tensor of VectorizedArray）。VectorizedArray是一个双数或浮点数的短数组，其长度取决于使用的特定计算机系统。例如，基于x86-64的系统支持流式SIMD扩展（SSE），处理器的矢量单元可以通过一条CPU指令处理两个双数（或四个单精度浮点数）。较新的处理器（大约从2012年起）支持所谓的高级向量扩展（AVX），有256位操作数，可以分别使用四个双数和八个浮点数。矢量化是一个单指令/多数据（SIMD）的概念，也就是说，一条CPU指令被用来同时处理多个数据值。通常情况下，有限元程序不会明确使用矢量化，因为这个概念的好处只体现在算术密集型操作中。大部分典型的有限元工作负载的内存带宽是有限的（对稀疏矩阵和向量的操作），额外的计算能力是无用的。

不过，在幕后，优化的BLAS包可能会严重依赖矢量化。另外，优化的编译器可能会自动将涉及标准代码的循环转化为更有效的矢量化形式（deal.II在矢量更新的常规循环中使用OpenMP SIMD pragmas）。然而，数据流必须非常有规律，才能让编译器产生高效的代码。例如，受益于矢量化的原型操作（矩阵-矩阵乘积）的自动矢量化在大多数编译器上都失败了（截至2012年初编写本教程并在2016年底更新时，gcc和英特尔编译器都无法为 FullMatrix::mmult  ]函数，甚至在更简单的情况下也不行，即矩阵边界是编译时常量而不是 FullMatrix::mmult). 中的运行时常量。主要原因是在最内层循环（即应用矢量化的地方）要处理的信息不一定是矢量长度的倍数，留下部分资源没有使用。此外，有可能被一起处理的数据可能没有以连续的方式布置在内存中，或者没有对处理器需要的地址边界进行必要的对齐。或者编译器可能无法证明数据阵列在一次加载几个元素时不会重叠。

因此，在deal.II的无矩阵实现中，我们选择在最适合于有限元计算的层次上应用矢量化。单元的计算通常对所有单元完全相同（除了从向量读写时使用的间接寻址中的索引），因此SIMD可以用来一次处理几个单元。在下面的所有内容中，你可以考虑用一个向量数组来保存几个单元的数据。记住，它与空间维度和元素数量无关，例如在Tensor或Point中。

请注意，矢量化取决于代码所运行的CPU和代码所编译的对象。为了给你的计算机生成最快的FEEvaluation内核，你应该用所谓的<i>native</i>处理器变体编译deal.II。当使用gcc编译器时，可以通过在cmake构建设置中设置变量<tt>CMAKE_CXX_FLAGS</tt>为<tt>"-march=native"</tt>来启用它（在命令行中，指定<tt>-DCMAKE_CXX_FLAGS="-march=native"</tt>，更多信息见deal.II阅读手册）。其他编译器也有类似的选项。我们在这个例子的run()函数中输出当前的矢量化长度。


<a name="Runningmultigridonlargescaleparallelcomputers"></a><h3>Running multigrid on large-scale parallel computers</h3>


如上所述，无矩阵框架中的所有组件都可以通过MPI使用域分解轻松实现并行化。由于在deal.II中通过p4est（详见 step-40 ）很容易获得大规模的并行网格，而且基于单元的循环与无矩阵评估<i>only</i>需要在每个处理器上将网格分解成大小基本相同的块，因此编写一个与分布式内存一起工作的并行程序相对来说不需要做什么。虽然其他使用MPI的教程程序依赖于PETSc或Trilinos，但本程序使用deal.II自己的并行向量设施。

deal.II的并行向量类， LinearAlgebra::distributed::Vector, 持有解决方案的处理器本地部分，以及重影DoF的数据字段，即由远程处理器拥有的DoF，但由当前处理器拥有的单元访问。在 @ref GlossLocallyActiveDof "术语表 "中，这些自由度被称为本地活动自由度。函数 MatrixFree::initialize_dof_vector() 提供了一个设置这种设计的方法。请注意，悬挂的节点可以与额外的重影自由度有关，这些自由度必须包括在分布式矢量中，但不属于 @ref
GlossLocallyActiveDof "词汇表 "意义上的局部活动自由度的一部分。此外，分布式向量持有本地拥有但被其他处理器需要的DoF的MPI元数据。这个向量类设计的一个好处是对重影项的访问方式。在向量的存储方案中，数据阵列延伸到解决方案的处理器本地部分之外，有更多的向量条目可用于重影自由度。这为所有本地活动自由度提供了一个连续的索引范围。(注意，索引范围取决于网格的具体配置。)由于无矩阵操作可以被认为是在做性能关键的线性代数，而性能关键的代码不能把时间浪费在做MPI全局到MPI局部的索引转换上，一个MPI等级的局部索引空间的可用性是很重要的。这里访问事物的方式是直接数组访问。这是通过 LinearAlgebra::distributed::Vector::local_element(), 提供的，但实际上很少需要，因为所有这些都发生在FEEvaluation的内部。

 LinearAlgebra::distributed::Vector 的设计与我们之前在 step-40 和 step-32 中使用的 PETScWrappers::MPI::Vector 和 TrilinosWrappers::MPI::Vector 数据类型类似，但由于我们不需要这些库的其他并行功能，我们使用deal.II的 LinearAlgebra::distributed::Vector 类而不是在这个教程程序中链接另一个大型库。还要注意的是，PETSc和Trilinos向量不提供对直接数组访问的幽灵条目的细粒度控制，因为它们抽象出了必要的实现细节。<a name="CommProg"></a> <h1> The commented program</h1>
...
为了提高效率，在无矩阵实现中进行的操作需要在编译时了解循环长度，这些长度是由有限元的度数给出的。因此，我们收集了两个模板参数的值，可以在代码中的一个地方改变。当然，我们可以把有限元的度数作为一个运行时的参数，通过编译所有可能的度数（比如，1到6之间）的计算核，并在运行时选择合适的核。在这里，我们只是选择二阶 $Q_2$ 元素并选择维度3作为标准。
...
我们为泊松问题定义一个可变系数函数。它类似于 step-5 中的函数，但我们使用 $a(\mathbf
x)=\frac{1}{0.05 + 2\|\bf x\|^2}$ 的形式，而不是不连续的。这只是为了证明这种实现的可能性，而不是在物理上有什么意义。我们定义系数的方式与早期教程程序中的函数相同。有一个新函数，即带有模板参数 @p number. 的 @p value 方法。 
...
在函数实现中，我们假设数字类型重载了基本的算术运算，所以我们只需像平常一样写代码。然后，基类函数 @p value 是由带有双倍类型的模板函数计算而来，以避免重复代码。
...
下面这个类被称为 <code>LaplaceOperator</code> ，实现了微分运算符。就所有的实用目的而言，它是一个矩阵，也就是说，你可以询问它的大小（成员函数  <code>m(), n()</code>  ），你可以将它应用于一个矢量（ <code>vmult()</code>  函数）。当然，与实数矩阵的区别在于，这个类实际上并不存储矩阵的<i>elements</i>，而只知道如何计算运算符应用于向量时的动作。   


描述矩阵大小、从MatrixFree对象初始化以及通过vmult()和Tvmult()方法实现矩阵-向量乘积的各种接口的基础结构，是由这个类派生的 MatrixFreeOperator::Base 类提供的。这里定义的LaplaceOperator类只需要提供几个接口，即通过vmult()函数中使用的apply_add()方法来实现运算符的实际操作，以及计算底层矩阵对角线项的方法。我们需要对角线来定义多梯度平滑器。由于我们考虑的是一个具有可变系数的问题，我们进一步实现了一个可以填充系数值的方法。   


请注意，文件 <code>include/deal.II/matrix_free/operators.h</code> 中已经包含了通过类 MatrixFreeOperators::LaplaceOperator. 实现拉普拉斯的方法。出于教育目的，在这个教程程序中重新实现了这个算子，解释了其中的成分和概念。   


这个程序利用了集成在deal.II中的有限元算子应用的数据缓存。这个数据缓存类被称为MatrixFree。它包含局部和全局自由度之间的映射信息（Jacobian）和索引关系。它还包含约束条件，如来自悬挂节点或迪里切特边界条件的约束。此外，它可以在所有单元上以%并行方式发出一个循环，确保只有不共享任何自由度的单元被处理（这使得循环在写入目标向量时是线程安全的）。与 @ref threads 模块中描述的WorkStream类相比，这是一个更先进的策略。当然，为了不破坏线程安全，我们在写入类全局结构时必须小心。   
...
顺便提一下，如果我们在同一个网格和自由度上实现了几个不同的操作（比如质量矩阵和拉普拉斯矩阵），我们将为每个操作者定义两个像现在这样的类（源自 MatrixFreeOperators::Base 类），并让它们都引用一般问题类中的同一个MatrixFree数据缓存。通过 MatrixFreeOperators::Base 的接口要求我们只提供一组最小的函数。这个概念允许编写具有许多无矩阵操作的复杂应用代码。   
...
为了初始化系数，我们直接赋予它上面定义的系数类，然后选择方法 <code>coefficient_function.value</code> 与矢量数（编译器可以从点数据类型中推断出来）。下面将解释FEEvaluation类（及其模板参数）的使用。
...
这里是这个类的主要功能，矩阵-向量乘积的评估（或者，一般来说，有限元算子评估）。这是在一个函数中完成的，它需要四个参数，MatrixFree对象，目标和源向量，以及要处理的单元格范围。MatrixFree类中的方法 <code>cell_loop</code> 将在内部用一些单元格范围来调用这个函数，这些单元格范围是通过检查哪些单元格可以同时工作来获得的，这样写操作就不会引起任何竞赛条件。请注意，循环中使用的单元格范围并不是直接指当前网格中的（活动）单元格数量，而是一个单元格批次的集合。 换句话说，"单元 "可能是一个错误的开始，因为FEEvaluation将几个单元的数据分组在一起。这意味着在正交点的循环中，我们实际上是将几个单元的正交点作为一个块来看待。这样做是为了实现更高的矢量化程度。 这种 "单元 "或 "单元批 "的数量存储在MatrixFree中，可以通过 MatrixFree::n_cell_batches(). 查询。与deal.II单元迭代器相比，在这个类中，所有的单元都被布置在一个普通的数组中，不直接知道水平或相邻关系，这使得通过无符号整数来索引单元成为可能。   


拉普拉斯运算符的实现非常简单。首先，我们需要创建一个对象FEEvaluation，它包含计算核，并有数据字段来存储临时结果（例如，在几个单元格集合上的所有正交点上评估的梯度）。请注意，临时结果不会使用大量的内存，而且由于我们用元素顺序指定模板参数，数据被存储在堆栈中（没有昂贵的内存分配）。通常，只需要设置两个模板参数，维度作为第一个参数，有限元的度数作为第二个参数（这等于每个维度的自由度数减去FE_Q元素的一个）。然而，在这里我们也希望能够使用浮点数来进行多网格预处理，这是最后一个（第五个）模板参数。因此，我们不能依赖默认的模板参数，因此必须填写第三和第四个字段。第三个参数指定每个方向的正交点的数量，其默认值等于元素的度数加1。第四个参数设置分量的数量（在PDEs系统中也可以评估向量值函数，但默认是标量元素），最后一个参数设置数字类型。   
...
这个函数实现了对 Base::apply_add() 接口的所有单元的循环。这是用MatrixFree类的 @p cell_loop 完成的，它接受该类的operator()，参数为MatrixFree, OutVector, InVector, cell_range。当使用MPI并行化工作时（但没有线程），如本教程程序中所做的，单元格循环对应于以下三行代码。   
...
下面的函数实现了算子对角线的计算。计算无矩阵算子评估的矩阵项，结果比评估算子更复杂。从根本上说，我们可以通过在<i>all</i>单位向量上应用算子来获得算子的矩阵表示。当然，这将是非常低效的，因为我们需要进行<i>n</i>运算符的评估来检索整个矩阵。此外，这种方法会完全忽视矩阵的稀疏性。然而，在单个单元上，这是一种方法，而且实际上效率并不低，因为单元内的所有自由度之间通常存在着耦合。   


我们首先将对角线向量初始化为正确的并行布局。这个向量被封装在基类 MatrixFreeOperators::Base. 中DiagonalMatrix类型的一个名为inverse_diagonal_entries的成员中，这个成员是一个共享指针，我们首先需要初始化，然后得到代表矩阵中对角线条目的向量。至于实际的对角线计算，我们再次使用MatrixFree的cell_loop基础设施来调用一个名为local_compute_diagonal()的本地工作程序。由于我们只写进一个向量，而没有任何源向量，我们用一个<tt>unsigned int</tt>类型的假参数来代替源向量，以便与cell_loop接口确认。在循环之后，我们需要将受Dirichlet边界条件约束的向量条目设置为1（要么是MatrixFree内部AffineConstraints对象描述的边界上的条目，要么是自适应多网格中不同网格层次之间的索引）。这是通过函数 MatrixFreeOperators::Base::set_constrained_entries_to_one() 完成的，并与Base算子提供的矩阵-向量乘积中的设置相匹配。最后，我们需要反转对角线条目，这是基于Jacobi迭代的Chebyshev平滑器所要求的形式。在循环中，我们断言所有条目都是非零的，因为它们应该从积分中获得了正的贡献，或者被约束并被 @p set_constrained_entries_to_one() 下面的cell_loop处理。
...
在本地计算循环中，我们通过循环本地矩阵的所有列来计算对角线，并将条目1放在<i>i</i>的槽中，将零条目放在所有其他槽中，也就是说，我们一次对一个单位向量应用单元微分算子。调用 FEEvaluation::evaluate, 的内部部分是对正交点的循环，而 FEEvalution::integrate, 则与local_apply函数完全相同。之后，我们挑选出本地结果的第<i>i</i>个条目，并将其放入一个临时存储器（因为我们在下一次循环迭代时覆盖了 FEEvaluation::get_dof_value() 后面数组中的所有条目）。最后，临时存储被写到目标向量中。注意我们是如何使用 FEEvaluation::get_dof_value() 和 FEEvaluation::submit_dof_value() 来读取和写入FEEvaluation用于积分的数据字段，并在另一方面写入全局向量中。   


鉴于我们只对矩阵的对角线感兴趣，我们干脆扔掉一路走来计算过的本地矩阵的所有其他条目。虽然计算完整的单元格矩阵，然后扔掉除对角线以外的所有东西看起来很浪费，但整合的效率很高，所以计算并没有花费太多时间。请注意，对于多项式程度来说，每个元素的算子评估的复杂度是 $\mathcal
...
O((p+1)^{d+1})$ 操作中的和因数化技术来计算对角线，这涉及到特别适应的内核&mdash;但由于这种内核只在特定的环境下有用，而对角线计算通常不在关键路径上，所以它们没有在deal.II中实现。)    


请注意，在向量上调用distribution_local_to_global以将对角线条目累积到全局矩阵的代码有一些限制。对于具有悬挂节点约束的操作者来说，将一个受约束的DoF的积分贡献分配到distribution_local_to_global调用中的其他几个条目上，这里使用的向量接口并不完全计算对角线条目，而是将一些位于本地矩阵对角线上的贡献，最终在全球矩阵的非对角线位置上堆积到对角线上。正如<a
href="http://dx.doi.org/10.4208/cicp.101214.021015a">Kormann (2016),
section 5.3</a>中所解释的，该结果在离散化精度上是正确的，但在数学上并不平等。在这个教程程序中，不会发生任何危害，因为对角线只用于没有悬挂节点约束出现的多网格水平矩阵。
...
这个类是基于  step-16  中的一个类。然而，我们用我们的无矩阵实现取代了SparseMatrix<double>类，这意味着我们也可以跳过稀疏性模式。请注意，我们用有限元的度数作为模板参数来定义LaplaceOperator类（该值在文件的顶部定义），并且我们用浮点数来表示多网格级的矩阵。   


该类还有一个成员变量，用来记录在我们真正去解决问题之前设置整个数据链的所有详细时间。此外，还有一个输出流（默认情况下是禁用的），可以用来输出各个设置操作的细节，而不是默认情况下仅打印出的摘要。   


由于这个程序被设计成与MPI一起使用，我们还提供了通常的 @p pcout 输出流，只打印MPI等级为0的处理器的信息。这个程序使用的网格可以是基于p4est的分布式三角图（在deal.II被配置为使用p4est的情况下），否则就是一个只在没有MPI的情况下运行的串行网格。
...
接下来，初始化所有层次上的多网格方法的矩阵。数据结构MGConstrainedDoFs保留了受边界条件制约的指数信息，以及不同细化层次之间的边缘指数，如 step-16 教程程序中所述。然后，我们穿过网格的各个层次，在每个层次上构造约束和矩阵。这些紧跟原始网格上的系统矩阵的构造，除了在访问层级信息而不是活动单元的信息时，在命名上略有不同。
...
组装功能非常简单，因为我们所要做的就是组装右手边。多亏了FEEvaluation和所有缓存在MatrixFree类中的数据，我们从 MatrixFreeOperators::Base, 中查询，这可以在几行中完成。由于这个调用没有被包装成一个 MatrixFree::cell_loop （这将是一个替代方案），我们必须不要忘记在装配的最后调用compress()，将右手边的所有贡献发送给各自自由度的所有者。
...
在第0层，我们以不同的方式初始化平滑器，因为我们想使用切比雪夫迭代作为求解器。PreconditionChebyshev允许用户切换到求解器模式，其中迭代次数在内部选择为正确值。在附加数据对象中，通过选择多项式程度为 @p numbers::invalid_unsigned_int. 来激活这一设置，然后算法将攻击粗级矩阵中最小和最大之间的所有特征值。切比雪夫平滑器的步数是这样选择的：切比雪夫收敛估计值保证将残差减少到变量@p smoothing_range中指定的数量。请注意，对于求解来说， @p smoothing_range 是一个相对的公差，并且选择小于1，在这种情况下，我们选择三个数量级，而当只有选定的特征值被平滑时，它是一个大于1的数字。     


从计算的角度来看，只要粗粒度适中，Chebyshev迭代是一个非常有吸引力的粗粒度求解器。这是因为Chebyshev方法只执行矩阵-向量乘积和向量更新，这通常比其他迭代方法中涉及的内积更好地并行到具有几万个核心的最大集群规模。前者只涉及（粗）网格中邻居之间的局部通信，而后者需要在所有处理器上进行全局通信。
...
对于这些接口矩阵的实现，已经有一个预定义的类 MatrixFreeOperators::MGInterfaceOperator ，它将例程 MatrixFreeOperators::Base::vmult_interface_down() 和 MatrixFreeOperators::Base::vmult_interface_up() 包装在一个带有@p vmult()和 @p Tvmult() 操作的新类中（最初是为矩阵编写的，因此期待这些名称）。请注意，vmult_interface_down是在多网格V型循环的限制阶段使用的，而vmult_interface_up是在延长阶段使用的。     
...
多网格程序的设置非常简单，与 step-16 相比，在求解过程中看不到任何区别。所有的魔法都隐藏在 LaplaceOperator::vmult 操作的实现背后。请注意，我们通过标准输出打印出求解时间和累积的设置时间，也就是说，在任何情况下，而设置操作的详细时间只有在构造函数中的detail_times标志被改变的情况下才会打印。
...
这里是数据输出，它是  step-5  的简化版本。我们对细化过程中产生的每个网格使用标准的VTU（=压缩的VTK）输出。此外，我们还使用了一种针对速度而不是磁盘使用量进行优化的压缩算法。默认设置（针对磁盘使用的优化）使得保存输出的时间是运行线性求解器的4倍，而设置 DataOutBase::VtkFlags::compression_level 到 DataOutBase::VtkFlags::best_speed 则将其降低到只有线性求解的四分之一的时间。   
...
在运行程序之前，我们输出一些关于检测到的矢量化水平的信息，正如在介绍中讨论的那样。
...
由于这个例子解决的是与 step-5 相同的问题（除了系数不同），所以关于解决方案没有什么可说的。我们还是展示一张图片，通过等值线和体积渲染来说明解决方案的大小。

  <img src="https://www.dealii.org/images/steps/developer/step-37.solution.png" alt="">   

更感兴趣的是评估多网格求解器的某些方面。当我们在二维运行这个程序时，对于二次元（ $Q_2$ ），我们得到以下输出（当在一个核心上以发布模式运行）。
...
与 step-16 一样，我们看到随着自由度的增加，CG的迭代次数保持不变。恒定的迭代次数（加上最佳的计算特性）意味着计算时间随着问题大小从一个周期到另一个周期的四倍而大约翻了两番。该代码在存储方面也非常有效。大约200-400万个自由度适合于1GB的内存，也见下面的MPI结果。一个有趣的事实是，尽管没有建立矩阵，但解决一个线性系统比设置要便宜（大约一半的时间花在 DoFHandler::distribute_dofs() 和 DoFHandler::distribute_mg_dofs() 的调用上）。这表明这种方法的效率很高，但也表明deal.II数据结构的设置相当昂贵，设置成本必须在几个系统求解中摊销。

如果我们在三个空间维度上运行程序，则变化不大。由于我们使用统一的网格细化，我们得到的元素数量是八倍，每个周期的自由度大约是八倍。
...
由于某个网格上的 $\mathcal Q_4$ 个元素对应于一半网格大小的 $\mathcal Q_2$ 个元素，我们可以将第四周期使用四度多项式的运行时间与第五周期使用二次多项式的运行时间进行比较，两者都是210万自由度。令人惊讶的效果是，尽管多用了一次线性迭代， $\mathcal Q_4$ 元素的求解器实际上比四次方的情况略快。高阶多项式的速度与低阶多项式类似，甚至比低阶多项式更快，这种效果是通过和分解进行无矩阵算子评估的主要优势之一，见<a
href="http://dx.doi.org/10.1016/j.compfluid.2012.04.012">matrix-free
paper</a>。这与基于矩阵的方法有根本的不同，后者随着多项式度数的增加和耦合的密集，每个未知数的成本会越来越高。

此外，高阶的设置也会变得更便宜一些，这是因为需要设置的元素更少。

最后，让我们看看度数为8的时间，这相当于低阶方法的另一轮网格细化。
...
在这里，初始化似乎比以前慢得多，这主要是由于矩阵对角线的计算，实际上是在每个单元上计算一个729 x 729的矩阵，并丢弃除对角线外的所有东西。然而，求解时间再次非常接近四元案例，表明理论上预期的随着多项式程度的线性增长几乎完全被更好的计算特性和高阶方法在几个单元上的自由度份额较小而增加了评估复杂性的事实所抵消。

<a name="Comparisonwithasparsematrix"></a><h3>Comparison with a sparse matrix</h3> 


为了了解无矩阵实现的能力，我们通过测量问题初始化的计算时间（分布自由度、设置和装配矩阵、设置多网格结构）以及无矩阵变体和基于稀疏矩阵的变体的实际解法，将上述三维例子的性能与基于稀疏矩阵的实现进行了比较。如上图所示，我们将预处理程序建立在浮点数上，将实际的矩阵和向量建立在双数上。测试在英特尔酷睿i7-5500U笔记本处理器上运行（两个核心，支持<a
href="http://en.wikipedia.org/wiki/Advanced_Vector_Extensions">AVX</a>，即用一条CPU指令可以完成对双数的四次操作，这在FEEvaluation中被大量使用），优化模式和两个MPI行列。
...
该表清楚地表明，无矩阵实现的求解速度是两倍以上，而在初始化成本方面则是六倍以上。随着问题大小被放大8倍，我们注意到时间通常也会上升8倍（因为求解器的迭代次数恒定为6次）。主要的偏差是在5k到36k自由度的稀疏矩阵中，时间增加了12倍。这是处理器中的（L3）缓存不能再容纳矩阵-向量乘积所需的所有数据的阈值，所有的矩阵元素必须从主内存中获取。

当然，这种情况不一定适用于所有情况，因为在有些问题上，对矩阵项的了解可以使解算器的效果更好（如当系数的变化比上例中更强烈时）。此外，这也取决于计算机系统。目前的系统具有良好的内存性能，因此稀疏矩阵的性能相当好。尽管如此，对于本例中使用的<i>Q</i><sub>2</sub>元素，无矩阵的实现已经给出了一个不错的速度。这一点对于时间依赖性或非线性问题尤其明显，在这些问题中，稀疏矩阵需要一次又一次地被重新组合，而有了这个类，这就变得容易多了。当然，由于产品的复杂性更好，当元素的阶数增加时，该方法获得了越来越大的优势（无矩阵的实现每个自由度的成本为4<i>d</i><sup>2</sup><i>p</i>，而稀疏矩阵的成本为2<i>p<sup>d</sup></i>，所以无论如何它在阶数为4及以上的3d中会获胜）。
...
正如介绍和代码中的注释所解释的，这个程序可以用MPI并行运行。事实证明，几何多网格方案工作得非常好，可以扩展到非常大的机器。据作者所知，这里显示的几何多网格结果是截至2016年底用deal.II完成的最大计算，在<a
href="https://www.lrz.de/services/compute/supermuc/systemdescription/">complete
SuperMUC Phase 1</a>的多达147456个核心上运行。超过1000个核心的可扩展性的要素是，没有任何依赖于全局问题大小的数据结构被完整地保存在一个处理器上，并且通信不是太频繁，以避免遇到网络的延迟问题。 对于用迭代求解器求解的PDEs，通信延迟往往是限制因素，而不是网络的吞吐量。以SuperMUC系统为例，两个处理器之间的点对点延迟在1e-6到1e-5秒之间，取决于MPI网络中的距离。这一类的矩阵-向量产品与 @p LaplaceOperator 涉及几个点对点的通信步骤，与每个核心上的计算交错进行。由此产生的矩阵-向量乘积的延迟约为1e-4秒。全局通信，例如一个 @p MPI_Allreduce 操作，在MPI网络中的所有等级上累积每个等级的单一数字之和，其延迟为1e-4秒。这个程序中使用的多网格V型循环也是全局通信的一种形式。想一想发生在单个处理器上的粗略网格求解。在开始之前，它积累了来自所有处理器的贡献。当完成后，粗网格解决方案被转移到更细的层次，在那里越来越多的处理器帮助平滑，直到细网格。从本质上讲，这是在网络中的处理器上的一个树状模式，并由网格控制。相对于 @p MPI_Allreduce 的操作，还原中的树是根据MPI网络中的实际链接进行优化的，多网格V型循环是根据网格的划分来进行的。因此，我们不能期望有同样的优化效果。此外，多网格循环并不是简单地在细化树上走来走去，而是在做平滑的时候在每一层上进行通信。换句话说，多网格中的全局通信更具挑战性，与提供较少优化机会的网格有关。测得的V型周期的延迟在6e-3和2e-2秒之间，即与60到200个MPI_Allreduce操作相同。

下图显示了在 $\mathcal Q_3$ 元素上进行的缩放实验。沿着这条线，问题的大小随着核心数量的增加而保持不变。当内核数量增加一倍时，人们期望计算时间减半，灰色虚线表示。结果显示，在达到0.1秒左右的绝对时间之前，该实现显示了几乎理想的行为。解算器的公差已经被设定为解算器执行五次迭代。这种绘制数据的方式是该算法的<b>strong scaling</b>。当我们进入到非常大的核心数时，曲线会提前变平，这是因为SuperMUC的通信网络，距离较远的处理器之间的通信会稍慢一些。

  <img src="https://www.dealii.org/images/steps/developer/step-37.scaling_strong.png" alt="">   

此外，该图还包含了<b>weak scaling</b>的结果，列出了当处理器核数和元素数都以相同速度增加时，算法的表现。在这种情况下，我们期望计算时间保持不变。在算法上，CG的迭代次数恒定在5次，所以我们从这一点来看是好的。图中的线条是这样排列的：每个数据系列中的左上角点代表每个处理器的相同大小，即131,072个元素（或每个核心大约350万个自由度）。表示理想的强缩放的灰色线条相隔8个相同的系数。结果再次表明，缩放比例几乎是理想的。当从288个核到147456个核时，并行效率在75%左右，每个核的局部问题大小为75万自由度，在288个核上需要1.0秒，在2304个核上需要1.03秒，在18000个核上需要1.19秒，在147000个核上需要1.35秒。这些算法对处理器的利用率也达到了很高。在147k核心上最大的计算在SuperMUC上达到约1.7 PFLOPs/s，其中算术峰值为3.2 PFLOPs/s。对于一个迭代式PDE求解器来说，这是一个非常高的数字，而且通常只有密集线性代数才会达到显著的数字。稀疏线性代数被限制在这个数值的十分之一。

正如介绍中提到的，无矩阵方法减少了数据结构的内存消耗。除了由于更少的内存传输而带来的更高的性能外，该算法还允许将非常大的问题装入内存。下图显示了随着我们增加问题的大小，直到计算耗尽内存的上限时的计算时间。我们对1k核、8k核和65k核进行了计算，发现问题的大小几乎可以在两个数量级上进行理想的扩展。这张图中显示的最大的计算涉及2920亿（ $2.92 \cdot 10^{11}$ ）个自由度。在147k核的DG计算中，上述算法也被运行，涉及多达5490亿（2^39）个自由度。

  <img src="https://www.dealii.org/images/steps/developer/step-37.scaling_size.png" alt="">   

最后，我们注意到，在对上述大规模系统进行测试的同时，deal.II中的多网格算法也得到了改进。原始版本包含了基于MGSmootherPrecondition的次优代码，其中一些MPI_Allreduce命令（检查所有向量条目是否为零）在每一层的平滑操作上都要进行，这在65k核以上的系统中才会变得明显。然而，下面的图片显示，改进已经在较小的规模上得到了回报，这里显示的是对 $\mathcal Q_5$ 元素在多达14336个核心上的计算。
...
正如代码中所解释的，这里介绍的算法准备在自适应细化的网格上运行。如果只有部分网格被细化，多网格循环将以局部平滑的方式运行，并沿界面施加迪里希条件，这些界面的细化程度不同，通过 MatrixFreeOperators::Base 类进行平滑。由于自由度在层次上的分布方式，将层次单元的所有者与第一个下级活动单元的所有者联系起来，在MPI中不同的处理器之间可能存在不平衡，这限制了可扩展性，约为2-5倍。
...
如上所述，该代码已准备好用于局部自适应h-精化。对于泊松方程，我们可以采用KellyErrorEstimator类中实现的Kelly误差指标。然而，我们需要小心处理平行向量的鬼魂指数。为了评估误差指标中的跳跃项，每个MPI进程需要知道本地相关的DoF。然而 MatrixFree::initialize_dof_vector() 函数只用一些本地相关的DoF来初始化向量。在向量中提供的鬼魂指数是一个严格的集合，只有那些在单元积分中被触及的指数（包括约束条件的解决）。这种选择有性能上的原因，因为与矩阵-向量乘积相比，发送所有本地相关的自由度会过于昂贵。因此，原样的解决方案向量不适合KellyErrorEstimator类。诀窍是改变分区的幽灵部分，例如使用一个临时向量和 LinearAlgebra::distributed::Vector::copy_locally_owned_data_from() ，如下所示。
...
这个程序只用MPI来并行化。作为一种选择，MatrixFree循环也可以在混合模式下发布，例如，在一个集群的节点上使用MPI并行化，在一个节点的共享内存区域内通过Intel TBB使用线程。要使用这种方式，就需要在主函数的MPI_InitFinalize数据结构中同时设置线程数，并将 MatrixFree::AdditionalData::tasks_parallel_scheme 设置为partition_color，以便真正并行地进行循环。这个用例在  step-48  中讨论。

<a name="InhomogeneousDirichletboundaryconditions"></a><h4> Inhomogeneous Dirichlet boundary conditions </h4>


所介绍的程序假定了同质的Dirichlet边界条件。当去到非均质条件时，情况就有点复杂了。为了理解如何实现这样的设置，让我们首先回顾一下这些条件是如何在数学公式中出现的，以及它们是如何在基于矩阵的变体中实现的。从本质上讲，非均质狄里奇条件将解决方案中的一些节点值设定为给定值，而不是通过变分原理来确定它们，@f{eqnarray*}
...
N_D$  。然后我们把这个解的表示插入到弱的形式中，例如上面所示的拉普拉斯，并把已知量移到右边。@f{eqnarray*}
...
在这个公式中，方程对所有的基函数 $\varphi_i$ 与 $i\in N \setminus \mathcal N_D$ 进行检验，这些基函数与迪里切条件约束的节点无关。

在deal.II的实现中，右侧的积分 $(\nabla \varphi_i,\nabla \varphi_j)_\Omega$ 已经包含在我们在每个单元上组装的局部矩阵贡献中。当使用 AffineConstraints::distribute_local_to_global() 时，如 step-6 和 step-7 教程程序中首次描述的那样，我们可以通过将本地矩阵的列<i>j</i>和行<i>i</i>相乘来说明非均质约束<i>j</i>的贡献。 ] 的局部矩阵根据积分 $(\varphi_i,
\varphi_j)_\Omega$ 乘以不均匀性，然后从全局右手边向量中的位置<i>i</i>中减去所得，也可参见 @ref
constraints 模块。实质上，我们使用一些从方程左侧被消除的积分来最终确定右侧的贡献。当首先将所有条目写进左手边的矩阵，然后通过 MatrixTools::apply_boundary_values(). 消除矩阵的行和列时，也会涉及类似的数学。 

原则上，属于受限自由度的成分可以从线性系统中消除，因为它们不携带任何信息。在实践中，在deal.II中，我们总是保持线性系统的大小不变，以避免处理两种不同的编号系统，避免对两种不同的索引集产生混淆。为了确保在不向受限行添加任何东西时，线性系统不会变得奇异，我们再向矩阵对角线添加假条目，否则与实际条目无关。

在无矩阵方法中，我们需要采取不同的方法，因为@p LaplaceOperator类代表了<b>homogeneous</b>算子的矩阵-向量乘积（最后一个公式的左手边）。 传递给 MatrixFree::reinit() 的AffineConstraints对象是否包含不均匀约束并不重要，只要它代表一个<b>linear</b>算子， MatrixFree::cell_loop() 调用将只解决约束的均匀部分。

在我们的无矩阵代码中，非均质条件的贡献最终的右手边计算与矩阵算子完全解耦，由上述不同的函数处理。因此，我们需要明确地生成进入右手边的数据，而不是使用矩阵装配的副产品。由于我们已经知道如何在一个向量上应用算子，我们可以尝试对一个向量使用这些设施，在那里我们只设置Dirichlet值。
...
或者等价地，如果我们已经将不均匀约束填入AffineConstraints对象。
...
然后我们可以将向量 @p solution 传递给@p LaplaceOperator::vmult_add() 函数，并将新的贡献添加到@p system_rhs向量中，该向量在 @p LaplaceProblem::assemble_rhs() 函数中被填充。然而，这个想法并不奏效，因为vmult()函数内部使用的 FEEvaluation::read_dof_values() 调用假定所有约束条件的值都是同质的（否则运算符就不是线性运算符，而是仿射运算符）。为了同时检索不均匀性的值，我们可以选择以下两种策略中的一种。
...
FEEvaluation类有一个设施，正好解决了这个要求。对于非均质的Dirichlet值，我们确实希望在从向量 @p solution. 中读取数据时跳过隐式施加的均质（Dirichlet）约束。例如，我们可以扩展@p LaplaceProblem::assemble_rhs() 函数来处理非均质的Dirichlet值，如下所示，假设Dirichlet值已被插值到对象中 @p constraints:   
...
在这段代码中，我们用忽略所有约束条件的 FEEvaluation::read_dof_values_plain() 代替了用于暂定解向量的 FEEvaluation::read_dof_values() 函数。由于这种设置，我们必须确保其他约束条件，例如通过悬挂节点，已经正确地分布到输入向量中，因为它们没有像 FEEvaluation::read_dof_values_plain(). 那样被解决。 在循环内部，我们然后评估拉普拉斯，并用 @p LaplaceOperator 类中的 FEEvaluation::submit_gradient() 重复二次导数调用，但符号调换，因为我们想根据上述公式减去右侧向量的迪里希条件的贡献。当我们调用 FEEvaluation::integrate() 时，我们将关于值槽和第一导数槽的两个参数设置为真，以说明在正交点的循环中添加的两个项。一旦右手边组装完毕，我们就继续求解同质问题的线性系统，比如说涉及到一个变量 @p solution_update. 在求解之后，我们可以将 @p solution_update 添加到包含最终（非同质）解决方案的 @p solution 向量中。

请注意，拉普拉斯的负号与我们需要建立右手边的强制力的正号一起，是一个更普遍的概念。我们所实现的只不过是牛顿的非线性方程方法，但应用于线性系统。我们在迪里切特边界条件方面使用了对变量  @p solution  的初始猜测，并计算出了残差  $r = f - Au_0$  。然后，线性系统被解为  $\Delta u = A^{-1} (f-Au)$  ，我们最后计算出  $u = u_0 + \Delta u$  。对于一个线性系统，我们显然在一次迭代后就能得到精确的解。如果我们想将代码扩展到非线性问题，我们会将 @p assemble_rhs() 函数重新命名为一个更具描述性的名字，如@p assemble_residual()，计算残差的（弱）形式，而 @p LaplaceOperator::apply_add() 函数将得到残差相对于解变量的线性化。
...
获得重新使用@p LaplaceOperator::apply_add() 函数的右手边的第二个选择是添加第二个LaplaceOperator，跳过Dirichlet约束。为此，我们初始化第二个MatrixFree对象，它没有任何边界值约束。这个 @p matrix_free 对象然后被传递给一个 @p LaplaceOperator 类实例@p inhomogeneous_operator，它只用于创建右手边。
...
这种技术的一个更复杂的实现可以重新使用原始的MatrixFree对象。这可以通过用多个块初始化MatrixFree对象来实现，其中每个块对应于不同的AffineConstraints对象。这样做需要对LaplaceOperator类进行大量的修改，但是库中的 MatrixFreeOperators::LaplaceOperator 类可以做到这一点。关于如何设置块的更多信息，请参见 MatrixFreeOperators::Base 中关于块的讨论。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-37.cc" 。 
