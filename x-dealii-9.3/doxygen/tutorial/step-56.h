...
本教程的目的是为斯托克斯方程创建一个高效的线性求解器，并将其与其他方法进行比较。 在这里，我们将使用带有几何多网格的FGMRES作为预处理速度块，我们将在结果部分表明，这比 step-22 中使用的线性求解器（包括 "可能的扩展 "中描述的方案）从根本上来说是一种更好的方法。 从根本上说，这是因为只有使用多网格才有可能得到 $O(n)$ 的求解时间，其中 $n$ 是线性系统的未知数的数量。使用Timer类，我们收集一些统计数据来比较设置时间、求解时间和迭代次数。我们还计算误差以确保我们所实现的是正确的。
...
请注意，我们使用的是变形张量，而不是 $\Delta u$ （关于两者之间的区别的详细描述可以在 step-22 中找到，但总的来说，变形张量的物理性更强，也更昂贵）。

<a name="LinearSolverandPreconditioningIssues"></a><h3> Linear %Solver and Preconditioning Issues </h3> 


离散方程的弱形式自然导致了以下速度和压力场的节点值的线性系统。@f{eqnarray*}
...
我们的目标是比较几种解决方法。 虽然 step-22 使用 "Schur补数法 "分两步解决线性系统，但我们本着 step-22 "结果 "一节中概述的方法，使用带有高效预处理程序的FMGRES一次性解决该块系统。其思路如下：如果我们找到一个块状预处理器 $P$ ，使矩阵 
...
是简单的，那么使用该预处理程序的迭代求解器将在几次迭代中收敛。注意，我们在这里做的是正确的预处理。 使用舒尔补码 $S=BA^{-1}B^T$ ，我们发现 
...
是一个不错的选择。让 $\widetilde{A^{-1}}$ 是 $A^{-1}$ 的近似值， $\widetilde{S^{-1}}$ 是 $S^{-1}$ 的近似值，我们看到@f{eqnarray*}
P^{-1} =
...
正如在 step-22 、 $-M_p^{-1}=:\widetilde{S^{-1}} \approx
S^{-1}$ 中所讨论的，其中 $M_p$ 是压力质量矩阵，通过使用CG与ILU作为预处理程序进行近似求解，而 $\widetilde{A^{-1}}$  ]是通过多种方法之一得到的：用CG和ILU作为预处理程序求解线性系统，仅仅使用ILU的一次应用，用CG和GMG（如 step-16 中描述的几何多网格）作为预处理程序求解线性系统，或者仅仅执行GMG的一个V-循环。
...
测试问题是一个 "制造的解决方案"（详见 step-7 ），我们选择 $u=(u_1,u_2,u_3)=(2\sin (\pi x), - \pi y \cos
...
如果你在deal.II手册中查找创建一个从 <code>Function@<dim@></code> 派生的类所需的内容，你会发现这个类有许多 @p virtual 函数，包括 Function::value(),   Function::vector_value(),   Function::value_list(), 等等，所有这些都可以被重载。 deal.II的不同部分将需要这些特定函数中的不同部分。这在一开始会让人感到困惑，但幸运的是，你真正需要实现的只有 @p value(). ，函数类中的其他虚函数在里面有默认的实现，会默认调用你对 @p value 的实现。

请注意，我们的参考方案满足了  $\nabla \cdot u = 0$  。此外，压力被选择为平均值为零。 对于 step-7 的 "制造解决方案的方法"，我们需要找到 $\bf
f$ ，以便。
...
因为我们在线性系统中没有强制要求平均压力为零，所以我们需要在求解后对解决方案进行后处理。为了做到这一点，我们使用 VectorTools::compute_mean_value() 函数来计算压力的平均值，以便从压力中减去它。


<a name="DoFHandlers"></a><h3> DoF Handlers </h3>


我们在这里实现几何多网格的方式只在速度变量（即上面描述的 $A$ 矩阵）上执行，而不是压力。我们可以用不同的方式来实现这一点，包括将所有的粗网格操作视为作用于 $2\times
2$ 块系统，我们只考虑左上角的块。另外，我们也可以通过真正只考虑整个有限元离散化的速度部分的线性系统来实现。后者是我们在这里想要使用的方式。

为了实现这一点，我们需要能够提出诸如 "我可以只拥有DoFHandler的一部分吗？"之类的问题。在编写这个程序的时候，这是不可能的，所以为了满足我们的需求，我们只是为速度创建一个单独的、第二个DoFHandler。然后，我们只基于这个第二DoFHandler为多网格预处理程序建立线性系统，并简单地将第一块（整体）向量转移到整个第二DoFHandler的对应向量中。为了实现这一目标，我们必须保证两个DoFHandler对象中的（速度）自由度排序的<i>order</i>是相同的。事实上，首先在两个对象上分配自由度，然后在两个对象上使用相同的DoFRenumbering操作序列，就可以做到这一点。


<a name="DifferencesfromtheStep22tutorial"></a><h3> Differences from the Step 22 tutorial </h3> 


 step-56 和 step-22 之间的主要区别是，我们使用块状求解器而不是 step-22 中使用的Schur Complement方法。这种方法的细节可以在 step-22 的 "可能的扩展 "部分的 "块状Schur补码预处理 "小节中找到。对于速度块的预处理，我们从<a href="https://aspect.geodynamics.org">ASPECT</a>中借用了一个名为 @p BlockSchurPreconditioner 的类，该类可以选择求解 $A$ 的逆，或者只对其应用一个预处理扫频，这分别为我们提供了一种昂贵和廉价的方法。<a name="CommProg"></a> <h1> The commented program</h1>
...
在下文中，我们将实现一个预处理程序，它扩展了  step-22  的结果部分所讨论的想法。具体来说，我们1.使用一个上块三角的预处理器，因为我们想使用右预处理。2.可选择允许使用速度块的内部求解器，而不是单一的预处理程序应用。3.不使用InverseMatrix，而是明确地调用SolverCG。这种方法也用于ASPECT代码（见https://aspect.geodynamics.org），该代码在模拟地幔对流的背景下求解斯托克斯方程，该代码已被用于解决成千上万个处理器上的问题。   
...
下面利用分量掩码，只对速度的边界值进行插值，这在矢量值dealii step-20 教程中进一步解释。
...
在这里，就像在 step-16 中一样，我们有一个函数来组装多网格预处理程序所需的水平和界面矩阵。
...
计算平均压力 $\frac{1}{\Omega} \int_{\Omega} p(x) dx $ ，然后从每个压力系数中减去它。这将导致压力的平均值为零。这里我们利用了压力是分量 $dim$ 和有限元空间是节点的事实。
...
我们首先运行代码，确认有限元解以混合有限元问题的误差分析所预测的正确速率收敛。给定足够平滑的精确解 $u$ 和 $p$ ，Taylor-Hood元素 $Q_k \times Q_{k-1}$ 的误差应该是 
...
例如，见Ern/Guermond "有限元的理论与实践"，第4.2.5节第195页。这确实是我们观察到的，以 $Q_2 \times Q_1$ 元素为例（这就是代码中的做法，但在 <code>main()</code> 中很容易改变）。
...
让我们比较一下使用UMFPACK的直接求解方法和我们选择 $\widetilde {A^{-1}}=A^{-1}$ 和 $\widetilde{S^{-1}}=S^{-1}$ 的两种方法，用CG求解 $A,S$ 的线性系统。然后CG的预处理程序是ILU或GMG。下表总结了求解器的迭代、时间和虚拟内存（VM）的峰值使用。
...
3. 3. 对于ILU来说， $A$ 的（内部）迭代次数随着细化而增加，导致求解时间的线性扩展性较差。相比之下， $A$ 的内部迭代次数在GMG中保持不变，导致求解时间几乎完美的缩放。
...
介绍中还概述了另一种对整个系统进行预处理的方案，即我们不选择上表中的 $\widetilde
{A^{-1}}=A^{-1}$ ，而是在 $\widetilde{A^{-1}}$ 中只分别使用GMG或ILU的单一预处理程序。

这实际上是在代码中实现的。目前，布尔值 <code>use_expensive</code> in <code>solve()</code> 被设置为 @p true.  上面提到的选项是通过设置为 @p false. 得到的。 

你会发现的是，如果你这样使用GMG，FGMRES的迭代次数在细化中保持不变。这意味着多重网格是最优的，并且不受  $h$  的影响。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-56.cc"  。 
