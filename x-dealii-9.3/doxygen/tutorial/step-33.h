...
too much trouble.
  ...
  @note  程序使用<a
href="http://trilinos.org">Trilinos</a>线性求解器（这些可以在Trilinos的Aztec/Amesos包中找到）和自动微分包Sacado，也是Trilinos的一部分。deal.II必须被配置为使用Trilinos。请参考<a
href="../../readme.html#trilinos">ReadMe</a>文件，以了解如何做到这一点。

  @note  虽然这个程序很好地演示了自动微分的使用，但它并没有表达欧拉方程求解器的技术水平。对于这个方程有更快、更准确的方法，你应该看看 step-67 和 step-69 以了解如何更有效地解决这个方程。
...
描述可压缩的无粘性气体运动的方程（所谓气体动力学的欧拉方程）是一个基本的守恒定律系统。在空间维度 $d$ 中，它们读作@f[
\partial_t \mathbf{w} + \nabla \cdot \mathbf{F}(\mathbf{w}) =
\mathbf{G}(\mathbf w),
@f]，解 $\mathbf{w}=(\rho v_1,\ldots,\rho v_d,\rho,
E)^{\top}$ 由 $\rho$ 流体密度、 ${\mathbf v}=(v_1,\ldots v_d)^T$ 流速（因此 $\rho\mathbf v$ 为线性动量密度）和 $E$ 气体的能量密度组成。我们将上述方程解释为  $\partial_t \mathbf{w}_i + \nabla \cdot \mathbf{F}_i(\mathbf{w}) = \mathbf
G_i(\mathbf w)$  ,  $i=1,\ldots,dim+2$  。

对于欧拉方程，通量矩阵 $\mathbf F$ （或通量函数系统）被定义为（这里显示为 $d=3$ 的情况）@f{eqnarray*}
  \mathbf F(\mathbf w)
  =
  \left(
  \begin{array}{ccc}
...
而我们将选择仅将重力的影响作为特定的右手边强制力，由@f{eqnarray*}
  \mathbf G(\mathbf w)
  =
  \left(
  \begin{array}{c}
...
其中 $\mathbf g=(g_1,g_2,g_3)^T$ 表示重力矢量。这样一来，整个方程组就成了。@f{eqnarray*}
...
这些方程分别描述了动量、质量和能量的守恒。该系统由一个定义压力的关系封闭。  $p =
...
考虑到这是一个双曲问题，与 step-12 中讨论的简单问题的风格相同，具体化以通常的方式进行：我们选择一个有限元空间 $V_h$ ，并针对我们的（矢量值的）测试函数 $\mathbf{z} \in V_h$ 积分我们的守恒定律。 然后我们通过部分积分，用<i> numerical </i>通量 $\mathbf{H}$ 、@f{eqnarray*}
...
在边界上，我们必须说明外部跟踪 $\mathbf{w}^-$ 是什么。根据边界条件，我们规定以下两种情况。  <ul>   <li>  流入边界： $\mathbf{w}^-$ 被规定为期望值。  <li>  超音速流出边界： $\mathbf{w}^- = \mathbf{w}^+$   <li>  亚音速流出边界： $\mathbf{w}^- = \mathbf{w}^+$  除了能量变量被修改为支持规定的压力 $p_o$  ，即 $\mathbf{w}^- =(\rho^+, \rho v_1^+, \dots, \rho v_d^+, p_o/(\gamma -1) + 0.5 \rho |\mathbf{v}^+|^2)$   <li>  反射边界：我们设定 $\mathbf{w}^-$  ，使 $(\mathbf{v}^+ + \mathbf{v}^-) \cdot \mathbf{n} = 0$  和 $\rho^- = \rho^+,E^-=E^+$  。  </ul>   

关于这些问题的更多信息可以在Ralf Hartmann的博士论文中找到，例如（"可压缩欧拉方程的自适应有限元方法"，博士论文，海德堡大学，2002）。

我们使用一个时间步进方案来替代上述方程中的时间导数。为简单起见，我们将 $ \mathbf{B}({\mathbf{w}_{n}})(\mathbf z) $ 定义为时间步长的空间残差 $n$  。
...
因此，在每个时间步长，我们的完全离散化是，应用于任何测试函数 $\mathbf z$ 的残差等于零。@f{eqnarray*}
...
在下面的实现中，我们为函数 $\mathbf H$ 选择Lax-Friedrichs通量，即 $\mathbf{H}(\mathbf{a},\mathbf{b},\mathbf{n}) =
\frac{1}{2}(\mathbf{F}(\mathbf{a})\cdot \mathbf{n} +
\mathbf{F}(\mathbf{b})\cdot \mathbf{n} + \alpha (\mathbf{a} - \mathbf{b}))$ ，其中 $\alpha$ 是输入文件中指定的一个固定数字，或者 $\alpha$ 是一个与网格有关的值。在后一种情况下，它被选为 $\frac{h}{2\delta T}$ ，其中 $h$ 是应用通量的面的直径，而 $\delta T$ 是当前的时间步长。

有了这些选择，将残差等效为零就会产生一个非线性方程组  $R(\mathbf{W}_{n+1})=0$  。我们通过牛顿迭代来解决这个非线性系统（与 step-15 中解释的方法相同），即通过迭代@f{eqnarray*}
...
直到 $|R(\mathbf{W}^k_{n+1})|$ （残差）足够小。通过用有限元空间的节点基础而不是所有的 $\mathbf z$ 进行测试，我们得到一个 $\delta \mathbf W$ 的线性系统：@f{eqnarray*}
\mathbf R'(\mathbf{W}^k_{n+1})\delta \mathbf{W}^k_{n+1} & = & -
\mathbf R(\mathbf{W}^{k}_{n+1}).
@f} 

一般来说，这个线性系统既不是对称的，也没有任何特定的确定性属性。我们将使用直接求解器或Trilinos的GMRES实现来解决它。从<a href="#Results">results shown below</a>中可以看出，这种全隐式迭代收敛速度非常快（通常为3步），并且具有牛顿方法所期望的二次收敛顺序。
...
href="http://trilinos.org" target="_top">Trilinos</a>框架内的一个包，提供了一个C++模板类 <code>Sacado::Fad::DFad</code> （ <code>Fad</code> 代表 "向前自动微分"），支持基本的算术运算符和函数，如 <code> sqrt, sin, cos, pow, </code> 等。为了使用这个功能，人们声明一个这种类型的变量集合，然后将这个集合中的一些变量表示为自由度，其余的变量是独立变量的函数。 这些变量在算法中被使用，随着变量的使用，它们对自由度的敏感度被持续更新。

可以想象，对于整个雅各布矩阵来说，这可能是非常昂贵的：自变量的数量是 $\mathbf W^k$ ，因变量是向量 $\mathbf
R(\mathbf W^k)$ 的元素。这两个向量很容易有几万个元素或更多。 然而，需要注意的是，并非 $\mathbf R$ 的所有元素都依赖于 $\mathbf W^k$ 的所有元素：事实上， $\mathbf R$ 中的一个条目只依赖于 $\mathbf W^k$ 的一个元素，如果两个相应的形状函数以弱形式重叠和耦合。

具体来说，定义当前单元上的残差可能依赖的最小独立AD变量集是明智的：在每个元素上，我们定义那些对应于定义在这个单元上的自由度的独立变量（或者，如果我们必须计算单元之间的跳跃项，则对应于定义在两个相邻单元上的自由度），而因变量是本地残差向量的元素。不这样做，即把<i>all</i>的元素 $\mathbf W^k$ 定义为独立的，将导致大量零的计算非常昂贵：局部残差向量的元素几乎独立于解向量的所有元素，因此它们的导数为零；然而，试图计算这些零很容易占用整个程序计算时间的90%甚至更多，正如这个程序首次编写几年后一个学生无意中做的实验所示。


回到自动计算雅各布的问题上。作者将这种方法与手工编码的雅各布式并列用于不可压缩的Navier-Stokes问题，发现Sacado方法与使用手工编码的雅各布式一样快，但无限简单，而且不容易出错。由于使用自动差分只需要编码残差 $R(\mathbf{W})$ ，确保代码的正确性和维护代码变得非常简单--雅各布矩阵 $\mathbf R'$ 基本上是由计算残差 $\mathbf
R$ 的相同代码计算的。

说了这么多，这里有一个非常简单的例子，展示了Sacado的使用方法。
...
输出是 $c(a,b)=2a+\cos(ab)$ 在 $a=1,b=2$ 的导数 $\frac{\partial c(a,b)}{\partial a},
\frac{\partial c(a,b)}{\partial b}$  。

应该注意的是，Sacado提供了更多的自动差分能力，而不是本程序中使用的小子集。 然而，理解上面的例子就足以理解Sacado在这个欧拉流程序中的使用。

<a name="Trilinossolvers"></a><h3> Trilinos solvers </h3>

该程序使用Aztec迭代求解器，或Amesos稀疏直接求解器，两者均由Trilinos软件包提供。 这个软件包本身就是为了用于并行程序而设计的，但是，它也可以像这里一样方便地用于串行程序。 Epetra软件包是基本的矢量/矩阵库，解算器是在此基础上建立的。 这个非常强大的包可以用来描述矢量的平行分布，并定义对这些矢量进行操作的稀疏矩阵。 请查看注释代码以了解更多关于这些求解器在例子中的使用细节。

<a name="Adaptivity"></a><h3> Adaptivity </h3> 

这个例子使用了一个特殊的细化指标，在冲击型问题中显示了一些有用性，在包括下坡流的例子中也是如此。 我们根据密度的平方梯度进行细化。悬空节点的处理是通过计算不同细化水平的单元的数值通量来实现的，而不是像迄今为止的所有其他教程程序那样使用AffineConstraints类。 通过这种方式，这个例子结合了连续和DG的方法论。它还简化了雅各布系数的生成，因为我们不必通过用于计算它的自动微分来跟踪约束自由度。

  @note  而这个程序是在2008年写的，我们不知道有什么出版物会真正使用这种方法。然而，A. Dedner, R. Kl&ouml;fkorn, and M. Kr&auml;nkel（"Continuous Finite-Elements on Non-Conforming Grids Using Discontinuous Galerkin Stabilization", Proceedings of Finite Volumes for Complex Applications VII - Methods and Theoretical Aspects, Springer, 2014）的一篇较新的论文接近。

此外，我们强制规定了细化水平的最大数量，以控制细化。 笔者的经验是，对于时间相关问题的适应性，如果不注意的话，细化很容易导致模拟戛然而止，因为时间步长的限制，如果网格在领域的任何部分变得太细的话。 在这个例子中，细化的数量被限制，让用户指定在网格的任何地方出现的最大细化水平。 这样一来，细化就不会使模拟速度减慢到停滞不前。 当然，这纯粹是一种启发式的策略，如果作者的顾问听说了，作者可能会被永远放逐出有限元误差估计界。

<a name="Inputdeckinitialandboundaryconditions"></a><h3>Input deck, initial and boundary conditions</h3> 


我们使用一个输入文件平台来驱动模拟。 通过这种方式，我们可以改变边界条件和其他重要的模拟属性，而不必重新编译。 关于格式的更多信息，请看<a href="#Results">results section</a>，在那里我们更详细地描述了一个输入文件的例子。
...
这个程序的实现被分成三个基本部分。  <ul>   <li>   <code>EulerEquations</code>  类，它封装了完全描述欧拉方程具体内容的一切。这包括通量矩阵 $\mathbf F(\mathbf W)$ 、数值通量 $\mathbf F(\mathbf
  W^+,\mathbf W^-,\mathbf n)$ 、右手边 $\mathbf G(\mathbf W)$ 、边界条件、细化指标、输出的后处理，以及类似的需要了解解向量和方程的各个组成部分的含义的东西。

    <li>  一个命名空间，处理与运行时参数有关的一切。

    <li>  处理时间步进、外部非线性和内部线性求解、组装线性系统以及驱动这一切的顶层逻辑的 <code>ConservationLaw</code> 类。  </ul>   

采用这种方法的原因是，它将程序中的各种关注点分开： <code>ConservationLaw</code> 是以这样一种方式编写的，即相对简单地将其适应于不同的方程组。人们只需为其他双曲方程重新实现 <code>EulerEquations</code> 类的成员，或用其他方程来增加现有方程（例如通过增加变量，或通过增加化学等）。然而，这样的修改不会影响时间步进，也不会影响非线性求解器，如果正确的话，因此， <code>ConservationLaw</code> 中的任何内容都不需要修改。

同样，如果我们想改进线性或非线性求解器，或时间步进方案（如<a
href="#Results">results section</a>末尾所暗示的），那么这根本不需要修改 <code>EulerEquations</code> 。<a name="CommProg"></a> <h1> The commented program</h1>
...
然后，正如在介绍中提到的，我们使用各种Trilinos软件包作为线性求解器以及自动微分。这些都在下面的include文件中。
...
为了结束本节，将dealii库中的所有内容引入本程序内容将进入的命名空间。
...
在这里，我们为这个特定的守恒定律系统定义了通量函数，以及几乎所有其他与气体动力学欧拉方程有关的东西，原因在介绍中已经讨论过了。我们将所有这些归入一个结构，该结构定义了与通量有关的一切。这个结构的所有成员都是静态的，也就是说，这个结构没有由实例成员变量指定的实际状态。更好的方法是使用命名空间，而不是一个拥有所有静态成员的结构--但是命名空间不能被模板化，而且我们希望结构中的一些成员变量取决于空间维度，我们以通常的方式使用模板参数来引入。
...
首先是几个变量，它们以一种通用的方式描述我们的解向量的各个组成部分。这包括系统中分量的数量（欧拉方程中每个空间方向的动量都有一个条目，加上能量和密度分量，总共有 <code>dim+2</code> 个分量），以及描述第一动量分量、密度分量和能量密度分量在解向量中的索引的函数。请注意，所有这些%数都取决于空间维度；以通用的方式定义它们（而不是通过隐含的惯例）使我们的代码更加灵活，并使以后的扩展更加容易，例如在方程中加入更多的分量。
...
在这个程序中，当生成图形输出的时候，我们需要指定解变量的名称，以及各种成分如何分组为矢量和标量场。我们可以在这里进行描述，但为了使与欧拉方程有关的事情在这里得到解决，并使程序的其他部分尽可能地通用，我们在以下两个函数中提供这类信息。
...
接下来，我们定义气体常数。我们将在紧随这个类的声明之后的定义中把它设置为1.4（与整数变量不同，比如上面的变量，静态常数浮点成员变量不能在C++的类声明中初始化）。这个1.4的值代表了由两个原子组成的分子的气体，例如空气，它几乎完全由 $N_2$ 和 $O_2$ 组成，直到小的痕迹为止。
...
在下文中，我们将需要从保守变量的矢量中计算动能和压力。我们可以根据能量密度和动能 $\frac 12 \rho |\mathbf v|^2
= \frac{|\rho \mathbf v|^2}{2\rho}$ 做到这一点（注意，独立变量包含动量分量 $\rho v_i$ ，而不是速度 $v_i$ ）。
...
我们将通量函数 $F(W)$ 定义为一个大矩阵。 这个矩阵的每一行都代表该行成分的标量守恒定律。 这个矩阵的确切形式在介绍中给出。请注意，我们知道矩阵的大小：它的行数与系统的分量一样多， <code>dim</code> 列数一样多；我们没有为这样的矩阵使用FullMatrix对象（它的行数和列数是可变的，因此每次创建这样的矩阵时必须在堆上分配内存），而是立即使用一个矩形的数字阵列。     
...
首先计算出现在通量矩阵中的压力，然后计算对应于动量项的矩阵的第一  <code>dim</code>  列。
...
与描述通量函数 $\mathbf F(\mathbf w)$ 的方式相同，我们也需要有一种方法来描述右侧的强制项。正如介绍中提到的，我们在这里只考虑重力，这导致了具体的形式 $\mathbf G(\mathbf w) = \left(
g_1\rho, g_2\rho, g_3\rho, 0, \rho \mathbf g \cdot \mathbf v
\right)^T$ ，这里显示的是三维情况。更具体地说，我们将只考虑三维的 $\mathbf g=(0,0,-1)^T$ ，或二维的 $\mathbf g=(0,-1)^T$ 。这自然导致了下面的函数。
...
我们必须处理的另一件事是边界条件。为此，让我们首先定义我们目前知道如何处理的各种边界条件。
...
接下来的部分是实际决定在每一种边界上做什么。为此，请记住，从介绍中可以看出，边界条件是通过在给定的不均匀性 $\mathbf j$ 的边界外侧选择一个值 $\mathbf w^-$ ，以及可能在内部选择解的值 $\mathbf w^+$ 来指定的。然后将两者都传递给数值通量 $\mathbf H(\mathbf{w}^+, \mathbf{w}^-,
\mathbf{n})$ 以定义边界对双线性形式的贡献。     


边界条件在某些情况下可以为解矢量的每个分量独立指定。例如，如果分量 $c$ 被标记为流入，那么 $w^-_c = j_c$  。如果是流出，那么 $w^-_c
= w^+_c$  。这两种简单的情况在下面的函数中首先被处理。     


从C++语言的角度看，有一个小插曲使这个函数不讨人喜欢。输出向量 <code>Wminus</code> 当然会被修改，所以它不应该是一个 <code>const</code> 的参数。然而，在下面的实现中，它却成为了参数，而且为了使代码能够编译，它必须成为参数。原因是我们在 <code>Wminus</code> 类型为 <code>Table@<2,Sacado::Fad::DFad@<double@> @></code> 的地方调用这个函数，这是一个2d表，其指数分别代表正交点和向量分量。我们用 <code>Wminus[q]</code> 作为最后一个参数来调用这个函数；对2d表进行下标会产生一个代表1d向量的临时访问器对象，这正是我们在这里想要的。问题是，根据C++ 1998和2003标准，临时访问器对象不能被绑定到一个函数的非静态引用参数上，就像我们在这里希望的那样（这个问题将在下一个标准中以rvalue引用的形式得到解决）。 我们在这里把输出参数变成常量，是因为<i>accessor</i>对象是常量，而不是它所指向的表：那个表仍然可以被写到。然而，这个黑客是不愉快的，因为它限制了可以作为这个函数的模板参数的数据类型：一个普通的向量是不行的，因为当标记为 <code>const</code> 时，它不能被写到。由于目前没有好的解决方案，我们将采用这里显示的务实的，甚至是不漂亮的解决方案。
...
规定的压力边界条件要复杂一些，因为即使压力是规定的，我们在这里真正设定的是能量分量，它将取决于速度和压力。因此，尽管这似乎是一个迪里切特类型的边界条件，我们得到了能量对速度和密度的敏感性（除非这些也是规定的）。
...
我们规定速度（我们在这里处理的是一个特定的分量，所以速度的平均值是与表面法线正交的。 这就产生了整个速度分量的敏感度。
...
在这个类中，我们还想指定如何细化网格。将使用我们在 <code>EulerEquation</code> 类中提供的所有信息的 <code>ConservationLaw</code> 类对于它所求解的特定守恒定律是相当不可知的：因为它甚至不关心一个解矢量有多少分量。因此，它不可能知道合理的细化指标是什么。另一方面，在这里我们知道，或者至少我们可以想出一个合理的选择：我们简单地看一下密度的梯度，并计算 $\eta_K=\log\left(1+|\nabla\rho(x_K)|\right)$ ，其中 $x_K$ 是单元 $K$ 的中心。     


当然也有许多同样合理的细化指标，但这个指标确实如此，而且很容易计算。
...
最后，我们声明一个实现数据组件后处理的类。这个类解决的问题是，我们使用的欧拉方程的表述中的变量是保守的而不是物理的形式：它们是动量密度  $\mathbf m=\rho\mathbf
v$  ，密度  $\rho$  ，和能量密度  $E$  。我们还想把速度  $\mathbf v=\frac{\mathbf
m}{\rho}$  和压力  $p=(\gamma-1)(E-\frac{1}{2} \rho |\mathbf
v|^2)$  放入我们的输出文件中。     


此外，我们还想增加生成Schlieren图的可能性。Schlieren图是一种将冲击和其他尖锐界面可视化的方法。Schlieren "这个词是一个德语单词，可以翻译成 "条纹"--不过，用一个例子来解释可能更简单：比如说，当你把高浓度的酒精或透明的盐水溶液倒入水中时，你会看到schlieren；这两种物质的颜色相同，但它们的折射率不同，所以在它们完全混合之前，光线会沿着弯曲的光线穿过混合物，如果你看它，会导致亮度变化。这就是 "分光"。类似的效果发生在可压缩流中，因为折射率取决于气体的压力（以及因此的密度）。     


这个词的起源是指三维体积的二维投影（我们看到的是三维流体的二维图片）。在计算流体力学中，我们可以通过考虑其原因来了解这种效应：密度变化。因此，Schlieren图是通过绘制 $s=|\nabla \rho|^2$ 产生的；显然， $s$ 在冲击和其他高度动态的地方很大。如果用户需要的话（通过在输入文件中指定），我们希望除了上面列出的其他派生量之外，还能生成这些雪莲图。     


计算解决我们问题的派生量并将其输出到数据文件的算法的实现依赖于DataPostprocessor类。它有大量的文档，该类的其他用途也可以在  step-29  中找到。因此，我们不做广泛的评论。
...
这是唯一值得评论的功能。当生成图形输出时，DataOut和相关的类将在每个单元格上调用这个函数，以获取每个正交点的值、梯度、Hessians和法向量（如果我们正在处理面）。请注意，每个正交点的数据本身就是矢量值，即保守变量。我们在这里要做的是计算每个正交点上我们感兴趣的量。注意，为此我们可以忽略Hessians（"inputs.solution_hessians"）和法向量（"inputs.normals"）。
...
在函数的开始，让我们确保所有的变量都有正确的大小，这样我们就可以访问个别的向量元素，而不必怀疑我们是否可能读或写无效的元素；我们还检查 <code>solution_gradients</code> 向量是否只包含我们真正需要的数据（系统知道这一点，因为我们在下面的 <code>get_needed_update_flags()</code> 函数中这样说）。对于内向量，我们检查至少外向量的第一个元素有正确的内部大小。
...
然后在所有的正交点上循环，在那里做我们的工作。这段代码应该是不言自明的。输出变量的顺序是：首先是 <code>dim</code> 速度，然后是压力，如果需要的话，还可以是SCHELEN图。请注意，我们尝试使用 <code>first_momentum_component</code> 和 <code>density_component</code> 的信息，对输入向量中的变量顺序进行通用处理。
...
我们接下来的工作是定义一些包含运行时参数的类（例如求解器公差、迭代次数、稳定参数等等）。我们可以在主类中完成这个工作，但是我们把它和主类分开，以使程序更加模块化和容易阅读。所有与运行时参数有关的东西都将在以下命名空间中，而程序逻辑则在主类中。   


我们将把运行时参数分成几个独立的结构，并将其全部放入一个命名空间  <code>Parameters</code>  。在这些类中，有几个是为单独的组进行参数分组的，如用于求解器、网格细化或输出。这些类中的每一个都有函数  <code>declare_parameters()</code>  和  <code>parse_parameters()</code>  ，分别在ParameterHandler对象中声明参数子段和条目，并从这样的对象中检索实际的参数值。这些类在ParameterHandler的子段中声明他们所有的参数。   


下面这个命名空间的最后一个类结合了前面所有的类，从它们派生出来，并负责处理输入文件顶层的另外几个条目，以及其他一些奇怪的条目，这些条目在子段中太短了，不值得自己有一个结构。   


这里值得指出的是一件事。下面这些类中没有一个构造函数可以初始化各种成员变量。不过这不是问题，因为我们将从输入文件中读取这些类中声明的所有变量（或者间接地：一个ParameterHandler对象将从那里读取，而我们将从这个对象中获取数值），它们将以这种方式被初始化。如果输入文件中根本没有指定某个变量，这也不是问题。在这种情况下，ParameterHandler类将简单地采取默认值，这个默认值是在下面的类的 <code>declare_parameters()</code> 函数中声明一个条目时指定的。
...
这些类中的第一个涉及到线性内解器的参数。它提供的参数表明使用哪种求解器（GMRES作为一般非对称不定式系统的求解器，或稀疏直接求解器），要产生的输出量，以及调整阈值不完全LU分解（ILUT）的各种参数，我们使用它作为GMRES的预处理器。     
...
- ilut_atol, ilut_rtol: 在形成预处理程序时，对于某些问题，不好的条件（或者只是运气不好）会导致预处理程序的条件非常差。 因此，将对角线扰动添加到原始矩阵中，并为这个稍好的矩阵形成预处理程序会有帮助。 ATOL是一个绝对扰动，在形成预处理之前加到对角线上，RTOL是一个比例因子  $rtol \geq 1$  。

- ilut_drop: ILUT将放弃任何幅度小于此值的数值。 这是一种管理该预处理程序所使用的内存量的方法。     


每个参数的含义在  ParameterHandler::declare_entry  调用的第三个参数中也有简要描述  <code>declare_parameters()</code>  。
...
同样的，这里有几个参数决定了网格如何被细化（以及是否被细化）。关于冲击参数的具体作用，请参见下面的网格细化函数。
...
接下来是关于通量修改的部分，使其更加稳定。特别是提供了两个选项来稳定Lax-Friedrichs通量：要么选择 $\mathbf{H}(\mathbf{a},\mathbf{b},\mathbf{n}) =
\frac{1}{2}(\mathbf{F}(\mathbf{a})\cdot \mathbf{n} +
\mathbf{F}(\mathbf{b})\cdot \mathbf{n} + \alpha (\mathbf{a} -
\mathbf{b}))$ ，其中 $\alpha$ 是输入文件中指定的一个固定数字，要么 $\alpha$ 是一个与网格有关的值。在后一种情况下，它被选择为 $\frac{h}{2\delta T}$ ，其中 $h$ 是应用通量的面的直径，而 $\delta T$ 是当前的时间步长。
...
然后是一个关于输出参数的部分。我们提供产生Schlieren图（密度的平方梯度，一种可视化冲击前沿的工具），以及图形输出的时间间隔，以防我们不希望每个时间步骤都有输出文件。
...
最后是将这一切结合起来的类。它自己声明了一些参数，主要是参数文件顶层的参数，以及一些太小的部分，不值得自己的类。它还包含所有实际上与空间维度有关的东西，比如初始或边界条件。     


由于这个类是由上面所有的类派生出来的， <code>declare_parameters()</code> and <code>parse_parameters()</code> 函数也会调用基类的相应函数。     


请注意，这个类也处理输入文件中指定的初始和边界条件的声明。为此，在这两种情况下，都有像 "w_0值 "这样的条目，它代表了 $x,y,z$ 方面的表达式，将初始或边界条件描述为一个公式，随后将由FunctionParser类来解析。类似的表达方式还有 "w_1"、"w_2 "等，表示欧拉系统的 <code>dim+2</code> 守恒变量。同样，我们允许在输入文件中最多使用 <code>max_n_boundaries</code> 个边界指标，这些边界指标中的每一个都可以与流入、流出或压力边界条件相关联，同质边界条件被分别指定给每个组件和每个边界指标。     


用来存储边界指标的数据结构有点复杂。它是一个 <code>max_n_boundaries</code> 元素的数组，表示将被接受的边界指标的范围。对于这个数组中的每个条目，我们在 <code>BoundaryCondition</code> 结构中存储一对数据：首先是一个大小为 <code>n_components</code> 的数组，对于解向量的每个分量，表明它是流入、流出还是其他种类的边界，其次是一个FunctionParser对象，一次性描述这个边界ID的解向量的所有分量。     


 <code>BoundaryCondition</code> 结构需要一个构造器，因为我们需要在构造时告诉函数解析器对象它要描述多少个向量分量。因此，这种初始化不能等到我们在后面的 <code>AllParameters::parse_parameters()</code> 中实际设置FunctionParser对象所代表的公式。      
...
这里终于出现了一个类，它实际上是对我们上面定义的所有欧拉方程和参数的具体内容做一些事情。公共接口与以往基本相同（构造函数现在需要一个文件名来读取参数，这个文件名在命令行中传递）。私有函数接口也与通常的安排非常相似， <code>assemble_system</code> 函数被分成三个部分：一个包含所有单元的主循环，然后分别调用另外两个单元和面的积分。
...
前面的几个成员变量也是相当标准的。请注意，我们定义了一个映射对象，在整个程序中组装术语时使用（我们将把它交给每个FEValues和FEFaceValues对象）；我们使用的映射只是标准的 $Q_1$ 映射--换句话说，没有什么花哨的东西--但在这里声明一个映射并在整个程序中使用它将使以后在需要时改变它变得更简单。这实际上是相当相关的：众所周知，对于欧拉方程的跨音速模拟，如果边界近似没有足够高的阶数，计算甚至不能收敛 $h\rightarrow 0$  。
...
接下来是一些数据向量，对应于前一个时间步骤的解（ <code>old_solution</code> ），当前解的最佳猜测（ <code>current_solution</code> ；我们说<i>guess</i>是因为计算它的牛顿迭代可能还没有收敛，而 <code>old_solution</code> 是指前一个时间步骤的完全收敛的最终结果），以及下一个时间步骤的解的预测器，通过将当前和前一个时间步骤的解推算到未来。
...
这最后一组成员变量（除了最下面的保存所有运行时参数的对象和一个屏幕输出流，它只在要求verbose输出时打印一些东西）处理我们在这个程序中与Trilinos库的接口，它为我们提供了线性求解器。与在 step-17 和 step-18 中包括PETSc矩阵类似，我们需要做的是创建一个Trilinos稀疏矩阵而不是标准的deal.II类。该系统矩阵在每个牛顿步骤中被用于雅各布系数。由于我们不打算并行运行这个程序（不过用Trilinos数据结构也不会太难），所以我们不必考虑其他的事情，比如分配自由度。
...
每次改变网格时，都会调用下面这个（简单）的函数。它所做的就是根据我们在之前所有的教程程序中生成的稀疏模式来调整特里诺斯矩阵的大小。
...
这个函数和下面的两个函数是这个程序的核心。它们将牛顿方法应用于非线性守恒方程组所产生的线性系统组装起来。   


第一个函数将所有的装配部件放在一个例行程序中，为每个单元格/面派送正确的部件。 对这些对象的装配的实际实现是在下面的函数中完成的。   


在函数的顶部，我们做了常规的内务处理：分配FEValues、FEFaceValues和FESubfaceValues对象，这些对象是在单元格、面和子面（在不同细化水平上的相邻单元的情况下）上进行积分所必需的。请注意，我们并不需要所有这些对象的所有信息（如值、梯度或正交点的实际位置），所以我们只让FEValues类通过指定最小的UpdateFlags集来获得实际需要的信息。例如，当使用邻接单元的FEFaceValues对象时，我们只需要形状值。给定一个特定的面，正交点和 <code>JxW</code> 值与当前单元的相同，法向量已知是当前单元的法向量的负值。
...
然后在这个单元格的所有面中循环。 如果一个面是外部边界的一部分，那么就在那里装配边界条件（ <code>assemble_face_terms</code> 的第五个参数表示我们是在外部面还是内部面工作；如果是外部面，表示邻居自由度指数的第四个参数被忽略，所以我们传递一个空矢量）。
...
在第一种情况下，我们不需要做什么：我们使用的是连续有限元，在这种情况下，面条款不会出现在双线性表格中。在第二种情况下，如果我们强烈地执行悬挂节点约束，通常也不会导致面项的出现（就像迄今为止，只要我们使用连续有限元的所有教程程序一样--这种执行是由AffineConstraints类和 DoFTools::make_hanging_node_constraints). 一起完成的）在当前的程序中，我们选择在不同细化水平的单元之间的面弱地执行连续性，原因有二。(i)因为我们可以，更重要的是(ii)因为我们必须通过AffineConstraints类的操作将我们用来计算牛顿矩阵元素的自动微分穿起来。这是有可能的，但不是微不足道的，所以我们选择了这种替代方法。           


需要决定的是我们所处的两个不同细化水平的单元之间的接口的哪一边。           


让我们来看看邻居首先是更精细的情况。然后，我们必须在当前单元的面的子代上进行循环，并在每个子代上进行整合。我们在代码中洒下几个断言，以确保我们试图找出邻居的哪个子面与当前单元格的某个子面重合的推理是正确的--有点防御性的编程总是无害的。           
...
我们必须关注的另一种可能性是邻居是否比当前单元更粗（特别是，由于每个面只有一个悬挂节点的通常限制，邻居必须正好比当前单元更粗一级，这是我们用断言检查的东西）。同样，我们在这个界面上进行整合。
...
这个函数通过计算残差的单元部分来组合单元项，将其负数加到右手边的向量上，并将其相对于局部变量的导数加到雅各布系数（即牛顿矩阵）上。回顾一下，单元格对残差的贡献为 $R_i = \left(\frac{\mathbf{w}^{k}_{n+1} - \mathbf{w}_n}{\delta t} ,
\mathbf{z}_i \right)_K $   $ +
\theta \mathbf{B}(\mathbf{w}^{k}_{n+1})(\mathbf{z}_i)_K $   $ +
(1-\theta) \mathbf{B}(\mathbf{w}_{n}) (\mathbf{z}_i)_K $ ，其中 $\mathbf{B}(\mathbf{w})(\mathbf{z}_i)_K =
...
- (\mathbf{G}(\mathbf {w}), \mathbf{z}_i)_K $ 为 $\mathbf{w} = \mathbf{w}^k_{n+1}$ 和 $\mathbf{w} = \mathbf{w}_{n}$ ， $\mathbf{z}_i$ 为 $i$ 第1个向量值的测试函数。此外，标量积 $\left(\mathbf{F}(\mathbf{w}), \nabla\mathbf{z}_i\right)_K$ 可以理解为 $\int_K \sum_{c=1}^{\text{n\_components}}
\sum_{d=1}^{\text{dim}} \mathbf{F}(\mathbf{w})_{cd}
\frac{\partial z^c_i}{x_d}$ ，其中 $z^c_i$ 是 $i$ 第1个测试函数的 $c$ 分量。   


   


在这个函数的顶部，我们做了一些常规的内务工作，即分配一些我们以后需要的局部变量。特别是，我们将分配一些变量，用于保存 $k$ 次牛顿迭代后的当前解 $W_{n+1}^k$ （变量 <code>W</code> ）和前一时间步长的解 $W_{n}$ （变量 <code>W_old</code> ）的值。   


除了这些之外，我们还需要当前变量的梯度。 我们必须计算这些是有点遗憾的，我们几乎不需要。 一个简单的守恒定律的好处是，通量一般不涉及任何梯度。 然而，我们确实需要这些，用于扩散稳定化。   


我们存储这些变量的实际格式需要一些解释。首先，我们需要在每个正交点为解向量的 <code>EulerEquations::n_components</code> 分量取值。这就构成了一个二维表，我们使用deal.II的表类（这比 <code>std::vector@<std::vector@<T@> @></code> 更有效，因为它只需要分配一次内存，而不是为外向量的每个元素分配一次）。同样地，梯度是一个三维表，Table类也支持。   
...
接下来，我们必须定义自变量，我们将尝试通过解决一个牛顿步骤来确定这些自变量。这些自变量是我们在这里提取的局部自由度的值。
...
为了将这些变量标记为独立变量，下面的方法可以做到这一点，将 <code>independent_local_dof_values[i]</code> 标记为总共 <code>dofs_per_cell</code> 中的 $i$ 个独立变量。
...
在所有这些声明之后，让我们实际计算一些东西。首先， <code>W</code>, <code>W_old</code>, <code>grad_W</code> 和 <code>grad_W_old</code> 的值，我们可以通过使用公式 $W(x_q)=\sum_i \mathbf W_i \Phi_i(x_q)$ 从本地DoF值计算出来，其中 $\mathbf W_i$ 是（本地部分）解向量的 $i$ 第1个条目，而 $\Phi_i(x_q)$ 是在正交点 $x_q$ 评估的第2个矢量值形状函数的值。梯度可以用类似的方法计算。     


理想情况下，我们可以通过调用类似 FEValues::get_function_values 和 FEValues::get_function_gradients, 的东西来计算这些信息，但是由于（i）我们必须为此扩展FEValues类，（ii）我们不想让整个 <code>old_solution</code> 矢量fad类型，只有局部单元变量，我们明确编码上面的循环。在这之前，我们添加另一个循环，将所有的fad变量初始化为零。
...
我们现在已经有了所有的部件，所以要进行组装。 我们有一个通过系统组件的外循环，和一个通过正交点的内循环，在那里我们积累了对 $i$ 第三残差的贡献  $R_i$  。这个残差的一般公式在引言和本函数的顶部给出。然而，考虑到 $i$  第三个（矢量值）测试函数 $\mathbf{z}_i$ 实际上只有一个非零分量，我们可以将其简化一下（关于这个主题的更多内容可以在 @ref
vector_valued 模块中找到）。它将由下面的变量 <code>component_i</code> 表示。有了这个，残差项可以重新写成@f{eqnarray*}
R_i &=&
...
我们最初对残差的所有贡献进行正意义上的求和，这样我们就不需要对雅各布项进行负数。 然后，当我们对 <code>right_hand_side</code> 矢量求和时，我们要否定这个残差。
...
每一行（i）的残差将累积到这个fad变量中。 在这一行的装配结束时，我们将查询这个变量的敏感度，并将其加入到雅各布系数中。
...
在循环结束时，我们要把敏感度加入到矩阵中，并从右手边减去残差。Trilinos FAD数据类型让我们可以使用  <code>R_i.fastAccessDx(k)</code>  访问导数，因此我们将数据存储在一个临时数组中。然后，这些关于整行局部道夫的信息被一次性添加到特里诺斯矩阵中（支持我们选择的数据类型）。
...
在这里，我们的做法与前一个函数基本相同。在顶部，我们引入自变量。因为如果我们在两个单元之间的内部面上工作，也会使用当前的函数，所以自变量不仅是当前单元上的自由度，而且在内部面上的情况下，也是邻近单元上的自由度。
...
接下来，我们需要定义保守变量 ${\mathbf W}$ 在面的这一侧（ $ {\mathbf W}^+$ ）和另一侧（ ${\mathbf W}^-$ ）的值，对于 ${\mathbf W} =
{\mathbf W}^k_{n+1}$ 和 ${\mathbf W} = {\mathbf W}_n$  。"这一边 "的值可以用与前一个函数完全相同的方式计算，但注意 <code>fe_v</code> 变量现在是FEFaceValues或FESubfaceValues的类型。
...
计算 "对立面 "要复杂一些。如果这是一个内部面，我们可以像上面那样，简单地使用邻居的独立变量来计算它。
...
另一方面，如果这是一个外部边界面，那么 $\mathbf{W}^-$ 的值将是 $\mathbf{W}^+$ 的函数，或者它们将是规定的，取决于这里施加的边界条件的种类。     
...
其余的由一个实际上知道欧拉方程边界条件具体内容的函数来完成。请注意，由于我们在这里使用的是fad变量，敏感度将被适当地更新，否则这个过程将是非常复杂的。
...
现在我们有了 $\mathbf w^+$ 和 $\mathbf w^-$ ，我们可以去计算每个正交点的数值流量函数 $\mathbf H(\mathbf w^+,\mathbf
w^-, \mathbf n)$ 。在调用这个函数之前，我们还需要确定Lax-Friedrich的稳定参数。
...
现在以与前一个函数中的单元贡献完全相同的方式组装面项。唯一的区别是，如果这是一个内部面，我们还必须考虑到残余贡献对邻近单元自由度的敏感性。
...
在这里，我们实际解决了线性系统，使用Trilinos的Aztec或Amesos线性求解器。计算的结果将被写入传递给这个函数的参数向量中。其结果是一对迭代次数和最终的线性残差。
...
如果参数文件中指定应使用直接求解器，那么我们就会到这里。这个过程很简单，因为deal.II在Trilinos中提供了一个对Amesos直接求解器的封装类。我们所要做的就是创建一个求解器控制对象（这里只是一个假对象，因为我们不会进行任何迭代），然后创建直接求解器对象。在实际进行求解时，注意我们没有传递一个预处理程序。无论如何，这对直接求解器来说没有什么意义。 最后，我们返回求解器的控制统计信息&mdash；它将告诉我们没有进行过迭代，并且最终的线性残差为零，这里没有任何可能提供的更好的信息。
...
同样地，如果我们要使用迭代求解器，我们使用Aztec的GMRES求解器。我们也可以在这里使用Trilinos的迭代求解器和预处理类，但我们选择直接使用Aztec求解器。对于给定的问题，Aztec的内部预处理实现优于deal.II的包装类，所以我们在AztecOO求解器中使用ILU-T预处理，并设置了一堆可以从参数文件中修改的选项。         


还有两个实际问题。由于我们将右手边和求解向量建立为deal.II向量对象（而不是矩阵，后者是一个Trilinos对象），我们必须将Trilinos Epetra向量交给求解器。 幸运的是，他们支持 "视图 "的概念，所以我们只需发送一个指向deal.II向量的指针。我们必须为设置平行分布的向量提供一个Epetra_Map，这只是一个串行的假对象。最简单的方法是要求矩阵提供它的地图，我们要用它为矩阵-向量乘积做好准备。         


其次，Aztec求解器要我们传入一个Trilinos Epetra_CrsMatrix，而不是 deal.II包装类本身。所以我们通过trilinos_matrix()命令来访问Trilinos包装类中的实际Trilinos矩阵。Trilinos希望矩阵是非常量的，所以我们必须用const_cast手动删除常量。
...
这个函数是非常简单的。我们在这里并不假装知道一个好的细化指标会是什么。相反，我们假设 <code>EulerEquation</code> 类会知道这一点，所以我们只是服从于我们在那里实现的相应函数。
...
在这里，我们使用之前计算的细化指标来细化网格。在开始的时候，我们在所有的单元上循环，并标记那些我们认为应该被细化的单元。
...
然后，我们需要在进行细化的同时，将各种解向量从旧网格转移到新网格。SolutionTransfer类是我们的朋友；它有相当丰富的文档，包括例子，所以我们不会对下面的代码做太多评论。最后三行只是将其他一些向量的大小重新设置为现在的正确大小。
...
开始时，我们读取参数文件中指定的网格文件，设置DoFHandler和各种向量，然后在这个网格上插值给定的初始条件。然后，我们根据初始条件进行一系列的网格细化，以获得一个已经很好地适应初始解的网格。在这个过程结束时，我们输出初始解。
...
然后我们进入主时间步进循环。在顶部，我们简单地输出一些状态信息，以便人们可以跟踪计算的位置，以及显示非线性内迭代进度的表格的标题。
...
然后是牛顿内迭代，在每个时间步长中解决非线性问题。它的工作方式是将矩阵和右手边重置为零，然后组装线性系统。如果右手边的规范足够小，那么我们就宣布牛顿迭代已经收敛了。否则，我们求解线性系统，用牛顿增量更新当前解，并输出收敛信息。最后，我们检查牛顿迭代的次数是否超过了10次的限制--如果超过了，就说明迭代有可能出现了发散，继续迭代也没有什么好处。如果发生这种情况，我们就抛出一个异常，这个异常将在 <code>main()</code> 中被捕获，并在程序终止前显示状态信息。         


请注意，我们写AssertThrow宏的方式大体上等同于写<code>if (!(nonlin_iter @<= 10)) throw ExcMessage ("No convergence in nonlinear solver");</code>。唯一显著的区别是AssertThrow也确保被抛出的异常带有它产生的位置（文件名和行号）的信息。这在这里不是太关键，因为只有一个地方可能发生这种异常；然而，当人们想找出错误发生的地方时，它通常是一个非常有用的工具。
...
首先，我们更新时间，如果需要的话，产生图形输出。然后，我们通过近似 $\mathbf w^{n+1}\approx \mathbf w^n +
\delta t \frac{\partial \mathbf w}{\partial t} \approx \mathbf w^n
+ \delta t \; \frac{\mathbf w^n-\mathbf w^{n-1}}{\delta t} = 2
\mathbf w^n - \mathbf w^{n-1}$ 更新下一个时间步长的解决方案的预测器，以尝试使适应性更好地工作。 我们的想法是尝试在前面进行细化，而不是步入一个粗略的元素集并抹去旧的解决方案。 这个简单的时间推断器可以完成这个工作。有了这个，如果用户需要的话，我们再细化网格，最后继续进行下一个时间步骤。
...
我们用网格 <code>slide.inp</code> （该文件与本程序的源代码在同一目录下）和以下输入牌（可在同一目录下的 <code>input.prm</code> ）运行该问题。
...
这个输出报告了牛顿迭代的进度和时间步长。请注意，我们对牛顿迭代的实现确实显示了预期的二次收敛顺序：每一步的非线性残差的规范大致是前一步的规范的平方。这导致了我们在这里可以看到的非常快速的收敛。这一直保持到 $t=1.9$ 的时候，这时非线性迭代报告缺乏收敛性。
...
正如我们所看到的，当大质量的流体撞击到左下角时，会发生一些振荡，导致迭代的分歧。解决这个问题的一个懒办法是添加更多的粘性。如果我们将扩散功率设置为 $\eta = 1.5$ 而不是 $2.0$ ，模拟就能度过这一危机。那么，结果就会是这样。


  <img src="https://www.dealii.org/images/steps/developer/step-33.slide.ed2.gif " alt="" height="300">   

沉重的流体在重力作用下被拉下山坡，在那里与滑雪屋相撞，并被抛向空中！希望每个人都能逃过一劫。 希望每个人都能逃过一劫! 还有，我们可以看到由于人为的粘性，重质和轻质之间的界限很快就模糊了。
...
我们所选择的数值方案在人工黏度小的时候不是特别稳定，而在人工黏度大的时候则过于扩散。此外，众所周知，还有更先进的技术来稳定解决方案，例如流线扩散、最小二乘稳定项、熵粘性。
...
虽然牛顿法作为非线性求解器在时间步长足够小的情况下似乎效果很好，但线性求解器还可以改进。例如，在目前的方案中，只要我们使用迭代求解器，每个牛顿步骤都要重新计算ILU；同样，对于直接求解器，每个步骤都要计算牛顿矩阵的LU分解。这显然是一种浪费：从一个牛顿步骤到另一个牛顿步骤，可能还有不同的时间步骤，牛顿矩阵并没有发生根本性的变化：一个牛顿步骤的ILU或稀疏LU分解可能仍然是下一个牛顿或时间步骤的一个非常好的预处理。因此，避免重新计算是减少计算时间的一个好办法。

我们可以更进一步：由于接近收敛时，牛顿矩阵只发生一点点变化，我们可以定义一个准牛顿方案，在每次牛顿迭代中只重新计算残差（即右手边的向量），并重新使用牛顿矩阵。由此产生的方案很可能不是二次收敛的，我们不得不期望多做几次非线性迭代；然而，鉴于我们不必每次都花时间建立牛顿矩阵，由此产生的方案很可能更快。
...
在 ConservationLaw::assemble_cell_term 函数中计算的残差为 $R_i = \left(\frac{\mathbf{w}^{k}_{n+1} - \mathbf{w}_n}{\delta t}
    , \mathbf{z}_i \right)_K  +
      \theta \mathbf{B}({\mathbf{w}^{k}_{n+1}})(\mathbf{z}_i)_K +
      (1-\theta) \mathbf{B}({\mathbf{w}_{n}}) (\mathbf{z}_i)_K $ 这意味着我们在一个牛顿迭代步骤中计算了两次空间残差：一次是关于当前解 $\mathbf{w}^{k}_{n+1}$ ，另一次是关于最后一个时间步骤的解 $\mathbf{w}_{n}$ ，在通过一个时间步骤的所有牛顿迭代中保持相同。在牛顿迭代过程中缓存残差 $ \mathbf{B}({\mathbf{w}_{n}}) (\mathbf{z}_i)_K$ 的显式部分将节省大量的人力。


<a name="Otherconservationlaws"></a><h4>Other conservation laws</h4>


最后，作为超越立即解决欧拉方程的一个方向，这个程序非常努力地将所有具体到欧拉方程的实现分离到一个类（ <code>EulerEquation</code> 类），而将所有具体到组装矩阵和向量、非线性和线性求解器以及一般顶层逻辑的实现分离到另一个类（ <code>ConservationLaw</code> 类）。

通过替换这个类中的通量矩阵和数值通量的定义，以及那里定义的各种其他部分，应该可以将 <code>ConservationLaw</code> 类也应用于其他双曲守恒定律。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-33.cc" 。 
