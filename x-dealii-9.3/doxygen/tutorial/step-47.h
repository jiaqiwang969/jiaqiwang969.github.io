...
这对应于一个 "夹紧 "的结构，非零的 $j(\mathbf x)$ 意味着与水平面有一定的角度。
...
该方程的基本问题是，它的解有四个导数。在我们在 step-3 、 step-4 和其他几个教程中处理的拉普拉斯方程的情况下，人们乘以一个检验函数，进行积分，通过部分积分，最后在检验函数和检验函数上都只有一个导数--对于全局连续的函数，人们可以做到这一点，但在单元之间的界面上可能有结点。导数可能不在界面上定义，但那是在一个较低维的流形上（所以不在积分值中显示出来）。

但是对于双调方程来说，如果按照同样的程序，在整个领域（即所有单元的联盟）上使用积分，最终会在测试函数和试验函数上各有两个导数。如果使用通常的片状多项式函数，并在单元格界面上有结点，那么第一个导数将产生一个不连续的梯度，第二个导数在界面上有delta函数--但由于测试函数和试验函数的第二个导数都产生一个delta函数，我们将尝试对两个delta函数的乘积进行积分。例如，在1d中， $\varphi_i$ 是通常的片状线性 "帽子函数"，我们会得到这样的积分 
...
但这只有在以下情况下才行得通：(i)  $f(\cdot)$ 实际上在 $\hat x$ 处定义良好，(ii)如果它是有限的。另一方面，一个形式的积分 
...
那么下面的诀窍就会起作用（至少如果域是凸的，见下文）。与我们通过引入第二个变量从常规拉普拉斯方程得到 step-20 的混合拉普拉斯方程一样，我们在这里可以引入一个变量 $v=\Delta u$ ，然后可以用下面的 "混合 "系统取代上面的方程。
...
换句话说，我们最终得到的实质上是 $u,v$ 的两个耦合的拉普拉斯方程组，每个方程组都有迪里希勒型边界条件。我们知道如何解决这样的问题，使用 step-20 或 step-22 的技术为这个系统构建良好的求解器和预处理器应该不是很困难。所以这种情况处理起来非常简单。

  @note  值得指出的是，这只适用于边界有角的域，如果该域也是凸的--换句话说，如果没有重入角。  这听起来是一个相当随意的条件，但考虑到以下两个事实，它是有意义的。原始双调方程的解必须满足  $u\in H^2(\Omega)$  。另一方面，上面的混合系统重述表明， $u$ 和 $v$ 都满足 $u,v\in H^1(\Omega)$ ，因为这两个变量只解决一个泊松方程。换句话说，如果我们想保证混合问题的解 $u$ 也是原来的偏谐方程的解，那么我们需要能够以某种方式保证 $-\Delta u=v$ 的解实际上比只是 $H^1(\Omega)$ 更平稳。这一点可以作如下论证。对于凸域，<a href="https://en.wikipedia.org/wiki/Elliptic_operator#Elliptic_regularity_theorem">"elliptic
  regularity"</a>意味着，如果右侧 $v\in H^s$ ，那么 $u\in H^{s+2}$ 如果域是凸的并且边界足够光滑。(如果域的边界足够光滑，也可以保证这一点--但边界没有角的域在现实生活中不是很实用。)   我们知道 $v\in H^1$ ，因为它解决了方程 $-\Delta v=f$ ，但我们仍然留下了边界凸性的条件；我们可以证明多边形的凸域足以保证 $u\in H^2$ 在这种情况下的实现（光滑的有界凸域将导致 $u\in H^3$ ，但我们不需要这么多规则）。另一方面，如果域不是凸的，我们不能保证混合系统的解在 $H^2$ 中，因此可能得到一个不能等于原始双调方程的解。
...
混合系统的相同技巧在这里不起作用，因为我们最终会对 $u$ 有<i>both</i>迪里切特和诺伊曼边界条件，但对 $v$ 没有。


在20世纪90年代和21世纪初，这一难题的解决方案随着非连续加尔金法的出现而出现。与使用<i>discontinuous</i>形状函数处理拉普拉斯方程，通过惩罚不连续的大小来获得每个形状函数上有一个导数的方程的方案一样，我们可以使用使用<i>continuous</i>（但不是 $C^1$ 连续）形状函数的方案，惩罚导数的跳跃，以获得每个形状函数上有两个导数的方案。与拉普拉斯方程的内部惩罚（IP）方法相类似，这种用于双调方程的方案通常被称为 $C^0$ IP（或C0IP）方法，因为它使用 $C^0$ （连续但不可连续微分）形状函数与内部惩罚公式。


<a name="DerivationoftheC0IPmethod"></a><h3> Derivation of the C0IP method </h3>


我们以Susanne Brenner和Li-Yeng Sung在 "C  $^0$  多边形域上线性四阶边界值问题的内部惩罚方法" @cite Brenner2005 中提出的 $C^0$ IP方法为基础，该方法是针对具有 "钳制 "边界条件的双谐波方程而推出的。

如前所述，该方法依赖于使用 $C^0$ Lagrange有限元，其中 $C^1$ 的连续性要求被放宽并被内部惩罚技术所取代。为了推导这个方法，我们考虑一个 $C^0$ 形状函数 $v_h$ ，它在 $\partial\Omega$ 上消失。我们引入符号 $ \mathbb{F} $ 作为 $\mathbb{T}$ 的所有面的集合， $ \mathbb{F}^b $ 作为边界面的集合，而 $ \mathbb{F}^i $ 作为内部面的集合，以便在下面进一步使用。由于 $v_h$ 的高阶导数在每个界面 $e\in \mathbb{F}$ 上有两个值（由两个单元 $K_{+},K_{-} \in \mathbb{T}$ 共享），我们通过在 $e$ 上定义以下单值函数来应对这种不连续性。
...
为 $k =1,2$ （即梯度和二阶导数矩阵），其中 $\mathbf n$ 表示从 $K_+$ 到 $K_-$ 指向 $e$ 的单位向量法线。在文献中，这些函数分别被称为 "跳跃 "和 "平均 "操作。

为了得到 $C^0$ IP的近似值 $u_h$ ，我们将偏谐方程左乘以 $v_h$ ，然后对 $\Omega$ 进行积分。如上所述，我们不能用这些形状函数对 $\Omega$ 的所有部分进行积分，但我们可以对每个单元单独进行积分，因为形状函数只是每个单元的多项式。因此，我们首先在每个网格单元 $K \in {\mathbb{T}}$ 上使用下面的分项积分公式。
...
相反，我们要做的是认识到 $\nabla\Delta w_h = \text{grad}\,(\text{div}\,\text{grad}\, w_h)$ ，我们可以将这些操作重新排序为 $\nabla\Delta w_h = \text{div}\,(\text{grad}\,\text{grad}\, w_h)$ ，我们通常写成 $\text{grad}\,\text{grad}\, w_h = D^2 w_h$ 来表示这是二阶导数的 "Hessian "矩阵。通过这种重新排序，我们现在可以整合发散，而不是梯度算子，我们得到以下结果。
...
这里，冒号表示对其左边和右边的矩阵指数的双重收缩，也就是两个张量之间的标量乘积。两个向量 $a \otimes b$ 的外积得到了矩阵 $(a \otimes b)_{ij} = a_i b_j$  。

然后，我们对所有单元格 $K \in  \mathbb{T}$ 进行求和，并考虑到这意味着每个内部面在求和中出现两次。因此，如果我们把所有的东西分成单元格内部的积分之和和单元格界面的单独之和，我们就可以使用上面定义的跳跃和平均运算符。还有两个步骤。首先，由于我们的形状函数是连续的，形状函数的梯度可能是不连续的，但连续性保证了实际上只有梯度的法向分量是不连续的，而切向分量是连续的。其次，当网格大小为零时，产生的离散公式并不稳定，为了得到一个稳定的公式，收敛到正确的解，我们需要添加以下条款。
...
然后，在对出现的情况进行抵消后，我们得出以下双调方程的C0IP公式：找到 $u_h$ ，使 $u_h =
g$ 在 $\partial \Omega$ 上和 
...
在边界界面 $e \in \mathbb{F}^b$ 上，也确保在极限 $h\rightarrow 0$ ， $u_h$ 收敛为 $C^1$ 的连续函数。  $\gamma$ 被选择为足够大，以保证方法的稳定性。我们将在下面的程序中讨论我们的选择。


<a name="ConvergenceRates"></a><h4>Convergence Rates </h4>

在多边形域上，双调方程的弱解 $u$ 住在 $H^{2 +\alpha}(\Omega)$ 中，其中 $\alpha \in(1/2, 2]$ 是由 $\Omega$ 的角的内角决定。例如，只要 $\Omega$ 是凸的， $\alpha=1$ ； $\alpha$ 可能小于1，如果域有重心角，但如果所有内角之一接近 $\pi$ ， $\alpha$ 就接近于 $1$  。

现在假设 $C^0$ 的IP解 $u_h$ 被 $C^0$ 的形状函数近似为多项式程度 $p \ge 2$ 。然后，上面概述的离散化产生了下面讨论的收敛率。


<b>Convergence in the $C^0$ IP-norm</b> 

理想情况下，我们希望测量 "能量准则 "的收敛性  $\|D^2(u-u_h)\|$  。然而，这并不可行，因为同样，离散解 $u_h$ 并没有两个（弱）导数。相反，我们可以定义一个离散的（ $C^0$  IP）半规范，它 "等同于 "能量规范，如下所示。
...
当然，只有在精确解足够光滑的情况下，这才是真的。事实上，如果 $f \in H^m(\Omega)$ 有 $m \ge 0$ ， $u \in H^{2+\alpha}(\Omega)$ 其中有 $ 2 < 2+\alpha  \le m+4$ ，那么 $C^0$ IP方法的收敛率是 $\mathcal{O}(h^{\min\{p-1, \alpha\}})$  。换句话说，只有当解平滑到 $\alpha\ge p-1$ 时，才能期待最佳收敛率；这只有在以下情况下才会发生：(i) 域是凸的，边界足够平滑；(ii)  $m\ge p-3$  。当然，在实践中，解决方案是什么就是什么（与我们选择的多项式程度无关），那么最后一个条件可以等同于说，如果 $m$ 不大，那么选择 $p$ 大肯定没有意义。换句话说， $p$ 唯一合理的选择是 $p\le
m+3$ ，因为更大的多项式度数不会导致更高的收敛阶数。

在这个程序中，我们有点懒得去实际实现这个等价的seminorm--尽管它不是很困难，而且会成为一个很好的练习。相反，我们将在程序中简单地检查一下 "坏的" $H^2$ 语义准则是什么？
...
产生的结果。从理论上看，这个准则的收敛率当然不可能比 $|\cdot|_h$ 的收敛率高<i>worse</i>，因为它只包含必要项的一个子集，但至少可以想象到它会更好。还有一种情况是，即使程序中存在一个错误，我们也能得到最佳收敛率，而这个错误只会在 $|\cdot|_h$ 中出现的额外条款中显示出次优的收敛率。但是，人们可能希望，如果我们在破碎规范和下面讨论的规范中得到最优速率，那么这个程序确实是正确的。结果部分将证明，我们在所有显示的规范中都得到了最优率。


<b>Convergence in the $L_2$-norm</b> 

在  $L_2$  -规范中的最佳收敛率是  $\mathcal{O}(h^{p+1})$  提供  $p \ge 3$  。更多细节可以在  @cite Engel2002  的定理4.6中找到。
...
鉴于我们在最好的情况下期望 $\mathcal{O}(h^{p-1})$ 等同于 $H^2$ 半规则的规范，以及 $\mathcal{O}(h^{p+1})$ 等同于 $L_2$ 规范，人们可能会问在 $H^1$ 半规则中会发生什么，它是介于其他两个规范之间。一个合理的猜测是，我们应该期待 $\mathcal{O}(h^{p})$  。可能在某个地方有一篇论文证明了这一点，但我们也验证了这个猜想在下面的实验中是真实的。
...
g(\mathbf x)$ 和 $\Delta u(\mathbf x)= h(\mathbf x)$ 上的 $\partial\Omega$ --的IP方法的推导，可以通过对 $\mathcal{A}(\cdot,\cdot)$ 和 $\mathcal{F}(\cdot)$ 的适当修改得到，在书中的章节 @cite Brenner2011 中描述。


<a name="Thetestcase"></a><h3>The testcase</h3>


剩下要描述的最后一步是这个程序解决的问题。像往常一样，三角函数是一个既好又坏的选择，因为它不在我们可能寻求解的任何多项式空间中，同时又比实数解通常更平滑（这里，它在 $C^\infty$ 中，而实数解在凸多边形域上通常只在 $H^3$ 左右，如果域不是凸的，则在 $H^2$ 和 $H^3$ 之间）。但是，由于我们没有办法用相对简单的公式来描述现实问题的解决方案，所以我们只是用下面的方法，在域 $\Omega$ 的单位平方上。
...
该程序有 `ExactSolution::Solution` 和 `ExactSolution::RightHandSide` 类，可以编码这些信息。<a name="CommProg"></a><h1> The commented program</h1>
...
前面几个include文件已经在前面的例子中使用过了，所以我们在这里不再解释它们的含义。程序的主要结构与例如 step-4 的结构非常相似，因此我们包含了许多相同的头文件。
...
在下面的命名空间中，让我们定义精确解，我们将与数值计算的解进行比较。它的形式是 $u(x,y) = \sin(\pi x) \sin(\pi y)$ （只实现了2d情况），该命名空间还包含一个对应于产生该解的右手边的类。
...
下面几段代码更有意思。它们都与线性系统的装配有关。虽然组装单元内部项的难度不大--这在本质上就像组装拉普拉斯方程的相应项一样，你已经在 step-4 或 step-6 中看到了这是如何工作的，例如，困难在于公式中的惩罚项。这需要在单元格的界面上对形状函数的梯度进行评估。因此，至少需要使用两个FEFaceValues对象，但如果其中一个面是自适应细化的，那么实际上需要一个FEFaceValues和一个FESubfaceValues对象；我们还需要跟踪哪些形状函数在哪里，最后我们需要确保每个面只被访问一次。所有这些对于我们真正想要实现的逻辑（即双线性形式中的惩罚项）来说都是一笔不小的开销。因此，我们将利用FEInterfaceValues类--这是deal.II中的一个辅助类，它允许我们抽象出两个FEFaceValues或FESubfaceValues对象，直接访问我们真正关心的东西：跳跃、平均数等。   


但这还没有解决我们的问题，即当我们在所有单元格和它们的所有面中循环时，必须跟踪我们已经访问过哪些面。为了使这个过程更简单，我们使用了 MeshWorker::mesh_loop() 函数，它为这个任务提供了一个简单的接口：基于WorkStream命名空间文档中概述的想法， MeshWorker::mesh_loop() 需要三个函数对单元、内部面和边界面进行工作。这些函数在从头开始的对象上工作，以获得中间结果，然后将其计算结果复制到复制数据对象中，由一个复制器函数将其复制到全局矩阵和右侧对象中。   


然后，下面的结构提供了这种方法所需的从头开始和复制对象。你可以查阅WorkStream命名空间以及 @ref threads "多处理器并行计算 "模块，以了解更多关于它们通常如何工作的信息。
...
下一个构建模块是在网格的每个内部面组装惩罚项。正如 MeshWorker::mesh_loop(), 文档中所描述的，这个函数接收到的参数表示一个单元和其相邻的单元，以及（对于这两个单元中的每一个）我们必须整合的面（以及潜在的子面）。同样，我们也得到了一个从头开始的对象，以及一个用于放置结果的副本对象。     


这个函数本身有三个部分。在顶部，我们初始化FEInterfaceValues对象，并创建一个新的 `CopyData::FaceData` 对象来存储我们的输入。这将被推到`copy_data.face_data`变量的末尾。我们需要这样做，因为我们为一个给定的单元整合的面（或子面）的数量因单元而异，这些矩阵的大小也不同，取决于面或子面邻近的自由度是什么。正如 MeshWorker::mesh_loop(), 文档中所讨论的，每次访问一个新的单元时，复制对象都会被重置，因此我们推送到`copy_data.face_data()`末尾的内容实际上就是后来的`copier`函数在复制每个单元的贡献到全局矩阵和右侧对象时所能看到的。
...
第二部分涉及到确定惩罚参数应该是什么。通过观察双线性形式中各种项的单位，很明显，惩罚必须具有 $\frac{\gamma}{h_K}$ 的形式（即超过长度尺度的一个），但应该如何选择无维数 $\gamma$ 并不先验。从拉普拉斯方程的不连续Galerkin理论来看，人们可能猜想正确的选择是 $\gamma=p(p+1)$ 是正确的选择，其中 $p$ 是所用有限元的多项式程度。我们将在本方案的结果部分更详细地讨论这个选择。       
...
第三部分是条款的组合。由于这些条款包含了矩阵和右手边的条款，所以现在稍微复杂一些。前者与上面所说的内部面完全相同，如果我们只是适当地定义了跳跃和平均（这就是FEInterfaceValues类所做的）。后者需要我们评估边界条件 $j(\mathbf x)$ ，在当前情况下（我们知道确切的解决方案），我们从 $j(\mathbf x) = \frac{\partial u(\mathbf x)}{\partial
{\mathbf n}}$ 中计算出来。然后，要添加到右手边矢量的项是  $\frac{\gamma}{h_e}\int_e
...
在设置了所有这些之后，剩下的就是创建一个scratch和copy数据对象，并调用 MeshWorker::mesh_loop() 函数，然后翻阅所有的单元格和面，调用各自的worker，然后是copyer函数，将东西放入全局矩阵和右侧。作为一个额外的好处， MeshWorker::mesh_loop() 以并行方式完成所有这些工作，使用你的机器恰好有多少个处理器核心。
...
表演到此基本上结束了。其余的函数并不太有趣或新颖。第一个函数只是使用直接求解器来求解线性系统（另见 step-29  ）。
...
下一个函数评估了计算出的解和精确解之间的误差（在这里我们知道精确解，因为我们已经选择了右手边和边界值的方式，所以我们知道相应的解）。在下面的前两个代码块中，我们计算了 $L_2$ 准则和 $H^1$ 半准则的误差。
...
现在也计算一下 $H^2$ 半规范误差的近似值。实际的 $H^2$ 半规范要求我们对解 $u_h$ 的二阶导数进行积分，但考虑到我们使用的拉格朗日形状函数， $u_h$ 在单元间的界面上当然有结点，因此二阶导数在界面上是奇异的。因此，我们实际上只对单元的内部进行积分，而忽略了界面的贡献。这不是*等同于问题的能量准则，但是仍然给了我们一个关于误差收敛速度的概念。     


我们注意到，我们可以通过定义一个等同于能量准则的准则来解决这个问题。这将涉及到不仅要像我们下面所做的那样将细胞内部的积分相加，而且还要为 $u_h$ 的导数在界面上的跳跃增加惩罚项，并对这两种项进行适当的缩放。我们将把这个问题留给以后的工作。
...
同样无趣的是生成图形输出的函数。它看起来和 step-6 中的那个一模一样，比如说。
...
我们用介绍中讨论的右手边和边界值运行程序。这些将产生域 $\Omega = (0,1)^2$ 上的解 $u = \sin(\pi x) \sin(\pi y)$  。我们用 $Q_2$ 、 $Q_3$ 和 $Q_4$ 元素来测试这个设置，我们可以通过`main()`函数中的`fe_degree`变量来改变。随着网格的细化， $L_2$ 的收敛率、 $H^1$ 的近似值率和 $H^2$ 的近似值收敛率对于 $Q_2$ 应该是2、2、1左右（如介绍中所讨论的 $L_2$ 的规范为次优）；对于 $Q_3$ 是4、3、2；而对于 $Q_4$ 是5、4、3。

从文献来看，并不清楚惩罚参数 $\gamma$ 应该是什么。例如， @cite Brenner2009 指出它需要大于1，并选择 $\gamma=5$  。FEniCS/Dolphin教程选择它为 $\gamma=8$  ，见https://fenicsproject.org/docs/dolfin/1.6.0/python/demo/documented/biharmonic/python/documentation.html 。  @cite Wells2007 使用的 $\gamma$ 值大于Kirchhoff板的元素所属的边数（见他们的第4.2节）。这表明也许 $\gamma = 1$ ,  $2$ , 太小了；另一方面， $p(p+1)$ 的值将是合理的，其中 $p$ 是多项式的程度。通过与拉普拉斯方程的不连续Galerkin公式相比较，人们期望最后一个选择是可行的（例如，见 step-39 和 step-74 中的讨论），而且它在这里也将证明是可行的。但是我们应该检查 $\gamma$ 的哪个值是正确的，我们将在下面这样做；改变 $\gamma$ 在`assemble_system()`中定义的两个`face_worker`和`boundary_worker`函数中很容易。
...
</table>  我们可以看到 $L_2$ 的收敛率约为2， $H^1$  -seminorm收敛率约为2， $H^2$  -seminorm收敛率约为1。后两者与理论上的预期收敛率相符；对于前者，我们没有定理，但鉴于介绍中的评论，对其为次优的情况并不感到惊讶。
...
</table>  我们可以看到， $L_2$ 的收敛率约为4， $H^1$  -seminorm收敛率约为3，而 $H^2$  -seminorm收敛率约为2。当然，这符合我们的理论预期。
...
</table>  我们可以看到， $L_2$ 规范收敛率约为5， $H^1$  -seminorm收敛率约为4，而 $H^2$  -seminorm收敛率约为3。在最细的网格上， $L_2$ 规范收敛率比我们的理论预期小得多，因为线性求解器由于舍入而成为限制因素。当然，在这种情况下， $L_2$ 的误差也已经非常小了。
...
为了与上面的结果进行比较，现在让我们也考虑一下我们简单地选择 $\gamma=1$ 的情况。
...
p(p+1)$ 的结果相比，变化更大。同样，我们可以对 $Q_3$ 和 $Q_4$ 元素重复这种实验。在这两种情况下，我们会发现我们获得了大致的预期收敛率。那么，更感兴趣的可能是比较误差的绝对大小。在上表中，对于 $Q_2$ 情况，最细网格上的误差在 $\gamma=p(p+1)$ 和 $\gamma=2$ 情况下是可比的，而对于 $Q_3$ ， $\gamma=2$ 的误差远远大于 $\gamma=p(p+1)$ 。对于 $Q_4$ 的情况也是如此。


<a name="Conclusionsforthechoiceofthepenaltyparameter"></a><h3> Conclusions for the choice of the penalty parameter </h3>


关于应该使用哪种 "合理 "的惩罚参数选择的结论是， $\gamma=p(p+1)$ 产生了预期的结果。因此，它是目前编写的代码所使用的。
...
- 从理论的角度来看，上面的收敛结果只使用了 "破碎的" $H^2$ 半规范 $|\cdot|^\circ_{H^2}$ ，而不是 "等效的 "规范 $|\cdot|_h$  。这足以让我们相信，这个程序并没有从根本上被破坏。然而，测量我们有理论结果的实际规范的误差可能是有趣的。例如，使用FEInterfaceValues类与 MeshWorker::mesh_loop() 结合，实现这一补充应该不会过于困难，其精神与我们用于装配线性系统的精神相同。


<a name="Derivationforthesimplysupportedplates"></a> <h4> Derivation for the simply supported plates </h4>


  类似于实施中解决的 "夹持 "边界条件，我们将推导出 $C^0$ IP有限元方案，用于简支板。  @f{align*}{
    \Delta^2 u(\mathbf x) &= f(\mathbf x)
    \qquad \qquad &&\forall \mathbf x \in \Omega,
    u(\mathbf x) &= g(\mathbf x) \qquad \qquad
    &&\forall \mathbf x \in \partial\Omega, \\
  ...
  我们用测试函数 $v_h$ 乘以偏谐方程，并对 $ K $ 进行积分，得到。  @f{align*}{
  ...
  对所有单元格 $K \in  \mathbb{T}$ 进行求和，因为 $\Delta u_h$ 的法线方向在两个单元格共享的每条内边上都指向相反的方向， $v_h = 0$ 在 $\partial \Omega$ 上，@f{align*}{
  \sum_{K \in \mathbb{T}} \int_{\partial K} v_h \frac{\partial (\Delta u_h)}{\partial \mathbf{n}} = 0,
  @f} 

  并根据单元格界面上的跳跃定义，@f{align*}{
  ...
  其中 $\mathbb{F}^i$ 是内部面的集合。  这导致我们得出@f{align*}{
  ...
  为了使离散问题对称化和稳定化，我们加入对称化和稳定化项。  我们最终得到双调方程的 $C^0$ IP有限元方案：找到 $u_h$ ，使 $u_h =g$ 在 $\partial \Omega$ 和@f{align*}{
  \mathcal{A}(v_h,u_h)&=\mathcal{F}(v_h) \quad \text{holds for all test functions } v_h,
  @f} 上 
  ...
  这个边界案例的实现与 "钳制 "版本类似，只是在系统组装时不再需要 "边界_工人"，并且根据配方改变右手边。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-47.cc"   
