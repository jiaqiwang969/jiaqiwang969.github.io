...
这个例子是基于2d中的拉普拉斯方程，涉及的问题是，如果膜受到某种外力的偏转，但也受到障碍物的限制，会发生什么？换句话说，想想一个弹性膜在边界被夹在一个矩形框架上（我们选择 $\Omega =
\left[-1,1\right]^2$ ），由于重力作用而下垂。如果膜下有一个障碍物，阻止它达到平衡位置，如果重力是唯一存在的力，现在会发生什么？在目前的例子程序中，我们将考虑在膜下有一个楼梯的障碍物，重力将膜推向它。

这个问题通常被称为 "障碍物问题"（也见<a
href="http://en.wikipedia.org/wiki/Obstacle_problem">this Wikipedia article</a>），它的结果是一个变分不等式，而不是一个变分方程，当放到弱形式时。下面我们将从经典的表述中推导出它，但在我们继续讨论数学问题之前，让我们展示一下我们将在本教程程序中考虑的问题的解决方案是怎样的，以获得一些我们应该期待的直觉。
...
与 $u\in H^2(\Omega)$  .    $u$ 是一个标量值函数，表示膜的垂直位移。第一个方程被称为平衡条件，有一个面积密度为  $f$  的力。这里，我们将考虑这个力是重力。第二个方程被称为胡克定律，即应力 $\sigma$ 与位移 $u$ 的梯度成正比（比例常数，通常用 $E$ 表示，这里被设定为1，但不失一般性；如果它是常数，它可以被放入右边的函数）。在边界，我们有零迪里希特条件。显然，前两个方程可以结合起来，得到 $-\Delta u \ge f$  。

直观地说，重力作用是向下的，所以 $f(\mathbf x)$ 是一个负函数（我们在这个程序中选择 $f=-10$ ）。那么第一个条件意味着作用在膜上的总力是重力加上一些正的东西：即障碍物在它们两个接触的地方对膜施加的向上的力。这个额外的力有多大？我们还不知道（我们也不知道它实际作用的 "位置"），但它必须使膜不能穿透障碍物。

上面的第四个等式和最后一个不等式构成了障碍条件，它必须在整个领域的每一点上都成立。这两个条件中的后者意味着膜必须在任何地方都高于障碍物 $g(\mathbf x)$ 。倒数第二个等式，通常称为 "互补条件"，说的是在膜不与障碍物接触的地方（即那些 $\mathbf x$ 和 $u(\mathbf x) - g(\mathbf x) \neq 0$ 的地方），那么 $-\Delta u=f$ 在这些地方；换句话说，没有额外的力作用在那里，正如预期的那样。另一方面，在 $u=g$ 的地方，我们可以有 $-\Delta u-f
...
这是变分不等式的典型形式，不仅 $v$ 出现在双线性形式中，实际上 $v-u$  也出现在双线性形式中。原因是这样的：如果 $u$ 不受约束，那么我们可以在 $G$ 中找到测试函数 $v$ ，这样 $v-u$ 可以有任何符号。通过选择测试函数 $v_1,v_2$ 使 $v_1-u = -(v_2-u)$ ，可以看出，如果两边事实上相等，那么不等式只能对 $v_1$ 和 $v_2$ 都成立，也就是说，我们得到一个变异的平等。

另一方面，如果 $u=g$ ，那么 $G$ 只允许检验函数 $v$ ，所以事实上 $v-u\ge 0$  。这意味着我们不能像上面那样用 $v-u$ 和 $-(v-u)$ 来测试这个方程，所以我们不能再得出两边实际上相等的结论。因此，这就模仿了我们上面讨论互补性条件的方式。
...
上面的变分不等式在工作中是很尴尬的。因此我们想把它重新表述为一个等价的鞍点问题。我们引入拉格朗日乘子 $\lambda$ 和拉格朗日乘子 $V$ 、 $K \dealcoloneq \{\mu\in V': \langle\mu,v\rangle\geq 0,\quad \forall
v\in V, v \le 0 \}$ 的凸锥 $K\subset V'$ 、 $V'$ 对偶空间，其中 $\langle\cdot,\cdot\rangle$ 表示 $V'$ 和 $V$ 之间的对偶性配对。直观地说， $K$ 是所有 "非正函数 "的锥体，除了 $K\subset (H_0^1)'$ ，所以也包含了除正函数外的其他对象。这就产生了。
...
Glowinski, Lions and Tr&eacute;moli&egrave;res.中已经说明了这个鞍点问题的 $(u,\lambda)\in V\times K$ 的存在和唯一性。Numerical Analysis of Variational Inequalities, North-Holland, 1981. 
...
有不同的方法来解决变分不等式。作为一种可能性，你可以把鞍点问题理解为一个带有不等式约束的凸二次方程序（QP）。

为了达到这个目的，让我们假设我们用相同的有限元空间离散 $u$ 和 $\lambda$ ，例如通常的 $Q_k$ 空间。然后我们会得到方程@f{eqnarray*}
 &A U + B\Lambda = F,&\\
 &[BU-G]_i \geq 0, \quad \Lambda_i \leq 0,\quad \Lambda_i[BU-G]_i = 0
\qquad \forall i.&
@f} 

其中 $B$ 是所选有限元空间上的质量矩阵，上述指数 $i$ 是针对位于域内部的自由度集合 $\cal S$ 中的所有自由度（我们在周边有迪里希条件）。然而，如果我们在组合产生这个质量矩阵的所有项时使用一个特殊的正交规则，即一个正交公式，其中正交点只位于定义形状函数的插值点；因为除了一个形状函数外，所有的形状函数在这些位置都是零，所以我们得到一个对角线质量矩阵，即 
...
其中 $g_h$ 是 $g$ 的一个合适的近似值。然后， $B_{ii}$ 和 $G_i$ 定义中的积分由梯形法则来近似。有了这个，上面的方程可以重述为@f{eqnarray*}
 &A U + B\Lambda = F,&\\
...
在这个程序中，我们选择 $c = 100$ 。这是一种惩罚参数，取决于问题本身，需要选择足够大的参数；例如，如果我们使用7个全局细化，使用当前程序对 $c = 1$ 没有收敛作用）。) 

经过一番头疼之后，人们可以说服自己，上面的不等式可以等效地改写为@f{equation*}
 C([BU]_i,\Lambda_i) = 0, \qquad \forall i\in{\cal S}.
@f}

我们在这里将使用的原始-双重活动集策略是一个迭代方案，它基于这个条件来预测下一个活动集和非活动集 $\mathcal{A}_k$ 和 $\mathcal{F}_k$ （也就是那些指数 $i$ 的互补集，对其来说 $U_i$ 要么等于要么不等于障碍物 $B^{-1}G$ 的值）。关于这种方法的更深入的处理，见Hintermueller, Ito, Kunisch: 作为半光滑牛顿方法的原始-对偶主动集策略，SIAM J. OPTIM.，2003年，第13卷，第3期，第865-888页。
...
1. 初始化 $\mathcal{A}_k$ 和 $\mathcal{F}_k$  ，使 $\mathcal{S}=\mathcal{A}_k\cup\mathcal{F}_k$ 和 $\mathcal{A}_k\cap\mathcal{F}_k=\emptyset$ 并设置 $k=1$  。2. 找到满足@f{align*}
  AU^k + B\Lambda^k &= F,\\
  [BU^k]_i &= G_i\quad&&\forall i\in\mathcal{A}_k,\\
  \Lambda_i^k &= 0\quad&&\forall i\in\mathcal{F}_k.
 @f}的原始-双数对 $(U^k,\Lambda^k)$  。

 注意第二个和第三个条件意味着 $|S|$ 的未知数是固定的，第一个条件产生了确定 $U$ 和 $\Lambda$ 所需的剩余 $|S|$ 方程。3. 用@f{equation*}
 \begin{split}
...
4. 如果 $\mathcal{A}_{k+1}=\mathcal{A}_k$ （然后，显然也是 $\mathcal{F}_{k+1}=\mathcal{F}_k$ ），则停止，否则设置 $k=k+1$ 并进入步骤（2）。

该方法被称为 "原始-双重"，因为它同时使用原始变量（位移 $U$ ）和双重变量（拉格朗日乘数 $\Lambda$ ）来确定下一个活动集。

在本节的最后，让我们补充两点意见。首先，对于任何满足这些条件的原始-对偶 $(U^k,\Lambda^k)$ ，我们可以区分以下几种情况。

1.   $\Lambda^k_i + c([BU^k]_i - G_i) < 0$  （i主动）。    <br>  然后是 $[BU^k]_i<G_i$ 和 $\Lambda^k_i=0$ （渗透）或 $\Lambda^k_i<0$ 和 $[BU^k]_i=G_i$ （压载）。2.  2.  $\Lambda^k_i + c([BU^k]_i - G_i)\geq 0$  (i不活动)。    <br>  然后是 $[BU^k]_i\geq G_i$ 和 $\Lambda^k_i=0$ （无接触）或 $\Lambda^k_i\geq0$ 和 $[BU^k]_i=G_i$ （解压负荷）。

第二，上面的方法在直觉上似乎是正确的，也是有用的，但有点临时性的。然而，它可以通过以下方式简明地推导出来。为此，请注意，我们要解决非线性系统@f{eqnarray*}
 &A U + B\Lambda = F,&\\
 &C([BU-G]_i, \Lambda_i) = 0,
\qquad \forall i.&
@f}

我们可以通过始终围绕前一个迭代进行线性化（即应用牛顿方法）来迭代求解，但为此我们需要对不可微的函数 $C(\cdot,\cdot)$ 进行线性化。也就是说，它是可微的，事实上我们有@f{equation*}
...
我们将矩阵 $A,B$ 以及向量以自然的方式分割成行和列，其索引属于活动集 ${\mathcal{A}_k}$ 或非活动集 ${\mathcal{F}_k}$ 。

我们也可以通过设置 $\delta U^k \dealcoloneq
U^{k+1} - U^k$ 和 $\delta \Lambda^k \dealcoloneq \Lambda^{k+1} - \Lambda^k$ 并将所有已知项带到右手边来解决我们感兴趣的变量，而不是求解更新 $\delta U, \delta \Lambda$ ，。这就得到了@f{equation*}
\begin{pmatrix}
 A_{\mathcal{F}_k\mathcal{F}_k} & A_{\mathcal{F}_k\mathcal{A}_k} & B_{\mathcal{F}_k} & 0\\
 A_{\mathcal{A}_k\mathcal{F}_k} & A_{\mathcal{A}_k\mathcal{A}_k} & 0 & B_{\mathcal{A}_k}\\
 0 & 0 & Id_{\mathcal{F}_k} & 0\\
...
我们甚至可以进一步推动这一点。很容易看出，我们可以消除第三行和第三列，因为这意味着 $\Lambda_{\mathcal{F}_k} = 0$ ：@f{equation*}
\begin{pmatrix}
...
这表明，事实上我们只需要解决位于活动集上的拉格朗日乘数。通过考虑第二行，我们将通过@f{equation*}
 \Lambda^k_S = B^{-1}\left(f_{\mathcal{S}} - A_{\mathcal{S}}U^k_{\mathcal{S}}\right).
@f}恢复全部的拉格朗日乘数向量。

由于第三行和 $B_{\mathcal{A}_k}$ 是一个对角线矩阵的事实，我们能够直接计算出 $U^k_{\mathcal{A}_k}=B^{-1}_{\mathcal{A}_k}G_{\mathcal{A}_k}$ 。因此，我们也可以把线性系统写成如下。@f{equation*}
\begin{pmatrix}
...
幸运的是，这种形式很容易得出：我们只需建立通常的拉普拉斯线性系统@f{equation*}
\begin{pmatrix}
...
本教程与  step-4  非常相似。程序的总体结构遵循 step-4 ，但略有不同。
...
如果你想了解当前的程序，你可能想阅读一下 step-4 的内容。<a name="CommProg"></a> <h1> The commented program</h1>
...
这个类提供了描述障碍物问题所需的所有函数和变量。它与我们在 step-4 中要做的事情很接近，所以相对简单。唯一真正的新组件是计算主动集合的update_solution_and_constraints函数和一些描述线性系统原始（无约束）形式所需的变量（ <code>complete_system_matrix</code> 和 <code>complete_system_rhs</code> ），以及主动集合本身和主动集合公式中用于缩放拉格朗日乘数的质量矩阵 $B$ 的对角线。其余内容与 step-4 相同。
...
在下文中，我们定义了描述右侧函数、迪里切特边界值和障碍物高度的类，作为 $\mathbf x$ 的函数。在这三种情况下，我们都从Function@<dim@>派生出这些类，尽管在 <code>RightHandSide</code> 和 <code>Obstacle</code> 的情况下，这更多的是出于惯例而非必要，因为我们从未将这类对象传递给库。在任何情况下，鉴于我们选择了 $f=-10$ ,  $u|_{\partial\Omega}=0$  ，右手和边界值类的定义是明显的。
...
请注意，在调用这个函数时，约束对象只包含边界值约束；因此我们不必在最后的复制-本地-全局步骤中注意保留矩阵项的值，这些项以后可能受到活动集的约束。   


还要注意的是，只有当我们实际上有 $Q_1$ 元素时，使用梯形规则的技巧才有效。对于更高阶的元素，我们需要使用一个正交公式，在有限元的所有支持点都有正交点。构建这样一个正交公式其实并不难，但不是这里的重点，所以我们只是在函数的顶部断言，我们对有限元的隐含假设事实上得到了满足。
...
在某种意义上，这是本程序的核心功能。 它更新了引言中讨论的受限自由度的活动集，并从中计算出AffineConstraints对象，然后可以用来在下一次迭代的解中消除受限自由度。同时，我们将解决方案的受限自由度设置为正确的值，即障碍物的高度。   


从根本上说，这个函数是相当简单的。我们必须对所有自由度进行循环，并检查函数 $\Lambda^k_i +
c([BU^k]_i - G_i) = \Lambda^k_i + cB_i(U^k_i - [g_h]_i)$ 的符号，因为在我们的例子中 $G_i = B_i[g_h]_i$  。为此，我们使用介绍中给出的公式，通过该公式我们可以计算出拉格朗日乘数，作为原始线性系统的残差（通过变量 <code>complete_system_matrix</code> and <code>complete_system_rhs</code> 给出。在这个函数的顶部，我们使用一个属于矩阵类的函数来计算这个残差。
...
计算 contact_force[i] = - lambda[i] * diagonal_of_mass_matrix[i] 。
...
下一步是重置活动集和约束对象，并在所有自由度上开始循环。由于我们不能只是在解向量的所有元素上循环，这使事情变得稍微复杂，因为我们没有办法找出一个自由度与哪个位置相关；然而，我们需要这个位置来测试一个自由度的位移是大于还是小于这个位置的障碍物的高度。     


我们通过对所有单元和定义在每个单元上的DoF进行循环来解决这个问题。在这里，我们使用 $Q_1$ 函数来描述位移，对于该函数，自由度总是位于单元格的顶点上；因此，我们可以通过询问顶点来获得每个自由度的索引及其位置。另一方面，这显然对高阶元素不起作用，因此我们增加了一个断言，确保我们只处理所有自由度都位于顶点的元素，以避免在有人想玩增加解的多项式程度时用非功能性代码绊倒自己。     


循环处理单元格而不是自由度的代价是，我们可能会不止一次遇到一些自由度，即每次我们访问与给定顶点相邻的一个单元格时。因此，我们必须跟踪我们已经接触过的顶点和尚未接触的顶点。我们通过使用一个标志数组 <code>dof_touched</code>  来做到这一点。
...
现在我们知道我们还没有碰过这个顶点，让我们得到那里的位移函数的值以及障碍函数的值，并以此来决定当前顶点是否属于活动集。为此，我们使用上面和介绍中给出的函数。           
...
在最后一步，我们将迄今为止从活动集得到的对DoF的约束集添加到那些由Dirichlet边界值产生的约束，并关闭约束对象。
...
关于求解函数，其实没什么好说的。在牛顿方法的背景下，我们通常对非常高的精度不感兴趣（为什么要求一个高度精确的线性问题的解，而我们知道它只能给我们一个非线性问题的近似解），因此我们使用ReductionControl类，当达到一个绝对公差（为此我们选择 $10^{-12}$ ）或当残差减少某个系数（这里是 $10^{-3}$ ）时停止反复。
...
这是一个对一切都有顶层控制的函数。 它并不长，而且事实上相当直接：在主动集方法的每一次迭代中，我们都会组装线性系统，求解它，更新主动集并将解投射回可行集，然后输出结果。只要主动集在前一次迭代中没有变化，迭代就会终止。   


唯一比较棘手的部分是，我们必须在第一次迭代组装好线性系统（即矩阵和右手边）后保存它。原因是这是唯一的步骤，我们可以在没有任何接触约束的情况下访问建立的线性系统。我们需要这个来计算其他迭代中解决方案的残差，但是在其他迭代中，我们形成的线性系统中对应于约束自由度的行和列被消除了，因此我们不能再访问原始方程的全部残差。
...
而这就是主函数。它遵循所有其他主函数的模式。调用初始化MPI是因为我们在这个程序中建立线性求解器的Trilinos库需要它。
...
一旦活动集不再变化，迭代就结束了（此时它有5399个受限自由度）。代数前提条件显然工作得很好，因为我们只需要4-6次CG迭代来解决线性系统（尽管这也与我们对线性求解器的精度要求不高有很大关系）。
...
图片显示，在第一步中，解决方案（在没有任何约束条件的情况下被计算出来的）是如此的弯曲，以至于几乎每一个内部点都必须被弹回第一步函数，产生一个不连续的解决方案。在活动集迭代的过程中，这种不切实际的膜的形状被抚平了，与最下层阶梯的接触消失了，解决方案也稳定下来。

除此以外，程序还输出了拉格朗日乘数的值。请记住，这些是接触力，所以在接触集上只应该是正的，而在接触集之外是零。另一方面，如果一个拉格朗日乘数在活动集上是负的，那么这个自由度必须从活动集上删除。下面的图片显示了迭代1、9和18中的乘数，我们用红色和棕色表示正值，蓝色表示负值。
...
很容易看到，正值在接触集的内部很好地收敛为适度的值，在台阶的边缘有很大的向上的力，正如人们所期望的那样（以支持那里的膜的大曲率）；在活动集的边缘，乘数最初是负的，导致接触集缩小，直到在迭代18，不再有负的乘数，算法已经收敛了。
...
与本教程中的任何程序一样，有许多明显的扩展和实验的可能性。第一个很清楚：引入适应性。接触问题是自适应网格的主要候选者，因为解决方案有沿其不太规则的线条（膜和障碍物之间建立接触的地方）和解决方案非常光滑的其他区域（或者，在目前情况下，在与障碍物接触的地方是恒定的）。在目前的程序中加入这一点应该不会造成太多困难，但要为此找到一个好的误差估计器并非易事。

一个更具挑战性的任务是扩展到三维。这里的问题不在于简单地让所有的东西都在三维中运行。相反，当一个三维体变形并与一个障碍物接触时，障碍物并不像这里的情况那样，在域内作为一个约束体的力量发挥作用。相反，接触力只作用于物体的边界。那么不等式就不在微分方程中，而实际上在（诺伊曼型）边界条件中，尽管这导致了一种类似的变分不等式。在数学上，这意味着拉格朗日乘数只存在于表面，当然，如果方便的话，它也可以通过零扩展到域中。在目前的程序中，我们不需要明确地形成和存储这个拉格朗日乘数。

在三维情况下，另一个有趣的问题是考虑有摩擦的接触问题。在几乎每个机械过程中，摩擦都有很大的影响。对于建模，我们必须考虑到接触面的切向应力。我们还必须注意到，摩擦给我们的问题增加了另一个非线性。

另一个不简单的修改是实现一个更复杂的构成法，如非线性弹性或弹塑性材料行为。这里的困难是要处理通过非线性构成法产生的额外非线性。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-41.cc" 。 
