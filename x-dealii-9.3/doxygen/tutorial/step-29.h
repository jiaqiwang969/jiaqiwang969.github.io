...
一个经常出现的问题是如何用deal.II解决涉及复值函数的问题。对于许多问题，与其直接使用复值有限元，不如将复值函数分成实部和虚部，并使用单独的标量有限元场来离散它们中的每一个，这样做往往更为方便。基本上，这相当于把一个复值方程看作是两个实值方程的系统。这个简短的例子演示了如何在deal.II中通过使用 <code>FE_system</code> 对象来堆叠代表实部和虚部的两个有限元域来实现。(相反的方法，保持所有的复值，在另一个教程程序中演示：见 step-58 )。当分成实部和虚部时，这里涉及的方程就属于矢量值问题。关于这个主题的顶层概述可以在 @ref vector_valued 模块中找到。

除了这个讨论，我们还讨论了ParameterHandler类，它提供了一种方便的方法，在运行时从配置文件中读取参数，而不需要重新编译程序代码。


<a name="Problemsetting"></a><h3>Problem setting</h3> 


这个程序的最初目的是模拟由一个几何形状可变的换能器透镜产生的超声波的聚焦特性。最近在医学成像方面的应用，不仅将超声波用于成像，而且还能激发材料中的某些局部效应，如光学特性的变化，然后可以用其他成像技术来测量。这些方法的一个重要因素是能够将超声波的强度集中在材料的某一特定部分，最好是一个点，以便能够检查该特定位置的材料特性。

为了推导出这个问题的模型，我们认为超声波是由波浪方程支配的压力波。@f[
	\frac{\partial^2 U}{\partial t^2}	-	c^2 \Delta U = 0
@f] 其中 $c$ 是波速（为简单起见，我们假设它是常数）， $U
= U(x,t),\;x \in \Omega,\;t\in\mathrm{R}$  。边界 $\Gamma=\partial\Omega$ 分为两部分 $\Gamma_1$ 和 $\Gamma_2=\Gamma\setminus\Gamma_1$  ，其中 $\Gamma_1$ 代表换能器透镜， $\Gamma_2$ 是吸收边界（也就是说，我们要在 $\Gamma_2$ 上选择边界条件，使其模仿一个更大的域）。在 $\Gamma_1$ 上，换能器产生一个恒定频率 ${\omega}>0$ 和恒定振幅（我们在这里选择为1）的波。@f[
U(x,t) = \cos{\omega t}, \qquad x\in \Gamma_1
@f] 

如果没有其他（内部或边界）源，并且由于唯一的源具有频率 $\omega$ ，那么解决方案可以接受  $U(x,t) = \textrm{Re}\left(u(x)\,e^{i\omega
t})\right)$  形式的变量分离。复值函数 $u(x)$ 描述了频率为 ${\omega}$ 的波的振幅和相位（相对于源）的空间依赖性，其中振幅是我们感兴趣的量。通过将这种形式的解决方案插入波浪方程，我们看到，对于 $u$ ，我们有@f{eqnarray*}
...
为了在 $\Gamma_2$ 上找到合适的条件来模拟吸收边界，考虑一个频率为 ${\omega}$ 的波在 $k\in {\mathrm{R}^2}$ 方向上行驶。为了使 $V$ 能够解决波浪方程， $|k|={\frac{\omega}{c}}$ 必须成立。假设这个波以直角击中 $x_0\in\Gamma_2$ 的边界，即 $n=\frac{k}{|k|}$ ， $n$ 表示 $\Omega$ 在 $x_0$ 的外单位法线。然后在 $x_0$ ，这个波满足方程@f[
c (n\cdot\nabla V) + \frac{\partial V}{\partial t} = (i\, c\, |k| - i\, \omega) V = 0.
@f]。 因此，通过强制执行边界条件@f[
c (n\cdot\nabla U) + \frac{\partial U}{\partial t} = 0, \qquad x\in\Gamma_2,
@f]，以直角撞击边界 $\Gamma_2$ 的波将被完全吸收。另一方面，那些没有以直角撞击边界的波场部分不满足这个条件，将其作为边界条件强制执行会产生部分反射，即只有部分波会穿过边界，就像它不在这里一样，而剩下的部分波会被反射回域中。
...
这是一个亥姆霍兹方程（类似于 step-7 中的方程，但这次有 "坏符号"），在 $\Gamma_1$ 上有迪里希特数据，在 $\Gamma_2$ 上有混合边界条件。由于 $\Gamma_2$ 上的条件，我们不能只是分别处理 $u$ 的实部和虚部方程。然而，我们可以把 $u$ 的PDE看作是 $u$ 的实部和虚部的两个PDE系统， $\Gamma_2$ 的边界条件代表系统两部分之间的耦合项。这是按以下思路进行的。让 $v=\textrm{Re}\;u,\; w=\textrm{Im}\;u$ ，然后在 $v$ 和 $w$ 方面我们有以下系统。@f{eqnarray*}
...
对于带有 $\phi|_{\Gamma_1}=\psi|_{\Gamma_1}=0$ 的测试函数 $\phi,\psi$ ，经过通常的乘法，在 $\Omega$ 上的积分和应用部分积分，我们得到弱表述@f{eqnarray*}
...
\end{array}
\right)
@f] （不要被这里的右边为零所迷惑，那是因为我们还没有包括迪里希特边界数据）。由于非对角线块中的符号交替，我们已经可以看到这个系统是非对称的，事实上它甚至是不确定的。当然，没有必要选择空间 $V_h$ 和 $W_h$ 是相同的。然而，我们期望解的实部和虚部具有类似的性质，因此在实现中确实会采取 $V_h=W_h$ ，并且也会对两个空间使用相同的基函数 $\phi_i = \psi_i$ 。使用不同符号的原因只是让我们能够区分 $v$ 和 $w$ 的形状函数，因为这种区分在实施中起着重要作用。


<a name="Thetestcase"></a><h3>The test case</h3>


在计算中，我们将考虑波在单位方阵中的传播，超声波由换能器透镜产生，该透镜的形状像圆的一段，中心在 $(0.5, d)$ ，半径略大于 $d$ ；这种形状应该导致声波在圆心处的聚焦。改变 $d$ 会改变透镜的 "焦点"，影响 $u$ 强度的空间分布，我们主要关注的是 $|u|=\sqrt{v^2+w^2}$ 的聚焦程度。

在下面的程序中，我们将使用实部和虚部分离的表述来实现复值亥姆霍兹方程。我们还将讨论如何生成一个看起来像正方形的域，其中有一个轻微的凸起，模拟换能器（在 <code>UltrasoundProblem<dim>::make_grid()</code> 函数中），以及如何生成图形输出，不仅包含解分量 $v$ 和 $w$ ，而且在输出文件中直接包含幅度 $\sqrt{v^2+w^2}$ （在 <code>UltrasoundProblem<dim>::output_results()</code> ）。最后，我们使用ParameterHandler类来轻松读取参数，如焦距 $d$ 、波速 $c$ 、频率 $\omega$ ，以及在运行时从输入文件中读取其他一些参数，而不是在源代码中固定这些参数，因为每次我们想改变参数时，都必须重新编译。<a name="CommProg"></a> <h1> The commented program</h1> 
...
为了解决线性系统，我们将使用UMFPACK提供的稀疏LU分解（见SparseDirectUMFPACK类），为此需要以下头文件。 注意，为了编译这个教程程序，deal.II-library需要建立UMFPACK支持，默认情况下是启用的。
...
首先，我们为代表Dirichlet边界值的函数定义一个类。这在以前已经做过很多次了，因此不需要过多解释。   


由于有两个值 $v$ 和 $w$ 需要在边界处规定，我们必须告诉基类，这是一个有两个分量的向量值函数， <code>vector_value</code> 函数和它的表弟 <code>vector_value_list</code> 必须返回有两个条目的向量。在我们的例子中，这个函数非常简单，它只是对实部 $v$ 返回1，对虚部 $w$ 返回0，而不管它在哪个点被评估。
...
<a name="codeParameterReaderdeclare_parameterscode"></a> <h4><code>ParameterReader::declare_parameters</code></h4>
 ...
 <code>declare_parameters</code> 函数声明了我们的ParameterHandler对象将能够从输入文件中读取的所有参数，以及它们的类型、范围条件和它们出现的子节。我们将用一对大括号包住所有进入一个部分的条目，以迫使编辑器将它们缩进一级，从而使阅读哪些条目共同构成一个部分更加简单。
...
网格和几何参数包括应用于初始粗略网格的全局细化步数和换能器镜头的焦距 $d$ 。对于细化步数，我们允许在 $[0,\infty)$ 范围内取整数，其中 Patterns::Integer 对象的第二个参数被省略，表示半开区间。 对于焦点距离，任何大于零的数字都可以接受。
...
最后但并非最不重要的是，我们希望能够通过配置文件中的条目来改变输出的一些属性，如文件名和格式，这就是最后一节的目的。
...
由于不同的输出格式在生成输出时可能需要不同的参数（例如，postscript输出需要视角角度、线宽、颜色等），如果我们必须为库中支持的每一种可能的输出格式手工声明所有这些参数，那就很麻烦了。相反，每种输出格式都有一个 <code>FormatFlags::declare_parameters</code> 函数，它在自己的小节中声明了该格式的所有特定参数。下面调用 DataOutInterface<1>::declare_parameters 为所有可用的输出格式执行 <code>declare_parameters</code> ，以便为每一种格式创建一个自己的小节，为该特定输出格式声明参数。(上面 <code>@<1@></code> 的调用中模板参数的实际值在这里并不重要：该函数做同样的工作，与维度无关，但碰巧是在一个依赖模板参数的类中。)  要想知道哪种输出格式有哪些参数，你可以查阅DataOutBase类的文档，或者干脆在没有参数文件的情况下运行这个程序。然后它将创建一个文件，将所有声明的参数设置为默认值，这可以方便地作为一个起点，将参数设置为你想要的值。
...
这是在ParameterReader类中的主函数。 它从外部被调用，首先声明所有的参数，然后从输入文件中读取参数，文件名由调用者提供。对这个函数的调用完成后，可以使用 <code>prm</code> 对象来检索从文件中读入的参数值。
...
正如在介绍中提到的，我们真正追求的数量是超声波强度的空间分布，它对应于  $|u|=\sqrt{v^2+w^2}$  。现在我们可以只满足于在输出中拥有 $v$ 和 $w$ ，并使用合适的可视化或后处理工具从我们计算的解决方案中得出 $|u|$ 。然而，也有一种方法可以输出从deal.II的解决方案中得出的数据，我们在这里要利用这个机制。




到目前为止，我们一直使用 DataOut::add_data_vector 函数将包含输出数据的向量添加到一个DataOut对象中。 这个函数有一个特殊的版本，除了数据向量外，还有一个额外的参数类型为DataPostprocessor。当这个函数用于输出时，在每个要生成输出数据的点上，指定的DataPostprocessor对象的 DataPostprocessor::evaluate_scalar_field() 或 DataPostprocessor::evaluate_vector_field() 函数被调用，从数据向量代表的有限元函数的值、梯度和二阶导数计算输出量（在面相关数据的情况下，法向量也可用）。因此，这使我们可以输出任何可以从解的值及其导数中局部导出的数量。 当然，超声强度 $|u|$ 就是这样一个量，它的计算甚至不涉及任何 $v$ 或 $w$ 的导数。




实际上，DataPostprocessor类只提供了这个功能的接口，我们需要从它派生出我们自己的类，以实现接口所指定的功能。在最一般的情况下，我们必须实现几个成员函数，但如果输出量是一个单一的标量，那么其中的一些模板代码可以由一个更专业的类来处理，即DataPostprocessorScalar，我们可以从这个类派生出来。这就是 <code>ComputeIntensity</code> 类的作用。
...
实际的后处理发生在下面这个函数中。它的输入是一个存储函数值的对象（这里是向量值），代表给 DataOut::add_data_vector, 的数据向量在我们产生输出的所有评估点的评估值，以及一些代表导数的张量对象（我们在这里没有使用，因为 $|u|$ 只是从 $v$ 和 $w$ 计算出来）。派生量在 <code>computed_quantities</code> 向量中返回。请记住，这个函数只能使用由  <code>get_needed_update_flags</code>  指定的相应更新标志的数据。例如，我们可能不会在这里使用导数，因为我们对 <code>get_needed_update_flags</code> 的实现要求只提供函数值。
...
计算本身是直截了当的。我们遍历输出向量中的每个条目，并从 $v$ 和 $w$ 的相应值中计算出 $|u|$ 。我们通过创建一个复数 $u$ ，然后对结果调用 `std::abs()` 来实现。(我们可能想调用 `std::norm()`, ，但是在一个历史的怪圈中，C++委员会决定 `std::norm()` 应该返回绝对值的<i>square</i>--从而不满足数学家对所谓 "规范 "的属性要求。) 
...
最后，这里是这个程序的主类。 它的成员函数与前面的例子非常相似，特别是  step-4  ，成员变量的列表也不包含任何重大的惊喜。传递给构造函数的ParameterHandler对象被存储为一个引用，以便于从该类的所有函数中访问参数。 由于我们正在使用矢量值的有限元，我们使用的FE对象是FESystem类型的。
...
这里我们为我们的领域设置了网格。 正如论述中所提到的，几何形状只是一个单位正方形（2d），其边界部分代表换能器透镜，被一个圆的扇形所取代。
...
接下来，为换能器镜头的位置和焦点定义两个点，这就是圆的中心，圆的部分将形成边界的换能器部分。注意，这是程序中唯一一个在二维和三维中略有不同的地方。尽管本教程只涉及二维情况，但要使这个程序在三维中发挥作用，必要的补充是非常少的，因此我们选择将它们包括在内。
...
作为初始粗网格，我们采用一个简单的单位方格，每个方向上有5个细分。选择细分的数量是为了使我们想指定为换能器边界的线段 $[0.4,0.6]$ 被一个面所横跨。然后，我们通过所有的单元格，找到换能器所在的面，事实上，这只是X轴上从0.4到0.6的一条边。这是我们希望根据圆环形边界进行细化的地方，所以我们用不同的流形指标来标记这个边缘。由于我们将在换能器上设置迪里希特边界条件，所以我们也改变其边界指标。
...
对于换能器透镜的圆形部分，我们使用了一个SphericalManifold对象（当然，在2D中它只是代表一个圆），中心的计算方法如上。
...
现在，全局细化被执行。靠近换能器位置的单元将根据换能器透镜的圆型边界自动细化。
...
系统矩阵、稀疏模式和向量的初始化与前面的例子相同，因此不需要进一步评论。和前面的函数一样，我们也输出我们在这里所做的运行时间。
...
首先，我们从ParameterHandler对象中查询波速和频率，并将其存储在本地变量中，因为它们将在本函数中频繁使用。
...
像往常一样，对于计算积分，我们使用普通的高斯正交规则。由于我们的双线性形式涉及到 $\Gamma_2$ 上的边界积分，我们还需要一个正交法则用于面的表面积分，这是 $dim-1$ 维的。
...
FEValues对象将为我们评估形状函数。 对于涉及到 $\Omega$ 上的积分的双线性形式部分，我们需要形状函数的值和梯度，当然还有正交权重。 对于涉及边界积分的条款，只需要形状函数值和正交权数。
...
像往常一样，系统矩阵是逐个单元组装的，我们需要一个矩阵来存储局部单元的贡献，以及一个索引向量来将单元贡献转移到全局系统矩阵中的适当位置。
...
在这一点上，重要的是要记住，我们所处理的是一个有两个组件的有限元系统。由于我们构建这个FES系统的方式，即作为两个标量有限元场的笛卡尔乘积，每个形状函数只有一个非零分量（用deal.II的术语来说，它们是 @ref
GlossPrimitive "原始的"）。 因此，每个形状函数可以被看作是介绍中的 $\phi$ 's或 $\psi$ 's之一，同样，相应的自由度也可以归属于 $\alpha$ 或 $\beta$ 。然而，当我们遍历当前单元上的所有自由度时，它们并不以任何特定的顺序出现，因此我们无法立即决定索引为 $i$ 和 $j$ 的自由度是属于我们解决方案的实部还是虚部。 另一方面，如果你看一下介绍中的系统矩阵的形式，这个区别是至关重要的，因为它将决定当前一对DoF的贡献将归入系统矩阵的哪个块，因此我们需要从给定的两个形状函数中计算哪个量。 幸运的是，FESystem对象可以为我们提供这些信息，即它有一个函数 FESystem::system_to_component_index, ，为每个局部的DoF索引返回一对整数，其中第一个表示该DoF属于系统的哪个组成部分。这对整数中的第二个整数表示该DoF在标量基有限元域中的索引，但这一信息在这里并不相关。如果你想知道更多关于这个函数和原始矢量值元素背后的基本方案，请看 step-8 或 @ref vector_valued 模块，那里对这些主题有深入的解释。
...
如果DoF $i$ 和 $j$ 都属于同一个分量，即它们的形状函数都是 $\phi$ 的或都是 $\psi$ 的，贡献将最终出现在我们系统矩阵的一个对角块中，由于相应的条目是由同一公式计算的，我们不必理会它们是否真的是 $\phi$ 或 $\psi$ 形状函数。我们可以简单地通过遍历所有正交点并将其贡献相加来计算条目，其中形状函数的值和梯度由我们的FEValues对象提供。
...
你可能会认为，在向FEValues对象请求形状函数值或梯度时，我们必须指定要评估形状函数的哪个分量。然而，由于形状函数是原始的，它们只有一个非零分量，而FEValues类足够聪明，它知道我们肯定对这一个非零分量感兴趣。
...
我们还必须增加由于边界项的贡献。为此，我们对当前单元格的所有面进行循环，首先看它是否在边界上，其次看它是否有与 $\Gamma_2$ 相关的正确的边界指标，即我们有吸收边界条件的边界部分。
...
这些面肯定会对系统矩阵的非对角线块有所贡献，所以我们要求FEFaceValues对象为我们提供这个面上的形状函数值。
...
严格来说，检查形状函数是否在一个面上有支持并不是必要的：如果我们不检查它，我们就会简单地把刚好为零的项加到本地单元格矩阵中，因为至少有一个形状函数刚好为零。然而，我们可以通过添加上面的检查来节省这项工作。




在任何一种情况下，这些DoF都会对系统矩阵的对角线块的边界积分做出贡献。为了计算积分，我们在面的所有正交点上进行循环，并将贡献量与面正交规则所提供的正交权重相加。 与对角线块上的条目不同，这里的形状函数哪一个是 $\psi$ ，哪一个是 $\phi$ ，确实很重要，因为这将决定该条目的符号。 我们通过一个简单的条件语句来说明这一点，以确定正确的符号。由于我们已经检查了DoF $i$ 和 $j$ 属于不同的组件，这里只需测试其中一个属于哪个组件即可。
...
现在我们已经完成了这个单元，必须把它的贡献从本地系统矩阵转移到全球系统矩阵。为此，我们首先要得到这个单元斗室的全局索引列表... 
...
唯一剩下的是 $\Gamma_1$ 上的迪里希特边界值，其特征是边界指标1。迪里希特值由我们上面定义的 <code>DirichletBoundaryValues</code> 类提供。
...
正如在介绍中已经提到的，系统矩阵既不是对称的，也不是确定的，因此，如何提出一个迭代求解器和预处理器来很好地处理这个矩阵并不十分明显。 我们选择了另一种方式，用UMFPACK提供的稀疏LU分解来解决线性系统。这通常是二维问题的一个很好的首选，即使对于大量的DoF也能很好地工作。 SparseDirectUMFPACK类提供了UMFPACK的deal.II接口，它非常容易使用，使我们只需3行代码就能解决我们的线性系统。
...
分解之后，我们可以使用 <code>A_direct</code> 作为代表系统矩阵逆的矩阵，因此要计算出解决方案，我们只需与右手边的向量相乘。
...
在这里，我们按照参数文件中指定的格式输出我们的解 $v$ 和 $w$ 以及导出的量 $|u|$ 。从 $v$ 和 $w$ 导出 $|u|$ 的大部分工作已经在 <code>ComputeIntensity</code> 类的实现中完成，因此输出程序相当简单，与前面教程中的内容非常相似。
...
接下来我们从ParameterHandler中查询输出相关的参数。 DataOut::parse_parameters  调用作为  <code>ParameterReader::declare_parameters</code>  中  DataOutInterface<1>::declare_parameters  调用的对应部分。它从ParameterHandler收集所有与输出格式相关的参数，并相应地设置DataOut对象的相应属性。
...
现在我们从ParameterHandler提供的基本名称和DataOut类提供的后缀组合成文件名（默认后缀被设置为正确的类型，与通过parse_parameters()在.prm文件中设置的类型相匹配）。
...
最后是程序的 <code>main</code> 函数。它的结构与其他几乎所有的教程程序相同。唯一的例外是，我们定义了ParameterHandler和 <code>ParameterReader</code> 对象，并让后者从一个名为 <code>step-29.prm</code> 的文本文件中读入参数值。这样读出的值就被交给UltrasoundProblem类的一个实例。
...
可以看出，我们设置了  $d=0.3$  ，这相当于换能器镜头的焦点在  $x=0.5$  ,  $y=0.3$  。粗略的网格被细化了5次，结果是160x160个单元，输出结果以vtu格式写入。参数读取器可以理解更多与生成输出有关的参数，但是我们在这里不需要这些参数，因此坚持使用它们的默认值。
...
(当然，如果你在本地运行该程序，执行时间会有所不同。) 事实上，大部分时间都花在了组装系统矩阵和生成输出上，这是因为在调试模式下有许多断言需要检查。在发布模式下，程序的这些部分运行得更快，而求解线性系统的速度却几乎没有加快。
...
前两张图片显示了  $u$  的实部和虚部，而最后一张图片显示了强度  $|u|$  。人们可以清楚地看到，强度集中在镜头的焦点周围（0.5，0.3），焦点在 $x$ -方向上相当尖锐，但在 $y$ -方向上比较模糊，这是聚焦镜头的几何形状、其有限的光圈和问题的波浪性质所造成的。

因为五颜六色的图形总是很有趣，而且为了进一步强调聚焦效果，这里有另一组图像，强调强度在 $x$ -方向的实际聚焦情况。
...
每次我们对网格进行一次细化，所以每一步的单元和自由度的数量大约是四倍。可以看出，生成网格、设置自由度、组装线性系统和生成输出的规模相当接近于线性，而求解线性系统的操作，自由度的数量每增加4倍，就需要8倍的时间，也就是说，它是  ${\cal O}(N^{3/2})$  。这可以解释为（使用最优排序）有限元矩阵的带宽是  $B={\cal O}(N^{(dim-1)/dim})$  ，而使用LU分解解决带状线性系统的努力是  ${\cal O}(BN)$  。这也解释了为什么该程序也能在3D中运行（在改变了 <code>UltrasoundProblem</code> 对象的维度后），但扩展性很差，需要非常耐心才能完成对具有明显分辨率的网格上的线性系统的求解，尽管该程序的其他部分扩展性很好。
...
这个程序的一个明显的可能的扩展是在三维空间运行，毕竟我们周围的世界是三维的，而超声束在三维介质中传播。你可以通过简单地改变 <code>main()</code> 中主类的模板参数并运行它来尝试。但这不会让你走得很远：当然，如果你按照参数文件中的设置做5个全局细化步骤，就更不会了。你的内存会耗尽，因为网格（含 $(2^5)^3 \cdot 5^3=2^{15}\cdot 125 \approx 4\cdot 10^6$ 单元），特别是稀疏直接求解器会占用太多的内存。然而，如果你有时间的话，你可以用3个全局细化步骤来求解：在2011年初，直接求解大约需要半小时。然而，你会注意到，这个解是完全错误的：网格大小根本不够小，不能准确地解决解的波浪，你可以在解的图中看到这一点。因此，在这种情况下，如果你不想在这个问题上扔一个更大的（估计是%并行的）机器，那么自适应性是必不可少的。<a name="PlainProg"></a><h1> The plain program</h1> @include "step-29.cc" 。 
