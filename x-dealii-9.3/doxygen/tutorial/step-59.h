...
无矩阵算子评估使高阶多项式基的离散化得到非常有效的实现，这是由于一种叫做和因子化的方法。这个概念已经在 step-37 和 step-48 的教程程序中介绍过。在这个教程程序中，我们将这些概念扩展到包括面积分的非连续Galerkin（DG）方案，这是一类高阶特别普遍的方法。

无矩阵评估的基本思想与连续元素的评估相同。迭代求解器或多网格平滑器中出现的矩阵-向量乘积不是由经典的稀疏矩阵核来实现的，而是通过对底层积分的实时评估来隐含应用。对于用张量积正交规则集成的张量积形状函数，通过使用和因子化技术，这种评估特别有效，它将最初的 $(k+1)^{2d}$ 插值操作分解为 $d$ 维中涉及 $k$ 度的相关形状函数的 $(k+1)^d$ 正交点的 $d$ 一维操作，每个成本 $(k+1)^{d+1}$ 。在三维中，这将复杂度降低了  $k$  的两个次方。当以每个自由度的复杂度来衡量时，复杂度是 $\mathcal O(k)$ 的多项式程度。由于DG中存在面积分，也由于对正交点的操作涉及更多的内存传输，两者的规模都是 $\mathcal O(1)$ ，观察到的复杂度往往是中等 $k\leq 10$ 的常数。这意味着高阶方法可以用每秒自由度的相同吞吐量来评估低阶方法。
...
方程中的项代表分项积分后的单元积分，由于分项积分和插入平均通量而在元素界面产生的原始一致性项，为恢复基础矩阵的对称性而添加的邻接一致性项，以及系数为 $\sigma$ 的惩罚项，其大小等于单元在面的法线方向上的长度乘以 $k(k+1)$ ，见 step-39  。惩罚项的选择是为了使逆估计成立，并且最终的弱形式是强制性的，即在离散设置中是正定的。邻接一致性项和惩罚项涉及元素界面上的跳跃 $\jump{u_h}$ ，这在解析解中消失了 $u$  。因此，这些条款与原始PDE是一致的，确保该方法可以保留最佳收敛顺序。

在下面的实现中，我们通过将法向量 $\mathbf{n}^-$ 从跳跃项移到导数上，形成<i>normal</i>形式的导数 $\mathbf{n}^-\cdot \nabla u_h$ 来实现上述的弱形式。这使得在正交点上的实现稍微有效一些，因为我们只需要处理标量项而不是张量，而且在数学上是等价的。
...
-\mathbf{n}^-\cdot \nabla u^- + 2 g_\text{N}$ ，对于给定的迪里希特值 $g_\text{D}$  和纽曼值 $g_\text{N}$  。然后将这些表达式插入上述弱形式中。涉及已知量 $g_\text{D}$ 和 $g_\text{N}$ 的贡献最终被移到右手边，而未知值 $u^-$ 被保留在左手边，对矩阵项的贡献与内部面类似。经过这些操作，得到了与 step-39 中相同的弱形式。
...
deal.II的无矩阵框架为实现上述离散化方程的作用提供了必要的基础设施。相对于我们在 step-37 和 step-48 中使用的 MatrixFree::cell_loop() ，我们现在用 MatrixFree::loop() 建立一个代码，它需要三个函数指针，一个用于单元积分，一个用于内面积分，一个用于边界面积分（与 step-39 教程程序中使用的MeshWorker的设计相类似）。在这三个函数中的每一个，我们都会在正交点上实现各自的条款。对于向量项与正交点上的数值和梯度之间的插值，我们使用FEEvaluation类来处理单元贡献，FEFaceEvaluation类来处理面贡献。这些函数的基本用法已经在 step-37 教程程序中得到了广泛的讨论。

在 MatrixFree::loop(), 中，所有的内部面都正好被访问一次，所以必须确保计算测试函数 $v_h^-$ 和 $v_h^+$ 的贡献。考虑到两边的测试函数确实是独立的，上面的弱形式实际上意味着我们在用测试函数的法向导数进行测试时，向名为`phi_inner`和`phi_outer`的FEFaceEvaluation对象提交相同的贡献，而在用测试函数的值进行测试时，则提交相反符号的值，因为后者由于跳跃项而涉及相反符号。对于不同细化程度的单元之间的面，从细化的一侧进行整合，FEFaceEvaluation自动执行内插到粗略一侧的子面。因此，一个悬挂的节点永远不会明确地出现在用户实现的弱形式中。

每个面被精确访问一次的事实也适用于那些在不同处理器之间的子域边界的面，当用MPI并行化时，一个单元属于一个处理器，一个属于另一个。在 MatrixFree::reinit() 中的设置将面分成了两边，最终只报告了 MatrixFree::n_inner_face_batches() 和 MatrixFree::n_boundary_face_batches(), 中分别实际处理的面。注意，与 step-37 中讨论的单元积分相类似，deal.II在几个面上应用矢量化，以使用SIMD，在我们称之为<i>batch of faces</i>的东西上工作，只需一条指令。面的批次与单元的批次是独立的，尽管处理面的积分的时间与处理各自单元的积分的时间保持一致，以提高数据的定位性。

这个程序的另一个新特点是，我们不再像 FEEvaluation::read_dof_values() 或 FEEvaluation::distribute_local_to_global() 那样将向量访问从求值和积分步骤中分离出来，而是分别调用组合函数 FEEvaluation::gather_evaluate() 和 FEEvaluation::integrate_scatter(), 。这对面积分很有用，因为根据面的评估内容，并非所有单元的向量项都必须首先被触及。例如，考虑到节点元素FE_DGQ的情况，其节点点在元素表面。如果我们对面的形状函数值感兴趣，只有 $(k+ 1)^{d-1}$ 个自由度以非微不足道的方式对它们有贡献（用更专业的方式来说，只有 $(k+1)^{d-1}$ 个形状函数在面有非零支持，并且对 FiniteElement::has_support_on_face()). 个自由度返回真值。 当与单元的 $(k+1)^d$ 个自由度相比，这要少一个幂。
...
FE_DGQHermite元素属于FE_DGQ元素家族，即其形状函数是一维多项式的张量乘积，该元素是完全不连续的。与通常的FE_DGQ元素中的节点特征相反，FE_DGQHermite元素是一个基于Hermite-like概念的节点贡献和导数贡献的混合物。基础多项式类是 Polynomials::HermiteLikeInterpolation ，可以总结如下。对于三次多项式，我们用两个多项式来表示单位区间左端的函数值和第一次导数，  $x=0$  ，用两个多项式来表示函数值和第一次导数以及单位区间的右端，  $x=1$  。在相反的两端，形状函数的值和一阶导数都是零，确保四个基础函数中只有两个对各自端部的值和导数有贡献。然而，我们偏离了经典的Hermite内插法，没有严格地为值和一导数指定一个自由度，而是允许一导数是第一和第二形状函数的线性组合。这样做是为了改善插值的调节。另外，当度数超过三时，我们以类似拉格朗日的方式在元素内部增加节点点，并在 $x=0$ 和 $x=1$ 这两个点上结合双零。这些额外节点的位置是由一些雅可比多项式的零点决定的，在类的描述中已经解释过  Polynomials::HermiteLikeInterpolation.  。 

使用这个元素，我们只需要访问 $2(k+1)^{d-1}$ 的自由度来计算一个面上的值和导数。检查是否满足Hermite属性是在 FEFaceEvaluation::gather_evaluate() 和 FEFaceEvaluation::integrate_scatter() 中透明地完成的，这些检查基础的类型并尽可能地减少对数据的访问。显然，如果我们将 FEFaceEvaluation::read_dof_values() 与 FEFaceEvaluation::evaluate(), 分开，这将是不可能的，因为我们需要读取的条目数量取决于导数的类型（只有值，一阶导数，等等），因此必须交给`read_dof_values()`。

这种优化不仅对计算面积分有用，而且对MPI鬼层交换也有用。在一个天真的交换中，如果一个单元负责计算面的贡献，我们就需要将该单元的所有自由度发送给另一个处理器。由于我们知道在用FEFaceEvaluation进行的评估中只有部分自由度被触及，所以自然只交换相关的自由度。当与 LinearAlgebra::distributed::Vector. 结合时， MatrixFree::loop() 函数支持选定的数据交换。为了实现这一点，我们需要告诉循环我们要对脸部做什么样的评价，使用 MatrixFree::DataAccessOnFaces, 类型的参数，正如在下面 `LaplaceOperator::vmult()` 的实现中可以看到。在这种情况下，数据交换的方式如下。矢量中的幽灵层数据仍然假装代表所有的自由度，这样，FEFaceEvaluation可以继续读取数值，就像单元格是本地拥有的一样。数据交换例程负责将数据打包和解包成这种格式的任务。虽然这听起来很复杂，但我们将在下面的结果部分显示，通过与不指定面孔上的数据访问的基线代码进行性能比较，这确实是值得的。
...
按照 step-37 程序的传统，我们再次用共轭梯度求解器内的几何多网格预处理器来解决泊松问题。在这个教程程序中，我们没有计算对角线并使用基本的PreconditionChebyshev作为平滑器，而是选择了一种不同的策略。我们实现了一个块状Jacobi预处理程序，其中块状指的是一个单元上的所有自由度。我们没有在预处理程序中建立完整的单元格矩阵并应用其LU因子化（或逆），这种操作将受到严重的内存带宽限制，因此速度相当慢；我们通过一种特殊的技术，即快速对角线化方法，对块的逆进行近似。

该方法的理念是利用单元矩阵的结构。对于在笛卡尔网格上离散的恒定系数的拉普拉斯，单元矩阵 $L$ 可以写为 
...
在三维中可以写成@f{align*}{
L &= A_2 \otimes M_1 \otimes M_0 + M_2 \otimes A_1 \otimes M_0 + M_2 \otimes M_1 \otimes A_0
@f}矩阵 $A_0$ 和 $A_1$ 表示一维拉普拉斯矩阵（包括与当前单元值 $u^-_h$ 和 $v^-_h$ 相关的单元和面项）， $M_0$ 和 $M_1$ 是质量矩阵。请注意，一旦单元上有非恒定系数或几何形状不再恒定，这种简单的张量乘积结构就会消失。我们提到，类似的设置也可以用来用这个最终的张量积形式的矩阵来替代计算的积分，这将把算子评估的操作减少到一半以下。然而，考虑到这只适用于直角坐标单元和恒定系数，这是一个相当狭窄的情况，我们避免继续探讨这个想法。
...
和  $\Lambda_d$  是代表广义特征值的对角线矩阵  $\lambda$  。请注意，向量 $s$ 是这样的：它们同时对角化 $A_d$ 和 $M_d$ ，即 $S_d^{\mathrm T} A_d S_d =
\Lambda_d$ 和 $S_d^{\mathrm T} M_d S_d = I$ 。

deal.II库实现了一个使用这个概念的类，叫做TensorProductMatrixSymmetricSum。

在本程序中，我们坚持使用常数系数和笛卡尔网格，尽管基于张量积的近似版本对于更普遍的网格仍然是可能的，而且算子评估本身当然也是通用的。另外，我们也不关心自适应网格，因为多网格算法需要获取不同细化边缘的通量矩阵，如 step-39 中所解释的。然而，我们所做的一件事是仍然将我们的块状Jacobi预处理程序包裹在PreconditionChebyshev中。这个类使我们不必寻找适当的松弛参数（对于块-雅各比平滑器来说，二维的松弛参数约为0.7，三维的松弛参数约为0.5），并且通常比普通的雅各比平滑器增加了一些平滑效率，因为当设置切比雪夫多项式的度数为1或2时，它可以降低解的时间。

请注意，块状雅可比平滑法有一个额外的好处：快速对角线化方法也可以解释为从FE_DGQHermite的Hermite-like多项式转变为一个单元拉普拉斯是对角线的基础。因此，它抵消了基础的影响，无论我们使用FE_DGQHermite还是FE_DGQ，都会得到相同的迭代次数。这与使用只有对角线的PreconditionChebyshev类（点-Jacobi方案）形成了对比，在这种情况下，FE_DGQ和FE_DGQHermite的表现确实不同，FE_DGQ需要的迭代次数比FE_DGQHermite少2-5次，尽管对类似Hermite的形状函数做了修改以确保良好的条件。<a name="CommProg"></a><h1> The commented program</h1>

包含文件与 step-37 中的文件基本相同，只是用FE_DGQHermite类代替了FE_Q。所有关于面积分的无矩阵计算功能已经包含在`fe_evaluation.h`中。
...
与 step-37 一样，为了简单起见，我们将维数和多项式程度作为常数收集在程序的顶部。与 step-37 不同的是，这次我们选择了一个真正的高阶方法，度数为8，任何不使用和因式分解的实现都会变得非常慢，而使用MatrixFree的实现则提供了与度数为2或3时基本相同的效率。此外，本教程程序中的所有类都是模板化的，因此，通过在`main()`函数中添加适当度数的实例，可以很容易地在运行时从输入文件或命令行参数中选择度数。
...
LaplaceOperator "类与  step-37  中的相应类类似。一个重要的区别是，我们没有从 MatrixFreeOperators::Base 中派生出这个类，因为我们想呈现 MatrixFree::loop() 的一些额外特性，这些特性在通用类 MatrixFreeOperators::Base. 中是没有的。 我们从Subscriptor类中派生出这个类，以便能够在Chebyshev预处理程序中使用该运算器，因为该预处理程序通过SmartPointer存储底层矩阵。   
...
`%PreconditionBlockJacobi`类定义了我们对这个问题的自定义预处理程序。与基于矩阵对角线的 step-37 相反，我们在这里通过使用介绍中讨论的所谓快速对角线化方法，计算出非连续Galerkin方法中对角线块的近似反演。
...
第二个新特征是我们没有像 step-37 中那样实现`vmult_add()`函数（通过虚拟函数 MatrixFreeOperators::Base::vmult_add()), ，而是直接实现`vmult()`功能。由于单元和面的积分都将和到目的向量中，我们当然必须在某处将向量归零。对于DG元素，我们有两个选择&ndash；一个是在下面的`apply_cell`函数中使用 FEEvaluation::set_dof_values() 而不是 FEEvaluation::distribute_local_to_global() 。这是因为MatrixFree中的循环布局是这样的：单元积分总是在面积分之前触及给定的矢量条目。然而，这实际上只适用于完全不连续的基数，其中每个单元都有自己的自由度，不与邻近的结果共享。另一种设置，即这里选择的设置，是让 MatrixFree::loop() 来处理向量的归零问题。这可以被认为是简单地在代码中调用`dst = 0;`。对于像 `LinearAlgebra::distributed::Vector`, 这样的支持性向量来说，实现起来就比较麻烦了，因为我们的目标是不要一次就把整个向量清零。在足够小的几千个向量项上进行归零操作的好处是，在 FEEvaluation::distribute_local_to_global() 和 FEFaceEvaluation::distribute_local_to_global(). 中再次访问之前，被归零的向量项会保留在缓存中，因为无矩阵运算符的评估真的很快，仅仅归零一个大的向量就会相当于运算符评估时间的25%，而我们显然希望避免这种代价。这种将向量归零的选项也可用于 MatrixFree::cell_loop 和连续基数，尽管在 step-37 或 step-48 的教程程序中没有使用它。   


第三个新特征是我们提供在单元格、内面和边界面计算的函数的方式。MatrixFree类有一个叫做`loop'的函数，它接收三个函数指针，分别指向三种情况，允许分开实现不同的东西。正如在 step-37 中所解释的，这些函数指针可以是 `std::function` 对象或类的成员函数。在这种情况下，我们使用成员函数的指针。   


最后一个新特性是可以给 MatrixFree::DataAccessOnFaces 类型的最后两个参数，这个类将面积分的数据访问类型传递给并行向量的MPI数据交换例程 LinearAlgebra::distributed::Vector::update_ghost_values() 和 LinearAlgebra::distributed::Vector::compress() 。其目的是不发送相邻元素的所有自由度，而是将数据量减少到手头计算真正需要的程度。数据交换是一个真正的瓶颈，特别是对于高自由度的DG方法来说，因此一个更严格的交换方式往往是有益的。枚举字段 MatrixFree::DataAccessOnFaces 可以取值`none`，这意味着根本不做面的积分，这与 MatrixFree::cell_loop(), 的值`values`类似，意味着只在面上使用形状函数值（但不使用导数），而值`gradients`则意味着除了值之外还访问面的一导数。值 "未指定 "意味着所有自由度将被交换给位于处理器边界的面，并指定在本地处理器上进行处理。   


为了了解如何减少数据，想想节点元素FE_DGQ的情况，节点点在元素表面，在一个单元的 $(k+1)^d$ 自由度中，只有 $(k+1)^{d-1}$ 自由度对一个面的多项式 $k$ 空间维度的值有贡献。类似的减少也可以用于内部惩罚方法，该方法对面的数值和一阶导数进行评估。当在一维中使用类似Hermite的基时，最多只有两个基函数对值和导数有贡献。FE_DGQHermite类实现了这一概念的张量乘积，在介绍中已经讨论过。因此，每个面只需交换 $2(k+1)^{d-1}$ 个自由度，一旦 $k$ 个自由度大于4或5个，这显然是一种胜利。请注意，FE_DGQHermite的这种减少的交换在具有弯曲边界的网格上也是有效的，因为导数是在参考元素上取的，而几何体只在内部混合它们。因此，这与试图用连续的Hermite型形状函数获得 $C^1$ 的连续性是不同的，在这种情况下，非笛卡尔的情况会大大改变情况。显然，在非笛卡尔网格上，导数还包括超出法向导数的形状函数的切向导数，但这些也只需要元素表面的函数值。如果元素不提供任何压缩，循环会自动交换受影响单元的所有条目。
...
单元操作与  step-37  非常相似。不过我们在这里不使用系数。第二个区别是，我们用一个单一的函数调用 FEEvaluation::gather_evaluate() 代替了 FEEvaluation::read_dof_values() 后面的两个步骤，该函数内部调用了两个单独方法的序列。同样， FEEvaluation::integrate_scatter() 实现了 FEEvaluation::integrate() 之后的 FEEvaluation::distribute_local_to_global(). 的序列。 在这种情况下，这些新函数只是节省了两行代码。然而，我们用它们来与FEFaceEvaluation进行类比，在那里它们更重要，如下所述。
...
面部操作实现了与 step-39 相类似的内部惩罚方法的条款，正如在介绍中所解释的。我们需要两个评估器对象来完成这个任务，一个用于处理来自内部面的两边之一的单元格的解，另一个用于处理来自另一边的解。面积分的评价器被称为FEFaceEvaluation，并在构造函数的第二个槽中接受一个布尔参数，以指示评价器应属于两边中的哪一边。在FEFaceEvaluation和MatrixFree中，我们称两边中的一边为 "内部"，另一边为 "外部"。`外部'这个名字是指两边的评价器将返回相同的法向量。对于 "内部 "一侧，法向量指向外部，而另一侧则指向内部，并且与该单元的外部法向量相对应。除了新的类名之外，我们再次得到了一系列的项目，与 step-37 中讨论的类似，但在这种情况下是针对内部面的。请注意，MatrixFree的数据结构形成了面的批次，类似于单元积分的单元批次。一批中的所有面涉及不同的单元格编号，但在参考单元格中具有相同的面编号，具有相同的细化配置（无细化或相同的子面）和相同的方向，以保持SIMD操作的简单和高效。   
...
在给定的一批面，我们首先更新指向当前面的指针，然后访问向量。如上所述，我们将向量访问与评估结合起来。在面积分的情况下，对于FE_DGQHermite基础的特殊情况，可以减少对向量的数据访问，正如上面解释的数据交换。由于 $(k+1)^d$ 单元自由度中只有 $2(k+1)^{d-1}$ 被非零值或形状函数的导数所乘，这种结构可以被用于评估，大大减少了数据访问。减少数据访问不仅是有益的，因为它减少了飞行中的数据，从而有助于缓存，而且当从单元格索引列表中相距较远的单元格中收集数值时，对面的数据访问往往比对单元格积分更不规则。
...
在正交点的循环中，我们最终计算了对内部惩罚方案的所有贡献。根据介绍中的公式，测试函数的值被乘以解决方案中的跳跃乘以惩罚参数和实空间中的法线导数的平均值的差值。由于内侧和外侧的两个评估器由于跳跃而得到不同的符号，我们在这里用不同的符号传递结果。测试函数的正态导数会被内侧和外侧的解决方案中的负跳跃所乘。这个术语，被称为邻接一致性术语，根据其与原始一致性术语的关系，在代码中还必须包括 $\frac{1}{2}$ 的因子，由于测试函数槽中的平均数，它得到了二分之一的因子。
...
边界面函数大体上沿用了内部面函数。唯一的区别是，我们没有一个单独的FEFaceEvaluation对象为我们提供外部值  $u^+$  ，但我们必须从边界条件和内部值中定义它们  $u^-$  。正如介绍中所解释的，我们在Dirichlet边界上使用 $u^+ = -u^- + 2
g_\text{D}$ 和 $\mathbf{n}^-\cdot \nabla u^+ = \mathbf{n}^-\cdot \nabla
...
在这一点上，有一个陷阱。下面的实现使用一个布尔变量`is_dirichlet`来切换Dirichlet和Neumann情况。然而，我们解决的问题是，我们还想在一些边界上施加周期性的边界条件，即沿着 $x$ 方向的边界。人们可能会问，这里应该如何处理这些条件。答案是MatrixFree会自动将周期性边界视为技术上的边界，即两个相邻单元的解值相遇的内面，必须用适当的数值通量来处理。因此，周期性边界上的所有面将出现在`apply_face()`函数中，而不是这个函数中。
...
至于在单元元素上计算一维矩阵，我们简单地写下在矩阵的行和列以及正交点上的典型装配程序会做什么。我们一劳永逸地选择相同的拉普拉斯矩阵，对内部面使用系数0.5（但可能由于网格的原因，在不同方向上的缩放比例不同）。因此，我们在Dirichlet边界（正确的系数应该是导数项为1，惩罚项为2，见 step-39 ）或在Neumann边界（系数应该为0）犯了一个小错误。由于我们只在多网格方案中使用这个类作为平滑器，这个错误不会有任何重大影响，只是影响了平滑的质量。
...
接下来的两个语句所集合的左右边界项似乎有一些任意的符号，但这些都是正确的，可以通过查看 step-39 并在一维情况下为法向量插入值-1和1来验证。
...
一旦我们通过FEEvaluation访问函数访问了反Jacobian（我们取第4个正交点的那个，因为它们在笛卡尔单元的所有正交点上都应该是一样的），我们检查它是对角线的，然后提取原始Jacobian的行列式，即反Jacobian的行列式的逆值，并根据1D Laplacian乘以质量矩阵的 $d-1$ 拷贝，设置权重为 $\text{det}(J) / h_d^2$  。
...
在DG背景下，用于近似块状Jacobi预处理的vmult函数非常简单。我们只需要读取当前单元格批次的值，对张量积矩阵阵列中的给定条目进行逆运算，并将结果写回去。在这个循环中，我们覆盖了`dst`中的内容，而不是首先将条目设置为零。这对于DG方法来说是合法的，因为每个单元都有独立的自由度。此外，我们手动写出所有单元批的循环，而不是通过  MatrixFree::cell_loop().  我们这样做是因为我们知道我们在这里不需要通过MPI网络进行数据交换，因为所有的计算都是在每个处理器上的本地单元上完成的。
...
右手边的计算比  step-37  中更复杂一些。现在的单元项包括分析解的负拉普拉斯，`RightHandSide'，为此我们需要首先将VectorizedArray字段的Point，即一批点，通过分别评估VectorizedArray中的所有车道，分割成一个点。请记住，通道的数量取决于硬件；对于不提供矢量化的系统（或deal.II没有本征），它可能是1，但在最近英特尔架构的AVX-512上也可能是8或16。
...
g_\text{N}$ 被插入并以边界函数 $g_\text{D}$ 和 $g_\text{N}$ 展开，这个函数就是到函数 `LaplaceOperator::apply_boundary()` 的遗漏部分。有一点需要记住的是，我们把边界条件移到右手边，所以符号与我们在求解部分施加的相反。     


我们本可以通过 MatrixFree::loop 部分发出单元格和边界部分，但我们选择手动写出所有面的完整循环，以了解面指数的索引布局在MatrixFree中是如何设置的：内部面和边界面都共享索引范围，所有批次的内部面的数字都比批次的边界单元格低。两种变体的单一索引使我们可以很容易地在两种情况下使用相同的数据结构FEFaceEvaluation，它附着在同一个数据域上，只是位置不同。内层面的批次（其中一个批次是由于几个面合并成一个面进行矢量化）的数量由 MatrixFree::n_inner_face_batches(), 给出，而边界面的批次数量由 MatrixFree::n_boundary_face_batches(). 给出。 
...
由于我们在单元上手动运行循环，而不是使用 MatrixFree::loop(), ，我们一定不要忘记与MPI进行数据交换--或者说，实际上我们在这里不需要对DG元素进行数据交换，因为每个单元都有自己的自由度，而且单元和边界积分只对本地拥有的单元的量进行评估。与相邻子域的耦合只通过内表面积分来实现，我们在这里没有做这个。也就是说，在这里调用这个函数并没有什么坏处，所以我们这样做是为了提醒大家里面发生了什么 MatrixFree::loop().  。 
...
`run()`函数设置了初始网格，然后以常规方式运行多网格程序。作为一个域，我们选择一个矩形，在 $x$ 方向上有周期性的边界条件，在 $y$ 方向上的正面（即索引号为2的面，边界id等于0）有一个Dirichlet条件，在背面以及 $z$ 方向上的两个面有Neumann条件（边界id等于1）的三维情况。与 $y$ 和 $z$ 方向相比， $x$ 方向的域的范围有点不同（鉴于 "解决方案 "的定义，我们想在这里实现周期性的解决方案）。
...
与 step-37 中一样，我们以运行时间来评估多网格求解器。 在两个空间维度的8度元素中，可能的输出看起来如下。
...
与 step-37 一样，随着问题大小的增加，CG的迭代次数保持不变。迭代次数稍高，这是因为我们使用的切比雪夫多项式的度数较低（ step-37 中为2比5），而且内部惩罚离散化的特征值分布较大。尽管如此，13次迭代将残差减少了12个数量级，或者说每次迭代几乎是9个系数，这表明总体上是一个非常有效的方法。特别是，当使用12个核心时，我们可以在5秒内解决一个具有2100万自由度的系统，这是一个非常好的效率。当然，在二维中，我们很好地进入了8级多项式的四舍五入体系；事实上，大约83000个自由度或0.025秒就足以完全收敛这个（简单的）分析解。
...
我们清楚地看到每个自由度的效率最初是如何提高的，直到它达到多项式程度的最大值  $k=4$  。这种效果是令人惊讶的，不仅是因为较高的多项式度数通常会产生一个好得多的解决方案，而且特别是在考虑到基于矩阵的方案时，较高度数的密集耦合会导致吞吐量单调地下降（在三维中是急剧的， $k=4$ 比 $k=1$ 慢10倍以上！）。对于更高的度数，吞吐量减少了一些，这既是由于迭代次数的增加（从 $k=2,3,4$ 的12次增加到 $k=10$ 的19次），也是由于算子评估的 $\mathcal O(k)$ 复杂性。尽管如此，对于更高的多项式度数来说，作为求解时间的效率仍然会更好，因为它们有更好的收敛率（至少对于像这个问题这样简单的问题来说）。对于 $k=12$ ，我们在100万个DoFs的情况下已经达到了舍入精度（求解时间不到一秒），而对于 $k=8$ ，我们需要2400万个DoFs和8秒。对于 $k=5$ ，误差约为 $10^{-9}$ ，有5700万个DoFs，因此，尽管花了16秒，仍然离舍入很远。
...
最后，我们来看一下本教程程序中提出的一些特殊成分，即特别是FE_DGQHermite基础和 MatrixFree::DataAccessOnFaces. 的规范。 在下表中，第三行显示了上述优化的求解器，第四行显示了仅将 MatrixFree::DataAccessOnFaces  ]设置为 "未指定 "而不是最优的 "网格"，最后一行是用基本的FE_DGQ元素代替FE_DGQHermite，其中MPI交换的费用更高，而由 FEFaceEvaluation::gather_evaluate() 和 FEFaceEvaluation::integrate_scatter(). 完成的操作  
...
表中的数据显示，对于较高的多项式度数，不使用 MatrixFree::DataAccessOnFaces 会增加10%左右的成本。对于较低的度数，差异显然没有那么明显，因为体积与表面的比例更有利，需要交换的数据更少。如果只看矩阵-向量乘积，而不是这里显示的完整的多网格求解器，差异会更大，仅仅因为MPI通信，时间就会差20%左右。
...
另一种扩展程序的方式是包括对自适应网格的支持，对于这种支持，不同细化水平的边缘的界面操作是必要的，正如在  step-39  中所讨论的那样。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-59.cc"  。 
