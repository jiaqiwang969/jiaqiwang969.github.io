...
EAR-0949446 and EAR-1550901 and The University of California -- Davis.
  ...
  @note  作为本程序的先决条件，你需要同时安装p4est和PETSc或Trilinos库。在<a href="../../readme.html" target="body">README</a>文件中描述了deal.II与这些附加库的安装情况。
...
这个例子显示了deal.II中的多级函数在并行、分布式网格上的应用，并给出了几何多栅和代数多栅方法的比较。代数多网格(AMG)的预处理程序与  step-40  中使用的相同。考虑了两种几何多网格（GMG）的预处理方法：一种是类似于  step-16  中的基于矩阵的版本（但用于并行计算），一种是  step-37  中讨论的无矩阵版本。我们的目标是找出哪种方法能导致大型并行计算的最佳解算器。

本教程是基于  @cite clevenger_par_gmg  中的一个数值例子。请参阅该出版物，以了解交易.II中多网格实现的详细背景。我们将在下面的文字中总结一些结果。

代数多网格方法显然是最容易用deal.II实现的，因为诸如 TrilinosWrappers::PreconditionAMG 和 PETScWrappers::PreconditionBoomerAMG 这样的类本质上是黑箱预处理，即使是并行计算，也只需要几行就可以设置。另一方面，几何多网格方法需要对整个代码库进行修改--不是很多，但必须知道自己在做什么。

这个程序的结果将显示，代数和几何多栅方法的性能大致相当<i>when using matrix-based formulations</i>，而无矩阵的几何多栅方法对于这里所考虑的问题来说要好得多。另一个结论是，当每个处理器的未知数小于20,000个时，基于矩阵的几何多网格方法确实不能很好地扩展。
...
在域 $\Omega = [-1,1]^\text{dim} \setminus [0,1]^\text{dim}$ （二维为L形域，三维为Fichera角）上，如果 $\min(x,y,z)>-\frac{1}{2}$ ，则 $\epsilon = 100$ 。换句话说， $\epsilon$ 是沿着域的边缘或面跑到重合角的小，这在下图中会看到。

边界条件在整个边界上是 $u=0$ ，右手边是 $f=1$  。我们使用连续的 $Q_2$ 元素来表示离散的有限元空间 $V_h$ ，并使用基于残差的、单元的后验误差估计器 $e(K) = e_{\text{cell}}(K) + e_{\text{face}}(K)$ ，来自 @cite karakashian2003posteriori ，有 
...
来适应性地细化网格。这是KellyErrorEstimator类中使用的Kelly误差估计器的概括，该类驱动大多数其他教程程序中的网格细化）。下图显示了二维的求解和细化：  <img width="400px" src="https://www.dealii.org/images/steps/developer/step-50-2d-solution.png" alt="">  在三维中，求解看起来类似（见下文）。在左边你可以看到解决方案，在右边我们展示了靠近域中心的 $x$ 的切片，显示了自适应细化的网格。  <table width="60%" align="center">
  <tr>
    <td align="center">
...
</table> 在二维和三维中，你都可以看到自适应细化拾取了角部奇点和粘度跳跃的内部奇点，而沿分离两个粘度的线的界面（正确）没有被细化，因为它被充分解决了。这是因为由系数的跳跃导致的解的扭结与单元格界面对齐。


<a name="Workloadimbalanceforgeometricmultigridmethods"></a><h3>Workload imbalance for geometric multigrid methods</h3> 


如上所述，本程序的目的是展示代数和几何多网格方法在这个问题上的应用，并做到并行计算。使算法扩展到大型并行机器的一个重要组成部分是确保每个处理器有相同的工作量。更准确地说，重要的是没有一小部分处理器比其他处理器有更多的工作，因为如果是这样的话，很大一部分处理器会闲置，等待小部分处理器完成。相反，一小部分处理器的工作大大超过<i>less</i>并不是一个问题，因为大多数处理器继续生产，只有一小部分处理器在完成工作后闲置。) 

对于活跃的网格，我们使用 parallel::distributed::Triangulation 类，就像在 step-40 中做的那样，它使用外部库<a href="http://www.p4est.org/">p4est</a>中的功能在处理器之间分配活跃单元。对于多级层次结构中的非活动单元，deal.II实现了我们所说的 "第一子规则"，对于层次结构中的每个单元，我们递归地将一个单元的父级分配给第一个子单元的所有者。下面的数字给出了这样一个分布的例子。这里的左图表示使用空间填充曲线划分的二维网格样本的活动单元（这也是p4est用来划分单元的方法）；中间的图片给出了活动网格的树状表示；右图给出了单元的多级层次结构。颜色和数字代表不同的处理器。树上的圆形节点是非活动单元，使用 "长子规则 "进行分配。

  <img width="800px" src="https://www.dealii.org/images/steps/developer/step-50-workload-example.png" alt="">   

在这个例子的屏幕输出中，包括了一个 "分区效率 "的数值，这个数值由 MGTools::workload_imbalance(). 给出，将用 $\mathbb{E}$ 表示，量化了在多网格层次结构的每一层上没有完美工作平衡所产生的开销。这种不平衡在上面的例子中很明显：虽然 $\ell=2$ 层在三个处理器中的四个单元是尽可能的平衡，但粗略的 $\ell=0$ 层只有一个处理器的工作，而 $\ell=1$ 层只有两个处理器的工作，其中一个的工作是另一个的三倍。

对于定义 $\mathbb{E}$ ，需要注意的是，由于我们使用局部平滑来定义多网格层次（见 @ref mg_paper "多网格论文 "中对局部平滑的描述），一个单元的细化水平与该单元的多网格水平相对应。现在，让 $N_{\ell}$ 为 $\ell$ 层的单元数（包括活动和非活动单元）， $N_{\ell,p}$ 为进程 $p$ 所拥有的子集。我们还将用 $P$ 表示处理器的总数量。假设任何一个处理器的工作量与该处理器拥有的单元格数量成正比，每个处理器的最佳工作量为 
...
然后我们将 $\mathbb{E}$ 定义为最佳分区与当前分区的并行复杂度的比率 
...
然后，该值 MGTools::workload_imbalance()   $= 1/\mathbb{E}$ 代表我们预期GMG方法（vmults、assembly等）由于网状分区的不平衡而与完全负载平衡的工作负载相比在时间上的系数增加。我们将在下面的结果部分报告一连串的网格，并与观察到的减速进行比较，因为我们的处理器数量越来越大（通常，负载不平衡也会变大）。

这些考虑因素在 @cite clevenger_par_gmg 中得到了更详细的考虑，其中包含了对分区效率模型和不平衡对GMG V-周期时间的影响的全面讨论。总之， $\mathbb{E}$ 的值在很大程度上取决于所使用的局部网格细化程度，对于全局细化的网格有一个最佳值 $\mathbb{E} \approx 1$ 。通常对于自适应细化的网格，用于分配单个网格的处理器数量对 $\mathbb{E}$ 有负面影响，但只到一个平移点，即处理器数量增加时，不平衡度保持相对稳定，进一步细化对 $\mathbb{E}$ 的影响很小。最后， $1/\mathbb{E}$ 被证明可以准确地表示出对V型周期的计时所预期的并行扩展的减慢。
...
上面的考虑表明，我们必须期待在deal.II中实现的几何多网格算法的可扩展性有一定的限制，因为即使在网格的最细层是完全负载平衡的情况下，较粗的层也可能不是这样。同时，较粗层的权重较小（ $W_\ell$ 对 $W$ 的贡献较小），因为较粗层的单元较少，因此对整体运行时间的贡献不如较细层的大。换句话说，较粗层次的不平衡可能不会导致大的影响。
...
你可以像@code
  ./step-50 gmg_mb_2d.prm
@endcode那样执行程序，这将从给定的输入文件（这里是`mg_mb_2d.prm`）获取运行时参数。
...
如果你想，例如，在四个处理器上运行。(也就是说，如果你有这么多的处理器，程序也可以用`-np 28672`来运行)。<a name="CommProg"></a> <h1> The commented program</h1>
...
我们使用与 step-40 相同的策略，在PETSc和Trilinos之间切换。
...
<a name="Coefficientsandhelperclasses"></a> <h3>Coefficients and helper classes</h3>




MatrixFree操作符必须使用 dealii::LinearAlgebra::distributed::Vector 矢量类型。这里我们定义了复制到Trilinos向量的操作，以便与基于矩阵的代码兼容。注意，目前PETSc矢量类型不存在这种功能，所以必须安装Trilinos才能使用本教程中的MatrixFree求解器。
...
让我们继续讨论我们要解决的问题的描述。我们把右手边的函数设置为1.0。 @p value  返回VectorizedArray的函数被无矩阵代码路径所使用。
...
我们将使用ParameterHandler来在运行时传入参数。 结构 @p Settings 解析并存储这些参数，以便在整个程序中进行查询。
...
我们将在整个程序中使用以下类型。首先是基于矩阵的类型，然后是无矩阵类。对于无矩阵的实现，我们使用 @p float 中的水平运算符。
...
与 step-16 和 step-37 不同，我们将设置分成两部分：setup_system()和setup_multigrid()。下面是大多数教程中常见的主动网格的典型setup_system()函数。对于无矩阵，主动网格的设置类似于  step-37  ；对于基于矩阵（GMG和AMG求解器），其设置类似于  step-40  。
...
<a name="LaplaceProblemsetup_multigrid"></a> <h4>LaplaceProblem::setup_multigrid()</h4> 矩阵式求解器。




该函数为无矩阵和基于矩阵的GMG做多级设置。无矩阵设置类似于 step-37 ，而基于矩阵的设置类似于 step-16 ，只是我们必须使用适当的分布式稀疏模式。
...
汇编分为三个部分：`assemble_system()`，`assemble_multigrid()`，和`assemble_rhs()`。这里的`assemble_system()`函数组装并存储（全局）系统矩阵和基于矩阵的方法的右手边。它类似于  step-40  中的装配。
...
结果是一个类似于 step-37 中 "使用 FEEvaluation::read_dof_values_plain() 避免解决约束 "一节中的函数。




这个函数的原因是MatrixFree算子不考虑非同质的Dirichlet约束，而是将所有Dirichlet约束视为同质的。为了说明这一点，这里的右手边被组装成残差 $r_0 = f-Au_0$ ，其中 $u_0$ 是除Dirichlet值以外的零向量。然后在求解的时候，我们可以得出解决方案是  $u = u_0 + A^{-1}r_0$  。这可以看作是对初始猜测为  $u_0$  的线性系统进行的牛顿迭代。下面`solve()`函数中的CG解计算了 $A^{-1}r_0$ ，调用`constraints.distribution()`（直接在后面）增加了 $u_0$  。




显然，由于我们考虑的是一个零Dirichlet边界的问题，我们可以采取类似于 step-37  `assemble_rhs()`的方法，但是这个额外的工作允许我们在选择时改变问题的声明。




这个函数在积分循环中有两个部分：通过提交梯度的负值将矩阵  $A$  的负值应用于  $u_0$  ，通过提交值  $f$  添加右手边的贡献。我们必须确保使用`read_dof_values_plain()`来评估 $u_0$ ，因为`read_dof_vaues()`会将所有Dirichlet值设为0。




最后，system_rhs向量的类型是 LA::MPI::Vector, ，但MatrixFree类只对 dealii::LinearAlgebra::distributed::Vector. 起作用，因此我们必须使用MatrixFree函数性计算右手边，然后使用`ChangeVectorType`命名空间中的函数将其复制到正确类型。
...
无矩阵GMG方法的求解器与 step-37 相似，除了加入一些接口矩阵，完全与 step-16 类似。
...
将 LA::MPI::Vector 中的求解向量和右手边复制到 dealii::LinearAlgebra::distributed::Vector 中，这样我们就可以解决。
...
求解线性系统，更新解的重影值，复制回 LA::MPI::Vector 并分配约束。
...
我们使用FEInterfaceValues类来组装一个误差估计器，以决定哪些单元需要细化。请看介绍中对单元和面积分的确切定义。为了使用该方法，我们为 MeshWorker::mesh_loop() 定义了Scratch和Copy对象，下面的大部分代码在本质上与 step-12 中已经设置的一样（或者至少在精神上相似）。
...
残余单元格的汇编器  $h^2 \| f + \epsilon \triangle u \|_K^2$   
...
我们需要对每个内部面进行一次组装，但我们需要确保两个进程都能对本地拥有的单元格和幽灵单元格之间的面项进行组装。这可以通过设置 MeshWorker::assemble_ghost_faces_both 标志来实现。我们需要这样做，因为我们在这里没有交流误差估计器的贡献。
...
我们使用存储在向量 @p estimate_vector 中的单元估计器，并细化固定数量的单元（这里选择的是每一步中大约两倍的DoF数量）。
...
output_results()函数与许多教程中的函数相似（例如，见 step-40 ）。
...
这是一个类似于 step-40 的主函数，但我们要求用户传递一个.prm文件作为唯一的命令行参数（参见 step-29 和ParameterHandler类的文档中关于参数文件的完整讨论）。
...
然后，我们可以用 step-50 所在目录中提供的输入文件来调用程序，收集各种信息。使用这些文件，并调整网格细化步骤的数量，我们可以得出程序的扩展性如何的信息。

下表给出了该程序在256M DoFs和7,168个处理器上的弱扩展时间。(回顾一下，弱缩放在增加处理器数量的同时保持每个处理器的自由度数量不变；也就是说，它考虑的是越来越大的问题。) 这里， $\mathbb{E}$ 是介绍中的分区效率（也等于1.0/工作量不平衡），"Setup "是设置、设置多重网格、装配和装配多重网格的组合，"Prec "是预处理程序的设置。理想情况下，所有的时间在各个求解器的每个问题大小上保持不变，但是由于分区效率从最大问题大小到最小问题大小从0.371下降到0.161，我们期望看到GMG的时间大约增加 $0.371/0.161=2.3$ 倍。事实上，这与我们真正得到的结果非常接近。
...
另一方面，最后一组列中的代数多重网格相对来说不受网格层次不平衡度增加的影响（因为它不使用网格层次），时间的增长反而是由文献中记载的其他因素驱动的（最明显的是代数多重网格方法的某些部分的算法复杂度似乎是 ${\cal O}(N
\log N)$ 而不是几何多重网格的 ${\cal O}(N)$ ）。
...
一个更有趣的例子是涉及到一个更复杂的粗略网格（见 step-49 的启发）。这种情况下的问题是，网格层次的最粗层次实际上是相当大的，我们必须考虑如何有效地解决粗层次的问题。对于代数多网格方法来说，这不是一个问题，因为它们只是继续建立越来越粗的矩阵层次，而不管它们的几何来源）。

在这里的程序中，我们只是用共轭梯度法解决粗级问题，没有任何预处理。如果粗略问题真的很小，这是可以接受的--例如，如果粗略网格有一个单元，那么粗略网格问题在2D中是 $9\times 9$ 矩阵，在3D中是 $27\times 27$ 矩阵；对于我们在当前程序的 $L$ 形域上使用的粗略网格，这些尺寸在2D中是 $21\times 21$ ，在3D中是 $117\times 117$ 。但是，如果粗略的网格由数百或数千个单元组成，这种方法将不再起作用，并可能开始支配每个V形单元的整体运行时间。一个常见的方法是使用代数多网格预处理程序来解决粗网格问题；然而，这将需要组装粗矩阵（即使是无矩阵版本）作为AMG实现的输入。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-50.cc"   
