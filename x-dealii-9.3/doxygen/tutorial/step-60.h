...
在本教程中，我们考虑两个域的情况， $\Omega$ 在 $R^{\text{spacedim}}$ 中， $\Gamma$ 在 $R^{\text{dim}}$ 中，其中 $\Gamma$ 被嵌入 $\Omega$ 中（ $\Gamma \subseteq \Omega$  ）。我们想在 $\Omega$ 上解决一个偏微分方程，对问题的解决*在嵌入域* $\Gamma$ 上强制执行一些条件。

有两种有趣的情况。

- 嵌入域 $\Gamma$ 的几何维度`dim`与域 $\Omega$ 相同（`spacedim`），即 $\Gamma$ 的spacedim维度不为零，或 

- 嵌入域 $\Gamma$ 的内在维度`dim`小于 $\Omega$ （`spacedim`），因此其间隔维度为零；例如，它是一条嵌入二维域的曲线，或一个嵌入三维域的面。

在这两种情况下，定义限制算子 $\gamma$ 为算子，给定 $\Omega$ 上的连续函数，返回其在 $\Gamma$ 上的（连续）限制，即：。
...
众所周知，当 $\Gamma$ 的内在维度与 $\Omega$ 相同时，算子 $\gamma$ 可以扩展为 $H^1(\Omega)$ 上的连续算子，将 $H^1(\Omega)$ 中的函数映射到 $H^1(\Gamma)$ 中的函数。

当 $\Gamma$ 的维度相对于 $\Omega$ 少一个，且 $\Gamma$ 没有边界时，在一个不太规则的范围空间（即 $H^{1/2}(\Gamma)$ ）中也是如此。在这第二种情况下，算子 $\gamma$ 也被称为*轨迹算子，对于嵌入 $\Omega$ 中的Lipschitz同维一曲线和曲面 $\Gamma$ ，它是很好的定义（关于轨迹算子的进一步细节请阅读<a
href="https://en.wikipedia.org/wiki/Trace_operator">this wikipedia article</a>）。

共二维的情况要复杂一些，一般来说，不可能构造一个连续的跟踪算子，甚至不可能从 $H^1(\Omega)$ 到 $L^2(\Gamma)$ ，当 $\Gamma$ 的维度在二维和三维中分别为零或一的时候。

在本教程中，我们对 $\gamma$ 的进一步细节不感兴趣：我们认为扩展 $\gamma$ 是理所当然的，假设嵌入域的尺寸（`dim`）总是比嵌入域的尺寸 $\Omega$ （`spacedim`）小一或相等。

我们将解决以下微分问题：给定 $g$ 上的一个足够规则的函数 $\Gamma$ ，找到 $u$ 的解。
...
这是一个约束问题，我们要寻找一个在 $\partial\Omega$ 上满足同质边界条件的谐波函数 $u$ ，在约束条件 $\gamma u = g$ 下使用拉格朗日乘法器。

这个问题有一个物理解释：谐波函数，即满足拉普拉斯方程的函数，可以被认为是边界值被规定的膜的位移。那么，目前的情况相当于找到一个膜的形状，对它来说，不仅边界上的位移，而且 $\Gamma$ 上的位移也是规定的。例如，如果 $\Gamma$ 是二维空间的封闭曲线，那么这将是一个肥皂膜的模型，它被沿 $\partial \Omega$ 的线环和沿 $\Gamma$ 的第二个线环固定住。在 $\Gamma$ 是整个区域的情况下，你可以把它看作是在障碍物上伸展的膜，其中 $\Gamma$ 是接触区域。(如果接触面积不知道，我们有一个不同的问题--称为 "障碍物问题"--在 step-41 中建模) 。

作为第一个例子，我们研究 $\partial\Omega$ 上的零Dirichlet边界条件。如果我们在 $\partial\Omega$ 上应用零Neumann边界条件或两者的混合，也适用同样的方程。

通过引入两个无限维空间 $V(\Omega)$ 和 $Q^*(\Gamma)$ ，可以得出变分公式，分别用于解 $u$ 和拉格朗日乘子 $\lambda$ 。

将第一个方程乘以 $v \in V(\Omega)$ ，第二个方程乘以 $q \in
Q(\Gamma)$ ，在可能的情况下进行部分积分，并利用 $\partial\Omega$ 的边界条件，我们得到以下变分问题。
...
其中 $(\cdot, \cdot)_{\Omega}$ 和 $(\cdot, \cdot)_{\Gamma}$ 分别代表 $L^2$ 中的标量积和 $\Gamma$ 。

对变量公式的检查告诉我们，空间 $V(\Omega)$ 可以被看作是 $H^1_0(\Omega)$  。空间 $Q(\Gamma)$ ，在同维度为零的情况下，应取为 $H^1(\Gamma)$ ，而在同维度为一的情况下应取为 $H^{1/2}(\Gamma)$ 。

因此函数 $g$ 应该在 $H^1(\Gamma)$ （共维度为零的情况）或 $H^{1/2}(\Gamma)$ （共维度为一的情况）。这使得我们在 $Q^*(\Gamma)$ 中有一个拉格朗日乘数 $\lambda$ ，它是 $H^{-1}(\Gamma)$ 或 $H^{-1/2}(\Gamma)$  。

对于上述问题的离散化，有两种选择。可以选择匹配的离散化，即 $\Gamma$ 的三角化与 $\Omega$ 的三角化一致，也可以选择以完全独立的方式离散化这两个域。

对于我们上面提出的简单问题，第一种选择显然更有意义：对 $\Omega$ 使用单一的三角法就可以了，然后根据 $\Gamma$ 施加某些约束。这种方法的一个例子是在 step-40 中研究的，其中解决方案必须保持在一个障碍物之上，这是在 $\Omega$ 上施加约束实现的。

为了解决更复杂的问题，例如域 $\Gamma$ 与时间有关的问题，第二种方法可能是更可行的解决方案。处理不对齐的网格本身就很复杂：为了说明如何做，我们研究一个简单的问题。

我们在这里描述的技术在文献中使用了许多名称之一：<b>immersed finite element method</b>、<b>fictitious boundary method</b>、<b>distributed Lagrange multiplier method</b>等。其主要原理是两个网格和两个有限元空间的离散化保持完全独立。这种技术对于模拟流固耦合问题特别有效，其中嵌入结构的配置是问题本身的一部分，人们要解决一个（可能是非线性的）弹性问题来确定 $\Gamma$ 的（与时间有关的）配置，以及 $\Omega
\setminus \Gamma$ 的（可能是非线性的）流动问题，加上流体和固体之间界面上的耦合条件。

在这个教程程序中，我们把事情弄得简单一些，我们假设嵌入域的配置是以两种可能的方式之一给出的。

- 变形映射  $\psi: \Gamma_0 \mapsto \Gamma \subseteq \Omega$  ，定义在  $\Gamma_0$  上的连续有限维空间上，对于任何点  $x \in \Gamma_0$  ，其坐标  $\psi(x)$  在  $\Omega$  中表示。

- 作为 $x\in \Gamma_0$ 的位移映射 $\delta \psi(x) = \psi(x)-x$ ，代表任何点 $x$ 的位移矢量，以使 $x$ 变形为其实际配置 $\psi(x) = x +\delta\psi(x)$ 。
...
拉格朗日乘数 $\lambda$ 和用户提供的函数 $g$ 是通过另一个有限维空间`embedded_dh`和另一个有限元`embedded_fe`定义的，使用相同的参考域。为了考虑到域的变形，MappingFEField或MappingQEulerian对象被初始化为`embedded_configuration`向量。

在嵌入空间中，一个标准的有限维空间`space_dh`被构建在嵌入网格`space_grid`上，使用有限元素`space_fe`，几乎逐字逐句地遵循  step-6  中的方法。

我们分别用\f[
V_h(\Omega) = \text{span} \{v_i\}_{i=1}^n
\f]和\f[
Q_h(\Gamma) = \text{span} \{q_i\}_{i=1}^m
\f]表示空间 $V$ 和 $Q$ 的离散化，其中 $n$ 是`space_dh`的尺寸， $m$ 是`embedded_dh`的尺寸。
...
虽然矩阵 $K$ 是 $\Omega$ 上泊松问题的标准刚度矩阵，矢量 $G$ 是 $\Gamma$ 上带有强制项 $g$ 的有限元问题的标准右手矢量（例如，见 step-3 ），但矩阵 $C$ 或其转置 $C^T$ 是非标准的，因为它们将两个非匹配网格的信息结合在一起。

特别是，在计算 $C$ 的一个条目时出现的积分，是在 $\Gamma$ 上计算的。在有限元中，我们通常将这个积分分成来自用于离散化 $\Gamma$ 的三角形的所有单元的贡献，我们将 $K$ 上的积分转换为参考元素 $\hat K$ 上的积分，其中 $F_{K}$ 是从 $\hat K$ 到 $K$ 的映射，并使用正交公式计算 $\hat K$ 上的积分。
...
计算这个总和是不容易的，因为我们必须评估  $(v_j \circ F_{K})
(\hat x_i)$  。一般来说，如果 $\Gamma$ 和 $\Omega$ 没有对齐，那么 $F_{K}(\hat x_i)$ 这个点相对于 $\Omega$ 来说是完全任意的，除非我们想出办法将 $V_h(\Omega)$ 的所有基函数插值在 $\Omega$ 上的任意点，否则我们无法计算矩阵 $C$ 的一个条目所需的积分。

为了评估 $(v_j \circ F_{K}) (\hat x_i)$ ，需要采取以下步骤（如下图所示）。

- 对于 $\Gamma$ 中的给定单元 $K$ ，计算实点 $y_i \dealcoloneq F_{K} (\hat
x_i)$ ，其中 $x_i$ 是用于对 $K
\subseteq \Gamma$ 进行积分的正交点之一。
...
一个更好的解决方案是使用一个方便的包装器在一个点的集合上执行前三个步骤。  GridTools::compute_point_locations().  如果人们实际上对计算完整的耦合矩阵感兴趣，那么可以调用方法 NonMatching::create_coupling_mass_matrix(), ，该方法以有效的方式执行上述步骤，重复使用所有可能的数据结构，并将昂贵的步骤聚集在一起。这就是我们在本教程后面要使用的函数。

我们通过迭代求解器来解决最终的鞍点问题，应用于Schur补数 $S$ （其构造例如在 step-20 中描述），我们使用LinearOperator类构造 $S$ 。


<a name="Thetestcase"></a><h3>The testcase</h3> 


我们在这里解决的问题与 step-4 相同，不同的是我们对一个嵌入域 $\Gamma$ 施加了一些约束。本教程是以独立于维度的方式编写的，在结果部分我们展示了如何改变`dim`和`spacedim`。

本教程是在`dim`等于1，`spacedim`等于2的情况下编译的。如果你想在嵌入维度`spacedim`等于3的情况下运行程序，你很可能想改变 $\Gamma$ 的参考域，例如，你从文件中读到的东西，或者你后来变形为更有趣的封闭球体。

在默认情况下， $\Gamma$ 的同维度为1，本教程的程序实现了虚构边界法。事实证明，同样的技术也用在了变量沉浸式有限元方法中，上面定义的耦合算子 $C$ 在几乎所有这些非匹配方法中都是一样的。

嵌入域被假定包括在 $\Omega$ 中，我们把它当作单位平方 $[0,1]^2$  。虚域 $\Gamma$ 的定义可以通过参数文件修改，可以给出从参考区间 $[0,1]$ 到 $\Omega$ 中的一条曲线的映射。

如果该曲线是封闭的，那么结果将类似于在边界为 $\Gamma$ 的网格上运行相同的问题。在非封闭的 $\Gamma$ 上，程序也会很高兴地运行，尽管在这些情况下，问题的数学表述更加困难，因为 $\Gamma$ 本身就有一个边界，相对于域 $\Omega$ 而言，它具有二维。
...
<a name="References"></a><h3>References</h3>
  ...
  <li>  Boffi, D., L. Gastaldi, L. Heltai, and C.S. Peskin. 2008. "论沉浸边界法的超弹性公式"。应用力学和工程中的计算机方法197（25-28）。

  <li>  Heltai, L., and F. Costanzo. 2012. "浸没式有限元方法的变量实现"。应用力学和工程中的计算机方法》229-232。  </ul>  <a name="CommProg"></a> <h1> The commented program</h1>


<a name="Includefiles"></a> <h3>Include files</h3> 其中大部分已在其他地方介绍过，我们只对新的进行评论。
...
这个类用来定义一个公共接口，供那些想使用一个全局ParameterHandler来处理参数的类使用。该类提供了一个静态的ParameterHandler成员，即 ParameterAcceptor::prm, ，并实现了 "命令设计模式"（例如，见E. Gamma, R. Helm, R. Johnson, J. Vlissides, 设计模式。Elements of Reusable Object-Oriented Software, Addison-Wesley Professional, 1994. https://goo.gl/FNYByc）。) 




ParameterAcceptor提供了一个全局订阅机制。每当一个从ParameterAcceptor派生出来的类的对象被构造出来，一个指向该派生类型的对象的指针就会被注册，同时在参数文件中也有一个部分条目。这种注册表在调用单一函数 ParameterAcceptor::initialize("file.prm") 时被遍历，该函数反过来确保所有存储在全局注册表中的类都声明它们将使用的参数，在声明了这些参数后，它读取`file.prm`的内容来解析实际参数。
...
一些方法，例如 GridTools::find_active_cell_around_point, 大量使用了这些非标准的操作。如果你需要多次调用这些方法，那么将这些数据结构存储在某个地方就变得很方便。  GridTools::Cache 正是这样做的，让你可以访问以前计算过的对象，或者在飞行中计算它们（然后将它们存储在类中供以后使用），并确保每当三角测量被更新时，相关的数据结构也被重新计算。
...
这些参数将被用来初始化Triangulation<dim,spacedim>（用于 $\Gamma$ ）和Triangulation<spacedim,spacedim>（用于 $\Omega$ ）。   


与其他教程程序相比，一个新奇之处在于大量使用了  std::unique_ptr.  这些指针的行为与经典指针一样，其优点是可以进行自动的内部管理：一旦unique_ptr超出范围，所包含的对象就会被自动销毁，即使它在一个容器内或者有一个例外。此外，它不允许有重复的指针，这可以防止所有权问题。我们这样做，是因为我们希望能够 i) 构建问题，ii) 读取参数，iii) 根据参数文件中指定的内容初始化所有对象。   
...
`Parameters`类派生于ParameterAcceptor。这使得我们可以在其构造函数中使用 ParameterAcceptor::add_parameter() 方法。     
...
嵌入网格 $\Omega$ 和嵌入网格 $\Gamma$ 之间的交互是通过 $C$ 的计算来处理的，这涉及到 $\Omega$ 的所有单元与 $\Gamma$ 的部分重叠：对这些单元的更高细化可能会提高我们的计算质量。为此，我们定义了 "delta_refinement"：如果它大于零，那么我们将空间网格中包含嵌入网格顶点的每个单元及其邻居标记出来，执行细化，并重复这个过程 "delta_refinement "次。
...
边界id的列表，在这里我们施加同质Dirichlet边界条件。在其余的边界id上（如果有的话），我们施加同质的Neumann边界条件。作为一个默认的问题，我们在 $\partial \Omega$ 上设置了零迪里希特边界条件。 
...
我们这里唯一的非常规函数是`setup_coupling()`方法，用于生成耦合矩阵的稀疏性模式  $C$  。
...
ParameterAcceptorProxy类是一个 "透明 "的包装器，它源自ParameterAcceptor和作为其模板参数传递的类型。在构造时，参数被分成两部分：第一个参数是转发给ParameterAcceptor类的 std::string, ，并包含应该用于这个类的部分的名称，而所有其余的参数都被转发给模板类型的构造器，在这里是转发给 Functions::ParsedFunction 构造器。     


这个类允许你结合ParameterAcceptor注册机制使用现有的类，只要这些类有`declare_parameters()`和`parse_parameters()`成员。     


这里就是这种情况，使得利用 Functions::ParsedFunction 类相当容易：不要求用户在代码中为RHS、边界函数等创建新的Function对象（就像在其他大多数教程中那样），这里我们允许用户使用deal.II接口到muParser（http://muparser.beltoforion.de），其中函数的规范不是在编译时完成，而是在运行时，使用一个字符串，被解析成一个实际的Function对象。     
...
与我们对 Functions::ParsedFunction 类所做的类似，我们对ReductionControl类重复同样的做法，允许我们为我们稍后使用的Schur complement迭代求解器指定所有可能的停止标准。
...
你现在可以使用相对路径（例如，`ParameterAcceptor("My Other Class")`）而不是绝对路径（例如，`ParameterAcceptor("/first/second/third/My Other Class")`）来构造从ParameterAcceptor派生的另一个类，获取。  <div class=CodeFragmentInTutorialComment>
...
反过来， ParameterAcceptor::prm.add_parameter 也是如此。 
...
- 在 ParameterAcceptor::prm 中附加一个*动作，当文件被解析时，或者当一个条目被设置时，它会更新传递给`add_parameter()`的变量的值，将其设置为输入文件中指定的值（当然，是在输入文件被解析并将文本表示转换为变量的类型之后）。
...
函数 `DistributedLagrangeProblem::setup_grids_and_dofs()` 是用来设置有限元空间的。注意 `std::make_unique` 是如何用来创建包裹在 `std::unique_ptr` 对象内的对象的。
...
初始化 $\Omega$ ：构造三角形并将其包裹到 `std::unique_ptr` 对象中。
...
接下来，我们使用 GridGenerator::hyper_cube(). 实际创建三角形，最后一个参数被设置为true：这激活了着色（即为边界的不同部分分配不同的边界指标），我们用它来分配Dirichlet和Neumann条件。
...
在这个教程程序中，我们不仅对 $\Omega$ 进行全局细化，还允许根据 $\Gamma$ 的位置进行局部细化，根据`parameters.delta_refinement`的值，我们用来决定我们应该对 $\Omega$ 进行多少轮局部细化，对应于 $\Gamma$ 的位置。     


有了这个映射，现在就可以通过调用方法 DoFTools::map_dofs_to_support_points. 来查询与`嵌入式_dh'相关的所有支持点的位置。      
...
一旦我们有了嵌入有限元空间的支持点，我们就想确定嵌入空间的哪些单元包含哪些支持点，以便有机会在必要的地方，即嵌入网格的地方细化嵌入网格。这可以手动完成，通过循环每个支持点，然后为嵌入空间的每个单元调用方法 Mapping::transform_real_to_unit_cell ，直到我们找到一个返回单位参考单元中的点，或者可以用更智能的方式完成。     
...
利用最大速度的最简单的方法是调用一个专门的方法， GridTools::compute_point_locations, ，它将在第一个点的搜索过程中存储大量有用的信息和数据结构，然后在后续的点中重复使用所有这些信息。     
...
为了构造一个良好的耦合插值算子 $C$ ，对嵌入域和被嵌入域之间的网格的相对尺寸有一些约束。耦合算子 $C$ 和空间 $V$ 和 $Q$ 必须满足一个inf-sup条件，以使问题有一个解决方案。事实证明，只要空间 $V$ 和 $Q$ 之间相互兼容，非匹配 $L^2$ 投影就满足这样的inf-sup条件（例如，只要它们被选为引言中描述的那些）。     
...
我们现在设置 $\Omega$ 和 $\Gamma$ 的DoF：因为它们基本上是独立的（除了 $\Omega$ 的网格在 $\Gamma$ 周围更加精细之外），这个过程是标准的。
...
根据定义，我们要解决的系统的Rhs只涉及一个零矢量和 $G$ ，它只用 $\Gamma$ 的DoF来计算。
...
创建耦合稀疏模式是一个复杂的操作，但可以使用 NonMatching::create_coupling_sparsity_pattern, 轻松完成，它需要两个DoFHandler对象，耦合的正交点，一个DynamicSparsityPattern（然后需要像往常一样复制到稀疏模式中），嵌入和嵌入三角的分量掩码（我们留空）以及嵌入和嵌入三角的映射关系。
...
这两个输出例程的唯一区别是，在第二种情况下，我们希望输出当前配置的数据，而不是参考配置的数据。这可以通过将实际的embedded_mapping传递给 DataOut::build_patches 函数来实现。该映射将负责输出实际变形配置的结果。
...
与其他教程程序中的情况不同，这里我们使用ParameterAcceptor风格的初始化，即首先构造所有对象，然后对静态方法 ParameterAcceptor::initialize 发出一次调用，以填充从ParameterAcceptor派生的类的所有参数。       
...
对于默认问题， $u$ 在 $\Gamma$ 上的值被设置为常数 $1$ ：这就像在 $\Gamma$ 上施加一个常数迪里切特边界条件，被视为 $\Omega$ 在 $\Gamma$ 内的部分的边界。同样，在 $\partial
\Omega$ 上我们有零迪里希特边界条件。
...
我们得到一个看起来 "花花绿绿 "的域，在这里我们施加一个线性边界条件  $g=x-.5$  。这个测试表明，该方法在从其边界条件中恢复一个完全线性的函数方面实际上是相当准确的，即使网格没有对齐，我们也得到了一个相当好的结果。
...
- 使用位移代替变形，将 $\Gamma_0$ 映射到 $\Gamma$ 。 

<a name="Moregeneraldomains"></a><h4> More general domains </h4>


我们已经在其他教程中看到（例如在 step-5 和 step-54 中）如何从输入文件中读取网格。对于这个教程程序来说，一个很好的概括是允许用户从参数文件本身选择要读取的网格，而不是在教程程序本身硬编码网格类型。
...
在这里，一个好的预处理程序是什么并不明显。我们用舒尔补码解决的物理问题是将迪里切特数据  $g$  与拉格朗日乘数的值联系起来  $\lambda$  。  $\lambda$ 可以解释为法线梯度的*跳跃，需要强加在 $u$ 上，跨越 $\Gamma$ ，以获得Dirichlet数据  $g$ 。

所以 $S$ 是某种诺伊曼到迪里切特的地图，我们希望有一个好的迪里切特到诺伊曼地图的近似值。一种可能性是使用 $\Gamma$ 上的问题的边界元素近似，并构建与 $\Gamma$ 相关的泊松问题的超星形算子的粗略近似，这正是迪里切特到诺依曼映射。

<a name="ParallelCode"></a><h4> Parallel Code </h4> 


这里提出的简单代码可以作为更复杂问题的起点，要解决这些问题，需要在并行代码上运行，可能使用分布式网格（见 step-17 ， step-40 ，以及 parallel::shared::Triangulation 和 parallel::distributed::Triangulation). 的文档）。 

当并行使用非匹配网格时，会出现一个问题：为了计算矩阵 $C$ ，一个进程需要关于实空间同一区域的两个网格的信息，但是，当使用分布式网格时，这种信息可能无法获得，因为存储在特定处理器上的 $\Omega$ 三角形的本地所有部分可能与存储在同一处理器上的 $\Gamma$ 三角形的本地所有部分不在同一地点。
...
然而，这种策略很可能会更加昂贵，因为根据定义，嵌入网格要比嵌入网格大，而且分布这两个网格中最大的网格更有意义，保持最小的网格在所有处理器中共享。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-60.cc"   
