...
本教程取决于  step-19  ,  step-32  ,  step-60  。
...
在本教程中，我们考虑了层流体系中的混合问题。这类问题出现在从化学工程到发电（如涡轮机械）等广泛的应用中。混合问题特别难以用数值解决，因为它们通常涉及一个容器（有固定的边界，可能还有复杂的几何形状，如挡板），由域 $\Omega$ 表示，和一个（或多个）沉浸和旋转的叶轮（由域 $\Omega^{\text{imp}}$ 表示）。我们希望解决流动方程的领域是这两个领域之间的（与时间有关的）差异，即。  $\Omega\setminus\Omega^{\text{imp}}$  . 
...
在这种情况下，一个可行的替代策略是使用非匹配方法（类似于我们在 step-60 中所做的），其中一个背景固定网格（可能会或不会在时间上进行局部细化以更好地捕捉固体运动）与一个旋转的独立网格相耦合。

为了保持与 step-60 中使用的相同符号，我们使用 $\Omega$ 来表示 ${\mathbb R}^{\text{spacedim}}$ 中代表流体和叶轮容器的域，并且我们在 ${\mathbb R}^{\text{dim}}$  ]来表示整个叶轮（当它的`spacedim`度量非负值时，也就是说，当我们可以把它表示为维数`dim`等于`spacedim`的网格时），薄叶轮的同维度表示，或者只是整个叶轮的边界。

域 $\Gamma$ 被嵌入到 $\Omega$ （ $\Gamma \subseteq \Omega$ ）中，它是不匹配的：一般来说，它不与体积网格的任何特征对齐。我们在 $\Omega$ 上求解一个偏微分方程，通过一些惩罚技术在嵌入域 $\Gamma$ 上强制执行一些问题的解决条件。在当前情况下，条件是流体在 $\Gamma$ 上各点的速度等于固体叶轮在该点的速度。

我们在这里描述的技术在文献中使用了许多名称之一：<b>immersed finite element method</b>和<b>fictitious boundary
method</b>等。 其主要原理是两个网格的离散化保持完全独立。在本教程中，这种方法被用来求解由斯托克斯方程描述的粘性流体的运动，该流体被一个刚性的非变形叶轮搅动。

因此， $\Omega$ 中求解的方程是蠕动流的斯托克斯方程（即 $\text{Re}\rightarrow 0$ 中的流动），并且在与叶轮相关的移动*嵌入域* $\Gamma$ 上应用无滑动边界条件。然而，这个教程可以很容易地扩展到其他方程（例如，纳维-斯托克斯方程、线性弹性方程等）。它可以被看作是 step-60 的一个自然扩展，它可以通过MPI使用分布式并行计算架构解决大型问题。

然而，与 step-60 相反， $\Gamma$ 上的Dirichlet边界条件是弱加的，而不是通过使用Lagrange乘法器，并且我们集中处理两个完全分布式三角形的耦合（这种组合在 step-60 的实现中是不可能的）。

当人们想在嵌入域上执行条件时，有两种有趣的情况出现  $\Gamma$  。

- 嵌入域 $\Gamma$ 的几何维度`dim`与域 $\Omega$ 相同（`spacedim`），也就是说， $\Gamma$ 的spacedim维度不为零。在这种情况下，对 $\Gamma$ 施加Dirichlet边界的边界条件是通过体积惩罚完成的。如果施加的惩罚只取决于速度，这通常被称为 $\mathcal{L}^2$ 惩罚，而如果惩罚同时取决于速度及其梯度，则是 $\mathcal{H}^1$ 惩罚。 $\mathcal{L}^2$  惩罚的情况与Darcy型方法非常相似。对 $\mathcal{L}^2$ 和 $\mathcal{H}^1$ 两种惩罚方法都进行了广泛的分析（例如，见 @cite Angot1999 ）。

- 嵌入域 $\Gamma$ 的内在维度`dim`小于 $\Omega$ 的维度（`spacedim`），因此它的spacedim维度为零；例如，它是一条嵌入二维域的曲线，或一个嵌入三维域的曲面。当然，这在物理上是不可能的，但是如果金属片的厚度可以忽略不计的话，我们可以把在流体中运动的非常薄的金属片视为本质上的低维。在这种情况下，通过应用<a href="https://en.wikipedia.org/wiki/Joachim_Nitsche">Nitsche</a>方法（见 @cite Freund1995 ）对 $\Gamma$ 施加弱边界条件。

这两种方法都有非常相似的要求，并导致高度相似的计算公式。因此，我们几乎以同样的方式对待它们。

在本教程中，我们对 $\Gamma$ 的进一步细节不感兴趣：我们假设嵌入域的尺寸（`dim`）总是比嵌入域的尺寸 $\Omega$ （`spacedim`）小一或相等。

我们将解决以下微分问题：给定 $\Gamma$ 上的一个足够规则的函数 $g$ ，找到 $(\textbf{u},p)$ 的解。
...
这个方程，我们通过缩放时间单位的方式将其规范化，使粘度的数值为1，描述了缓慢的粘性流动，如蜂蜜或岩浆。本教程的主要目的是说明如何用惩罚方法，以弱的方式将速度场条件 $\mathbf{u} = \mathbf{g}$ 强加于非匹配的 $\Gamma$ 。关于斯托克斯问题的更广泛的讨论，包括体力、不同的边界条件和求解策略，可以在  step-22  中找到。

让我们首先考虑整个领域中的斯托克斯问题  $\Omega$  。我们寻找一个速度场 $\mathbf{u}$ 和一个压力场 $p$ ，满足斯托克斯方程的同质边界条件 $\partial\Omega$ 。

斯托克斯方程的微弱形式首先通过将其写成矢量形式@f{eqnarray*}
  \begin{pmatrix}
    {-\Delta \textbf{u} + \nabla p}
    \\
    {-\textrm{div}\;\textbf{u}}
...
与矢量值测试函数 $\phi = \begin{pmatrix}\textbf{v} \\ q\end{pmatrix}$ 形成左边的点积，并在域 $\Omega$ 上积分，得到以下方程组。@f{eqnarray*}
  (\mathrm v,
...
其中 $(\cdot, \cdot)_{\Omega}$ 代表 $L^2$ 的标量积。这与 step-22 中使用的变分形式相同。

这种变异形式没有考虑到嵌入域。与 step-60 相反，我们并不强行执行 $\textbf{u}$ 对 $\Gamma$ 的约束，而是通过惩罚项弱行执行。

对这种弱的边界条件的分析取决于 $\Gamma$ 的间隔维度是正的（如果`dim`等于`spacedim`）还是零（如果`dim`小于`spacedim`）。我们讨论这两种情况。
...
对 $\Gamma$ 的积分是低维积分。可以证明（见 @cite Freund1995 ），存在一个正的常数 $C_1$ ，所以如果 $\beta > C_1$ ，边界的弱强加条件将是一致和稳定的。在 $\Gamma$ 上的前两个附加积分（上式中的第二行）在通过部分积分后自然出现，此时我们不假设 $\mathbf{v}$ 在 $\Gamma$ 上是零。

上文等式中的第三行包含两个条款，是为了确保弱形式的一致性，还有一个稳定条款，是为了强制执行边界条件，其误差与近似误差一致。一致性项和稳定项是用实际的边界数据添加到右边的  $\mathbf{g}$  。

当 $\mathbf{u}$ 满足 $\Gamma$ 上的条件 $\mathbf{u}=\mathbf{g}$ 时， $\Gamma$ 上的所有一致性和稳定性积分都被抵消，就剩下通常的斯托克斯流的弱形式，也就是说，上述表述是一致的。
...
注意第三行和第四行的第一个项的不同符号。在这种情况下，稳定性和一致性条件成为  $\beta > 0$  。在对称情况下， $\beta$ 的值取决于 $h$ ，一般选择 $\beta = C h^{-1} $ ， $h$ 是衡量被整合面的大小， $C$ 是一个常数，以便 $1 \leq C \leq 10$  。这就像人们通常用Nitsche惩罚方法来执行Dirichlet边界条件一样。

另一方面，非对称方法与人们对非连续Galerkin方法的非对称内部惩罚方法（"NIPG "方法 @cite Riviere1999 ）执行连续性的方式有关。即使非对称情况在稳定参数的可能选择方面似乎更有优势，我们还是选择了对称离散化，因为在这种情况下，可以证明对偶问题也是一致的，导致解决方案不仅能量规范以正确的顺序收敛，而且其 $L^2$ 规范也是如此。此外，得到的矩阵仍然是对称的。

上述公式是在假设领域被精确离散化的情况下工作的。然而，如果叶轮的变形是一个刚体运动，就有可能人为地将斯托克斯问题的解扩展到螺旋桨本身，因为刚体运动也是斯托克斯问题的解。我们的想法是在 $\Omega^{\text{imp}}$ 内解决同样的问题，在 $\Gamma$ 上施加同样的边界条件，使用同样的惩罚技术，并用在 $\Omega$ 上全局连续的测试函数 $\mathbf{v}$ 来测试。

这导致了以下（中间）公式。@f{multline*}{
(\nabla \textbf{v}, \nabla \textbf{u})_{\Omega} - (\textrm{div}\;  \textbf{v}, p)_{\Omega}
...
其中跳跃项，用 $\lbrack \cdot \rbrack$ 表示，是相对于法向量 $\textbf{n}$ 的一个固定方向计算的。2的系数出现在 $\beta$ 前面，因为我们看到 $\Gamma$ 的每一部分两次，一次来自流体内部，一次来自在其中移动的障碍物内部。对于 $\Gamma$ 上的所有其他积分，我们对 $\Gamma$ 的每一部分都访问了两次，但符号相反，因此得到了跳跃项）。

这里我们注意到，与不连续的Galerkin方法不同，测试和试验函数在 $\Gamma$ 上是连续的。此外，如果 $\Gamma$ 不与单元边界对齐，所有的跳跃项也是零，因为一般来说，有限元函数空间在每个单元内都是平滑的，如果 $\Gamma$ 穿过一个单元，只在有限的点上与它的边界相交，所有在 $\Gamma$ 上的贡献，除了稳定化的，可以从公式中忽略，导致以下变分公式的最终形式。
...
在 step-60 中，约束条件的施加需要以拉格朗日乘数的形式增加新的变量。本教程的程序不是这种情况。使用Nitsche方法施加边界条件只修改了系统矩阵和右手边，没有增加额外的未知数。然而，嵌入域上的速度矢量 $\textbf{u}$ 不会与规定的速度 $\textbf{g}$ 完全匹配，而只能达到与有限元方法的插值误差相同的数值误差。此外，与 step-60 一样，我们仍然需要在不匹配的嵌入网格上进行积分，以构建施加边界条件所需的边界项 $\Gamma$  。
...
这里， $\Gamma$ 上的积分只是体积的一部分的积分。因此， $\mathcal{L}^2$ 的惩罚包括增加一个体积项，约束流体的速度与 $\Gamma$ 内刚体的速度相一致。在这种情况下， $\beta$ 必须被选择得足够大，以确保 $\Gamma$ 中的迪里希特边界条件得到充分尊重，但又不能太高，以保持系统矩阵的适当调节。

 $\mathcal{H}^1$ 的惩罚可以用类似的方式构建，在惩罚中加入粘性成分，以阻尼 $\Gamma$ 内的速度梯度。
...
注意 $L^2$ 的惩罚（`dim`等于`spacedim`）和Nitsche的惩罚（`dim`等于`spacedim-1`）导致了完全相同的数值实现，这要感谢deal.II的独立维度能力。


<a name="Representationofand"></a><h4>Representation of Ω and Γ</h4>


在本教程中，嵌入网格 $\Gamma$ 和嵌入网格都是用 parallel::distributed::Triangulation. 来描述的。 这两个三角形可以通过GridGenerator命名空间中的函数或者通过读取其他应用程序（例如GMSH，见 step-49 中的讨论）生成的网格文件来构建。这比之前在  step-60  中所做的略微更普遍。

无论是在 "dim=spacedim "还是 "dim<spacedim "的情况下，增加浸没边界法，只是在系统矩阵和系统的右手边引入了额外的项，这些项是在 $\Gamma$ 上积分的结果。这并没有改变必须解决的问题的变量数量。因此，挑战是与必须进行的积分有关  $\Gamma$  。

在有限元中，我们把这个积分分成来自用于离散化 $\Gamma$ 的所有单元的贡献，我们把 $K$ 上的积分转化为参考元素 $\hat K$ 上的积分，其中 $F_{K}$ 是从 $\hat K$ 到 $K$ 的映射，并使用正交公式计算 $\hat K$ 上的积分。比如说。
...
(\hat x_i)$ 。一般来说，如果 $\Gamma$ 和 $\Omega$ 没有对齐，那么 $y_i = F_{K}(\hat x_i)$ 这个点相对于 $\Omega$ 来说是完全任意的，除非我们想出一个办法，将 $V_h(\Omega)$ 的所有基函数插在 $\Omega$ 上的一个任意点，否则我们无法计算出所需的积分。


为了评估 $(v_j \circ F_{K}) (\hat x_i)$ ，需要采取以下步骤（如下图所示）。
...
- 找到 $\Omega$ 中 $y_i$ 所在的单元。我们将称这个单元为 $T$  。

- 找出 $T$ 内 $y_i$ 的参考坐标。为此，我们需要将参考元素 $\hat T$ 转换为元素 $T$ ： $\hat y_i = G^{-1}_{T} (y_i)$ 的映射 $G_T$ 的逆向。

- 在这一点上评估  $\Omega$  网格的基函数  $v_j$   $\hat y_i$  。这也是比较简单的，使用FEValues。
...
在  step-60  中，上面的第二到第四步是通过依次调用来计算的。
...
虽然这种方法对目前的情况可行，但它不容易用于使用分布式三角的平行模拟。事实上，由于嵌入域 $\Gamma$ 单元上的正交点的位置与嵌入三角形的位置不一致，而且 $\Gamma$ 是不断移动的，这就要求代表 $\Gamma$ 的三角形被完整地存储在所有处理器中。随着处理器的数量和 $\Gamma$ 中单元格数量的增加，这将导致内存方面的严重瓶颈。因此，在这一步骤中寻求另一种策略。


<a name="Usingparticlestotrack"></a><h4>Using particles to track Γ</h4> 


记住，对于惩罚方法（ $\mathcal{L}^2$ 或 $\mathcal{H}^1$ ）和Nitsche方法，我们要计算被正交所近似的积分。也就是说，我们需要计算\f[
\beta (\textbf{v},\textbf{u})_{\Gamma} =
\sum_{K\in \Gamma} \sum_{i=1}^{n_q}  \big(\hat{\textbf{u}}(\hat x_i)  (\textbf{v} \circ F_{K}) (\hat x_i) J_K (\hat x_i) w_i \big)
\f]。 如果你跟随上面的讨论，那么你会记得 $\textbf{u}$ 和 $\textbf{v}$ 是定义在流体网格上的形状函数。唯一定义在实体网格上的是。  $F_K(\hat x_i)$  ，是实体单元上正交点的位置，是 $\Gamma$ 的一部分， $J_K$ 是其雅各布系数的行列式， $w_i$ 是相应的正交重量。

现在要认识到的重要部分是这样的。  $w_i$ 是正交公式的一个属性，不随时间变化。此外， $F_K$ 的雅各布矩阵本身随着固体障碍物在流体中的移动而变化，但由于固体被认为是不变形的（它只平移和旋转，但不扩张），雅各布矩阵的行列式保持不变。因此，乘积 $J_K(\hat x_i) w_i$ （我们通常用`JxW`表示）在每个正交点上保持不变。因此，我们唯一需要跟踪的是位置 $x_i=F_K(\hat x_i)$ --但这些位置随着实体域的速度移动。

换句话说，我们实际上根本不需要保持实体网格。我们所需要的只是位置 $x_i(t)$ 和相应的`JxW`值。由于这两个属性都是附着在实体材料上的点属性（或点向量），它们可以被理想化为一组不相连的无限小的 "粒子"，它们随着实体的运动携带所需的`JxW`信息。deal.II有能力以ParticleHandler类的形式在大规模并行计算中分配和存储这样一组粒子（关于实现的细节见 @cite GLHPW2018  ），我们将在本教程中使用这一功能。

因此，这一步采取的方法如下。

- 为域  $\Gamma$  创建一个  parallel::distributed::Triangulation  。

- 在 $\Gamma$ 上的正交点位置创建 Particles::Particle  。

- 调用 Particles::ParticleHandler::insert_global_particles() 函数，将粒子分布在处理器上，*遵循实体三角剖分*。

- 将 "JxW "值作为一个 "属性 "附加到每个 Particles::Particle 对象。

这种结构的生成相对昂贵，但每次模拟必须只生成一次。一旦生成了 Particles::ParticleHandler 并将所需信息附加到粒子上，就可以利用粒子在ParticleHandler内按单元分组的事实，对 $\Gamma$ 进行积分，使我们能够。

- 在 $\Omega$ 中至少包含一个粒子的所有单元上进行循环操作 

- 在给定单元中的所有粒子上循环操作 

- 计算积分并填充全局矩阵。

由于 Particles::ParticleHandler 可以管理粒子从一个处理器到另一个处理器的交换，嵌入的三角结构可以通过移动粒子而被移动或变形。与这种位移相关的唯一约束是，颗粒的位移距离不应大于一个单元的大小。这是因为这是 Particles::ParticleHandler 能够跟踪离开当前单元的粒子现在所处的单元的极限。

一旦整个问题（斯托克斯问题和沉没边界施加）被集合起来，最终的鞍点问题由迭代求解器解决，应用于舒尔补数 $S$ （其构造例如在 step-22 中描述），我们使用LinearOperator类构建 $S$ 。
...
本教程程序使用了一些关于对流体内部的非匹配界面施加速度条件的技术。关于更多的背景材料，你可能想查一下以下参考资料。  @cite Freund1995  ,  @cite Angot1999  ,  @cite Glowinski1999  ,  @cite Boffi2008  ,  @cite Heltai2012  。<a name="CommProg"></a> <h1> The commented program</h1>


<a name="Includefiles"></a> <h3>Include files</h3> 其中大部分已在其他地方介绍过，我们只对新的进行评论。靠近顶部的开关允许在PETSc和Trilinos线性代数功能之间进行选择，与  step-40  和  step-50  中的开关类似。
...
这些是与  step-60  有关的唯一新的包含文件。在本教程中，实体和流体之间的非匹配耦合是通过一个中间数据结构来计算的，该结构可以跟踪实体的正交点在流体网格中的位置如何演变。这个数据结构需要跟踪描述实体域的每个单元上的正交点的位置、正交权重，如果实体域是同维度的，还需要跟踪每个点的法向量。




Deal.II通过ParticleHandler类在Particles命名空间中提供这些设施。ParticleHandler是一个允许你管理粒子集合的类（类型为 Particles::Particle), 的对象，代表具有一些附加属性（如ID）的点的集合，浮动在 parallel::distributed::Triangulation. 命名空间中的方法和类允许人们轻松实现Particle-In-Cell方法和分布式三角形的粒子追踪。
...
在生成网格时，我们允许从文件中读取网格，如果deal.II的构建支持OpenCASCADE，我们也允许读取CAD文件，并将其作为网格的流形描述符（见 step-54 中对OpenCASCADE命名空间中的各种流形描述符的详细描述) 
...
与我们在 step-60 中所做的类似，我们设置了一个持有我们问题的所有参数的类，并从ParameterAcceptor类派生出来，以简化参数文件的管理和创建。   
...
然而，由于这个类将作为 "const "引用传递给StokesImmersedProblem类，我们必须确保我们仍然可以在这里定义的Function类派生的对象中正确设置时间。为了做到这一点，我们声明 `StokesImmersedProblemParameters::rhs` 和 `StokesImmersedProblemParameters::angular_velocity` 成员都是 "可变 "的，并定义以下的小辅助方法，将其时间设置为正确的值。
...
该类的其余部分主要由描述模拟及其离散化细节的成员变量组成。下面的参数是关于输出的位置、空间和时间离散化（默认是 $Q_2\times Q_1$ Taylor-Hood离散化，该离散化对速度使用2度的多项式），以及在我们再次生成图形输出之前应该经过多少时间步骤。
...
我们在此说明另一种从参数文件中创建Triangulation的方法，该方法 GridGenerator::generate_from_name_and_arguments(), 接收GridGenerator命名空间中的函数名称，其参数为一个字符串，代表参数的元组。     


在 Patterns::Tools::Convert 类中详细解释了将参数从字符串解析成字符串的机制，该类用于将字符串转换为大多数基本STL类型（向量、映射、图元）和基本deal.II类型（点、张量、BoundingBox等）。     


一般来说，可以用等级1的统一元素表示的对象（即 std::vector<double>,  Point<dim>,  std::set<int>,  等）是用逗号分隔的。额外的等级采取分号，允许你将字符串解析为 `std::vector<std::vector<double>>`, 或例如 `std::vector<Point<dim>>`, 类型的对象，如`0.0, 0.1; 0.1, 0.2`。这个字符串可以被解释为两个Point对象的向量，或者一个双数向量的向量。     


当条目不统一时，比如在元组的情况下，我们用冒号来分隔各个条目。例如，像`5: 0.1, 0.2`这样的字符串可以用来解析一个类型为 `std::pair<int,  Point<2>>的对象或者一个 `std::tuple<int,   std::vector<double>>`.  的对象。      
...
我们允许用户选择两种最常见的细化策略，即 "固定数 "或 "固定分数"，这些策略参考了 GridRefinement::refine_and_coarsen_fixed_fraction() 和 GridRefinement::refine_and_coarsen_fixed_number(). 的方法。      
...
一旦角速度被提供为一个函数对象，我们就通过下面这个派生自函数类的类来重构点状实体速度。它通过假设实体以给定的角速度绕原点（或三维中的 $z$ 轴）旋转，提供实体在给定位置的速度值。
...
下一节包含了该类的 "私有 "成员。第一个方法类似于前面例子中的方法。然而，它不仅负责生成流体的网格，而且还负责生成固体的网格。第二个方法是计算最大的时间步长，保证每个粒子最多移动一个单元。这对于确保 Particles::ParticleHandler 能够找到粒子最终所在的单元是很重要的，因为它只能从一个单元看向它的近邻（因为在并行设置中，每个MPI进程只知道它拥有的单元以及它们的近邻）。
...
接下来的两个函数初始化该类中使用的 Particles::ParticleHandler 对象。我们有两个这样的对象。一个代表被动追踪器，用于绘制流体粒子的轨迹，而另一个代表固体的材料粒子，它们被放置在固体网格的正交点上。
...
其余的函数求解线性系统（看起来与 step-60 中的线性系统几乎相同），然后对解进行后处理。refine_and_transfer()方法仅在每一个 "refinement_frequency "步骤中被调用，以适应网格，并确保所有在细化前的时间步骤中计算的场都正确地转移到新的网格中。这包括矢量场，以及粒子信息。同样地，我们只在每一个`输出_频率`的步骤中调用两个输出方法。
...
接下来是与  step-60  有关的主要创新之一。这里我们假设固体和流体都是完全分布的三角形。这使得问题可以扩展到非常大的自由度，代价是要沟通所有非匹配三角形之间的重叠区域。这一点特别棘手，因为我们没有对两个三角形的各个子域的相对位置或分布做出假设。特别是，我们假设每个进程只拥有 "solid_tria "的一部分，以及 "fluid_tria "的一部分，不一定在同一个物理区域，也不一定重叠。     
...
这个信息被 Particles::ParticleHandler 类用来交换（使用某对某的通信模式）所有的粒子，因此每个进程都知道生活在它所拥有的流体子域所占区域上的粒子。     
...
与 step-22 中的做法类似，我们使用一个块系统来处理问题的斯托克斯部分，并且非常密切地遵循那里的做法。
...
让我们转到这个程序的粒子方面。有两个 Particles::ParticleHandler 对象用于耦合固体和流体，以及描述被动追踪器。在许多方面，这些对象的作用类似于离散化中使用的DoFHandler类，也就是说，它们提供了一个粒子的枚举，并允许查询每个粒子的信息。
...
对于每个示踪粒子，我们需要计算其当前位置的速度场，并使用离散时间步进方案更新其位置。我们使用分布式线性代数对象来做这件事，这些对象存储了每个粒子的位置或速度的坐标。也就是说，这些向量有`tracer_particle_handler.n_global_particles() * spacedim`项，我们将以一种方式来存储这些向量的一部分，以便在所有进程中进行划分。(隐含地，我们在这里假设每个粒子的`spacedim'坐标被存储在向量的连续条目中)。因此，我们需要确定每个向量条目的所有者是谁。我们将这个所有者设定为等于在时间 $t=0$ 产生该粒子的进程。这个信息被储存在每个进程的`locally_owned_tracer_particle_coordinates`索引集中。     
...
本教程程序的关键点之一是两个独立的 parallel::distributed::Triangulation 物体之间的耦合，其中一个物体可能相对于另一个物体运动和变形（可能有较大的变形）。当流体和实体的三角形都是 parallel::distributed::Triangulation, 类型时，每个进程只能访问这两个三角形中每个单元的局部拥有的部分。如上所述，一般情况下，本地拥有的域是不重叠的。     


为了允许不重叠的 parallel::distributed::Triangulation 对象之间有效地交换信息，该库的一些算法要求用户以每个进程的轴对齐边界盒集合的形式，提供三角形的本地拥有部分所占区域的粗略描述，这些边界盒提供了域的本地拥有部分的全覆盖。这种信息就可以用于这样的情况：人们需要向已知位置周围的单元格的所有者发送信息，而不知道这个所有者实际上是谁。但是，如果我们知道每个进程拥有的几何区域或体积的边界盒集合，那么我们就可以确定可能拥有该位置所在单元的所有进程的一个子集：即其边界盒包含该点的所有进程。与其向所有进程发送与该位置相关的信息，不如只向具有点对点通信基元的一小部分进程发送信息。你会注意到，这也允许典型的时间与内存的权衡：我们愿意存储的关于每个进程拥有的区域的数据越多--以更精细的边界盒信息的形式--我们必须执行的通信就越少。     


我们通过收集一个向量（长度为 Utilities::MPI::n_mpi_processes()) 的BoundingBox对象的向量）来构建这些信息。我们使用extract_rtree_level()函数填充这个向量，并允许用户选择要提取的树的哪一级。这个 "级别 "对应的是与边界框重叠的区域应该有多粗/多细。     
...
如果我们走到这一步，那么三角图已经读完了，我们就可以给它附加正确的流形描述了。只有在deal.II支持OpenCASCADE的情况下，我们才会执行接下来的几行代码。对于地图中的每个条目，我们尝试打开相应的CAD文件，分析它，并根据其内容，选择一个 OpenCASCADE::ArcLengthProjectionLineManifold （如果CAD文件包含一个`TopoDS_Edge'或一个`TopoDS_Wire'）或一个 OpenCASCADE::NURBSPatchManifold, ，如果文件包含一个面。请注意，如果CAD文件不包含单一的线、边或面，在生成Manifold时将会抛出一个断言。     


我们使用 Patterns::Tools::Convert 类来为我们完成从字符串到流形ID和文件名之间的映射的转换。
...
现在我们检查 "形状 "中包含多少个面。OpenCASCADE本质上是三维的，所以如果这个数字是零，我们就把它解释为线型流形，否则就解释为 OpenCASCADE::NormalToMeshProjectionManifold 在`spacedim`=3时，或 OpenCASCADE::NURBSPatchManifold 在`spacedim`=2时。
...
我们使用这个技巧，因为 OpenCASCADE::NormalToMeshProjectionManifold 只在spacedim=3下实现。上面的检查确保了事情的实际运作是正确的。
...
一旦实体和流体网格被创建，我们开始填充 Particles::ParticleHandler 对象。我们要处理的第一个对象是用来跟踪流体中的被动追踪器的。这些东西只是沿途传送，从某种意义上说，它们的位置并不重要：我们只是想用它们来观察流体被传送的位置。我们可以使用任何我们选择的方式来确定它们的初始位置。一个方便的方法是将初始位置创建为我们所选择的形状的网格顶点，这个选择由参数文件中的一个运行时参数决定。   
...
在这个实现中，我们使用FE_Q有限元空间的支持点来创建追踪器，这些支持点定义在一个临时网格上，然后被丢弃。在这个网格中，我们只保留与支撑点相关的 Particles::Particle 对象（存储在 Particles::ParticleHandler 类中）。   
 ...
 Particles::ParticleHandler 类提供了插入一组粒子的可能性，这些粒子实际生活在活动过程所拥有的域的一部分。然而，在这种情况下，这个功能是不够的。作为任意网格（与流体网格不匹配）上的FE_Q对象的本地拥有的支持点所产生的粒子没有理由位于流体网格的本地拥有的子域的同一物理区域内。事实上，这种情况几乎不会发生，特别是由于我们要跟踪粒子本身发生了什么。   
...
每个如此创建的粒子都有一个唯一的ID。在下面的算法中的某个时刻，我们将需要包含每个粒子的位置和速度信息的向量。这个向量的大小为`n_particles * spacedim`，我们必须以一种方式来存储这个向量的元素，以便每个并行进程 "拥有 "那些对应于它所拥有的粒子坐标的元素。换句话说，我们必须在所有进程中划分0和`n_particles * spacedim`之间的索引空间。我们可以通过查询`tracer_particle_handler`的本地相关粒子的ID来做到这一点，并构建需要的索引，将所有粒子的位置和速度存储在一个（平行分布的）矢量中，其中我们隐含地假设我们将每个位置或速度的坐标存储在`spacedim`连续的矢量元素中（这就是 IndexSet::tensor_priduct() 函数的作用）。
...
这是通过利用 Particles::Particle 类的 "属性 "概念实现的。我们可以（以一种有效的内存方式）在一个 Particles::ParticleHandler 对象内为每个 Particles::Particle 对象存储任意数量的`双`数。我们利用这种可能性来存储实体网格的正交点的JxW值。   
...
由于我们已经存储了正交点的位置，我们可以使用这些位置直接使用`solid_particle_handler`插入粒子，而不必通过 Particles::Generators 函数。
...
我们接下来构建分布式块状矩阵和向量，用于解决问题中出现的线性方程。这个函数改编自 step-55 ，我们参考这个步骤进行彻底解释。
...
我们组装系统矩阵、预处理矩阵和右手边。该代码改编自 step-55 ，基本上是 step-27 也有的，如果你知道斯托克斯方程是什么样子的，那就是相当标准的。
...
这个函数用FGMRES求解线性系统，有一个对角线块的预处理和一个对角线块的代数多网格（AMG）方法。该预处理程序对 $(0,0)$ （即速度-速度）块应用V循环，对 $(1,1)$ 块应用质量矩阵的CG（这是我们对舒尔补码的近似值：上面组装的压力质量矩阵）。
...
同样，我们通过 Particles::DataOut 对象将粒子（无论是来自实体还是追踪器）写成单一的压缩vtu文件。这个简单的对象不写附加在粒子上的 "属性 "信息，而只写它们的id--但是，无论如何，我们并不关心这些粒子位置的 "JxW "值，所以我们可能想要可视化的信息并没有丢失。
...
这个函数现在负责协调整个模拟过程。它与其他时间相关的教程程序非常相似--以 step-21 或 step-26 为例。在开始时，我们输出一些状态信息，同时将所有当前参数保存到输出目录中的一个文件中，以便于重现。
...
默认的问题是生成一个同维度的零叶轮，由一个旋转的矩形网格组成，在一个方向上旋转半个时间单位，在相反方向上旋转半个时间单位，恒定的角速度等于  $\approx 2\pi \frac{\text{rad}}{\text{time unit}}$  。因此，叶轮做了半个旋转，并返回到原来的位置。下面的动画显示了速度的大小，固体叶轮和示踪粒子的运动。
...
这部电影显示了灰色的旋转障碍物（实际上是用足够大的点绘制的固体粒子的叠加，使它们重叠），<a
href="https://en.wikipedia.org/wiki/Streamlines,_streaklines,_and_pathlines">streamlines
of the fluid flow</a>为浅色（包括在模拟过程中特定时间形成的角顶点），而示踪粒子为蓝色调。
...
这使得本教程程序中使用的技术与教程 step-60 中介绍的技术相结合，可以解决带有分布式拉格朗日乘数的流体结构相互作用问题，在 parallel::distributed::Triangulation 对象上。

上面的时间显示，目前的预处理策略对Nitsche惩罚的效果并不好，如果我们想瞄准更大的问题，应该想出一个更好的预处理方法。此外，应该实施检查点重启策略，以允许较长的模拟被中断和恢复，例如在 step-69 教程中就有这样做。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-70.cc" 。 
