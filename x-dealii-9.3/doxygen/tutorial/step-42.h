...
这个例子是 step-41 的扩展，考虑了三维接触问题，具有各向同性硬化的弹塑性材料行为。换句话说，它考虑的是，如果把一个刚性障碍物推入一个三维体（接触问题），其变形是由弹塑性材料法则（一种只能容纳一定最大应力的材料）支配的，随着变形的累积，该材料会硬化。为了说明我们打算做什么，在讨论太多细节之前，让我们只展示一张解决方案的图片（可变形体是一个立方体--实际上只显示了一半--，障碍物对应于一个汉字，下面会讨论）。

  <img src="https://www.dealii.org/images/steps/developer/step-42.CellConstitutionLi2.png" alt="">   


这个问题的描述意味着，与 step-41 相比，我们必须照顾到一个额外的非线性因素：材料行为。由于我们在这里考虑的是一个三维问题，我们还必须考虑到这样一个事实，即现在接触区域是在可变形体的边界，而不是在内部。最后，与 step-41 相比，在处理线性系统和不等式约束时，我们还必须处理悬挂节点的问题，因为我们希望使用自适应网格；在后一种情况下，我们将不得不处理优先考虑悬挂节点的约束还是不等式的约束更重要。

由于在三维空间中很容易达到几百万个自由度，即使使用自适应网格细化，我们决定使用Trilinos和p4est来并行运行我们的代码，在 step-40 的框架上进行并行化。其他并行化的指针可以在  step-32  中找到。
...
这里，这些方程中的第一个通过四阶顺应性张量 $A$ 定义了应变 $\varepsilon(\mathbf u)=\frac{1}{2}\left(\nabla \mathbf u
  + \nabla \mathbf u^T\right)$ 和应力 $\sigma$ 之间的关系； $\varepsilon^p$ 提供了应变的塑性成分以确保应力不超过屈服应力。我们将只考虑各向同性的材料，对于这些材料， $A$ 可以用Lam&eacute;模量 $\lambda$ 和 $\mu$ 表示，或者用体模量 $\kappa$ 和 $\mu$ 表示。第二个方程是力的平衡；我们在这里将不考虑任何体力，并假定  $\mathbf f=0$  。第三行的互补条件意味着如果 $\mathcal{F}(\sigma)< 0$ ，则 $\varepsilon^p=0$ ，但当且仅当 $\mathcal{F}(\sigma) = 0$ ， $\varepsilon^p$ 可能是一个非零张量，特别是在这种情况下， $\varepsilon^p$ 必须指向 $\partial
\mathcal{F}(\sigma)/\partial \sigma$ 方向。不等式 $\mathcal{F}(\sigma)\le 0$ 是一个事实的陈述，即塑性材料只能支持有限的应力；换句话说，如果外力会导致应力 $\sigma$ ，那么它们会产生塑性变形 $\mathcal{F}(\sigma)> 0$ 的反应。这种<i>yield function</i>的典型形式是 $\mathcal{F}(\sigma)=|\sigma^D|-\sigma_{\text{yield}}$ ，其中 $\tau^D
= \tau - \dfrac{1}{3}tr(\tau)I$ 是张量的偏离部分， $|\cdot|$ 表示Frobenius规范。
...
大多数材料--尤其是金属--都有这样的特性，即它们在变形后会出现一些硬化。换句话说， $\sigma_{\text{yield}}$ 随着变形而增加。在实践中，导致硬化的不是弹性变形，而是塑性成分。有不同的构成法则来描述这些材料行为。最简单的称为线性各向同性硬化，由流动函数描述  $\mathcal{F}(\sigma,\varepsilon^p) = \vert\sigma^D\vert - (\sigma_0 +
...
一般来说，处理不等式是相当笨拙的。在这里，我们必须处理两个问题：塑性和接触问题。正如本页顶部提到的论文中详细描述的那样，至少可以用一种方式重新表述塑性，使其看起来像一个非线性，然后我们可以用牛顿方法来处理。这在数学上略显棘手，因为非线性不只是一些平滑的函数，而是在应力达到屈服应力的地方有结点；然而，对于这种<i>semismooth</i>函数，可以证明牛顿方法仍然收敛。

在不涉及细节的情况下，我们也将摆脱作为独立变量的应力，而只用位移来工作  $\mathbf u$  。最终，这种重述的目的是我们希望最终得到一个对称的、正定的问题--比如一个线性化的弹性问题，其空间可变系数由塑性行为产生--需要在每个牛顿步骤中得到解决。我们希望如此，因为有高效和可扩展的方法来解决这样的线性系统，如用代数多重网格的CG预设。这与类似于混合拉普拉斯的鞍点问题（见 step-20 ）相反，如果我们继续使用包含位移和应力的混合公式，我们会得到鞍点问题，对于这个问题， step-20 已经提示了构建良好的求解器和预处理器是多么困难。
...
- 对接触条件运行一个主动集方法，与我们在 step-41 中的方式基本相同。
...
如前所述，我们将通过应用牛顿方法来处理算子 $P_\Pi$ 的非线性问题，尽管该算子在严格意义上是不可微的。然而，它满足了<i>slant</i>的可微条件，这就足以使牛顿方法发挥作用。由此产生的方法被称为<i>semi-smooth Newton method</i>，听起来令人印象深刻，但实际上只是一个牛顿方法应用于具有适当选择的 "导数 "的半光滑函数。

在当前情况下，我们将通过在每次迭代中求解 $i$ 以下方程（仍然是不等式，但已线性化）来运行我们的迭代。
...
这个张量是 $P_\Pi(C\cdot)$ 围绕 $\varepsilon^D(\mathbf u^{i-1})$ 的（形式）线性化。对于我们这里考虑的线性各向同性材料，投影仪的体积和剪切分量由@f{gather*}
...
请注意，这个问题对应于一个线性弹性接触问题，其中 $I_\Pi$ 扮演着弹性张量 $C=A^{-1}$ 的角色。事实上，如果材料在某一点上没有塑性，那么 $I_\Pi=C$  。然而，在材料具有塑性的地方， $I_\Pi$ 是一个空间变化的函数。在任何情况下，我们必须为牛顿迭代 $\tilde {\mathbf u}^{i}$ 解决的系统使我们更接近重写问题的目标，使我们能够使用众所周知的椭圆系统的求解器和预处理器。

作为对牛顿方法的最后说明，让我们提一下，正如牛顿方法常见的那样，我们需要通过控制步长来使其全球化。换句话说，虽然上面的系统求解的是 $\tilde {\mathbf u}^{i}$ ，但最终的迭代结果将是 

@f{align*}
  {\mathbf u}^{i} = {\mathbf u}^{i-1} + \alpha_i (\tilde {\mathbf u}^{i} - {\mathbf u}^{i-1})
@f} 

其中右侧括号中的差值扮演了传统牛顿方向的角色，即 $\delta {\mathbf u}^{i}$ 。我们将使用标准的直线搜索来确定 $\alpha^i$ 。


<a name="ActiveSetmethodstosolvethesaddlepointproblem"></a><h3>Active Set methods to solve the saddle point problem</h3>


这个要在每个牛顿步骤中解决的线性化问题基本上与 step-41 一样。唯一的区别在于接触区是在边界而不是在域内。但这没有进一步的后果，所以我们参考 step-41 的文献，唯一的提示是 $\mathcal{S}$ 这次包含了接触边界 $\Gamma_C$ 的所有顶点。和那里一样，我们需要做的是保持一个自由度子集的固定，导致额外的约束，可以写成一个鞍点问题。然而，正如论文中所讨论的，通过以适当的方式写这些约束，消除自由度之间的耦合，我们最终会得到一组节点，这些节点基本上只是附加了迪里希特值。


<a name="Overallalgorithm"></a><h3>Overall algorithm</h3> 


上述算法结合了阻尼半光滑牛顿法（我们用于非线性构成法）和半光滑牛顿法，用于接触。它的工作原理如下。  <ol>   <li>  初始化主动集和非主动集  $\mathcal{A}_i$  和  $\mathcal{F}_i$  ，使  $\mathcal{S} = \mathcal{A}_i \cup \mathcal{F}_i$  和  $\mathcal{A}_i \cap
 \mathcal{F}_i = \emptyset$  并设置  $i = 1$  。这里， $\mathcal{S}$ 是位于可能发生接触的域的表面的所有自由度的集合。 起始值 $\hat U^0 \dealcoloneq
 P_{\mathcal{A}_k}(0)$ 满足我们的障碍条件，也就是说，我们将初始零位移投射到可行位移集上。

   <li>  组建牛顿矩阵  $A_{pq} \dealcoloneq a'(
 U^{i-1};\varphi_p,\varphi_q)$  和右手边  $F(\hat U^{i-1})$  。 这些对应于线性化的牛顿步骤，暂时忽略了接触不等式。

   <li>  找到满足@f{align*}
 A\tilde U^i + B\Lambda^i & = F, &\\
 \left[B^T\tilde U^i\right]_p & = G_p & \forall p\in\mathcal{A}_i,\\
 \Lambda^i_p & = 0 & \forall p\in\mathcal{F}_i.
 @f}的原始-双重对 $(\tilde U^i,\Lambda^i)$  。

 如同 step-41 ，我们可以通过从第一个方程中消除 ${\cal A}_i$ 中的那些自由度来获得这个问题的解决方案，并得到一个线性系统 $\hat {\hat A}(U^{i-1}) \tilde U^i = \hat {\hat H}(U^{i-1})$ 。




   <li>  通过应用直线搜索和计算 $U^{i-1}$ 和 $\tilde U^i$ 的线性组合来减弱 $i>2$ 的牛顿迭代。这需要找到一个 $\alpha^i_l \dealcoloneq 2^{-l},(l=0,\ldots,10)$ ，使@f{gather*}U^i \dealcoloneq \alpha^i_l\bar U^i +
 (1-\alpha^i_l)U^{i-1}@f} 
 ...
 \f}与 ${\hat R}\left({\mathbf u}\right)=\left(P_{Pi}(C\varepsilon(u)),\varepsilon(\varphi^{i}_p\right)$ ，除了(i)我们设置 ${\hat R}\left({\mathbf u}\right)=0$ 的元素 $p\in\mathcal{A}_i$ ，和(ii)对应悬挂节点的元素，我们以通常的方式消除。
   ...
   <li>  用@f{gather*}\mathcal{A}_{i+1} \dealcoloneq \lbrace p\in\mathcal{S}:\Lambda^i_p +
 ...
 这里， $P_{\mathcal{A}}(U)$ 是 $\mathcal{A}$ 中的活动部件对间隙@f{gather*}P_{\mathcal{A}}(U)_p \dealcoloneq \begin{cases}
 U_p, & \textrm{if}\quad p\notin\mathcal{A}\\
...
在该算法的第3步中，矩阵 $B\in\mathbb{R}^{n\times m}$ ,  $n>m$ 描述了位移和拉格朗日乘数（接触力）的基数的耦合，在我们的情况下它不是二次的，因为 $\Lambda^k$ 只定义在 $\Gamma_C$ ，即可能发生接触的表面。如文中所示，我们可以选择 $B$ 是一个每行只有一个条目的矩阵，（另见H&uuml;eber, Wohlmuth: A primal-dual active set strategy for non-linear multibody contact problems, Comput. Method Appl. Mech. Engrg. 194, 2005, pp.3147-3166）。) 矢量 $G$ 由间隙 $g$ @f{gather*}G_p = \begin{cases}
g_{h,p}, & \text{if}\quad p\in\mathcal{S}\\
0, & \text{if}\quad p\notin\mathcal{S}.
\end{cases}@f}的合适近似值定义。
...
由于我们的程序是在三维空间中运行的，程序所进行的计算是很昂贵的。因此，使用自适应网格细化是在可接受的运行时间内的一个重要步骤。为了使我们的生活更轻松，我们简单地选择已经在deal.II中实现的KellyErrorEstimator。我们把包含位移 $u$ 的解向量交给它。正如我们将在结果中看到的，它产生了一个相当合理的接触区和塑性的自适应网格。


<a name="Implementation"></a><h3>Implementation</h3>


本教程基本上是 step-40 和 step-41 的混合体，但我们没有使用PETSc，而是让Trilinos库来处理线性代数的并行化问题（就像 step-32 中那样）。由于我们试图解决一个类似于 step-41 的问题，我们将使用同样的方法，但现在是并行的。

一个困难是如何处理来自Dirichlet条件的约束，悬挂的节点和由接触产生的不平等条件。为此，我们创建了三个AffineConstraints类型的对象，它们描述了各种约束条件，我们将在每次迭代中适当地组合。

与 step-41 相比，该程序有一些新的类。

  <ul>   <li>   <code>ConstitutiveLaw</code>  描述材料的塑性行为 

  <li>   <code>SphereObstacle</code>  描述了一个球体，它作为障碍物被推入可变形的弹塑性体。  是用这个类还是下一个类来描述障碍物，由输入参数文件决定。

  <li>   <code>ChineseObstacle</code> （和一个辅助类）是一个允许我们从文件中读入障碍物的类。在我们将在结果部分展示的例子中，这个文件将是 <code>'obstacle_file.dat'</code> ，并对应于显示力或力量的中文、日文或韩文符号的数据（见http://www.orientaloutpost.com/："这个词可用于激励--它也可以意味着力量/运动/推动/力。它可以是任何使你继续前进的内部或外部事物。这是用中文表达动机的最安全方式。如果你的听众是日本人，请看另一个关于动机的条目。这是日语和韩语中的一个词，但它的意思是 "动力 "或 "动能"（没有你可能正在寻找的动机的意思）"）。实质上，我们将假装有一个印章（即对应于平底障碍物的面具，没有中间高度的碎片），我们把它压在身体里。有关的符号看起来如下（也可参见本节顶部的图片，了解最终结果是怎样的）。

    <img src="https://www.dealii.org/images/steps/developer/step-42.character.png" alt="" width="25%">   </ul> 。 

除此以外，让我们只对以下方面进行评论。  <ul>   <li>  程序允许你通过参数文件从两个不同的粗略网格中选择。它们是立方体 $[0,1]^3$ 或半球体，其开放面朝向正方向 $z$ 。

  <li> 在这两种情况下，我们将假定惯例，即可能与障碍物接触的边界部分具有边界指标一。对于这两种网格，我们假定这是一个自由表面，即身体要么在那里接触，要么没有力作用于它。对于半球体，弯曲部分的边界指标为零，我们在那里施加零位移。对于盒子，我们沿底部施加零位移，但允许沿边的垂直位移（尽管没有水平位移）。  </ul> <a name="CommProg"></a><h1> The commented program</h1>


<a name="Includefiles"></a> <h3>Include files</h3> 这组包含文件在这个时候已经没有什么惊喜了。
...
最后，我们包括两个系统头文件，让我们为输出文件创建一个目录。第一个头文件提供了 <code>mkdir</code> 功能，第二个头文件让我们确定在 <code>mkdir</code> 失败时发生了什么。
...
这个类提供了一个构成法的接口，即应变  $\varepsilon(\mathbf u)$  和应力  $\sigma$  之间的关系。在这个例子中，我们使用的是具有线性、各向同性的硬化的弹塑性材料行为。这种材料的特点是杨氏模量  $E$  ，泊松比  $\nu$  ，初始屈服应力  $\sigma_0$  和各向同性硬化参数  $\gamma$  。 对于 $\gamma =
0$ ，我们得到完美的弹塑性行为。   


正如描述这个程序的论文中所解释的，第一个牛顿步骤是用一个完全弹性的材料模型来解决的，以避免同时处理两种非线性（塑性和接触）。为此，这个类有一个函数 <code>set_sigma_0()</code> ，我们以后用它来简单地设置 $\sigma_0$ 为一个非常大的值--基本上保证实际应力不会超过它，从而产生一个弹性材料。当我们准备使用塑性模型时，我们使用相同的函数，将 $\sigma_0$ 设置回其适当的值。 由于这种方法，我们需要将 <code>sigma_0</code> 作为这个类的唯一非静态成员变量。
...
\nu$ （弹性模量和泊松数），使用Lam&eacute;参数 $\lambda,mu$ 或其他几个常用的约定。在这里，构造器以 $E,\nu$ 的形式接受材料参数的描述，但由于这被证明不是出现在塑性投影仪方程中的系数，我们立即将它们转换为更合适的体模和剪模的集合 $\kappa,\mu$ 。 此外，构造器将 $\sigma_0$ （无任何塑性应变的屈服应力）和 $\gamma$ （硬化参数）作为参数。在这个构造器中，我们还计算了应力-应变关系的两个主成分及其线性化。
...
这是构成法的主成分。它计算四阶对称张量，根据上面给出的投影，当在一个特定的应变点评估时，将应变与应力联系起来。我们需要这个函数来计算 <code>PlasticityContactProblem::residual_nl_system()</code> 中的非线性残差，我们将这个张量与正交点给出的应变相乘。计算遵循引言中列出的公式。在比较那里的公式和下面的实现时，记得 $C_\mu : \varepsilon = \tau_D$ 和 $C_\kappa : \varepsilon = \kappa
...
该函数返回线性化的应力应变张量，围绕前一个牛顿步骤 $u^{i-1}$ 的解线性化  $i-1$  。 参数 <code>strain_tensor</code> （通常表示为 $\varepsilon(u^{i-1})$ ）必须作为参数传递，并作为线性化点。该函数在变量stress_strain_tensor中返回非线性构成法的导数，在stress_strain_tensor_linearized中返回线性化问题的应力-应变张量。 见 PlasticityContactProblem::assemble_nl_system ，其中使用了这个函数。
...
下面这个类是可以从输入文件中选择的两个障碍中的第一个。它描述了一个以位置 $x=y=0.5, z=z_{\text{surface}}+0.59$ 和半径 $r=0.6$ 为中心的球体，其中 $z_{\text{surface}}$ 是可变形体的（平）表面的垂直位置。该函数的 <code>value</code> 返回给定 $x,y$ 值的障碍物位置，如果该点实际位于球体下方，则返回一个不可能干扰变形的大正值，如果它位于球体的 "阴影 "之外。
...
下面两个类描述了介绍中概述的障碍，即汉字。两个中的第一个， <code>BitmapFile</code> 负责从以pbm ascii格式存储的图片文件中读入数据。这个数据将被双线性插值，从而提供一个描述障碍物的函数。(下面的代码显示了如何通过在给定的数据点之间进行内插来构建一个函数。人们可以使用在本教程程序写完后引入的 Functions::InterpolatedUniformGridData, ，它正是我们在这里想要的，但看看如何用手来做是有启发的）。)      


我们从文件中读取的数据将被存储在一个名为 obstacle_data 的双倍数  std::vector  中。 这个向量构成了计算单片双线性函数的基础，作为一个多项式插值。我们将从一个文件中读取的数据由零（白色）和一（黑色）组成。     


 <code>hx,hy</code> 变量表示 $x$ 和 $y$ 方向的像素间的间距。  <code>nx,ny</code> 是这些方向上每个像素的数量。   <code>get_value()</code> 返回图像在给定位置的值，由相邻的像素值插值而成。
...
以下两个函数返回坐标为 $i,j$ 的给定像素的值，我们将其与定义在位置 <code>i*hx, j*hy</code> 的函数值相识别，并在任意坐标 $x,y$ 处对两个函数中第一个函数返回的点值做双线性插值。在第二个函数中，对于每个 $x,y$ ，我们首先计算离 $x,y$ 左下方最近的像素坐标的（整数）位置，然后计算这个像素内的坐标 $\xi,\eta$ 。我们从下方和上方截断这两种变量，以避免在评估函数时超出其定义的范围而可能发生的舍入误差问题。
...
这是本程序的主类，提供描述非线性接触问题所需的所有函数和变量。它接近于 step-41 ，但有一些额外的功能，如处理悬挂节点，牛顿方法，使用Trilinos和p4est进行并行分布式计算。处理悬空节点使生活变得有点复杂，因为我们现在需要另一个AffineConstraints对象。我们为接触情况下的主动集合方法创建一个牛顿方法，并处理构成法的非线性算子。   
...
就成员变量而言，我们先用这些变量来表示这个程序运行的MPI宇宙，用一个流来让一个处理器产生输出到控制台（见 step-17 ），还有一个变量用来为程序的各个部分计时。
...
下一组描述了网格和有限元空间。特别是，对于这个并行程序，有限元空间有与之相关的变量，表明哪些自由度存在于当前的处理器上（索引集，也见 step-40 和 @ref distributed 文档模块），以及各种约束：那些由悬挂节点、由Dirichlet边界条件和由接触节点的活动集施加的约束。在这里定义的三个AffineConstraints变量中，第一个只包含悬挂节点的约束，第二个也包含与Dirichlet边界条件相关的约束，第三个则是这些约束加上接触约束。     


变量 <code>active_set</code> 包括那些由接触约束的自由度，我们用 <code>fraction_of_plastic_q_points_per_cell</code> 来跟踪每个单元上应力等于屈服应力的正交点的分数。后者仅用于创建显示塑性区的图形输出，而不是用于任何进一步的计算；该变量是该类的成员变量，因为该信息是作为计算残差的副产品计算的，但仅在很久之后才使用。(注意，该向量是一个长度等于<i>local mesh</i>上活动单元数量的向量；它从未被用来在处理器之间交换信息，因此可以是一个普通的deal.II向量)。
...
下一个变量块对应的是解决方案和我们需要形成的线性系统。特别是，这包括牛顿矩阵和右手边；对应于残差（即牛顿右手边）但我们没有消除各种约束的向量，该向量用于确定在下一次迭代中需要约束哪些自由度；以及对应于介绍中简要提及并在随文讨论的 $B$ 矩阵对角线的向量。
...
下一个区块涉及到构建起始网格。我们将使用下面的辅助函数和 <code>make_grid()</code> 的第一块来构造一个对应于半球形的网格。deal.II有一个函数可以创建这样的网格，但是它的位置和方向都是错误的，所以我们需要在使用它之前对它进行一些移动和旋转。   


供以后参考，如 GridGenerator::half_hyper_ball(), 的文件中所述，半球体的平坦表面的边界指标为0，而其余部分的边界指标为1。
...
或者，创建一个超立方体网格。创建后，按如下方式分配边界指标。  <div class=CodeFragmentInTutorialComment>


[1.x.130]


</div>  换句话说，立方体的边的边界指标是8。底部的边界指标是6，顶部的指标是1。我们通过循环所有面的所有单元并查看单元中心的坐标值来设置这些指标，并在以后评估哪个边界将携带迪里希特边界条件或将受到潜在接触时利用这些指标。(在目前的情况下，网格只包含一个单元，它的所有面都在边界上，所以严格来说，所有单元的循环和查询一个面是否在边界上都是不必要的；我们保留它们只是出于习惯：这种代码可以在许多程序中找到，基本上都是这种形式。) 
...
在下文中，每一组操作都被放入一个大括号封闭的块中，该块被块顶部声明的变量计时（ TimerOutput::Scope 变量的构造器开始计时部分，在块的末端调用的析构器又停止计时）。
...
这个函数是从前面的函数中分解出来的，它计算与迪里希勒型边界条件相关的约束，并通过与来自悬挂节点的约束合并，将它们放入 <code>constraints_dirichlet_and_hanging_nodes</code> 变量中。   


正如介绍中所述，我们需要区分两种情况。

- 如果领域是一个盒子，我们将底部的位移设置为零，并允许沿侧面的Z方向的垂直运动。如 <code>make_grid()</code> 函数所示，前者对应于边界指标6，后者对应于8。
...
以下函数是我们在 <code>solve_newton()</code> 函数中每次牛顿迭代时调用的第一个函数。它的作用是将解决方案投射到可行集上，并更新接触或穿透障碍物的自由度的活动集。   
...
第二部分是在所有单元上的循环，在这个循环中，我们看每一个定义了自由度的点，是否活动集条件为真，我们需要把这个自由度添加到接触节点的活动集上。正如我们一直所做的，如果我们想在单个点上评估函数，我们用一个FEValues对象（或者，这里是FEFaceValues对象，因为我们需要检查表面的接触）和一个适当选择的正交对象来做。我们通过选择定义在单元格面上的形状函数的 "支持点 "来创建这个面的正交对象（关于支持点的更多信息，请参见这个 @ref GlossSupport "词汇表条目"）。因此，我们有多少个正交点就有多少个面的形状函数，在正交点上循环就相当于在面的形状函数上循环。有了这个，代码看起来如下。
...
在这个函数的最后，我们在处理器之间交换数据，更新 <code>solution</code> 变量中那些被其他处理器写入的幽灵元素。然后，我们将Dirichlet约束和那些来自悬挂节点的约束合并到已经包含活动集的AffineConstraints对象中。我们通过输出主动约束自由度的总数来结束这个函数，对于这个自由度，我们对每个处理器拥有的主动约束自由度的数量进行加总。这个本地拥有的受限自由度的数量当然是活动集和本地拥有的自由度集的交集的元素数量，我们可以通过在两个IndexSets上使用 <code>operator&</code> 得到。
...
考虑到问题的复杂性，可能会让人感到惊讶的是，在每次牛顿迭代中，我们必须解决的线性系统的组合实际上是相当简单的。下面的函数建立了牛顿的右手边和牛顿矩阵。它看起来相当简单，因为繁重的工作发生在对 <code>ConstitutiveLaw::get_linearized_stress_strain_tensors()</code> 的调用中，特别是在 AffineConstraints::distribute_local_to_global(), 中使用我们之前计算的约束。
...
在计算了应力-应变张量及其线性化之后，我们现在可以把矩阵和右手边的部分组合起来。在这两者中，我们需要线性化的应力-应变张量乘以 $\varphi_i$ 的对称梯度，即术语 $I_\Pi\varepsilon(\varphi_i)$ ，所以我们引入这个术语的缩写。回顾一下，该矩阵对应于随附出版物的符号中的双线性形式 $A_{ij}=(I_\Pi\varepsilon(\varphi_i),\varepsilon(\varphi_j))$ ，而右手边是 $F_i=([I_\Pi-P_\Pi
C]\varepsilon(\varphi_i),\varepsilon(\mathbf u))$ ，其中 $u$ 是当前的线性化点（通常是最后的解决方案）。这可能表明，如果材料是完全弹性的（其中 $I_\Pi=P_\Pi$ ），右手边将为零，但这忽略了一个事实，即右手边还将包含由于接触而产生的非均质约束的贡献。                   
...
这就是说，在略微滥用函数名称的情况下，它实际上做了更多的事情。例如，它还计算对应于牛顿残差的向量，但不消除受限自由度。我们需要这个向量来计算接触力，并最终计算出下一个活动集。同样，通过跟踪我们在每个单元上遇到的显示塑性屈服的正交点的数量，我们也可以计算出 <code>fraction_of_plastic_q_points_per_cell</code> 矢量，随后我们可以输出这个矢量来可视化塑性区。在这两种情况下，作为线条搜索的一部分，这些结果是不必要的，因此我们可能会浪费少量的时间来计算它们。同时，无论如何，这些信息是作为我们在这里需要做的事情的自然副产品出现的，而且我们想在每个牛顿步骤结束时收集一次，所以我们不妨在这里做。   
...
- 对于悬空节点，我们必须将 AffineConstraints::set_zero 函数应用于newton_rhs。如果求解值为 $x_0$ 的悬空节点有一个与障碍物接触的邻居 $x_1$ 和一个没有接触的邻居 $x_2$ ，这是必须的。因为前者的更新将是规定的，所以悬挂的节点约束将有一个不均匀性，看起来像  $x_0 = x_1/2 +
\text{gap}/2$  。所以右侧的相应条目是无意义的非零值。这些值我们必须设置为零。

- 和 step-40 一样，在求解或使用解决方案时，我们需要在有和没有鬼魂元素的向量之间进行洗牌。   
...
除此以外，这个函数的顶部部分应该是相当明显的。我们将变量 <code>previous_residual_norm</code> 初始化为可以用双精度数字表示的最负值，这样，比较当前残差是否小于上一步的残差，在第一步中总是失败。
...
在我们计算了当前牛顿步骤的试解 $\tilde{\mathbf u}$ 之后，情况就变得有点棘手了。我们处理的是一个高度非线性的问题，所以我们必须用直线搜索的方式来抑制牛顿方法。为了理解我们是如何做到这一点的，请回忆一下，在我们的公式中，我们在每一个牛顿步骤中计算一个试解，而不是新旧解之间的更新。由于解集是一个凸集，我们将使用直线搜索，尝试以前的解和试验解的线性组合，以保证阻尼解再次出现在我们的解集中。我们最多应用5个阻尼步骤。         
...
最后一步是检查收敛性。如果活动集在所有处理器中没有变化，并且残差小于阈值 $10^{-10}$  ，那么我们就终止在当前网格上的迭代。
...
<a name="PlasticityContactProblemrefine_grid"></a> <h4>PlasticityContactProblem::refine_grid</h4> 迭代结束。




如果你已经在deal.II教程中走到这一步，下面这个细化网格的函数应该不会再对你构成任何挑战。它对网格进行细化，可以是全局的，也可以是使用Kelly误差估计器的，如果这样要求的话，还可以将上一个网格的解转移到下一个网格。在后一种情况下，我们还需要再次计算活动集和其他数量，为此我们需要由  <code>compute_nonlinear_residual()</code>  计算的信息。
...
在我们到达 <code>run()</code> 之前的其余三个函数都与生成输出有关。下面一个是尝试显示变形体的变形构造。为此，这个函数接收一个位移矢量场，通过先前计算的位移来移动网格（局部）的每个顶点。在生成图形输出之前，我们将以当前的位移场调用这个函数，在生成图形输出之后，我们将以负的位移场再次调用这个函数来撤销对网格的改变。   
...
在函数的其余部分，我们在每个处理器上生成一个VTU文件，以该处理器的子域ID为索引。在第一个处理器上，我们随后也创建了一个 <code>.pvtu</code> 文件，对VTU文件的<i>all</i>进行索引，这样就可以一次性读取整个输出文件集。这些 <code>.pvtu</code> 被Paraview用来描述整个并行计算的输出文件。然后我们再为Paraview的竞争者--VisIt可视化程序做同样的事情，创建一个匹配的 <code>.visit</code> 文件。
...
和其他所有的教程程序一样， <code>run()</code> 函数包含整体逻辑。这里没有太多的内容：本质上，它在所有的网格细化循环中执行循环，并在每个循环中，将事情交给 <code>solve_newton()</code> 中的牛顿求解器，并调用函数来创建如此计算的解决方案的图形输出。然后输出一些关于运行时间和内存消耗的统计数据，这些数据是在这个网格的计算过程中收集的。
...
<a name="Thecodemaincodefunction"></a> <h3>The <code>main</code> function</h3> 
 ...
 <code>main()</code> 函数真的没有什么内容。看起来他们总是这样。
...
包含这个程序的目录还包含一些输入参数文件，可以用来创建各种不同的模拟。例如，用 <code>p1_adaptive.prm</code> 参数文件（用球作为障碍物，用盒子作为领域）在16个核心上运行该程序，会产生这样的输出。
...
图片显示了自适应细化以及细胞在与球接触过程中被塑化的程度。请记住，我们考虑每个正交点的应力偏差部分的规范，以查看是否有弹性或塑性行为。蓝色意味着这个单元只包含弹性正交点，与所有正交点都被塑化的红色单元相反。在顶面的中间--网格最细的地方--非常仔细地看可以看到由障碍物引起的凹陷。这就是 <code>move_mesh()</code> 函数的结果。然而，由于我们在这里考虑的障碍物的压痕非常小，所以很难辨别这种效果；可以玩玩将网格的顶点位移为计算位移的倍数。
...
像往常一样，有多种可能性来扩展这个程序。从算法的角度来看，这个程序在写的时候就已经走得很远了，使用了接触不等式、塑性非线性和线性求解器的最佳可用算法。然而，就更现实的情况而言，人们希望用这个程序做一些事情。  <ul>   <li>  将程序从静态扩展到准静态情况，也许可以通过选择后向-欧拉模式来进行时间离散化。一些理论结果可以在Jörg Frohne的博士论文中找到，<i>FEM-Simulation
der Umformtechnik metallischer Oberfl&auml;chen im Mikrokosmos</i>，德国锡根大学，2011。

  <li>  考虑有摩擦的接触问题也是一个有趣的进展。在几乎每个机械过程中，摩擦都有很大的影响。 为了模拟这种情况，我们必须考虑到接触面的切向应力。摩擦也给我们的问题增加了另一个不等式，因为只要切向应力不超过某个极限，身体和障碍物通常会粘在一起，超过这个极限，两个身体就会互相滑过。

  <li> 如果我们已经模拟了摩擦性接触，下一步要考虑的是接触区的发热。由两个物体之间的摩擦引起的热量提高了可变形物体的温度，并引起一些材料参数的变化。

  <li>  对于接触和塑性，实施更精确的、与问题相适应的误差估计器可能是有意义的。  </ul>  <a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-42.cc" 。 
