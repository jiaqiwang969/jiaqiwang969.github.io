...
undertaken. This work was supported by EPSRC grant no EP/K032208/1.
  ...
  @note  作为本程序的先决条件，你需要安装PETSc或Trilinos和p4est库。在<a href="../../readme.html"
target="body">README</a>文件中描述了deal.II和这些附加库的安装情况。

<a name="Intro"></a> <a name="Introduction"></a><h1>Introduction</h1>


在 step-40 的基础上，本教程展示了如何使用MPI与PETSc或Trilinos进行线性代数，并行解决具有多个组件的线性PDEs。为此，我们回到了  step-22  中讨论的斯托克斯方程。编写本教程的动机是在 step-40 （并行拉普拉斯）和 step-32 （针对时间相关问题的并行耦合斯托克斯与布西尼斯克）之间提供一个中间步骤（双关）。
...
我们正在求解满足斯托克斯方程的速度 $\textbf{u}$ 和压力 $p$ ，即@f{eqnarray*}
...
确保你阅读（甚至更好：尝试） step-22 中 "可能的扩展 "部分的 "Block Schur complement preconditioner "中所描述的内容。就像那里描述的那样，我们将使用Krylov方法和块状预处理器来解决块状系统。

我们的目标是为线性系统构造一个非常简单的（也许是最简单的）最优预处理。如果预处理系统的迭代次数与网格大小无关，则该预处理程序被称为 "最优 "或 "最佳复杂性" $h$  。你可以将这一定义扩展到要求与所使用的处理器数量（我们将在结果部分讨论）、计算域和网格质量、测试案例本身、有限元空间的多项式程度等无关。

为什么恒定的迭代次数被认为是 "最佳 "的？假设离散化的PDE给出了一个有N个未知数的线性系统。因为来自有限元离散化的矩阵是稀疏的，矩阵-向量乘积可以在O(N)时间内完成。先决条件的应用充其量也只能是O(N)（例如可以用多网格方法来做）。如果解决线性系统所需的迭代次数与 $h$ 无关（因此也与N无关），那么解决该系统的总成本将是O(N)。不可能战胜这个复杂度，因为即使是查看右手边的所有条目也已经需要O(N)的时间。更多信息见 @cite elman2005 ，第2.5章（多网格）。

这里描述的预处理程序比 step-22 中描述的更简单，通常需要更多的迭代，因此需要更多的时间来解决。在考虑预处理程序时，最优性并不是唯一重要的衡量标准。但是一个最优的、昂贵的预处理程序通常比一个便宜的、非最优的预处理程序更可取。这是因为，最终，随着网格尺寸越来越小，线性问题越来越大，前者将最终击败后者。
...
有了这个 $P$ 的选择，假设我们准确地处理了 $A^{-1}$ 和 $S^{-1}$ （这是一个 "理想化 "的情况），预处理的线性系统有三个独立于 $h$ 的不同特征值，因此是 "最优 "的。 见 @cite elman2005 中6.2.1节（特别是第292页）。作为比较，使用 step-22 中理想版本的上块三角预处理(也用于 step-56 )将使所有特征值都等于1。

我们将使用 $P^{-1}$ 中（几乎）独立于 $h$ 的逆运算的近似值。在这种情况下，我们可以再次证明，特征值是独立于 $h$ 的。对于Krylov方法，我们选择MINRES，它对分析很有吸引力（迭代次数被证明与 $h$ 无关，见上述书中6.2.1章的其余部分），从计算的角度看很好（例如比GMRES更简单、更便宜），而且适用（矩阵和预处理器是对称的）。

对于近似，我们将使用质量矩阵在压力空间的CG求解来近似 $S^{-1}$ 的作用。请注意，质量矩阵在光谱上等同于 $S$ 。我们可以预期CG的迭代次数与 $h$ 无关，即使使用ILU这样简单的预处理程序。

对于速度块的逼近 $A$ ，我们将执行一个AMG V-循环。在实践中，这种选择并不完全独立于 $h$ ，这可以解释迭代次数的轻微增加。一个可能的解释是，最粗的层次将被精确解决，而最粗矩阵的层次数和大小是不可预测的。


<a name="Thetestcase"></a><h3>The testcase</h3> 


我们将根据经典的Kovasznay问题构建一个制造的解决方案，见  @cite kovasznay1948laminar  。这里有一个用x速度着色的解决方案的图像，包括速度的流线。

   <img src="https://www.dealii.org/images/steps/developer/step-55.solution.png" alt="">   

不过，我们在这里必须作弊，因为我们要解决的不是非线性的Navier-Stokes方程，而是没有对流项的线性Stokes系统。因此，为了重现完全相同的解，我们用科瓦兹内问题的解来制造解的方法。这将有效地把对流项移到右手边  $f$  。

右手边是用脚本 "reference.py "计算的，我们使用精确解来进行边界条件和误差计算。<a name="CommProg"></a> <h1> The commented program</h1>
...
下面这段代码与 step-40 相同，可以在PETSc和Trilinos之间切换。
...
这个类通过函数 InverseMatrix::vmult(). 暴露了应用给定矩阵的逆的动作，在内部，逆并不是明确形成的。相反，一个带有CG的线性求解器被执行。这个类扩展了 step-22 中的InverseMatrix类，增加了一个指定预处理程序的选项，并允许在vmult函数中使用不同的矢量类型。
...
主类与  step-40  非常相似，只是矩阵和向量现在是块状版本，而且我们为拥有的和相关的DoF存储一个  std::vector<IndexSet>  ，而不是一个IndexSet。我们正好有两个IndexSets，一个用于所有速度未知数，一个用于所有压力未知数。
...
Kovasnay流定义在域[-0.5, 1.5]^2上，我们通过向 GridGenerator::hyper_cube. 传递最小和最大值来创建该域。 
...
与 step-40 相比，块状矩阵和向量的构造是新的，与 step-22 等串行代码相比也是不同的，因为我们需要提供属于我们处理器的行的集合。
...
最后，我们以正确的尺寸构建块向量。带有两个 std::vector<IndexSet> 的函数调用将创建一个重影向量。
...
像在 step-56 中一样，我们减去平均压力，以允许对我们的参考解进行误差计算，其平均值为零。
...
你可以在对流项 $b
\cdot \nabla u$ 中规定精确的流动解，即 $b$  。如果你把右手边设置为零，这应该会得到与原问题相同的解。
...
使得探索自适应网格细化变得简单。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-55.cc" 。 
