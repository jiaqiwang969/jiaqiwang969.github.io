...
其中 $\Psi=\Psi(\mathbf x)$ 上的Dirichlet边界条件也可以用Neumann或Robin条件代替； $L$ 是一个算子，一般也包含微分算子。

在合适的条件下，上述方程有一组解 $\Psi_\ell,\varepsilon_\ell$  ,  $\ell\in {\cal I}$  ，其中 $\cal I$ 可以是一个有限或无限的集合（在后一种情况下，它可能是一个离散的或有时至少部分是一个连续的集合）。在这两种情况下，让我们注意到，我们要计算的不再只是一个单一的解，而是一组解（各种特征函数和相应的特征值）。从数字上寻找这类特征值问题的所有特征值（特征函数）的问题是一个艰巨的挑战。事实上，如果集合 $\cal I$ 是无限的，这个挑战当然是难以解决的。 然而，大多数时候，我们实际上只对这些值（函数）的一小部分感兴趣；幸运的是，我们在这个教程程序中使用的SLEPc库的接口允许我们选择特征谱的哪一部分和我们想要解决的多少个解决方案。

在这个程序中，我们使用的eigenspectrum求解器是由deal.II提供的类，这些类包裹着<a href="http://www.grycap.upv.es/slepc/" target="_top">SLEPc</a>库的线性代数实现；SLEPc本身建立在<a
href="http://www.mcs.anl.gov/petsc/" target="_top">PETSc</a>库的线性代数内容上。

<a name="Intro"></a> <a name="Introduction"></a><h1>Introduction</h1>


静态量子力学的基本方程是薛定谔方程，它模拟了粒子在外部势中的运动  $V(\mathbf x)$  。粒子由一个波函数 $\Psi(\mathbf x)$ 描述，该波函数满足（非维度化）形式的关系 
...
因此，这个粒子只能存在于一定数量的特征态中，这些特征态对应于作为该方程解的能量特征值 $\varepsilon_\ell$ 。量子力学的正统（哥本哈根）解释认为，如果一个粒子具有能量 $\varepsilon_\ell$ ，那么在位置 $\mathbf x$ 发现它的概率与 $|\Psi_\ell(\mathbf
x)|^2$ 成正比，其中 $\Psi_\ell$ 是对应于这个特征值的特征函数。

为了在数值上找到这个方程的解，即一组成对的特征值/特征函数，我们使用通常的有限元方法，将左边的方程与测试函数相乘，通过部分积分，在有限维空间中通过近似 $\Psi(\mathbf
x)\approx\Psi_h(\mathbf x)=\sum_{j}\phi_j(\mathbf x)\tilde\psi_j$ 寻找解，其中 $\tilde\psi$ 是膨胀系数的向量。然后我们立即得出以下方程，该方程将连续特征值问题离散化：@f[ \sum_j [(\nabla\phi_i,
...
在这个程序中，我们对波函数 $\Psi$ 使用了Dirichlet边界条件。从有限元代码的角度来看，这意味着只有内部的自由度是<i>freedom</i>的实数度：边界上的自由度不是自由的，但毕竟是被迫有一个零值。另一方面，有限元方法从以下事实中获得了许多力量和简单性：我们只是在每个单元上做同样的事情，而不必过多地考虑一个单元在哪里，它是否与一个不太精细的单元相接，因而有一个悬挂的节点，或者与边界相邻。所有这些检查都会使有限元线性系统的装配难以编写，甚至更难阅读。

因此，当然，当你用DoFHandler对象分配自由度时，你并不关心你列举的一些自由度是否在迪里切特边界。他们都会得到数字。我们只需要在以后应用边界值的时候照顾到这些自由度。有两种基本的方法（要么使用 MatrixTools::apply_boundary_values() <i>after</i>装配线性系统，要么使用 AffineConstraints::distribute_local_to_global() <i>during</i>装配；详见 @ref constraints "约束模块"），但两者的结果是一样的：一个线性系统的总行数等于<i>all</i>自由度的数目，包括那些位于边界的自由度。然而，受迪里希特条件约束的自由度与线性系统的其他部分分开，方法是将相应的行和列清零，在对角线上放一个正条目，并在右手边放相应的迪里希特值。
...
这里，下标 $i$ 和 $b$ 分别对应于内部和边界自由度。内部自由度满足线性系统 $A_i U_i=F_i$ ，在内部产生正确的解，边界值由 $U_b = D_b^{-1} F_b$ 决定，其中 $D_b$ 是一个对角矩阵，由消除边界自由度的过程产生，而 $F_b$ 的选择方式是 $U_{b,j}=D_{b,jj}^{-1} F_{b,j}$  ]对每个边界自由度都有正确的边界值 $j$ 。 对于好奇的人来说，矩阵 $D_b$ 的条目是将修改后的局部贡献加入全局矩阵的结果，对于局部矩阵，如果非零，对角线元素被设置为其绝对值；否则，它们被设置为对角线绝对值的平均值。这个过程保证了 $D_b$ 的条目是正的，并且大小与对角线的其他条目相当，确保所产生的矩阵不会因为涉及到大小迥异的矩阵条目的舍入而产生不合理的精度损失。最终出现在对角线上的实际数值是很难预测的，你应该把它们当作任意的、不可预测的，但却是正的。) 

对于 "常规 "线性系统，这一切都导致了正确的解决方案。另一方面，对于特征值问题，这就不那么简单了。在那里，消除边界值会影响到我们在当前教程程序中要解决的两个矩阵 $A$ 和 $M$ 。消除边界值后，我们就会得到一个可以这样划分的特征值问题。
...
这种形式清楚地表明，有两组特征值：我们关心的那些，以及来自分离问题的虚假特征值@f[
  D_A \tilde \Psi_b = \epsilon_h D_M \Psi_b.
@f]这些特征值是虚假的，因为它们是由一个只在边界节点上操作的特征值系统产生的--这些节点不是<i>freedom</i>的实数度。当然，由于两个矩阵 $D_A,D_M$ 是对角线，我们可以准确地量化这些虚假的特征值：它们是 $\varepsilon_{h,j}=D_{A,jj}/D_{M,jj}$ （其中指数 $j$ 正好对应于受迪里切特边界值约束的自由度）。

那么，如何处理这些问题呢？第一部分是识别我们的特征值求解器何时发现其中的一个。为此，程序通过计算所有受限自由度上表达式 $\varepsilon_{h,j}=D_{A,jj}/D_{M,jj}$ 的最小和最大，计算并打印出这些特征值所在的区间。在下面的程序中，这已经足够了：我们发现这个区间位于我们感兴趣并计算的最小特征值和相应的特征函数的集合之外，所以这里我们不需要做什么。

另一方面，我们可能会发现我们在这个程序中计算的一个特征值恰好在这个区间内，在这种情况下，我们不会立即知道它是一个虚假的还是一个真正的特征值。在这种情况下，我们可以在计算完两个矩阵后简单地缩放其中一个矩阵的对角线元素，从而将它们从特征谱的感兴趣的频率上移开。这可以通过使用以下代码来完成，确保所有假的特征值正好等于 $1.234\cdot 10^5$  。
...
下面的程序本质上只是 step-4 的一个稍加修改的版本。不同的地方有以下几点。

  <ul>   

  <li>  主类（名为 <code>EigenvalueProblem</code> ）现在不再有单一的解向量，而是有一整套我们要计算的各种特征函数的向量。此外， <code>main</code> 函数对这里的一切都有顶层控制，它通过 <code>SlepcInitialize</code> 和 <code>SlepFinalize</code> 同时初始化和最终确定与SLEPc和PETSc的接口。 </li>   

  <li>  我们使用PETSc的矩阵和向量，如 step-17 和 step-18 ，因为这是SLEPc特征值求解器的要求。 </li>   

  <li>  函数 <code>EigenvalueProblem::solve</code> 与本教程中迄今所见的任何函数都完全不同，因为它不只是求解一个线性系统，而是实际求解特征值问题。它建立在SLEPc库上，更直接的是建立在类 SLEPcWrappers::SolverKrylovSchur.</li> 的deal.II SLEPc包装器上。 

  <li>  我们使用ParameterHandler类来描述一些输入参数，如势的确切形式 $V({\mathbf
x})$  ，网格的全局细化步数，或我们要解决的特征值数量。我们可以在这方面做得更多，但只限于在运行时选择一些实际的输入文件参数。为了看看在这方面可以做什么，看看  @ref step_29  "  step-29  " 和  step-33  。 </li>   

  <li>  我们使用FunctionParser类，使潜在的  $V(\mathbf
x)$  成为运行时参数，可以在输入文件中指定为一个公式。 </li>   

  </ul>   

程序的其余部分以一种相当直接的方式从 step-4 开始。<a name="CommProg"></a> <h1> The commented program</h1>
...
正如在介绍中提到的，这个程序本质上只是 step-4 的一个稍作修改的版本。因此，以下大部分的include文件都是在那里使用的，或者至少是在以前的教程程序中已经使用过的。
...
下面是主类模板的类声明。它看起来和在  step-4  中已经展示过的差不多。
...
最后，我们将有一个对象，包含对我们自由度的 "约束"。如果我们有自适应细化的网格（目前的程序中没有），这可能包括悬挂节点约束。在这里，我们将存储边界节点的约束  $U_i=0$  。
...
下一个函数在域 $[-1,1]^d$ 上创建一个网格，根据输入文件的要求对其进行多次细化，然后将DoFHandler附加到其上，并将矩阵和向量初始化为正确的大小。我们还建立了对应于边界值的约束条件  $u|_{\partial\Omega}=0$  。   


对于矩阵，我们使用PETSc包装器。这些包装器能够在非零项被添加时根据需要分配内存。这似乎是低效的：我们可以先计算稀疏模式，用它来初始化矩阵，然后在我们插入条目时，我们可以确定我们不需要重新分配内存和释放之前使用的内存。一种方法是使用这样的代码。  <div class=CodeFragmentInTutorialComment>


[1.x.70]


</div> 代替下面两个 <code>reinit()</code> 对刚度和质量矩阵的调用。   
...
x)\varphi_j(\mathbf x)$ 和 $M^K_{ij} = \int_K \varphi_i(\mathbf
x)\varphi_j(\mathbf x)$ 组装全局刚度和质量矩阵。如果你看过以前的教程程序，这个函数应该会很熟悉。唯一新的东西是使用我们从输入文件得到的表达式设置一个描述势 $V(\mathbf x)$ 的对象。然后我们需要在每个单元的正交点上评估这个对象。如果你见过如何评估函数对象（例如，见 step-5 中的系数），这里的代码也会显得相当熟悉。
...
上述调用的输出是一组向量和数值。在特征值问题中，特征函数只确定到一个可以任意固定的常数。由于对特征值问题的原点一无所知，SLEPc除了将特征向量归一到 $l_2$ （向量）准则外，没有其他选择。不幸的是，这个规范与我们从特征函数角度可能感兴趣的任何规范没有什么关系： $L_2(\Omega)$ 规范，或者也许是 $L_\infty(\Omega)$ 规范。     


让我们选择后者，并重新调整特征函数的尺度，使其具有 $\|\phi_i(\mathbf x)\|_{L^\infty(\Omega)}=1$ 而不是 $\|\Phi\|_{l_2}=1$ （其中 $\phi_i$ 是 $i$ 第三个特征<i>function</i>， $\Phi_i$ 是相应的结点值矢量）。对于这里选择的 $Q_1$ 元素，我们知道函数 $\phi_i(\mathbf x)$ 的最大值是在其中一个节点上达到的，所以 $\max_{\mathbf
x}\phi_i(\mathbf x)=\max_j (\Phi_i)_j$ ，使得在 $L_\infty$ 准则下的归一化是很简单的。请注意，如果我们选择 $Q_k$ 元素与 $k>1$ ，这就不那么容易了：在那里，一个函数的最大值不一定要在一个节点上达到，所以 $\max_{\mathbf x}\phi_i(\mathbf x)\ge\max_j (\Phi_i)_j$ （尽管平等通常几乎是真的）。
...
唯一值得讨论的是，由于潜能在输入文件中被指定为一个函数表达式，如果能将其与特征函数一起以图形形式表示出来就更好了。实现这一目标的过程相对简单：我们建立一个代表 $V(\mathbf x)$ 的对象，然后将这个连续函数插值到有限元空间。其结果我们也附加到DataOut对象上，以便进行可视化。
...
该问题的输入由一个输入文件 <code>\step-36.prm</code> 作为参数，例如，该文件可以包含以下文本。
...
这里，域内电势为零，我们知道特征值由  $\lambda_{(mn)}=\frac{\pi^2}{4}(m^2+n^2)$  给出，其中  $m,n\in{\mathbb N^+}$  。特征函数是正弦和余弦，在 $m$ 和 $n$ 方向有 $x$ 和 $y$ 个周期。这与我们的程序产生的输出相匹配。
...
============================ Running \step-36
   ...
   Job done.  @endcode 这些特征值正是对应于 $(m,n)=(1,1)$ 、 $(1,2)$ 和 $(2,1)$ 、 $(2,2)$ 和 $(3,1)$ 等对。相应的特征函数的可视化将看起来像这样。
...
在操场上玩一些游戏总是值得的! 所以这里有几个建议。
  ...
  <li>  上面使用的势（称为<i>infinite well</i>，因为它是一个由无限高的墙包围的平坦势）很有趣，因为它允许有分析上已知的解决方案。然而，除此之外，它是相当无聊的。也就是说，通过在输入文件中设置不同的势来玩弄这个势是很容易的。例如，让我们假设我们想在2d中使用以下势。@f[
  V(x,y) = \left\{
       \begin{array}{ll}
...
@f] 换句话说，在半径为0.75的圆的两个扇面中，电势是-100，在另外两个扇面中是-5，而在圆外是零。我们可以通过在输入文件中使用以下内容来实现这一点。
...
输出文件还包含一个内插的势的版本，看起来像这样（注意，正如预期的那样，最低的几个特征模式的概率密度 $|\Psi(\mathbf x)|^2$ 只有在势最低的地方，即在势的内圈的右上角和左下角的部分才是重要的）。
...
<td></td>
  ...
  <li>  在我们对问题的推导中，我们假设粒子被限制在一个域中 $\Omega$ ，并且在这个域的边界，其存在的概率 $|\Psi|^2$ 为零。这相当于解决所有 ${\mathbb R}^d$ 上的特征值问题，并假设能量势只在 $\Omega$ 区域内是有限的，而在区域外是无限的。相对来说，可以证明 $|\Psi(\mathbf x)|^2$ 在 $\mathbf x$ 的所有位置上， $V(\mathbf
x)=\infty$  。那么问题来了，如果我们的势不是这种形式，也就是说，没有一个势是无限的有界域，会发生什么？在这种情况下，可能值得只考虑一个非常大的边界域，其中 $V(\mathbf x)$ 即使不是无限的，也至少是非常大的。像这样玩一玩，探索一下当我们使计算区域越来越大时，频谱和特征函数如何变化。

  <li>  如果我们研究简单的谐波振荡器问题 $V(\mathbf x)=c|\mathbf x|^2$ 会怎样？这个势正是上一段讨论的形式，具有超球面对称性。人们可能想用一个大的外半径的球面域，来近似于全空间问题（例如，通过调用 GridGenerator::hyper_ball). ）。 

  <li>  上面的图显示了波函数  $\Psi(\mathbf x)$  ，但感兴趣的物理量实际上是粒子在位置  $\mathbf x$  的概率密度  $|\Psi(\mathbf x)|^2$  。一些可视化程序可以从输入文件中的数据计算出派生量，但我们也可以在创建输出文件时立即这样做。这样做的工具是DataPostprocessor类，可以和DataOut类一起使用。如何做到这一点的例子可以在  step-29  和  step-33  中找到。

  <li>  如果盒子里的粒子有%的内部自由度会怎样？例如，如果该粒子是一个自旋-  $1/2$  粒子？在这种情况下，我们可能要开始解决一个矢量值的问题，而不是。

  <li>  我们这里的deal.II库的实现使用了PETScWrappers和SLEPcWrappers，适合在串行机器结构上运行。然而，对于更大的网格和更多的自由度，我们可能希望在并行架构上运行我们的应用程序。上述代码的并行实现在这里可能特别有用，因为广义的特征谱问题比前面大多数教程中考虑的标准问题的解决成本更高。幸运的是，将上述程序修改为与MPI兼容是一个相对简单的过程。关于如何做到这一点的简述可以在  @ref
step_17  "  step-17  " 中找到。
...
并像往常一样在  <code>make_grid_and_dofs()</code>  中初始化它们。
...
注意我们是如何使用精确分解（使用SparseDirectUMFPACK）作为ARPACK的前置条件的。  </ul>  <a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-36.cc" 。 
