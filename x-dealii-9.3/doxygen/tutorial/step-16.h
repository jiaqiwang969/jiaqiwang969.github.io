...
这个例子显示了deal.II中多级函数的基本用法。它几乎解决了与 step-6 中使用的相同的问题，但展示了使用多网格作为预处理程序时必须提供的东西。特别是，这要求我们定义一个层次结构，提供从一个层次到下一个层次以及返回的转移算子，并提供每个层次上的拉普拉斯算子的表示。

为了使微分方程系统和块状预处理程序具有足够的灵活性，在开始使用多层次方法之前，必须创建一些不同的对象，尽管大部分需要做的事情是由deal.II本身提供。这些对象是 

  - 处理网格间转移的对象；我们使用MGTransferPrebuilt类来处理，它几乎完成了库中的所有工作。

  - 最粗层次的求解器；在这里，我们使用MGCoarseGridHouseholder。

  - 所有其他级别的平滑器，在我们的例子中，这将是使用SOR作为基础方法的 mg::SmootherRelaxation 类。

  - 和 mg::Matrix, 类，该类有一个特殊的水平乘法，即我们基本上在每个网格水平上存储一个矩阵，并允许与之相乘。

这些对象中的大多数只在实际解决线性系统的函数中需要。在这里，这些对象被组合到一个多网格类型的对象中，其中包含V型循环的实现，它又被预处理程序PreconditionMG使用，准备插入到LAC库的线性求解器中。

这里实现的自适应细化网格的多网格方法遵循 @ref mg_paper "多网格论文 "中的纲要，该论文描述了交易二中的基本实现，也介绍了很多术语。首先，我们必须区分层次网格，即与粗网格有相同细化距离的单元，以及由层次结构中的活动单元组成的叶子网格（在较早的工作中，我们将其称为全局网格，但这个术语被过度使用）。最重要的是，叶子网格与最细层次上的层次网格不完全相同。下面的图片显示了我们认为的 "水平网格"。

  <p align="center">
  @image html "multigrid.png" ""
</p>   

这个网格中的精细层次只包括定义在细化单元上的自由度，但不延伸到领域中未被细化的那部分。虽然这保证了整体工作的增长，因为 ${\cal O}(N)$ 是最佳多网格复杂度所必需的，但它导致了在定义平滑的地方和在个别层次上定义的算子的边界条件时的问题，如果层次边界不是外部边界。这些问题在上面引用的文章中进行了详细讨论。

<a name="Thetestcase"></a><h3>The testcase</h3> 


我们在这里解决的问题与 step-6 相似，主要有两个不同点：第一，多网格预处理，显然。我们还改变了系数的不连续性，使局部装配器看起来不会比必要的更复杂。<a name="CommProg"></a> <h1> The commented program</h1>
...
现在，这些是多级方法所需的include。第一个文件声明了如何处理多网格方法每个层次上的Dirichlet边界条件。对于自由度的实际描述，我们不需要任何新的包含文件，因为DoFHandler已经实现了所有必要的方法。我们只需要为更低的层次分配自由度。




其余的包含文件涉及到多网格作为线性算子（求解器或预处理器）的机制。
...
我们将使用 MeshWorker::mesh_loop 来循环计算单元，所以在这里包括它。
...
-  <code>assemble_multigrid</code> 函数，它组装了对应于中间层的离散算子的矩阵。
...
然后，我们有两个新的矩阵，只需要在自适应网格上进行局部平滑的多网格方法。它们在细化区域的内部和细化边缘之间传递数据，正如在 @ref mg_paper "多网格论文 "中详细描述的那样。     
...
关于三角结构的构造函数只有一个简短的评论：按照惯例，deal.II中的所有自适应精化三角结构在单元格之间的面的变化不会超过一个级别。然而，对于我们的多网格算法，我们需要一个更严格的保证，即网格在连接两个单元的顶点上的变化也不超过细化级别。换句话说，我们必须防止出现以下情况。   
...
现在是关于多网格数据结构的事情。首先，我们调整多级对象的大小，以容纳每一级的矩阵和稀疏模式。粗层次是零（现在是强制性的，但在未来的修订中可能会改变）。注意，这些函数在这里采取的是一个完整的、包容的范围（而不是一个起始索引和大小），所以最细的级别是 <code>n_levels-1</code>  。我们首先要调整容纳SparseMatrix类的容器的大小，因为它们必须在调整大小时释放它们的SparsityPattern才能被销毁。
...
现在，我们必须在每个层面上提供一个矩阵。为此，我们首先使用 MGTools::make_sparsity_pattern 函数在每个层次上生成一个初步的压缩稀疏模式（关于这个主题的更多信息，请参见 @ref Sparsity 模块），然后将其复制到我们真正想要的那个。下一步是用拟合的稀疏模式初始化接口矩阵。     


值得指出的是，界面矩阵只有位于较粗和较细的网格之间的自由度的条目。因此，它们甚至比我们的多网格层次中的各个层次的矩阵还要稀疏。因此，我们使用一个专门为此目的而建立的函数来生成它。
...
cell_worker函数被用来在给定的单元上组装矩阵和右手边。这个函数用于活动单元生成system_matrix，并在每个层次上建立层次矩阵。   
...
下面的函数在网格的活动单元上装配线性系统。为此，我们向Mesh_loop()函数传递两个lambda函数。cell_worker函数重定向到同名的类成员函数，而copyer是这个函数特有的，它使用约束条件将本地矩阵和向量复制到相应的全局矩阵。
...
下一个函数是建立矩阵，定义每一级网格上的多栅方法。集成的核心与上面的相同，但是下面的循环将遍历所有的现有单元，而不仅仅是活动的单元，并且必须将结果输入正确的层级矩阵。幸运的是，MeshWorker对我们隐藏了大部分的内容，因此这个函数和之前的函数的区别只在于汇编器的设置和循环中的不同迭代器。   
...
这是另一个在支持多网格求解器（或者说，事实上，我们使用多网格方法的预处理器）方面有明显不同的函数。   


让我们从建立多层次方法的两个组成部分开始：层次间的转移算子和最粗层次上的求解器。在有限元方法中，转移算子来自所涉及的有限元函数空间，通常可以用独立于所考虑问题的通用方式计算。在这种情况下，我们可以使用MGTransferPrebuilt类，给定最终线性系统的约束和MGConstrainedDoFs对象，该对象知道每个层次的边界条件和不同细化层次之间接口的自由度，可以从具有层次自由度的DoFHandler对象建立这些转移操作的矩阵。   


下面几行的第二部分是关于粗网格求解器的。由于我们的粗网格确实非常粗，我们决定采用直接求解器（最粗层次矩阵的Householder分解），即使其实现不是特别复杂。如果我们的粗网格比这里的5个单元多得多，这里显然需要更适合的东西。
...
多级求解器或预处理器的下一个组成部分是，我们需要在每一级上有一个平滑器。这方面常见的选择是使用松弛方法的应用（如SOR、Jacobi或Richardson方法）或求解器方法的少量迭代（如CG或GMRES）。 mg::SmootherRelaxation 和MGSmootherPrecondition类提供了对这两种平滑器的支持。这里，我们选择应用单一的SOR迭代。为此，我们定义一个适当的别名，然后设置一个平滑器对象。     


最后一步是用我们的水平矩阵初始化平滑器对象并设置一些平滑参数。 <code>initialize()</code> 函数可以有选择地接受额外的参数，这些参数将被传递给每一级的平滑器对象。在目前SOR平滑器的情况下，这可能包括一个松弛参数。然而，我们在这里将这些参数保留为默认值。对 <code>set_steps()</code> 的调用表明我们将在每个级别上使用两个前平滑步骤和两个后平滑步骤；为了在不同级别上使用可变数量的平滑器步骤，可以在对 <code>mg_smoother</code> 对象的构造器调用中设置更多选项。     


最后一步的结果是，我们使用SOR方法作为平滑器--它不是对称的--但我们在下面使用共轭梯度迭代（它需要对称的预处理），我们需要让多级预处理确保我们得到一个对称的算子，即使是非对称的平滑器。
...
下一个准备步骤是，我们必须将我们的水平矩阵和接口矩阵包裹在一个具有所需乘法函数的对象中。我们将为从粗到细的接口对象创建两个对象，反之亦然；多网格算法将在以后的操作中使用转置算子，允许我们用已经建立的矩阵初始化该算子的上下两个版本。
...
像上面的几个函数一样，这几乎完全是  step-6  中相应函数的复制。唯一的区别是对 <code>assemble_multigrid</code> 的调用，它负责形成我们在多网格方法中需要的每一级矩阵。
...
<pre> 周期0 活动单元数：80 自由度数：89 (按级别：8, 25, 89) CG迭代数。8 

周期1 活跃单元数：158 自由度数：183 (按级别：8, 25, 89, 138) CG迭代次数。9 
...
第五周期 活跃单元数: 2096 自由度数: 2317 (按级别: 8, 25, 89, 304, 779, 1214, 817) CG迭代次数: 11 

第六周期 活跃单元数: 3986 自由度数: 4366 (按级别: 8, 25, 89, 337, 836, 2270, 897, 1617) CG迭代次数: 10 

第7周期 活跃单元数：7574 自由度数：8350（按级别：8, 25, 89, 337, 1086, 2835, 2268, 1789, 3217） CG迭代次数。11 </pre> 
...
我们鼓励你生成solve()调用的时间，并与  step-6  进行比较。你会看到多网格方法在粗网格上有相当大的开销，但在细网格上，由于其最佳的复杂性，它总是胜过其他方法。

仔细检查这个程序的性能可以发现，它主要是由矩阵-向量操作主导的。  step-37 展示了一种通过使用无矩阵方法来避免这种情况的办法。

另一个途径是使用代数多栅方法。这里使用的几何多网格方法在实现上有时会有点尴尬，因为它需要所有这些额外的数据结构，如果程序要在%parallel的机器上通过MPI耦合运行，就会变得更加困难，比如说。在这种情况下，如果能使用一个黑盒预处理程序，就会更简单，该程序使用某种多栅层次结构以获得良好的性能，但可以自己计算出水平矩阵和类似的东西。代数多栅方法正是这样做的，我们将在 step-31 中使用它们来解决斯托克斯问题，在 step-32 和 step-40 中使用它们来解决平行变化。也就是说，这个例子程序的MPI并行版本可以在  step-50  中找到。

最后，人们可能想考虑如何将几何多网格用于其他类型的问题，特别是 @ref vector_valued "矢量值问题"。这是 step-56 的主题，我们将这里的技术用于斯托克斯方程。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-16.cc"  。 
