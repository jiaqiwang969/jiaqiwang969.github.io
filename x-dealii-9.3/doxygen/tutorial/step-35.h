...
本程序的目的是展示如何有效地解决不可压缩的时间依赖性纳维-斯托克斯方程。这些方程描述了粘性不可压缩流体的流动，内容如下 
...
@f]，其中 $n$ 是外部单位法线。 $\Gamma_2$ 上的边界条件经常被用来模拟外流条件。

在以前的教程中（例如见 step-20 和 step-22 ），我们已经看到了如何使用Schur补数方法来解决时间无关的斯托克斯方程。对于随时间变化的情况，在时间离散化之后，我们将得到一个系统，如 
...
其中 $\tau$ 是时间步长。尽管这个系统的结构与斯托克斯系统相似，因此它可以用Schur补充方法来解决，但事实证明，Schur补充的条件数与 $\tau^{-2}$ 成正比。这使得该系统非常难解，也意味着对于纳维-斯托克斯方程来说，这不是一个有用的求解途径。

<a name="Projection"></a> <a name="Projectionmethods"></a><h3> Projection methods </h3>


相反，我们需要想出一个不同的方法来解决时间依赖性的纳维-斯托克斯方程。其求解的困难来自于速度和压力是通过约束条件@f[
  \nabla \cdot u = 0,
@f]耦合的，而压力是Lagrange乘数。投影法的目的是将这个约束与扩散（拉普拉斯）算子解耦。

让我们简单描述一下投影方法在半离散情况下的样子。我们的目标是获得一个速度 $\{u^k\}$ 和压力 $\{p^k\}$ 的序列。我们还将得到一个辅助变量的序列 $\{\phi^k\}$ 。假设从初始条件和一阶方法的应用中我们已经找到了  $(u^0,p^0,\phi^0=0)$  和  $(u^1,p^1,\phi^1=p^1-p^0)$  。那么投影法包括以下步骤。  <ul>   <li>  <b>Step 0</b> 。外推法。定义一下。  @f[
    u^\star = 2u^k - u^{k-1}, \quad p^\sharp = p^k + \frac43 \phi^k - \frac13 \phi^{k-1}.
  @f]  <li>  <b>Step 1</b>。扩散步骤。我们发现 $u^{k+1}$ 可以解决单一线性方程@f[
  ...
  @f]  <li>  <b>Step 3</b>的 $\phi^{k+1}$ 。压力校正。这里我们有两个选项。      <ul>   <li>  <i>Incremental Method in Standard Form</i>。压力的更新方式是。      @f[
        p^{k+1} = p^k + \phi^{k+1}.
      @f]  <li>  <i>Incremental Method in Rotational Form</i>。在这种情况下，@f[
        p^{k+1} = p^k + \phi^{k+1} - \nu \nabla \cdot u^{k+1}.
      @f]  </ul>   </ul>  。 

在不详细说明的情况下，让我们对我们刚刚描述的投影方法说几句。  <ul>   <li>  平流项 $u\cdot\nabla u$ 被其<i>skew symmetric form</i> @f[
    u \cdot \nabla u + \frac12 \left( \nabla\cdot u \right) u.
  @f]取代，这与连续方程一致（因为 $\nabla\cdot u = 0$ ，尽管这对离散解来说不是点状的），而且需要它来保证时间步进方案的无条件稳定性。此外，为了使该术语线性化，我们使用 $u^{k+1}$ 的二阶外推法 $u^\star$  。    <li>  投射步骤是亥姆霍兹分解@f[
  ...
  @f] 事实上，如果我们在 $u^{k+1}$ 上使用这个分解，我们得到@f[
...
<p>标准增量方案和旋转增量方案是van Kan在 <ul> 中首次考虑的 <li>  J. van Kan, "A second-order accurate pressure-correction scheme for viscous incompressible flow", SIAM Journal on Scientific and Statistical Computing, vol. 7, no. 3, pp. 870-891, 1986  </ul>  并由Guermond在 <ul>   <li>  J.-L. Guermond, "非压缩流的二阶精确压力修正方案", SIAM Journal of Scientific Statistical Computing, vol. 7, no. L. Guermond, "Un résultat de convergence d'ordre deux en temps pour l'approximation des équations de Navier-Stokes par une technique de projection incrémentale", ESAIM: Mathematical Modelling and Numer Analysis, vol. 33, no. 1, pp. 169-189, 1999  </ul>  。事实证明，这种技术受到运动压力的非物理边界条件的影响，导致收敛率降低。为了防止这种情况，Timmermans等人在 <ul>   <li>  L. Timmermans, P. Minev, and F. Van De Vosse, "An approximate projection scheme for incompressible flow using spectral elements", International Journal for Numerical Methods in Fluids, vol. 22, no. 7, pp.673-688, 1996  </ul>  旋转压力校正投影方法，对运动压力使用发散校正。在 <ul>   <li>  J.-L. Guermond 和 J. Shen, "On the error estimates for the rotational pressure-correction projection methods", Mathematics of Computation, vol. 73, no. 248, pp.1719-1737, 2004  </ul>  中首次对Stokes问题进行了深入分析。</p> 

<a name ="fullydiscrete"></a> <a name="TheFullyDiscreteSetting"></a><h3> The Fully Discrete Setting </h3>

为了获得该方法的完全离散设置，我们一如既往地需要一个变量公式。鉴于边界条件的性质，这里有一个微妙的问题。当我们将方程乘以一个合适的检验函数时，出现的一个项是@f[
...
我们对投影方法的实现遵循<i>verbatim</i>上面的描述。然而，我们必须注意到，与其他大多数有多个求解分量的问题不同，我们没有使用矢量值的有限元。相反，我们对速度和压力的分量分别使用单独的有限元，并对这些分量使用不同的 <code>DoFHandler</code> 's。这样做的主要原因是，正如我们从方案的描述中看到的，速度和压力的 <code>dim</code> 分量是解耦的。因此，所有速度分量的方程看起来都是一样的，具有相同的系统矩阵，并且可以以%的速度平行求解。很明显，这种方法也有其缺点。例如，在集合矩阵和右手边时，我们需要保持几个 <code>DoFHandler</code> s和迭代器的同步；获得矢量值函数的固有量（如发散）变得有点尴尬，以及其他。
...
我们在顶壁、底壁和障碍物上都施加了无滑动边界条件。在左边，我们有流入的边界条件@f[
  u =
  \left( \begin{array}{c} 4 U_m y (H-y)/H^2 \\ 0 \end{array} \right),
@f]与 $U_m = 1.5$ ，即流入的边界条件对应于这个配置的Poiseuille流。最后，在右边的垂直壁上，我们规定速度的垂直分量和压力都应该是零。最后的时间  $T=10$  。<a name="CommProg"></a> <h1> The commented program</h1>
...
我们首先包括所有必要的deal.II头文件和一些C++相关的文件。它们中的每一个都在以前的教程程序中讨论过，所以我们在这里就不做详细介绍了。
...
这尤其包括辅助变量  $\phi$  的方程表述，为此我们声明了一个  <code>enum</code>  。接下来，我们声明一个类，它将读取和存储我们程序运行所需的所有参数。
...
接下来的几个结构和函数是用来做各种并行的事情。它们遵循 @ref threads 中提出的方案，使用WorkStream类。正如那里所解释的，这需要我们为每个装配器声明两个结构，一个是每任务数据，一个是从头开始的数据结构。然后，这些结构被移交给组装本地贡献的函数，并将这些本地贡献复制到全局对象上。     


这个程序的一个特点是，我们不只是有一个代表速度和压力的DoFHandler对象，而是为这两类变量使用单独的DoFHandler对象。当我们想把涉及这两个变量的条款，如速度的发散和压力的梯度，乘以各自的测试函数时，我们要为这种优化付费。在这样做的时候，我们不能再仅仅使用一个FEValues对象，而是需要两个，而且它们需要用单元格迭代器来初始化，这些迭代器指向三角形中的同一个单元格，但不同的DoFHandlers。     


为了在实践中做到这一点，我们声明了一个 "同步 "迭代器--一个内部由几个（在我们的例子中是两个）迭代器组成的对象，每次同步迭代器向前移动一步时，内部存储的每个迭代器也向前移动一步，从而始终保持同步状态。碰巧的是，有一个deal.II类可以促进这种事情。这里重要的是要知道，建立在同一个三角形上的两个DoFHandler对象将以相同的顺序走过三角形的单元。
...
在构造函数中，我们只是从作为参数传递的 <code>Data_Storage</code> 对象中读取所有数据，验证我们读取的数据是否合理，最后，创建三角测量并加载初始数据。
...
创建三角形，并对其进行所需次数的细化的方法。在创建三角形之后，它创建与网格相关的数据，即分配自由度和重新编号，并初始化我们将使用的矩阵和向量。
...
在这组方法中，我们初始化了稀疏模式、约束条件（如果有的话）并组装了不依赖于时间步长的矩阵  <code>dt</code>  。注意，对于拉普拉斯矩阵和质量矩阵，我们可以使用库中的函数来完成。因为这个函数的昂贵操作--创建两个矩阵--是完全独立的，我们原则上可以把它们标记为可以使用 Threads::new_task 函数进行%并行工作的任务。我们不会在这里这样做，因为这些函数在内部已经被并行化了，特别是由于当前的函数在每个程序运行中只被调用一次，所以在每个时间步长中不会产生费用。然而，必要的修改将是非常直接的。
...
对于梯度算子，我们首先初始化稀疏模式，并对其进行压缩。这里需要注意的是，梯度算子是从压力空间作用到速度空间的，所以我们必须处理两个不同的有限元空间。为了保持循环的同步，我们使用之前定义的别名，即 <code>PairedIterators</code> and <code>IteratorPair</code>  。
...
这是时间行进函数，从 <code>t_0</code> 开始，使用时间步长为 <code>dt</code> 的投影法进行时间推进，直到 <code>T</code>  。   


它的第二个参数 <code>verbose</code> 表示该函数是否应该输出它在任何特定时刻所做的信息：例如，它将说明我们是否正在进行扩散、投影子步骤；更新前置条件器等。我们没有使用像 <div class=CodeFragmentInTutorialComment>


[1.x.117]


</div> 那样的代码来实现这种输出，而是使用ConditionalOStream类来为我们做这个。该类接受一个输出流和一个条件，该条件表明你传递给它的东西是否应该被传递到给定的输出流，或者应该被忽略。这样，上面的代码就变成了 <div class=CodeFragmentInTutorialComment>
...
扩散步骤的实现。注意，昂贵的操作是函数末尾的扩散解，我们必须对每个速度分量做一次。为了加快进度，我们允许以%并行方式进行，使用 Threads::new_task 函数，确保 <code>dim</code> 的求解都得到处理，并被安排到可用的处理器上：如果你的机器有一个以上的处理器核心，并且这个程序的其他部分目前没有使用资源，那么扩散求解将以%并行方式运行。另一方面，如果你的系统只有一个处理器核心，那么以%并行方式运行将是低效的（因为它导致了，例如，缓存拥堵），事情将被顺序地执行。
...
下面的几个函数是处理集合平流项的问题，平流项是扩散步骤的系统矩阵的一部分，在每个时间步骤中都会发生变化。如上所述，我们将使用WorkStream类和其他设施，在所有单元上以%并行的方式运行装配循环，如 @ref threads 的文档模块中所述。
...
<a name="codeNavierStokesProjectionprojection_stepcode"></a> <h4><code>NavierStokesProjection::projection_step</code></h4> @code
  template <int dim>
  void NavierStokesProjection<dim>::assemble_advection_term()
  {
    vel_Advection = 0.;
...
这种方法绘制了当前的解决方案。主要的困难是，我们希望创建一个单一的输出文件，其中包含所有速度分量、压力以及流动的涡度的数据。另一方面，速度和压力存在于不同的DoFHandler对象中，因此不能用一个DataOut对象写入同一个文件。因此，我们必须更努力地把各种数据放到一个DoFHandler对象中，然后用它来驱动图形输出。   


我们不会在这里详细说明这个过程，而是参考 step-32 ，那里使用了一个类似的程序（并有记录），为所有变量创建一个联合的DoFHandler对象。   


我们还注意到，我们在这里将涡度作为一个单独的函数中的标量来计算，使用 $L^2$ 中的量 $\text{curl} u$ 投影到用于速度分量的有限元空间。然而，原则上，我们也可以从速度中计算出一个点状量，并通过 step-29 和 step-33 中讨论的DataPostprocessor机制来实现。
...
下面是一个辅助函数，通过将 $\text{curl} u$ 项投影到用于速度分量的有限元空间来计算涡度。这个函数只有在我们生成图形输出时才会被调用，所以不是很频繁，因此我们没有像对待其他装配函数那样，麻烦地使用WorkStream概念来并行化它。不过，如果需要的话，这应该不会太复杂。此外，我们在这里的实现只适用于2D，所以如果情况不是这样，我们就会保释。
...
让我们画出 $t=1,5,12,20,25$ （即时间步数200、1000、2400、4000和5000）的所得结果，其中左栏显示涡度，右栏显示速度场。
...
这些图像很好地显示了障碍物后面涡流链的发展和延伸，涡度的符号表明这是一个左转或右转的涡流。


<a name="Re500"></a> <a name="Re500"></a><h3> Re = 500 </h3>


我们可以将参数文件中的雷诺数 $Re$ ，改为 $500$ 的值。这样做，并在一定程度上减少时间步长，在 $t=20,40$ 时得到以下图像。
...
对于这个较大的雷诺数，我们观察到不符合物理规律的振荡，特别是涡度的振荡。离散化方案现在难以正确解决流动问题，而流动应该仍然是层状的和有组织的。这些现象是典型的离散化方案，它们在溶解度不足的情况下缺乏稳健性，其中溶解度不足意味着用网格尺寸而不是几何体的物理尺寸计算的雷诺数很大。我们看一下放大的障碍物后面的区域，以及我们在那里的网格尺寸。
...
的确，现在的涡度场看起来更加平滑了。虽然我们可以预期进一步细化网格也会抑制剩余的振荡，但我们应该采取措施在粗略分辨率的限制下获得一个强大的方案，如下所述。
...
这个方案可以向以下方向扩展。  <ul>   <li>  自适应网格细化。正如我们所看到的，我们在一个单一的固定网格上计算了所有的东西。  使用自适应网格细化可以提高精度，同时不明显增加计算时间。
    ...
    <li>  自适应时间步进。虽然目前显然没有关于可变时间步长的投影方法的理论，但实践表明，它们的表现非常好。

    <li>  高雷诺%数。正如我们从结果中看到的，增加雷诺数会大大改变离散化方案的行为。使用众所周知的稳定技术，我们可以计算这个问题或许多其他问题中的流动，当雷诺数非常大，计算成本要求空间分辨率，而流动只能得到有限的解决，特别是对于三维湍流。

    <li>  可变密度的不可压缩流。对于密度可变的不可压缩流的情况，有类似投影的方法。如果不同密度的流体混合，例如淡水和盐水，或者酒精和水，这种流动就会发挥作用。

    <li>  可压缩纳维-斯托克斯方程。这些方程适用于以下情况：速度高到足以使流体变得可压缩，但还没有快到我们进入一个粘度变得可以忽略不计的体系，纳维-斯托克斯方程需要被气体动力学的双曲欧拉方程所取代。如果速度超过音速的三分之一，可压缩性就开始成为一个因素，所以它对几乎所有的地面交通工具来说都不是一个因素。另一方面，商业喷气式飞机的飞行速度约为音速的85%，机翼上的流动变得明显超音速，在这种情况下，可压缩的纳维尔-斯托克斯方程也不再适用。然而，在这两者之间的范围内有重要的应用，如小型飞机或许多欧洲和东亚国家的快速列车。  </ul>  <a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-35.cc"   
