...
本教程程序介绍了泊松方程的 "弱加勒金 "有限元方法的实现。从某种意义上说，考虑这种方法的动机与 step-51 中的观点相同：我们想考虑不连续的形状函数，但需要解决这样一个事实：与通常的连续Galerkin方法相比，所产生的问题有更多的自由度（因为，例如，每个顶点带有与相邻单元一样多的自由度）。我们还必须解决这样一个事实：与连续Galerkin方法不同，<i>every</i>一个单元上的自由度与它的每个面邻单元上的所有自由度相耦合。因此，从 "传统 "非连续Galerkin方法得到的矩阵既大又相对密集。

 step-51 中的混合非连续Galerkin方法（HDG）和本教程中的弱Galerkin（WG）方法都是通过引入额外的自由度来解决耦合问题的，这些自由度的形状函数只存在于单元间的一个面上（即。在网格的 "骨架 "上），因此将相邻单元的自由度相互 "隔离"：单元自由度只与同一单元的其他单元自由度以及面的自由度耦合，而不与相邻单元的自由度耦合。因此，这些细胞自由度的形状函数的耦合确实正好耦合在一个细胞和定义在其面上的自由度上。
...
弱加尔金有限元方法（WGFEMs）使用离散弱函数来近似标量未知数，使用离散弱梯度来近似经典梯度。该方法最初是由王俊平和叶秀在论文<a href="https://doi.org/10.1016/j.cam.2012.10.003">
...
J. Comput. Appl. Math., 103-115, 2013</a>中提出。与连续Galerkin方法相比，弱Galerkin方法满足重要的物理特性，即局部质量守恒和体法通量连续。它的结果是一个SPD线性系统，通过网格细化可以获得最佳收敛率。
...
在这个程序中，我们将考虑一个测试案例，在单位平方域上的确切压力是 $p = \sin \left( \pi x\right)\sin\left(\pi y \right)$ ，具有同质的Dirichelet边界条件， $\mathbf{K}$ 是身份矩阵。然后我们将计算压力、速度和通量的 $L_2$ 误差。


<a name="WeakGalerkinscheme"></a><h3> Weak Galerkin scheme </h3>


上述泊松方程有一个解 $p$ ，需要满足问题的弱表述，@f{equation*}
...
弱Galerkin方法的想法是用一个<i>discontinuous function</i> $p_h$ 来近似精确的 $p$ 解。这个函数可能只在单元格之间的界面上不连续，由于我们也想沿着界面评估这个函数，我们不仅要规定它在单元格内部应该有什么值，而且还要规定它在界面上的值。我们的做法是， $p_h$ 实际上是一个元组， $p_h=(p^\circ,p^\partial)$ ，尽管它实际上只是一个单一的函数，它要么等于 $p^\circ(x)$ ，要么等于 $p^\partial(x)$ ，这取决于它是在位于细胞内部还是在细胞界面的点 $x$ 上被评估。

然后我们想把这个近似值简单地贴到上面的双线性表格中。这适用于我们必须在边界上评估测试函数 $q_h$ 的情况（我们只需取其界面部分 $q_h^\partial$ ），但我们必须小心处理梯度，因为它只在单元内部定义。因此，泊松方程的弱Galerkin方案是由@f{equation*}
\mathcal{A}_h\left(p_h,q \right) = \mathcal{F} \left(q_h \right),
@f}定义的 

对所有离散测试函数 $q_h$ ，其中@f{equation*}
...
关键的一点是，在这里，我们用<i>discrete weak gradient</i>算子 $\nabla_{w,d} p_h$ 取代了梯度 $\nabla p_h$ ，这对我们特殊定义的近似值 $p_h$ 是有意义的。

那么问题是这个算子如何工作。为此，让我们首先说说我们是如何看待压力的离散近似 $p_h$ 的。如上所述，"函数" $p_h$ 实际上由两部分组成：单元内部的数值 $p_h^\circ$ 和界面上的 $p_h^\partial$ 。我们必须为这两部分定义离散的（有限维）函数空间；在这个程序中，我们将用FE_DGQ来表示 $p_h^\circ$ 作为细胞内部的空间（在每个细胞上定义，但一般是沿界面不连续的），用FE_FaceQ表示 $p_h^\partial$ 作为界面上的空间。

然后让我们只考虑一个单元（因为上面的积分都是逐个单元定义的，而且弱离散梯度是逐个单元定义的）。 $p_h$ 对单元 $K$ 、 $p_h|_K$ 的限制就由一对 $(p_h^\circ|_K,p_h^\partial|_{\partial K})$ 组成。实质上，我们可以认为 $\nabla_{w,d} p_h$ 是定义在 $K$ 上的某个函数，它接近于梯度；特别是，如果 $p_h|_K$ 是一个可微函数的限制（对 $K$ 的内部和边界--这将使它在内部和边界之间连续），那么 $\nabla_{w,d} p_h$ 将只是精确梯度 $\nabla p_h$  。但是，由于 $p_h|_K$ 在 $K$ 的内部和边界之间不连续，我们需要一个更普遍的定义；此外，我们不能处理任意函数，因此要求 $\nabla_{w,d} p_h$ 也在一个有限元空间中（由于梯度是一个矢量，必须是矢量值的，并且由于弱梯度是在每个单元上单独定义的，因此在单元之间也将是不连续的）。

这样做的方法是以下列方式定义这个弱梯度算子 $\nabla_{w,d}|_K :
DGQ_k(K) \times DGQ_r(\partial K) \rightarrow RT_s(K)$ （其中 $RT_s(K)$ 是单元格 $K$ 上阶为矢量值的Raviart-Thomas空间）。@f{equation*}{
...
为所有测试函数  $\mathbf v_h \in RT_s(K)$  。这实质上是简单地应用了逐段积分公式。换句话说，对于一个给定的 $p_h=(p^\circ_h,p^\partial_h)$ ，我们需要把 $\nabla_{w,d} p_h|_K$ 看作是度数为 $s$ 的Raviart-Thomas函数，对于这个函数，左手边和右手边在所有测试函数中是相等的。

那么需要说明的关键点是以下几点。通常的梯度 $\nabla$ 是一个*本地*算子，它仅仅根据一个函数在某一点及其（无限小）邻域的值来计算导数，而弱离散梯度 $\nabla_{w,d}$ 不具有这种性质。它取决于它所应用的函数在整个单元上的值，包括单元的边界。然而，从上面 $\nabla_{w,d}$ 的定义中可以看出，两者都是线性算子，这将使我们在下面的讨论中可以通过矩阵来表示 $\nabla_{w,d}$ 。

  @note  可能值得指出的是，虽然弱离散梯度是Raviart-Thomas空间 $RT_s(K)$ 中每个单元 $K$ 上的一个元素，但它在单元之间是不连续的。另一方面，定义在整个网格上并由FE_RaviartThomas类实现的Raviart-Thomas空间 $RT_s=RT_s({\mathbb T})$ 代表了在单元间界面上具有连续法线分量的函数。这意味着<i>globally</i>， $\nabla_{w,d} p_h$ 不在 $RT_s$ 中，尽管它在 $RT_s(K)$ 中的每个单元 $K$ 上。  相反，它是在一个 "破碎的 "拉维亚特-托马斯空间中，下面我们将用符号 $DGRT_s$ 来表示。 这里的 "破碎 "一词是指 "把东西打碎 "的过程，而不是指 "没有功能 "的同义词）。因此，人们可能会（理所当然地）争辩说，在弱Galerkin文献中使用的符号有点误导性，但就像很多时候一样，这完全取决于使用某种符号的背景--在目前的背景下，对Raviart-Thomas空间或元素的提及总是被理解为对 "破碎 "空间的提及。
...
由于 $p_h$ 是有限元空间的一个元素，我们可以像往常一样在一个基中展开它，即我们可以写出@f{equation*}{
  p_h(\mathbf x) = \sum_j P_j \varphi_j(\mathbf x).
@f} 

这里，由于 $p_h$ 有两个分量（内部分量和界面分量），对于基函数 $\varphi_j(\mathbf x)$ 也必须如此，我们可以写成 $\varphi_j = (\varphi_j^\circ,\varphi_j^\partial)$  。如果你遵循 step-8 、 step-20 和 @ref vector_valued "向量值问题文件模块 "中的描述，就不会奇怪，对于 $j$ 的某些值， $\varphi_j^\circ$ 将为零，而对于 $j$ 的其他值， $\varphi_j^\partial$ 将为零-即，形状函数将是一种或另一种类型。然而，这在这里并不重要。重要的是，我们需要思考如何表示 $\nabla_{w,d} \varphi_j$ ，因为当我们想实现双线性形式@f{equation*}
\mathcal{A}_h\left(p_h,q_h\right)
  = \sum_{K \in \mathbb{T}}
    \int_K \mathbf{K} \nabla_{w,d} p_h \cdot \nabla_{w,d} q_h \;\mathrm{d}x,
@f}时，这显然是会出现在问题中的。
...
关键是 $\nabla_{w,d} \varphi_j$ 是已知的 "破碎的 "Raviart-Thomas空间 $DGRT_s$ 的一个成员。这意味着我们可以（在每个单元 $K$ 上分别表示）@f{equation*}
\nabla_{w,d} \varphi_j|_K
  = \sum_k C_{jk}^K \mathbf v_k|_K
@f}

其中的函数  $\mathbf v_k \in DGRT_s$  ，以及  $C^K$  是一个维度为 
...
使用这个扩展，我们可以很容易地使用上面的弱离散梯度的定义来定义这个矩阵是什么。@f{equation*}{
...
为所有测试函数  $\mathbf v_i \in DGRT_s$  。

这显然导致了一个形式为@f{equation*}{
...
在这最后一步中，我们假设指数 $i,j,k$ 只涉及在单元 $K$ 上活动的自由度，从而确保空间 $RT_s(K)$ 上的质量矩阵是可逆的）。等价地，利用矩阵 $M$ 的对称性，我们有@f{equation*}{
  C^K = \left(G^K\right)^{T} \left(M^K\right)^{-1}.
@f}

另外值得指出的是，矩阵 $C^K$ 和 $G^K$ 当然不是正方形，而是矩形的。


<a name="Assemblingthelinearsystem"></a><h3> Assembling the linear system </h3>


在解释了弱离散梯度是如何定义的之后，我们现在可以回到有关方程的线性系统应该如何组装的问题上。具体来说，利用上面显示的双线性形式 ${\cal A}_h$ 的定义，我们就需要计算对全局矩阵的局部贡献的元素，@f{equation*}{
...
因此，如果我们有每个单元格 $K$ 的矩阵 $C^K$ ，那么我们可以很容易地计算出 $K$ 单元格对矩阵 $A$ 的贡献 $A^K$ 如下。@f{equation*}{
  A^K_{ij} =
    \sum_k \sum_l C_{ik}^K C_{jl}^K
    H^K_{kl}
    =
...
实际上只是单元 $K$ 的质量矩阵，使用Raviart-Thomas基础并通过渗透性张量 $\mathbf K$ 加权。这里的推导表明，弱Galerkin方法实际上只需要我们计算每个单元 $C^K$ 和 $H^K$ 的矩阵，然后计算 $A^K = C^K H^K (C^K)^T$ ，这很容易计算。下面要显示的代码正是这样做的。

在计算出单元格 $K$ 对全局矩阵的贡献后，我们要做的就是将这些局部贡献 "分配 "到全局矩阵中。如何做到这一点，首先显示在  step-3  和  step-4  中。在当前的程序中，这将通过调用 AffineConstraints::distribute_local_to_global(). 来实现。 

一个线性系统当然也需要一个右手边。除了我们只需要对每个形状函数 $\varphi_i$ 使用单元格内部部分 $\varphi_i^\circ$ 外，这里没有与计算右手边相关的困难。


<a name="PostprocessingandiLsub2subierrors"></a><h3> Post-processing and <i>L<sub>2</sub></i><i>L<sub>2</sub></i>-errors </h3> 


前面几节的讨论已经给了我们一个线性系统，我们可以解决数值压力  $p_h$  。我们可以用它来计算变量 $\mathbf u = -{\mathbf K}\nabla p$ 的近似值，如果这是我们要解决的模型，它对应于介质在多孔介质中的流动速度。这种步骤--从离散问题的解中计算一个派生量--通常被称为 "后处理"。
...
在单元 $K$ 上，数值速度 $ \mathbf{u}_h = -\mathbf{K} \nabla_{w,d}p_h$ 可写为 
...
不幸的是， $\mathbf{K} \mathbf{v}_j$ 可能不在 $RT$ 空间（当然，除非如果 $\mathbf K$ 是常数乘以身份矩阵）。因此，为了在有限元程序中表示它，我们需要把它投射回我们可以处理的有限维空间。在这里，我们将使用 $L_2$ 投影来将其投射回（破碎的） $RT$ 空间。

我们将投影定义为 $ \mathbf{Q}_h \left( \mathbf{K}\mathbf{v}_j \right) =
\sum_{k} d_{jk}\mathbf{v}_k$ 在每个单元 $K$ 上的投影。对于任何  $j$  ,  $\left( \mathbf{Q}_h \left( \mathbf{Kv}_j \right),\mathbf{v}_k \right)_K =
\left( \mathbf{Kv}_j,\mathbf{v}_k \right)_K.$  所以，与其说是上面的公式，不如说是  $K$  单元上的数值速度变成了 @f{equation*}
...
我们有以下的系统来解决系数 $d_{jk}$ ：@f{equation*}
 \sum_j
...
其中 $| K |$ 是元素的面积， $\gamma$ 是元素的面， $\mathbf{n}$ 是每个面的单位法向量。这些规范中的最后一条衡量了网格单元之间界面上速度向量的法向分量的精度。缩放因子 $|K|/|\gamma|$ 的选择是为了随着网格大小的变化，缩放出界面集合的长度（或面积）的差异。

上面的第一个错误很容易用 VectorTools::integrate_difference. 计算出来，其他的需要更多的工作，在下面的代码中实现。<a name="CommProg"></a> <h1> The commented program</h1> 
...
接下来，我们定义系数矩阵 $\mathbf{K}$ （这里是身份矩阵）、迪里希特边界条件、右手边 $f = 2\pi^2 \sin(\pi x) \sin(\pi y)$ ，以及与这些选择相对应的 $K$ 和 $f$ 的精确解，即 $p =
\sin(\pi x) \sin(\pi y)$ 。
...
这个函数更有意思。正如介绍中所详述的，线性系统的装配要求我们评估形状函数的弱梯度，这是Raviart-Thomas空间中的一个元素。因此，我们需要定义一个Raviart-Thomas有限元对象，并有FEValues对象在正交点评估它。然后我们需要计算每个单元 $K$ 上的矩阵 $C^K$ ，为此我们需要在介绍中提到的矩阵 $M^K$ 和 $G^K$ 。   


有一点可能不是很明显，在之前的所有教程程序中，我们总是用DoFHandler的单元格迭代器来调用 FEValues::reinit() 。这样就可以调用诸如 FEValuesBase::get_function_values() 这样的函数，在单元格的正交点上提取有限元函数的值（用DoF值的向量表示）。为了使这种操作发挥作用，我们需要知道哪些向量元素对应于给定单元上的自由度--也就是说，正是DoFHandler类所提供的那种信息和操作。   
...
接下来我们通过使用 FullMatrix::gauss_jordan(). 对这个矩阵进行求逆，它将被用来计算后面的系数矩阵 $C^K$ 。值得回顾的是，在这次调用之后，`cell_matrix_M`实际上包含了 $M^K$ 的*逆。
...
从介绍中，我们知道定义 $C^K$ 的方程的右边 $G^K$ 是面积分和单元积分的区别。在这里，我们对内部的贡献的负值进行了近似。这个矩阵的每个分量都是多项式空间的一个基函数与拉维-托马斯空间的一个基函数的发散之间的乘积的积分。这些基函数被定义在内部。
...
@endcode 
  ...
  @p cell_matrix_C 是 $G^K$ 的转置与质量矩阵的逆之间的矩阵积（该逆存储在 @p cell_matrix_M): 中）。 
...
\mathrm{d}x$ 给出。我们在上一步计算了系数 $C$ ，因此在适当地重新排列循环后得到以下结果。
...
在这里计算与我们在`assemble_system()`函数中已经完成的相同的矩阵，当然是在CPU时间上浪费了。同样，我们从那里复制一些代码到这个函数中，这也是一个糟糕的主意。一个更好的实现可能会提供一个函数来封装这些重复的代码。我们也可以考虑使用计算效率和内存效率之间的经典权衡，在装配过程中每个单元只计算一次 $C^K$ 矩阵，把它们存储在某个地方，然后在这里重新使用它们。这就是 step-51 所做的，例如，`assemble_system()`函数需要一个参数来决定本地矩阵是否被重新计算，类似的方法是 
...
在介绍中，我们解释了如何计算单元上的数值速度。我们需要每个单元上的压力解值、格拉姆矩阵的系数和 $L_2$ 投影的系数。我们已经计算了全局解，所以我们将从全局解中提取单元解。当我们组装压力的系统矩阵时，已经计算了格拉姆矩阵的系数。我们在这里也要这样做。对于投影的系数，我们做矩阵乘法，即用格拉姆矩阵的倒数乘以 $(\mathbf{K} \mathbf{w}, \mathbf{w})$ 的矩阵作为组成部分。然后，我们将所有这些系数相乘，称之为β。数值速度是β和拉维尔特-托马斯空间的基础函数的乘积。
...
为了计算介绍中提到的矩阵 $D$ ，我们需要评估 $D=M^{-1}E$ ，正如介绍中所解释的。
...
我们现在可以计算局部速度未知数（相对于我们将项 $-\mathbf K \nabla_{w,d} p_h$ 投射到的Raviart-Thomas空间而言）。
...
这一部分是计算压力的 $L_2$ 误差。 我们定义了一个向量，用来保存每个单元上的误差的规范。接下来，我们使用 VectorTool::integrate_difference() 来计算每个单元上 $L_2$ 准则的误差。然而，我们实际上只关心解向量的内部分量的误差（我们甚至不能评估正交点的界面压力，因为这些都位于单元格的内部），因此必须使用一个权重函数，确保解变量的界面分量被忽略。这是通过使用ComponentSelectFunction来实现的，其参数表明我们要选择哪个分量（零分量，即内部压力）以及总共有多少分量（两个）。
...
在这个函数中，我们评估每个单元上的速度的 $L_2$ 误差，以及面的流量的 $L_2$ 误差。该函数依赖于之前计算过的`compute_postprocessed_velocity()`函数，该函数根据之前计算过的压力解来计算速度场。   
...
将所有单元和面的误差相加后，我们取平方根，得到 $L_2$ 的速度和通量误差。这些我们都输出到屏幕上。
...
在这两个输出文件中，内部和面的变量都被存储。对于界面输出，输出文件只是包含了内部压力对面的插值，但是因为没有定义从两个相邻的单元中得到的两个内部压力变量中的哪一个，所以最好在界面输出文件中忽略内部压力。相反，对于单元格内部输出文件，当然不可能显示任何界面压力 $p^\partial$ ，因为这些压力只适用于界面，而不是单元格内部。因此，你会看到它们被显示为一个无效的值（比如一个无穷大）。   


对于细胞内部的输出，我们还想输出速度变量。这有点棘手，因为它生活在同一个网格上，但使用不同的DoFHandler对象（压力变量生活在`dof_handler`对象上，达西速度生活在`dof_handler_dgrt`对象上）。幸运的是， DataOut::add_data_vector() 函数有一些变化，允许指定一个向量所对应的DoFHandler，因此我们可以在同一个文件中把两个DoFHandler对象的数据可视化。
...
<a name="Thecodemaincodefunction"></a> <h3>The <code>main</code> function</h3> 这是主函数。
...
我们在运行程序时，右手边将产生解  $p = \sin(\pi x) \sin(\pi y)$  ，并且在域  $\Omega = (0,1)^2$  中具有同质的迪里希特边界条件。此外，我们选择微分算子 $\mathbf{K}$ 中的系数矩阵作为身份矩阵。我们使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 、 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 和 $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 元素组合来测试这个设置，人们可以通过使用`main()`中`WGDarcyEquation`对象的适当构造参数来选择。然后我们将可视化单元内部和面上的压力值。随着网格的细化，压力、速度和流量的收敛率对于 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 应该是1，对于 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 应该是2，对于 $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 应该是3。
...
从图中我们可以看出，随着网格的细化，最大和最小的压力值正在接近我们预期的数值。由于网格是矩形网格，每个方向上的单元数是偶数，所以我们有对称的解决方案。从右边的三维图中，我们可以看到，在 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 上，压力在单元的内部是一个常数，正如预期的那样。
...
我们可以用下一个更高的多项式度数重复上面的实验。下面的数字是使用 $\mbox{WG}(Q_1,Q_1;RT_{[1]})$ 实现的内部压力和表面压力。网格被细化了4次。 与之前使用 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 的数字相比，在每个单元上，解不再是恒定的，因为我们现在使用双线性多项式来做近似。因此，在一个内部有4个压力值，在每个面上有2个压力值。
...
与 $\mbox{WG}(Q_0,Q_0;RT_{[0]})$ 组合的相应图像相比，现在的解决方案大大增加了准确性，特别是在界面上非常接近于连续，我们不再能够区分界面压力 $p^\partial$ 和相邻单元上的内部压力 $p^\circ$ 。
...
让我们再提高一个多项式度数。下面是使用 $WG(Q_2,Q_2;RT_{[2]})$ 实现的内部压力和面压，网格大小为 $h = 1/32$ （即5个全局网格细化步骤）。在程序中，我们在生成图形输出时使用`data_out_face.build_patches(fe.degree)`（参见 DataOut::build_patches()), 的文档，这里意味着我们将每个2d单元内部分成4个子单元，以便提供更好的二次多项式的可视化。  <table align="center">
  <tr>
...
和以前一样，我们可以使用 $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 组合生成压力、速度和流量的 $L_2$ 误差的收敛数据。
...
再一次， $\mbox{WG}(Q_2,Q_2;RT_{[2]})$ 的收敛率符合预期，其值在3左右。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-61.cc"  。 
