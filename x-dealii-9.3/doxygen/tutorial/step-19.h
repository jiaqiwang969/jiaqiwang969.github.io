...
awards DMS-1821210, EAR-1550901, and OAC-1835673.
  ...
  @note  deal.II中存在对粒子的支持，主要是由于Rene Gassmoeller的最初努力。如果你在自己的工作中使用粒子功能，请通过引用出版物 @cite GLHPW2018 来确认这项工作。

<a name="Intro"></a> <a name="Introduction"></a><h1>Introduction</h1>


一般来说，有限元方法，特别是deal.II，是为了解决偏微分方程而发明的--换句话说，是为了解决[连续体力学](https://en.wikipedia.org/wiki/Continuum_mechanics)问题。另一方面，有时人们想解决的问题是，跟踪单个物体（"粒子"）以及它们的位置如何演变是有用的。如果这只是导致一组常微分方程，例如，如果你想跟踪太阳系中行星随时间变化的位置，那么deal.II显然不是你合适的工具。另一方面，如果这种演变是由于与偏微分方程的解的相互作用，或者有一个网格来确定哪些粒子与其他粒子相互作用（例如在[平滑粒子流体力学（SPH）](https://en.wikipedia.org/wiki/Smoothed-particle_hydrodynamics)方法中），那么deal.II对你有支持。

我们这里要考虑的情况是带电粒子如何在电场中运动。作为动机，我们将考虑[阴极射线](https://en.wikipedia.org/wiki/Cathode_ray)。由一块被加热的带负电的金属（"阴极"）发出的电子，然后被电场加速到带正电的电极（"阳极"）。阳极通常是环形的，这样大部分电子可以以电子束的形式飞过孔。在过去，它们可能会照亮由[阴极射线管](https://en.wikipedia.org/wiki/Cathode-ray_tube)制成的电视的屏幕。而今天，电子束在[X射线机](https://en.wikipedia.org/wiki/X-ray_tube)、[电子束光刻](https://en.wikipedia.org/wiki/Electron-beam_lithography)、[电子束焊接](https://en.wikipedia.org/wiki/Electron-beam_welding)以及其他一些领域都很有用。

我们接下来要考虑的方程式如下。首先，我们需要描述电场。这是最容易做到的，注意到电动势 $V$ 满足方程@f[


  -\epsilon_0 \Delta V = \rho
@f]，其中 $\epsilon_0$ 是真空的介电常数， $\rho$ 是电荷密度。这是由我们将选择的边界条件所增强的，如下所示。
...
换句话说，我们在两个电极上规定电压 $+V_0$ 和 $-V_0$ ，在其他地方规定绝缘（诺伊曼）边界条件。由于粒子的动力学纯粹是由于电场 $\mathbf E=\nabla V$ ，我们也可以在两个电极上规定 $2V_0$ 和 $0$  --所有重要的是两个电极上的电压差。

考虑到这个电动势 $V$ 和电场 $\mathbf E=\nabla V$ ，我们可以用微分方程@f[
  m {\ddot {\mathbf x}}_i = e\mathbf E,
@f]来描述第 $i$ 个粒子的运动轨迹，其中 $m,e$ 是每个粒子的质量和电荷。在实践中，将其写成位置 $\mathbf x$ 和速度 $\mathbf v$ 的一阶微分方程系统很方便。
...
首先，在历史上，阴极射线使用非常大的电场将电子从金属中拉出来。这只产生一个相对较小的电流。我们可以通过加热阴极来做得更好：在这种情况下，统计学上的一部分电子有足够的热能离开金属；然后电场只要足够强，就可以把它们从宿主的吸引中拉出来。我们将以下列方式对此进行建模。如果（i）电场指向远离电极的地方，即如果 $\mathbf E \cdot \mathbf n < 0$ ，其中 $\mathbf n$ 是指向域外（进入电极）的面的法向量，以及（ii）电场超过一个阈值 $|\mathbf E|\ge E_\text{threshold}$ ，我们将创建一个新粒子。这肯定不是一个足够准确的模型，但对于我们目前的教程项目来说已经足够好了。

第二，原则上，我们必须通过@f[
  \rho(\mathbf x) = \sum_i e\delta(\mathbf x-\mathbf x_i).
@f]对电荷密度进行建模。

  @note 现在的问题是，在现实中，一台老式电视中的阴极射线管产生的电流大约为几毫安培。在粒子加速器的更高能量的光束中，电流可能只有几纳安培。但一个安培是每秒流动的 $6\times 10^{18}$ 个电子。现在，正如你将在结果部分看到的那样，我们实际上只模拟了几微秒（ $10^{-5}$ 秒），但这仍然导致非常非常多的电子 -- 远远超过我们希望用一个像目前这样小的程序来模拟的数量。因此，让我们假设每个粒子代表 $N$ 个电子。那么粒子的质量和电荷也是 $Nm$ 和 $Ne$ ，我们要解决的方程是@f[
  (Nm) {\ddot {\mathbf x}}_i = (Ne)\mathbf E,
@f]，这当然与上述情况完全相同。另一方面，这些电子 "团块 "的电荷密度由@f[
  \rho(\mathbf x) = \sum_i (Ne)\delta(\mathbf x-\mathbf x_i).
@f]给出。我们将在程序中实现这种形式，其中 $N$ 在程序中被选择得相当大，以确保粒子实际影响电场。这在实践中可能并不现实。在大多数情况下，没有足够的电子来实际影响整个电场。但现实主义不是我们的目标）。) 




 人们可能会问，为什么电场（或者说，电势）的方程没有时间导数，而电子位置的方程却有。从本质上讲，这是一个建模假设。我们假设粒子移动得很慢，以至于在任何时候电场都处于平衡状态。这就是说，换句话说，电子的速度远远小于光速。换句话说，我们可以用电极电压来重新表述  $V_0$  ：由于每伏特的电动势都会使电子加速约600公里/秒（忽略相对论效应），要求  $|\mathbf v_i\|\ll c$  等于说  $2V_0 \ll 500 \text{V}$  。在这个假设下（以及电子总数较少的假设），我们也可以忽略运动电荷产生的磁场，否则也会影响电子的运动。
...
由于电势对粒子位置的依赖性很强，我们不想将其作为一个耦合系统来解决，而是采用一种解耦的方法，即首先解决每个时间步长的电势，然后解决粒子位置。这与我们在 step-21 、 step-31 和 step-32 中所做的非常一致，仅举几例，都可以在 step-58 中讨论的算子分割方法的背景下理解。
...
当然，有许多更好的方法来做时间离散化（例如简单的[跃迁方案](https://en.wikipedia.org/wiki/Leapfrog_integration)），但这不是本教程程序的重点，因此我们将满足于我们在这里的内容。不过，我们将在本程序的<a href="#extensions">possibilities for extensions</a>部分对这个难题的一个部分进行评论）。

还有一个问题是我们应该如何选择时间步长  $\Delta t$  。这里的限制是， Particles::ParticleHandler 类需要跟踪每个粒子在哪个单元中。如果我们平行运行计算，这尤其是一个问题（例如，在 step-70 中），因为在这种情况下，每个进程只存储它所拥有的那些单元，外加一层 "幽灵单元"。这在这里并不重要，但一般来说，我们应该确保在每个时间步长中，一个粒子只从一个单元移动到它的任何一个近邻（面、边或顶点邻居）。如果我们能确保这一点，那么 Particles::ParticleHandler 就能保证能够弄清粒子最后在哪个单元。要做到这一点，一个有用的经验法则是，我们应该选择时间步长，使所有粒子的预期移动距离小于一个单元格直径。这里， $h_i$ 是粒子 $i$ 所在的单元的最短边缘的长度--本质上是对单元大小的衡量。

另一方面，一个粒子可能已经在一个单元的边界上，而邻近的单元可能曾经进一步细化。因此，那么穿过那个*相邻*单元的时间实际上将是上述数量的一半，建议@f[
  \Delta t \le \min_i \frac{\tfrac 12 h_i}{\|\mathbf v_i\|}.
@f] 

但即使这样也是不够好的。上面的公式使用公式@f[
\frac{{\mathbf x}_i^{(n)}-{\mathbf x}_i^{(n-1)}}{\Delta t} = {\mathbf v}_i^{(n)},
@f]更新粒子的位置，也就是说，使用*当前*速度 ${\mathbf v}_i^{n}$ 。但是在我们需要选择 $\Delta t$ 的时候，我们还没有当前的速度--也就是在我们更新了势 $V^{(n)}$ 之后，但在我们从 ${\mathbf v}_i^{(n-1)}$ 到 ${\mathbf v}_i^{(n)}$ 更新速度之前。我们有的只是  ${\mathbf v}_i^{(n-1)}$  。所以我们需要为我们的最终选择增加一个安全系数。@f[
  \Delta t^{(n)} =
  c_\text{safety} \min_i \frac{\tfrac 12 h_i}{\|\mathbf v_i^{(n-1)}\|}.
@f]  $c_\text{safety}$ 应该有多大？这取决于与 $\|\mathbf v_i^{(n)}\|$ 相比， $\|\mathbf v_i^{(n-1)}\|$ 可能被低估了多少，而这实际上是很容易评估的。如果沿途遇到的电场大致恒定，在一个时间步长中产生的速度为零的粒子，在每个连续的时间步长中大致会获得相等的速度增量。因此， $\|\mathbf v_i^{(n-1)}\|$ 和 $\|\mathbf v_i^{(n)}\|$ 之间的最大差异将是一个系数。因此，我们将选择 $c_\text{saftey}=0.5$ 。

只有另外一种情况我们应该考虑。在第一个时间步骤中会发生什么？在那里，任何要被移动的粒子刚刚被创造出来，但它们的速度是零。所以我们不知道我们应该为它们选择什么速度。当然，在所有其他时间步骤中，也有刚刚被创造出来的粒子，但一般来说，具有最高速度的粒子限制了时间步骤的大小，因此新创造出来的具有零速度的粒子并不重要。但是如果我们**只有这样的粒子？
...
@f] 不想让一个粒子移动超过 $\frac 12 h_i$ ，那么意味着我们应该选择时间步长为@f[
  ...
  \min_i
  \sqrt{ \frac{h_i m}{e \|\nabla V^{(1)}\| }}.
@f] 使用关于相邻单元可能小2倍的相同论点，然后得出时间步长为0的最终公式。@f[
  \Delta t
  =
  \min_i
...
在讨论了时间离散化之后，对空间离散化的讨论将很简短：我们使用二次有限元，即空间 $Q_2$ ，来近似计算电动势 $V$ 。在初始时间步骤中，网格被调整了几次。如果你读过 step-6 ，所有这些都是完全标准的，而且实现起来也没有规定任何形式的惊喜。
...
添加和移动粒子实际上在deal.II中并不十分困难。要添加一个粒子，本程序的`create_particles()`函数只需使用以下形式的代码片段。
...
换句话说，这与在 `std::set` 或 `std::map`: 中插入一个对象没有什么不同。 创建对象，设置其属性（这里是当前位置、其参考单元位置和其id）并调用`insert_particle`。唯一可能令人惊讶的是参考位置。为了评估诸如 $\nabla V(\mathbf x_i)$ 这样的东西，有必要在位置 $\mathbf x_i$ 评估有限元场。但这需要在参考单元 $\hat{\mathbf x}_i$ 上的点评估有限元形状函数。为了提高效率，每个粒子不仅要存储它的位置和它所在的单元，还要存储该点在单元的参考坐标系中所对应的位置。

这样一来，更新一个粒子的位置就不再困难了。我们只需要调用 

@code
  particle->set_location(new_location);
@endcode 

我们在 "move_particles() "函数中这样做。唯一的区别是，我们必须告诉 Particles::ParticleHandler 类也要找到该位置对应的单元（以及在并行计算时，哪个进程拥有该单元）。出于效率的考虑，这在更新所有粒子的位置后最容易完成，并通过 Particles::ParticleHandler::sort_particles_into_subdomains_and_cells() 函数实现。

当然，有些时候，一个粒子可能会离开相关的领域。在这种情况下， Particles::ParticleHandler::sort_particles_into_subdomains_and_cells() 不能找到周围的单元，而只是简单地删除该粒子。但是，追踪以这种方式丢失的粒子的数量往往是有用的，为此， Particles::ParticleHandler 类提供了一个可以附加的 "信号"。我们在主类的构造函数中展示了如何做到这一点，以计算每个时间步骤中损失了多少粒子。具体来说，这种工作方式是， Particles::ParticleHandler 类有一个 "信号"，人们可以附加一个函数，只要信号被触发就会执行。在这里，这看起来如下。
...
这有点拗口，但实际情况是这样的。我们声明一个lambda函数来 "捕获"`this`指针（这样我们就可以在lambda函数中访问周围对象的成员函数），它需要两个参数。

- 被 "丢失 "的粒子的一个引用。

- 它最后所在的单元格的引用。然后lambda函数用这些参数简单地调用 `CathodeRaySimulator::track_lost_particle` 函数。当我们把这个lambda函数附加到信号上时， Particles::ParticleHandler::sort_particles_into_subdomains_and_cells() 函数将为每个找不到新家的粒子触发信号。这让我们有机会记录下粒子的位置，并记录下关于它的统计数据。




  @note  在这个教程程序中，我们通过手工插入粒子，并在我们根据包括静电问题的解决在内的条件而特别选择的位置插入粒子。但在其他情况下，人们主要想把粒子作为被动对象使用，例如，追踪和可视化流体流动问题的流场。在这些情况下，在 Particles::Generators 命名空间中有许多函数可以自动生成粒子。这个命名空间中的一个函数也被用于 step-70 教程程序中，例如。
...
在这幅图中，边界上标有红色和蓝色的部分是阴极，保持在一个电势 $V=-V_0$  。阴极的红色部分是被加热的部分，导致电子离开金属，然后被电场加速（还显示了一些电场线）。边界的绿色部分是阳极，保持在 $V=+V_0$ 。边界的其余部分满足诺伊曼边界条件。

这个设置模仿了真实的设备。重心角导致电势 $V$ ，其导数（电场 $\mathbf E$ ）有一个奇点--换句话说，它在角的附近变得非常大，允许它把电子从金属上扯下来。这些电子然后被加速推向（绿色）阳极，阳极中间有一个孔，电子可以通过这个孔逃离设备，飞向屏幕，在那里它们激发 "荧光粉"，然后发出我们从这些老式电视屏幕上看到的光。阴极的非加热部分不受电子发射的影响--在代码中，我们将其标记为电子管的 "聚焦元件"，因为它的负电压会排斥电子，并确保它们不会从垂直于边界的阴极加热部分飞走，而是事实上将其路径弯曲到右侧的阳极上。

图中的电场线也说明了电场分别连接着负极和正极。电子经历的加速力是沿着这些场线的。最后，图片显示了计算中使用的网格，说明在重租角的顶端以及边界条件改变的所有地方都有奇异点；这些奇异点是可见的，因为网格在这些地方被细化。

实际的利益是要弄清楚从阴极发射的电子中有哪一部分真正通过了阳极上的孔--那些只是弹到阳极本身的电子除了将电能转化为热能外，实际上并没有什么用处。因此，在`track_lost_particle()`函数中（为每个离开域的粒子调用，见上文），我们将估计它可能离开域的位置并在输出中报告。




  @note 值得重申的是，这里使用的几何图形，以及这个程序的任何其他方面，都不是为了代表任何东西，甚至是半点现实的。教程是我们教授deal.II如何工作的工具，我们经常使用我们有某种直觉的情况，因为这有助于我们解释程序的输出，但这就是我们打算让程序除了作为教学工具之外做任何有用的事情的程度。<a name="CommProg"></a><h1> The commented program</h1>
...
本程序中使用的大部分include文件在 step-6 和类似程序中都是众所周知的。
...
新的文件只有以下三个。第一个声明了DiscreteTime类，它帮助我们在时间相关的模拟中跟踪时间。后面两个提供了所有的粒子功能，即跟踪位于网格上的粒子的方法（ Particles::ParticleHandler 类）和为可视化目的输出这些粒子的位置和属性的能力（ Particles::DataOut 类）。
...
按照惯例，我们把所有与程序细节相对应的东西都放到一个自己的命名空间中。在顶部，我们定义了一些常数，我们宁愿使用符号名称而不是硬编码的数字。




具体来说，我们为几何学各部分的 @ref GlossBoundaryIndicator "边界指标"，以及电子的物理特性和我们在这里使用的其他具体设置定义了数字。




对于边界指标，让我们从某个随机值101开始列举。这里的原则是要使用*不常见的数字。如果之前有`GridGenerator'函数设置的预定义边界指标，它们很可能是从0开始的小整数，但不在这个相当随机的选择范围内。使用下面这样的数字可以避免冲突的可能性，同时也减少了在程序中直接拼出这些数字的诱惑（因为你可能永远不会记得哪个是哪个，而如果它们从0开始，你可能会受到诱惑）。
...
下面是这个程序的主类。从根本上说，它的结构与 step-6 和其他许多教程程序相同。这包括大多数成员函数（其余的目的可能从它们的名字中不言自明），以及只有少数成员变量超出了 step-6 的范围，所有这些都与处理粒子有关。
...
那么，让我们开始实施。构造函数所做的实际上只是对顶部的所有成员变量进行简单的初始化。唯一值得一提的是`particle_handler'，它被交给了一个指向粒子所处的三角形的引用（目前当然还是空的，但是粒子处理程序存储了这个引用，一旦粒子被添加，就会使用它--这发生在三角形被建立之后）。它得到的另一个信息是每个粒子需要存储多少 "属性"。在这里，我们需要每个粒子记住的是它当前的速度，也就是一个带有`dim`分量的矢量。然而，每个粒子还有其他的内在属性， Particles::ParticleHandler 类会自动并始终确保这些属性是可用的；特别是，这些属性是粒子的当前位置、它所在的单元、它在该单元中的参考位置，以及粒子的ID。   
...
然后，下一个函数负责生成我们要解决的网格。回顾一下域的样子。  <p align="center">
<img
src="https://www.dealii.org/images/steps/developer/step-19.geometry.png"
alt="The geometry used in this program"
width="600">
</p>  我们将这个几何体细分为 $4\times 2$ 个单元的网格，看起来像这样。  <div class=CodeFragmentInTutorialComment>


[1.x.87]


</div> 这样做的方法是首先定义 $15=5\times 3$ 顶点的位置--在这里，我们说它们在整数点上，左边的中间点向右移动了一个`delta=0.5'的值。   
...
有了这些数组，我们就可以转向更高级别的数据结构了。我们创建一个CellData对象的向量，为每个要创建的单元存储相关的顶点以及 @ref GlossMaterialId "材料ID"（在这里我们将简单地设置为0，因为我们在程序中不使用它）。     


这些信息然后被交给 Triangulation::create_triangulation() 函数，网格会被两次全局细化。
...
该函数的其余部分循环所有单元及其面，如果一个面在边界上，则决定哪一个边界指标应该应用到它。如果你将代码与上面的几何图形相比较，各种条件应该是有意义的。     
...
本程序的下一个函数是处理与求解偏微分方程有关的各种对象的设置。它实质上是对 step-6 中相应函数的复制，不需要进一步讨论。
...
计算矩阵项的函数实质上也是对  step-6  中相应函数的复制。
...
请注意，最后一行不再包含积分，因此也没有出现 $dx$ ，这需要在我们的代码中出现`JxW`符号。         


对于一个给定的单元格 $K$ ，这个单元格对右边的贡献是 
...
最后一个与场相关的函数是细化网格的函数。我们将在第一个时间步骤中多次调用它，以获得一个能很好地适应解的结构的网格，特别是解决解中由于重心角和边界条件类型变化的地方而产生的各种奇异现象。你可能想再参考一下 step-6 以了解更多细节。
...
只有当电场强度超过阈值时，电子才能逃离阴极，而且关键是，如果电场指向*域内，电子才能逃离阴极。一旦我们检查了这一点，我们就在这个位置创建一个新的 Particles::Particle 对象，并将其插入到 Particles::ParticleHandler 对象中，并设置一个唯一的ID。                 


这里唯一不明显的是，我们还将这个粒子与我们当前所在的单元格的参考坐标中的位置联系起来。这样做是因为我们将在下游函数中计算诸如粒子位置的电场等量（例如，在每个时间步长中更新其位置时计算作用于它的力）。在任意坐标上评估有限元场是一个相当昂贵的操作，因为形状函数实际上只定义在参考单元上，所以当要求一个任意点的电场时，我们首先要确定这个点的参考坐标是什么。为了避免反复操作，我们一次性地确定这些坐标，然后将这些参考坐标直接存储在粒子上。
...
在所有这些插入结束时，我们让`particle_handler`更新一些关于它所存储的粒子的内部统计数据。
...
第二个与粒子有关的函数是在每个时间步骤中移动粒子的函数。要做到这一点，我们必须在所有单元、每个单元的粒子上进行循环，并评估每个粒子位置的电场。   
...
现在得到了其中一个粒子位置的电场，我们首先用它来更新速度，然后更新位置。要做到这一点，让我们首先从存储在粒子的属性中得到旧的速度，计算加速度，更新速度，并将这个新速度再次存储在粒子的属性中。回顾一下，这对应于介绍中讨论的以下一组更新方程中的第一个。
...
在更新了所有粒子的位置和属性（即速度）之后，我们需要确保`particle_handler`再次知道它们在哪个单元中，以及它们在参考单元的坐标系中的位置是什么。下面的函数就是这样做的。它还确保在并行计算中，如果粒子从一个处理器所拥有的子域移动到另一个处理器所拥有的子域，那么粒子就会从一个处理器移动到另一个处理器）。
...
正如介绍中提到的，我们必须以不同的方式对待第一个时间步长，因为在那里，粒子还没有可用，或者还没有我们计算合理步长所需的相关信息。下面的公式遵循导言中的讨论。
...
有了这个，`output_results()`函数就变得相对简单了。我们使用DataOut类，就像我们在以前几乎所有的教程程序中使用的那样，来输出解决方案（"电动势"），我们使用上面定义的后处理程序来输出其梯度（"电场"）。这一切都被写入一个VTU格式的文件中，同时将当前时间和时间步长与该文件联系起来。
...
输出粒子的位置和属性并不复杂。 Particles::DataOut 类扮演了粒子的DataOut类的角色，我们所要做的就是告诉该类从哪里获取粒子，以及如何解释属性的`dim'分量--即作为表示速度的单一矢量，而不是`dim'标量属性。剩下的就和上面一样了。
...
这个程序的主类的最后一个成员函数是驱动。在顶部，它通过在一连串越来越细的网格上求解问题（尚未创建粒子）来多次细化网格。
...
现在随着时间的推移做循环。这个步骤的顺序紧跟介绍中所讨论的算法的大纲。正如在DiscreteTime类的文档中详细讨论的那样，当我们将场和粒子信息向前移动一个时间步长时，存储在`time`变量中的时间与这些量的（部分）位置不一致（在DiscreteTime的字典中，这就是 "更新阶段"）。对`time.advance_time()`的调用通过将`time`变量设置为场和粒子已经处于的时间而使一切重新保持一致，一旦我们处于这个 "一致阶段"，我们就可以生成图形输出并将模拟的当前状态的信息写入屏幕。
...
程序的最后一个函数又是`main()`函数。自 step-6 以来，它在所有的教程程序中都没有变化，因此没有什么新的东西可以讨论。
...
当这个程序被运行时，它产生的输出看起来如下。``时间步数1 场自由度：4989 仿真中的粒子总数：20 这个时间步数损失的粒子数：0 
...
... 
  ...
  时间步数1000 场自由度：4989 仿真中的粒子总数。 44 这个时间步骤损失的粒子数：6 通过阳极损失的粒子数。0.0601266 
...
时间步数2091场自由度：4989模拟中的粒子总数。 44 这个时间步骤损失的粒子数：0 通过阳极损失的粒子数。0.0503338 
...
时间步数2093 场自由度：4989 仿真中的粒子总数。 44 这个时间步长损失的粒子数：2 通过阳极损失的粒子的分数。0.050308 
...
也就是说，将这个程序的结果可视化的一个更合适的方法是创建一个视频，显示这些电子是如何运动的，以及电场是如何随着它们的运动而变化的。
...
你在这里可以看到，边界的 "焦点元素 "是如何用其负电压排斥电子，并确保它们不会直接从阴极垂直飞走（就像它们在其轨迹的初始部分那样）。它还显示了电场线如何随着时间的推移而移动，以回应飞过的电荷--换句话说，粒子对电场的反馈，而电场本身又驱动着电子的运动。

这部电影表明，电子是以 "成串 "或 "突发 "的方式运动的。这种表象的一个因素是电影创作方式的一个伪命题。电影的每一帧都对应着一个时间步长，但时间步长是不同的。更具体地说，穿过最小单元的最快粒子决定了时间步长（见介绍中的讨论），因此，每当一个（快速）粒子穿过域的右边缘的小单元时，时间步长都很小；一旦粒子离开域，时间步长又会变长。通过绘制屏幕输出中显示的时间步长，可以很容易地看到这种减速-加速的效果。

然而，这其中的第二部分是真实的。仿真在开始时创造了一大群粒子，而在大约第300个时间步长之后，粒子就会减少。这可能是因为模拟中的粒子带有负电荷。它们降低了（同样带负电的电极）的电场强度，因此减少了阴极上的点的数量，在这些点上，电场强度超过了将电子从电极中吸引出来所需的阈值。
...
`assemble_system()`、`move_particles()`和`update_timestep_size()`函数都调用 Particles::ParticleHandler::particles_in_cell() 和 Particles::ParticleHandler::n_particles_in_cell() ，查询位于当前单元上的粒子信息。虽然这很方便，但也很低效。为了理解为什么会这样，我们需要知道粒子是如何存储在 Particles::ParticleHandler: 中的，即在一个数据结构中，粒子是以某种线性方式排序的，按它们所在的单元格排序。因此，为了找到与给定单元相关的粒子，这些函数需要搜索给定单元上的第一个（也可能是最后一个）粒子--这种努力需要花费 ${\cal O}(\log N)$ 次操作，其中 $N$ 是粒子的数量。但这是在每个单元上重复进行的；假设对于大型计算来说，单元和粒子的数量大致成正比，那么这些函数调用的累积成本是 ${\cal O}(N \log N)$ ，因此大于我们应该在程序的所有部分中争取的 ${\cal O}(N)$ 成本。

不过，我们可以使之更便宜。首先，我们可以不调用 Particles::ParticleHandler::n_particles_in_cell(), ，而是先调用 Particles::ParticleHandler::particles_in_cell() ，然后通过计算当前单元上最后一个粒子到第一个粒子的距离来计算单元上的粒子数。
...
当然，这些调用中的第一个仍然是 ${\cal O}(\log N)$ ，但至少第二个调用只需要与当前单元格上的粒子数成比例的计算时间，因此，当累积到所有单元格时，其成本为 ${\cal O}(N)$ 。

但我们甚至可以通过一些适当的算法设计摆脱这些调用中的第一个。这是因为粒子的排列方式与单元格相同，所以我们可以在单元格上移动时直接走动它们。下面的算法纲要就是这样做的。
...
在这段代码中，我们对每个单元都精确地触摸了一次，而且我们不必在大数据结构中搜索每个单元上的第一个或最后一个粒子。因此，该算法总共花费了 ${\cal O}(N)$ 来完成对所有粒子和所有单元的扫瞄。

对于本程序中存在这个问题的所有三个函数来说，实现这个方案并不十分困难。


<a name="Morestatisticsaboutelectrons"></a><h4> More statistics about electrons </h4>


该程序已经计算出通过阳极上的孔离开域的电子的比例。但是还有一些人们可能感兴趣的量。例如，这些粒子的平均速度。从每个粒子的属性中获得其速度并不是很困难，就像我们在`move_particles()`函数中所做的那样，并从中计算出统计数据。


<a name="Abettersynchronizedvisualization"></a><h4> A better-synchronized visualization </h4> 


如上所述，由于我们为每个时间步长创建输出，所以视频的不同帧之间存在着不同的时间差。一个更好的创建电影的方法是在固定的时间间隔内生成一个新的输出文件，不管每个这样的点之间有多少时间步骤。


<a name="Abettertimeintegrator"></a><h4> A better time integrator </h4> 


我们在这个程序中考虑的问题是一个耦合的、多物理学问题。但我们解决它的方法是首先计算（电）势场，然后更新粒子的位置。这就是所谓的 "算子分割法"，这个概念我们将在  step-58  中详细研究。

虽然要想出一个不涉及将问题分割成PDE部分和粒子部分的方法是很困难的，但人们*可以*（而且可能应该！）想出一个更好的方法来更新粒子的位置。具体来说，我们用来更新粒子位置的方程是 
...
在发布模式下，在写这篇文章时，该程序在作者的一台笔记本电脑上运行大约3.5分钟。这是可以接受的。但是，如果我们想让模拟变成三维的呢？如果我们想在任何时候都不使用最多约100个粒子（如这里使用的参数），而是使用100,000个？如果我们需要一个更细的网格？

在这些情况下，我们不仅要在单个处理器上运行程序，而且要在尽可能多的可用处理器上运行。这就要求对PDE的解决方案以及粒子进行并行化。在实践中，虽然有大量的挑战要使其高效和规模化，但这些挑战都是在deal.II本身解决的。例如， step-40 展示了如何将有限元部分并行化， step-70 展示了如何将粒子部分也并行化。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-19.cc" 。 
