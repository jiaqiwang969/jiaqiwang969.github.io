...
本例专门介绍了 <em> 非连续Galerkin方法 </em> ，简称为DG方法。它包括以下主题。  <ol>   <li>  用DG方法对线性平流方程进行离散化。    <li>  使用FEInterfaceValues组装跳跃项和单元间界面的其他表达。    <li>  使用 MeshWorker::mesh_loop().   </ol>  汇编系统矩阵。 

本程序特别关注的是DG方法的循环。这些问题变得特别复杂，主要是因为对于面的条款，我们必须分别区分边界、规则的内部面和有悬挂节点的内部面的情况。 MeshWorker::mesh_loop() 处理了单元和面迭代的复杂性，并允许为不同的单元和面项指定 "工作者"。面条款本身的整合，包括对自适应细化面的整合，是通过FEInterfaceValues类完成的。
...
@f]的制约。 这里， ${\mathbf \beta}={\mathbf \beta}({\bf x})$ 表示一个矢量场， $u$ 是（标量）解函数， $g$ 是边界值函数，@f[
\Gamma_- \dealcoloneq \{{\bf x}\in\Gamma, {\mathbf \beta}({\bf x})\cdot{\bf n}({\bf x})<0\}
@f]是域的边界的流入部分， ${\bf n}$ 表示边界的单位外向法线 $\Gamma$  。该方程是本教程 step-9 中已经考虑的平流方程的保守版本。


在每个单元 $T$ 上，我们从左边乘以一个测试函数 $v_h$ ，然后通过部分积分得到。@f[
  \left( v_h, \nabla \cdot (\beta u_h) \right)_T
= -(\nabla v_h, \beta u_h) + \int_\Gamma v_h u_h \beta \cdot n
@f] 当在所有单元格 $T$ 上求和时，边界积分是在所有内部和外部面上进行的，因此有三种情况。  <ol>   <li>  流入的外部边界（我们用给定的 $g$ 替换 $u_h$ ）。    $\int_{\Gamma_-} v_h g \beta \cdot n$   <li>  流出的外部边界。    $\int_{\Gamma_+} v_h u_h \beta \cdot n$   <li> 内面（两边的积分变成了跳跃，我们使用上风速度）。    $\int_F [v_h] u_h^{\text{upwind}} \beta \cdot n$   </ol> 。 

这里，跳跃被定义为 $[v] = v^+ - v^-$  ，其中上标指的是面的左（'+'）和右（'-'）值。如果 $\beta \cdot n>0$ ，上风值 $u^{\text{upwind}}$ 被定义为 $u^+$ ，否则 $u^-$ 。
...
为了实现这种双线性形式，我们需要用通常的方法计算单元项（第一个和）来实现单元上的积分，用FEInterfaceValues计算界面项（第二个和），以及边界项（其他两个和）。所有这些的求和是由 MeshWorker::mesh_loop(). 完成的。 
...
我们通过估计每个单元的梯度规范，自适应地细化网格，在一连串的网格上求解。在每个网格上求解后，我们以vtk格式输出解，并计算解的 $L^\infty$ 规范。由于精确的解是0或1，我们可以用这个来衡量数值解的过冲程度。<a name="CommProg"></a> <h1> The commented program</h1>
...
这里定义了不连续的有限元。它们的使用方式与所有其他有限元相同，不过--正如你在以前的教程程序中所看到的--用户与有限元类的交互并不多：它们被传递给 <code>DoFHandler</code> 和 <code>FEValues</code> 对象，就这些了。
...
考虑到流动方向，单元格 $[0,1]^2$ 的流入边界是右边界和下边界。我们在x轴上规定了不连续的边界值1和0，在右边界上规定了值0。这个函数在流出边界上的值将不会在DG方案中使用。
...
最后，一个计算并返回风场的函数  $\beta=\beta(\mathbf x)$  。正如介绍中所解释的，在2D中我们将使用一个围绕原点的旋转场。在3D中，我们只是不设置 $z$ 分量（即为零），而该函数在目前的实现中不能用于1D。
...
以下对象是我们在调用 MeshWorker::mesh_loop(). 时使用的抓取和复制对象。新对象是FEInterfaceValues对象，其工作原理类似于FEValues或FEFacesValues，只是它作用于两个单元格之间的接口，并允许我们以我们的弱形式组装接口项。
...
接下来的四个成员代表要解决的线性系统。  <code>system_matrix</code> and <code>right_hand_side</code>  是由  <code>assemble_system()</code>, the <code>solution</code>  生成的，在  <code>solve()</code>. The <code>sparsity_pattern</code>  中计算的，用于确定  <code>system_matrix</code>  中非零元素的位置 。
...
我们从生成稀疏模式开始。为此，我们首先用系统中出现的耦合物填充一个动态稀疏模式（DynamicSparsityPattern）类型的中间对象。在建立模式之后，这个对象被复制到 <code>sparsity_pattern</code> ，可以被丢弃。




为了建立DG离散的稀疏模式，我们可以调用与 DoFTools::make_sparsity_pattern, 类似的函数，该函数被称为 DoFTools::make_flux_sparsity_pattern: 。 
...
在这里我们看到了与手工组装的主要区别。我们不需要在单元格和面上写循环，而是将逻辑包含在对 MeshWorker::mesh_loop() 的调用中，我们只需要指定在每个单元格、每个边界面和每个内部面应该发生什么。这三个任务是由下面的函数里面的lambda函数处理的。
...
@endcode 
 ...
 <code>DerivativeApproximation</code> 类计算梯度的精度为浮点。这就足够了，因为它们是近似的，只作为细化指标。
...
该程序的输出包括一个自适应细化网格和数值解的vtk文件。最后，我们还用 VectorTools::integrate_difference(). 计算了解的L-无穷大规范。 
...
下面的 <code>main</code> 函数与之前的例子也类似，不需要评论。
...
在这个程序中，我们使用了不连续的元素。这是一个合理的问题，为什么不简单地使用正常的、连续的。当然，对于每个有数值方法背景的人来说，答案是显而易见的：连续Galerkin（cG）方法对于传输方程是不稳定的，除非特别增加稳定项。然而，DG方法<i>is</i>则是稳定的。用目前的程序来说明这一点并不十分困难；事实上，只需要做以下几个小的修改就可以了。
...
在细化迭代5中，图像不能再以合理的方式绘制成三维图。因此，我们展示了一个范围为 $[-1,2]$ 的彩色图（当然，精确解的解值位于 $[0,1]$ ，）。在任何情况下，很明显，连续Galerkin解表现出振荡行为，随着网格的细化越来越差。

如果人们出于某种原因想使用连续元素，有许多策略可以稳定cG方法。讨论这些方法超出了本教程程序的范围；例如，感兴趣的读者可以看看  step-31  。
...
鉴于在这种情况下确切的解决方案是已知的，进一步扩展的一个有趣途径是确认这个程序的收敛顺序。在目前的情况下，解是非光滑的，因此我们不能期望得到特别高的收敛阶数，即使我们使用高阶元素。但即使解<i>is</i>光滑，该方程也不是椭圆的，因此，我们应该获得等于最优插值估计的收敛阶数，这一点并不明显（例如，我们通过使用二次元可以获得 $h^3$ 在 $L^2$ 规范下的收敛）。

事实上，对于双曲方程来说，理论预测常常表明，最好的希望是低于内插估计值二分之一的阶。例如，对于流线扩散法（此处用于稳定传输方程解的DG法的一种替代方法），可以证明对于度数为 $p$ 的元素，在任意网格上的收敛阶数为 $p+\frac 12$ 。虽然在均匀细化的网格上观察到的顺序经常是 $p+1$ ，但人们可以构建所谓的彼得森网格，在该网格上实际上达到了更差的理论约束。这应该是比较简单的验证，例如使用 VectorTools::integrate_difference 函数。

一个不同的方向是观察运输问题的解决经常有不连续性，因此我们<i>bisect</i>每个坐标方向的每个单元的网格可能不是最佳的。相反，一个更好的策略是只在平行于不连续的方向上切割单元。这被称为 <i>anisotropic mesh refinement</i> ，是  step-30  的主题。<a name="PlainProg"></a> <h1> The plain program</h1>  @include "step-12.cc"  。 
